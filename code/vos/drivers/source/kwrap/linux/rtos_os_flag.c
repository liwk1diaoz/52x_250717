...
EXPORT_SYMBOL(vos_flag_max_cnt);
...
EXPORT_SYMBOL(vos_flag_used_cnt);
...
int _IOFUNC_FLAG_IOCMD_INFO(unsigned long arg)
{
	...
	ioarg.max_cnt = vos_flag_max_cnt();
	ioarg.used_cnt = vos_flag_used_cnt();

	if (copy_to_user((void *)arg, (void *)&ioarg, sizeof(VOS_FLAG_IOARG_INFO))) {
		DBG_ERR("copy_to_user failed\n");
		return -EFAULT;
	}
