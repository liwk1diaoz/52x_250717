...
#include <plat/hardware.h> //for NVT_PERIPHERAL_PHYS_BASE
...
void debug_dumpmem(ULONG addr, ULONG length)
{
	...
	if ((addr & NVT_PERIPHERAL_PHYS_BASE) == NVT_PERIPHERAL_PHYS_BASE) {
		map_addr = ioremap(addr, length+16);
		if (NULL == map_addr) {
			DBG_DUMP("ioremap() failed, addr 0x%lx, length %ld\r\n", addr, length);
			return;
		}
		p_u32 = (volatile UINT32 *)map_addr;
	} else {
