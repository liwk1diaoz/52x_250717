...
EXPORT_SYMBOL(vos_sem_used_cnt);
...
void vos_sem_dump(int (*dump)(const char *fmt, ...), int level)
{
	...
	for (i = 0; i < gsemnum; i++) {
		...
		if (FALSE == vos_task_chk_hdl_valid(p_semcell->own_tsk)) {
			...
			dump("Semaphore[%.3d, %32s] -> (Max: %.2d, Cur: %.2d), Owner: None\r\n",
			...
			dump("Semaphore[%.3d, %32s] -> * (Max: %.2d, Cur: %.2d), Owner: %s\r\n",
		...
		if (p_semcell->waitsk_count > 0) {
			...
			for (tsk_i = 0; tsk_i < WAIT_TASK_NUM_MAX; tsk_i++, p_waitsk++) {
				...
				task_name[TASK_COMM_LEN-1] = 0;
				dump("  Task[%.3d, %s], sleep_time = %lld ms \r\n",
...
int _IOFUNC_SEM_IOCMD_INFO(unsigned long arg)
{
	...
	ioarg.max_cnt = vos_sem_max_cnt();
	ioarg.used_cnt = vos_sem_used_cnt();

	if (copy_to_user((void *)arg, (void *)&ioarg, sizeof(VOS_SEM_IOARG_INFO))) {
		DBG_ERR("copy_to_user failed\n");
		return -EFAULT;
	}
