...
EXPORT_SYMBOL(vos_task_max_cnt);
...
EXPORT_SYMBOL(vos_task_used_cnt);
...
int _IOFUNC_TASK_IOCMD_INFO(unsigned long arg)
{
	...
	ioarg.max_cnt = vos_task_max_cnt();
	ioarg.used_cnt = vos_task_used_cnt();

	if (copy_to_user((void *)arg, (void *)&ioarg, sizeof(VOS_TASK_IOARG_INFO))) {
