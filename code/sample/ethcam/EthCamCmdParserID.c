...
void EthCamCmd_InstallID(UINT32 path_cnt)
{
	...
	for(i=0;i<path_cnt;i++){
		snprintf(name, sizeof(name) - 1, "FLG_ID_ETHCAMCMD_%d",(int)i);
		vos_flag_create(&FLG_ID_ETHCAMCMD[i], &cflg, name);
		snprintf(name, sizeof(name) - 1, "ETHCAMCMD_SEM_ID_%d",(int)i);
		vos_sem_create(&ETHCAMCMD_SEM_ID[i], 1, name);
		snprintf(name, sizeof(name) - 1, "ETHCAMSTR_SEM_ID_%d",(int)i);
