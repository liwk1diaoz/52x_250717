...
#define RESOLUTION_SET  1 //0: 2M(IMX290), 1:5M(OS05A) 2: 2M (OS02K10) 3: 2M (AR0237IR) 4: 2M (OV2718)
#if ( RESOLUTION_SET == 0)
...
static HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl)
{
    ...
    //Sensor interface choice
	  if (g_shdr_mode == 1) {
		if (RESOLUTION_SET == 2) {
			cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.serial_if_pinmux = 0x301;//PIN_MIPI_LVDS_CFG_CLK2 | PIN_MIPI_LVDS_CFG_DAT0 | PIN_MIPI_LVDS_CFG_DAT1
			printf("Using g_shdr_mode 2 lane\n");
		} else {
			...
			printf("Using g_shdr_mode 4 lane\n");
		}
	  } else {
		#if (RESOLUTION_SET == 0)
		printf("Using imx290 4 lane\n");
		...
		#elif (RESOLUTION_SET == 1)
		printf("Using OS052A 4 lane\n");
		cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.serial_if_pinmux = 0xf01;//0xf01;//PIN_MIPI_LVDS_CFG_CLK2 | PIN_MIPI_LVDS_CFG_DAT0 | PIN_MIPI_LVDS_CFG_DAT1 | PIN_MIPI_LVDS_CFG_DAT2 | PIN_MIPI_LVDS_CFG_DAT3
		#elif (RESOLUTION_SET == 2)
		printf("Using OS02K10 2 lane\n");
		cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.serial_if_pinmux = 0x301;//PIN_MIPI_LVDS_CFG_CLK2 | PIN_MIPI_LVDS_CFG_DAT0 | PIN_MIPI_LVDS_CFG_DAT1
		#elif (RESOLUTION_SET == 4)
		printf("Using OV2718 2 lane\n");
		cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.serial_if_pinmux = 0x301;//PIN_MIPI_LVDS_CFG_CLK2 | PIN_MIPI_LVDS_CFG_DAT0 | PIN_MIPI_LVDS_CFG_DAT1
		#endif
	  }
...
static HD_RESULT set_proc_param(HD_PATH_ID video_proc_path, HD_DIM* p_dim)
{
	...
	if (p_dim != NULL) { //if videoproc is already binding to dest module, not require to setting this!
		...
		if (g_prc_out_fmt == 0) video_out_param.pxlfmt = HD_VIDEO_PXLFMT_YUV420;
		if (g_prc_out_fmt == 1) video_out_param.pxlfmt = HD_VIDEO_PXLFMT_YUV420_NVX2;
		video_out_param.dir = HD_VIDEO_DIR_NONE;
