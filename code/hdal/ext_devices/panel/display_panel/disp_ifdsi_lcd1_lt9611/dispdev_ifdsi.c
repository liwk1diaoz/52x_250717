...
//#endif
	...
	UINT32                  ui_vvlaid_start,ui_vvlaid_end;
...
//#if _FPGA_EMULATION_
	...
	if (ui_pixel_pkt_mode_sel == DSI_PIXMODE_COMMAND_MODE) {
		...
		ui_vsa = p_mode->panel.ui_vsync_sync_width;//charles
		...
		if (ui_pixel_fmt == DSI_PIXEL_RGB_666_PACKETED) {
			...
			if (ui_video_sync_mode == DSI_VIDEOPKT_TYPE_SYNC_PULSE) {
				...
				ui_hsa=p_mode->panel.ui_hsync_sync_width;
				
				ui_hact_byte_count = p_mode->panel.ui_hsync_active_period*2;

				ui_hfp_byte_count = ui_hbp_byte_count = p_mode->panel.ui_hsync_back_porch;

				ui_bllp_byte_count = (p_mode->panel.ui_hsync_total_period - ui_hsa) * ui_lane_cnt + 663;
...
//          uiVIDEData = (uiVtotal_Sub_Vactive) * p_mode->panel.ui_hsync_total_period * 3 / 2;
		...
		ui_vvlaid_start = ui_vsa + p_mode->panel.ui_vsync_back_porch_odd;
		if (dsi_set_config(DSI_CONFIG_ID_VVALID_START, ui_vvlaid_start) != E_OK) { //charles
			DSI_ERR_MSG(" Configure VStart as 0x%08x error\r\n", ui_vvlaid_start);
		} else {
			DSI_CFG_MSG(" Configure VStart as %03d success\r\n", ui_vvlaid_start);
		}
		ui_vvlaid_end = ui_vsa + p_mode->panel.ui_vsync_back_porch_odd + p_mode->panel.ui_vsync_active_period -1;
		...
		if (dsi_set_config(DSI_CONFIG_ID_VVALID_END, ui_vvlaid_end) != E_OK) {//charles
			DSI_ERR_MSG(" Configure VEnd as %03d error\r\n", ui_vvlaid_end);
		} else {
			DSI_CFG_MSG(" Configure VEnd as %03d success\r\n", ui_vvlaid_end);
		}
	...
	//ui_bllp_byte_count = 8191;//charles
