...
//#if _FPGA_EMULATION_
	...
	if ((p_mode->panel.lcd_mode >= PINMUX_DSI_1_LANE_CMD_MODE_RGB565) && (p_mode->panel.lcd_mode <= PINMUX_DSI_1_LANE_VDO_SYNC_EVENT_RGB888)) {
		...
		if (dsi_set_config(DSI_CONFIG_ID_DATALANE_NO, DSI_DATA_LANE_0) != E_OK) {
			DSI_ERR_MSG(" Configure DSI_CONFIG_ID_DATALANE_NO fail\r\n");
		}
		ui_lane_cnt = 1;
	}
	...
	if (ui_pixel_pkt_mode_sel == DSI_PIXMODE_COMMAND_MODE) {
		if (dsi_set_config(DSI_CONFIG_ID_BLANK_CTRL, TRUE) != E_OK) {
			DSI_ERR_MSG(" Configure DSI_CONFIG_ID_BLANK_CTRL fail\r\n");
		}

		if (dsi_set_config(DSI_CONFIG_ID_INTER_PKT_LP, TRUE) != E_OK) {
			DSI_ERR_MSG(" Configure DSI_CONFIG_ID_INTER_PKT_LP fail\r\n");
		}
#if 0
...
//          uiVIDEData = (uiVtotal_Sub_Vactive) * p_mode->panel.ui_hsync_total_period * 3 / 2;
		...
		if (ui_video_sync_mode == DSI_VIDEOPKT_TYPE_SYNC_PULSE) {
			...
			if (dsi_set_config(DSI_CONFIG_ID_SYNCEVT_NULL_LEN, 0x20) != E_OK) {
				DSI_ERR_MSG(" Configure DSI_CONFIG_ID_SYNCEVT_NULL_LEN fail\r\n");
			}

			if (dsi_set_config(DSI_CONFIG_ID_SYNCEVT_SLICE_NO, 0) != E_OK) {
				DSI_ERR_MSG(" Configure DSI_CONFIG_ID_SYNCEVT_SLICE_NO fail\r\n");
			}
