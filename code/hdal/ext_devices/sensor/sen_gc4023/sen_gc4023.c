...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_gc4023, 1, 54, 000, 00);

//=============================================================================
// information
//=============================================================================
#define SEN_GC4023_MODULE_NAME    "sen_gc4023"
...
#define SENSOR_ID                  0x4023
#define SEN_I2C_ADDR 0x52 >> 1
#define SEN_I2C_COMPATIBLE "nvt,sen_gc4023"
...
//=============================================================================
static CTL_SEN_DRV_TAB *sen_get_drv_tab_gc4023(void);
static void sen_pwr_ctrl_gc4023(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb);
static ER sen_open_gc4023(CTL_SEN_ID id);
static ER sen_close_gc4023(CTL_SEN_ID id);
static ER sen_sleep_gc4023(CTL_SEN_ID id);
static ER sen_wakeup_gc4023(CTL_SEN_ID id);
static ER sen_write_reg_gc4023(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_read_reg_gc4023(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_chg_mode_gc4023(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj);
static ER sen_chg_fps_gc4023(CTL_SEN_ID id, UINT32 fps);
static ER sen_set_info_gc4023(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static ER sen_get_info_gc4023(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static UINT32 sen_calc_chgmode_vd_gc4023(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_calc_exp_vd_gc4023(CTL_SEN_ID id, UINT32 fps);
static void sen_set_gain_gc4023(CTL_SEN_ID id, void *param);
static void sen_set_expt_gc4023(CTL_SEN_ID id, void *param);
static void sen_set_preset_gc4023(CTL_SEN_ID id, ISP_SENSOR_PRESET_CTRL *ctrl);
static void sen_set_flip_gc4023(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
static ER sen_get_flip_gc4023(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
#if defined(__FREERTOS)
void sen_get_gain_gc4023(CTL_SEN_ID id, void *param);
void sen_get_expt_gc4023(CTL_SEN_ID id, void *param);
#else
static void sen_get_gain_gc4023(CTL_SEN_ID id, void *param);
static void sen_get_expt_gc4023(CTL_SEN_ID id, void *param);
#endif
static void sen_get_min_expt_gc4023(CTL_SEN_ID id, void *param);
static void sen_get_mode_basic_gc4023(CTL_SENDRV_GET_MODE_BASIC_PARAM *mode_basic);
static void sen_get_attr_basic_gc4023(CTL_SENDRV_GET_ATTR_BASIC_PARAM *data);
static void sen_get_attr_signal_gc4023(CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *data);
static ER sen_get_attr_cmdif_gc4023(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_CMDIF_PARAM *data);
static ER sen_get_attr_if_gc4023(CTL_SENDRV_GET_ATTR_IF_PARAM *data);
static void sen_get_fps_gc4023(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data);
static void sen_get_speed_gc4023(CTL_SEN_ID id, CTL_SENDRV_GET_SPEED_PARAM *data);
static void sen_get_mode_mipi_gc4023(CTL_SENDRV_GET_MODE_MIPI_PARAM *data);
static void sen_get_modesel_gc4023(CTL_SENDRV_GET_MODESEL_PARAM *data);
static UINT32 sen_calc_rowtime_gc4023(CTL_SEN_ID id, CTL_SEN_MODE mode);
static void sen_get_rowtime_gc4023(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data);
static void sen_set_cur_fps_gc4023(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_cur_fps_gc4023(CTL_SEN_ID id);
static void sen_set_chgmode_fps_gc4023(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_chgmode_fps_gc4023(CTL_SEN_ID id);
static void sen_ext_get_probe_sen_gc4023(CTL_SEN_ID id, CTL_SENDRV_GET_PROBE_SEN_PARAM *data);
//=============================================================================
// global variable
//=============================================================================
...
static CTL_SEN_CMD gc4023_mode_1[] = {
  //version 0.2
  //mclk 27Mhz
  //wpclk 216Mhz
  //rpclk 172.2Mhz
  ...
  {0x031c, 1, {0xce, 0x0}},
  {0x0219, 1, {0x47, 0x0}},
  {0x0342, 1, {0x04, 0x0}},//1200
  {0x0343, 1, {0xb0, 0x0}},
  {0x0259, 1, {0x05, 0x0}},
  {0x025a, 1, {0xa0, 0x0}},
  {0x0340, 1, {0x05, 0x0}},//1500
  {0x0341, 1, {0xdc, 0x0}},
  {0x0347, 1, {0x02, 0x0}},
  {0x0348, 1, {0x0a, 0x0}},
  {0x0349, 1, {0x08, 0x0}},
...
static CTL_SEN_DRV_TAB gc4023_sen_drv_tab = {
	sen_open_gc4023,
	sen_close_gc4023,
	sen_sleep_gc4023,
	sen_wakeup_gc4023,
	sen_write_reg_gc4023,
	sen_read_reg_gc4023,
	sen_chg_mode_gc4023,
	sen_chg_fps_gc4023,
	sen_set_info_gc4023,
...
static CTL_SEN_CMD sen_set_cmd_info_gc4023(UINT32 addr, UINT32 data)
{
	CTL_SEN_CMD cmd;
...
#if defined(__KERNEL__)
static void sen_load_cfg_from_compatible_gc4023(struct device_node *of_node)
{
...
static ER sen_open_gc4023(CTL_SEN_ID id)
{
	...
	#if defined(__KERNEL__)
	sen_i2c_reg_cb(sen_load_cfg_from_compatible_gc4023);
	#endif
...
static UINT32 sen_get_cmd_tab_gc4023(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	switch (mode) {
	case CTL_SEN_MODE_1:
		*cmd_tab = gc4023_mode_1;
		return sizeof(gc4023_mode_1) / sizeof(CTL_SEN_CMD);
...
static ER sen_chg_mode_gc4023(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj)
{
	...
	if (is_fastboot[id]) {
		...
		sen_set_chgmode_fps_gc4023(id, isp_builtin_get_chgmode_fps(id));
		sen_set_cur_fps_gc4023(id, isp_builtin_get_chgmode_fps(id));
		sen_set_gain_gc4023(id, &sensor_ctrl);
		sen_set_expt_gc4023(id, &sensor_ctrl);
		#endif
	...
	cmd_num = sen_get_cmd_tab_gc4023(chgmode_obj.mode, &p_cmd_list);
	if (p_cmd_list == NULL) {
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_gc4023(id, chgmode_obj.frame_rate);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_gc4023(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_gc4023(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
	...
	for (idx = 0; idx < cmd_num; idx++) {
		if (p_cmd_list[idx].addr == SEN_CMD_DELAY) {
			...
			cmd = sen_set_cmd_info_gc4023(0x0340, (sensor_vd >> 8) & 0x3F);
			rt |= sen_write_reg_gc4023(id, &cmd);
			cmd = sen_set_cmd_info_gc4023(0x0341, sensor_vd & 0xFF);
			rt |= sen_write_reg_gc4023(id, &cmd);		
			...
			sen_set_flip_gc4023(id, &flip);
		} else {
			cmd = sen_set_cmd_info_gc4023(p_cmd_list[idx].addr, p_cmd_list[idx].data[0]);
			rt |= sen_write_reg_gc4023(id, &cmd);
		}
	}
...
static ER sen_chg_fps_gc4023(CTL_SEN_ID id, UINT32 fps)
{
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_gc4023(id, fps);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_gc4023(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_gc4023(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
...
static ER sen_set_info_gc4023(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_SET_EXPT:
		sen_set_expt_gc4023(id, data);
		break;
	case CTL_SENDRV_CFGID_SET_GAIN:
		sen_set_gain_gc4023(id, data);
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		sen_set_flip_gc4023(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE1:
		sen_set_preset_gc4023(id, (ISP_SENSOR_PRESET_CTRL *)(data));
		break;
	default:
		return E_NOSPT;
	}
...
static ER sen_get_info_gc4023(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_EXPT:
		sen_get_expt_gc4023(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_GAIN:
		sen_get_gain_gc4023(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_BASIC:
		sen_get_attr_basic_gc4023((CTL_SENDRV_GET_ATTR_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_SIGNAL:
		sen_get_attr_signal_gc4023((CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_CMDIF:
		rt = sen_get_attr_cmdif_gc4023(id, (CTL_SENDRV_GET_ATTR_CMDIF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_IF:
		rt = sen_get_attr_if_gc4023((CTL_SENDRV_GET_ATTR_IF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_FPS:
		sen_get_fps_gc4023(id, (CTL_SENDRV_GET_FPS_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_SPEED:
		sen_get_speed_gc4023(id, (CTL_SENDRV_GET_SPEED_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_BASIC:
		sen_get_mode_basic_gc4023((CTL_SENDRV_GET_MODE_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_MIPI:
		sen_get_mode_mipi_gc4023((CTL_SENDRV_GET_MODE_MIPI_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODESEL:
		sen_get_modesel_gc4023((CTL_SENDRV_GET_MODESEL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_ROWTIME:
		sen_get_rowtime_gc4023(id, (CTL_SENDRV_GET_MODE_ROWTIME_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		rt = sen_get_flip_gc4023(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_PROBE_SEN:
		sen_ext_get_probe_sen_gc4023(id, (CTL_SENDRV_GET_PROBE_SEN_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE2:
		sen_get_min_expt_gc4023(id, data);
		break;	
	default:
		rt = E_NOSPT;
	}
...
static UINT32 sen_calc_chgmode_vd_gc4023(CTL_SEN_ID id, UINT32 fps)
{
	...
	sen_set_chgmode_fps_gc4023(id, fps);
	sen_set_cur_fps_gc4023(id, fps);
...
{
	{0x00, 0x00, 0x00, 0x07, 0x0f, 0x01, 0x00, 1000  },
	{0x80, 0x02, 0x00, 0x07, 0x0f, 0x01, 0x0B, 1188  },
	{0x01, 0x00, 0x00, 0x07, 0x0f, 0x01, 0x19, 1406  },
	{0x81, 0x02, 0x00, 0x07, 0x0f, 0x01, 0x2A, 1656  },
	{0x02, 0x00, 0x00, 0x08, 0x10, 0x02, 0x00, 2000  },
	{0x82, 0x02, 0x00, 0x08, 0x10, 0x02, 0x17, 2375  },
	{0x03, 0x00, 0x00, 0x08, 0x10, 0x02, 0x33, 2797  },
	{0x83, 0x02, 0x00, 0x09, 0x11, 0x03, 0x14, 3313  },
	{0x04, 0x00, 0x00, 0x0A, 0x12, 0x04, 0x00, 4000  },
	{0x80, 0x02, 0x20, 0x0A, 0x12, 0x04, 0x2F, 4734  },
	{0x01, 0x00, 0x20, 0x0B, 0x13, 0x05, 0x26, 5594  },
	{0x81, 0x02, 0x20, 0x0C, 0x14, 0x06, 0x28, 6641  },
	{0x02, 0x00, 0x20, 0x0D, 0x15, 0x08, 0x00, 8000  },
	{0x82, 0x02, 0x20, 0x0D, 0x15, 0x09, 0x1E, 9484  },
	{0x03, 0x00, 0x20, 0x0E, 0x16, 0x0B, 0x0C, 11203 },
	{0x83, 0x02, 0x20, 0x0E, 0x16, 0x0D, 0x11, 13266 },
	{0x04, 0x00, 0x20, 0x0F, 0x17, 0x10, 0x00, 16000 },
	{0x84, 0x02, 0x20, 0x0F, 0x17, 0x12, 0x3D, 18953 },
	{0x05, 0x00, 0x20, 0x10, 0x18, 0x16, 0x19, 22406 },
	{0x85, 0x02, 0x20, 0x10, 0x18, 0x1A, 0x22, 26547 },
	{0xb5, 0x04, 0x20, 0x11, 0x19, 0x20, 0x00, 32000 },
	{0x85, 0x05, 0x20, 0x11, 0x19, 0x25, 0x3A, 37922 },
	{0x05, 0x08, 0x20, 0x12, 0x1a, 0x2C, 0x33, 44797 },
	{0x45, 0x09, 0x20, 0x15, 0x1d, 0x35, 0x05, 53094 },
	{0x55, 0x0a, 0x20, 0x16, 0x1e, 0x40, 0x00, 64000 },
...
static void sen_set_gain_gc4023(CTL_SEN_ID id, void *param)
{
	...
	//DBG_ERR("digital_gain =%d,decimal=%d \r\n", digital_gain,decimal);
	
	cmd = sen_set_cmd_info_gc4023(0x031d, 0x2d);
	rt |= sen_write_reg_gc4023(id, &cmd);
	cmd = sen_set_cmd_info_gc4023(0x0614, (gain_data[0] >> 8) & 0xFF);
	rt |= sen_write_reg_gc4023(id, &cmd);
	cmd = sen_set_cmd_info_gc4023(0x0615, gain_data[0] & 0xFF);
	rt |= sen_write_reg_gc4023(id, &cmd);
	cmd = sen_set_cmd_info_gc4023(0x031d, 0x28);
	rt |= sen_write_reg_gc4023(id, &cmd);
	cmd = sen_set_cmd_info_gc4023(0x0218, (gain_data[1] >> 8) & 0xFF);
	rt |= sen_write_reg_gc4023(id, &cmd);
	cmd = sen_set_cmd_info_gc4023(0x1467, gain_data[1] & 0xFF);
	rt |= sen_write_reg_gc4023(id, &cmd);
	cmd = sen_set_cmd_info_gc4023(0x1468, (gain_data[2] >> 16) & 0xFF);
	rt |= sen_write_reg_gc4023(id, &cmd);
	cmd = sen_set_cmd_info_gc4023(0x00b8, (gain_data[0] >> 16) & 0xFF);
	rt |= sen_write_reg_gc4023(id, &cmd);
	cmd = sen_set_cmd_info_gc4023(0x00b9, (gain_data[1] >> 16) & 0xFF);
	rt |= sen_write_reg_gc4023(id, &cmd);
	cmd = sen_set_cmd_info_gc4023(0x0064, (decimal >> 6) & 0x0F);
	rt |= sen_write_reg_gc4023(id, &cmd);
	cmd = sen_set_cmd_info_gc4023(0x0065, (decimal & 0x3F) << 2);
	rt |= sen_write_reg_gc4023(id, &cmd);					
...
static void sen_set_expt_gc4023(CTL_SEN_ID id, void *param)
{
	...
	t_row = sen_calc_rowtime_gc4023(id, cur_sen_mode[id]);
	if (t_row == 0) {
	...
	// write exposure line
	// get fps
	chgmode_fps = sen_get_chgmode_fps_gc4023(id);
	...
	sen_set_cur_fps_gc4023(id, cur_fps);

	// calculate new vd
	sensor_vd = sen_calc_exp_vd_gc4023(id, cur_fps);
	...
	cmd = sen_set_cmd_info_gc4023(0x0202, (line >> 8) & 0x3F);
	rt |= sen_write_reg_gc4023(id, &cmd);
	cmd = sen_set_cmd_info_gc4023(0x0203, line & 0xFF);
	rt |= sen_write_reg_gc4023(id, &cmd);
...
static void sen_set_flip_gc4023(CTL_SEN_ID id, CTL_SEN_FLIP *flip)
{
	...
	cmd = sen_set_cmd_info_gc4023(0x022c, 0x00);
	rt |= sen_read_reg_gc4023(id, &cmd);
	temp_reg = cmd.data[0];
	...
	cmd = sen_set_cmd_info_gc4023(0x022c, temp_reg);
	rt |= sen_write_reg_gc4023(id, &cmd);
...
static void sen_ext_get_probe_sen_gc4023(CTL_SEN_ID id, CTL_SENDRV_GET_PROBE_SEN_PARAM *data)
{
	...
	cmd = sen_set_cmd_info_gc4023(0x3F0, 0x00);
	rt |= sen_read_reg_gc4023(id, &cmd);
	read_data_h = cmd.data[0];

	cmd = sen_set_cmd_info_gc4023(0x3F1, 0x00);
	rt |= sen_read_reg_gc4023(id, &cmd);
...
static void sen_get_fps_gc4023(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data)
{
	data->cur_fps = sen_get_cur_fps_gc4023(id);
	data->chg_fps = sen_get_chgmode_fps_gc4023(id);
}
...
static void sen_get_rowtime_gc4023(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data)
{
	...
	data->row_time = sen_calc_rowtime_gc4023(id, data->mode) * data->row_time_step;
}
...
#if defined(__FREERTOS)
void sen_get_i2c_id_gc4023(CTL_SEN_ID id, UINT32 *i2c_id)
{
	*i2c_id = sen_i2c[id].id;
}

void sen_get_i2c_addr_gc4023(CTL_SEN_ID id, UINT32 *i2c_addr)
{
	*i2c_addr = sen_i2c[id].addr;
}

int sen_init_gc4023(SENSOR_DTSI_INFO *info)
{
	...
	sprintf(compatible, "nvt,sen_gc4023");
	if (sen_common_check_compatible(compatible)) {
		...
		sprintf(node_path, "/sensor/sen_cfg/sen_gc4023");
	...
	reg_obj.pwr_ctrl = sen_pwr_ctrl_gc4023;
	reg_obj.det_plug_in = NULL;
	reg_obj.drv_tab = sen_get_drv_tab_gc4023();
	rt = ctl_sen_reg_sendrv("nvt_sen_gc4023", &reg_obj);
	if (rt != E_OK) {
...
void sen_exit_gc4023(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_gc4023");
}

#else
static int __init sen_init_gc4023(void)
{
...
static void __exit sen_exit_gc4023(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_gc4023");
}

module_init(sen_init_gc4023);
module_exit(sen_exit_gc4023);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION(SEN_GC4023_MODULE_NAME);
MODULE_LICENSE("GPL");
#endif
