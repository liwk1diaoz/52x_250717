...
#define TRANS_PROTOCOL TRANS_PROTOCOL_I2C
...
static void sen_set_expt_imx290(CTL_SEN_ID id, void *param)
{
	...
	UINT32 i = 0;
	ER rt = E_OK;
	static UINT32 pre_short_expt = 0xFFFFFFFF;
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		if((frame_cnt == 1)&&(pre_short_expt != 0xFFFFFFFF)) {
			line[frame_cnt] = pre_short_expt * 10 / t_row;
		} else {
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		pre_short_expt = sensor_ctrl->exp_time[1];
	}

	//DBG_DUMP("sen_expt = %d, %d, line = %d, %d, pre_short_expt = %d\r\n", sensor_ctrl->exp_time[0], sensor_ctrl->exp_time[1], line[0], line[1], pre_short_expt);
