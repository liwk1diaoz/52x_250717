...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_sc302iot, 1, 56, 000, 00);

//=============================================================================
// information
//=============================================================================
#define SEN_SC302IOT_MODULE_NAME     "sen_sc302iot"
...
#define SEN_I2C_COMPATIBLE "nvt,sen_sc302iot"
...
//=============================================================================
static CTL_SEN_DRV_TAB *sen_get_drv_tab_sc302iot(void);
static void sen_pwr_ctrl_sc302iot(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb);
static ER sen_open_sc302iot(CTL_SEN_ID id);
static ER sen_close_sc302iot(CTL_SEN_ID id);
static ER sen_sleep_sc302iot(CTL_SEN_ID id);
static ER sen_wakeup_sc302iot(CTL_SEN_ID id);
static ER sen_write_reg_sc302iot(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_read_reg_sc302iot(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_chg_mode_sc302iot(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj);
static ER sen_chg_fps_sc302iot(CTL_SEN_ID id, UINT32 fps);
static ER sen_set_info_sc302iot(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static ER sen_get_info_sc302iot(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static UINT32 sen_calc_chgmode_vd_sc302iot(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_calc_exp_vd_sc302iot(CTL_SEN_ID id, UINT32 fps);
static void sen_set_gain_sc302iot(CTL_SEN_ID id, void *param);
static void sen_set_expt_sc302iot(CTL_SEN_ID id, void *param);
static void sen_set_preset_sc302iot(CTL_SEN_ID id, ISP_SENSOR_PRESET_CTRL *ctrl);
static void sen_set_flip_sc302iot(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
static ER sen_get_flip_sc302iot(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
#if defined(__FREERTOS)
void sen_get_gain_sc302iot(CTL_SEN_ID id, void *param);
void sen_get_expt_sc302iot(CTL_SEN_ID id, void *param);
#else
static void sen_get_gain_sc302iot(CTL_SEN_ID id, void *param);
static void sen_get_expt_sc302iot(CTL_SEN_ID id, void *param);
#endif
static void sen_get_min_expt_sc302iot(CTL_SEN_ID id, void *param);
static void sen_get_mode_basic_sc302iot(CTL_SENDRV_GET_MODE_BASIC_PARAM *mode_basic);
static void sen_get_attr_basic_sc302iot(CTL_SENDRV_GET_ATTR_BASIC_PARAM *data);
static void sen_get_attr_signal_sc302iot(CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *data);
static ER sen_get_attr_cmdif_sc302iot(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_CMDIF_PARAM *data);
static ER sen_get_attr_if_sc302iot(CTL_SENDRV_GET_ATTR_IF_PARAM *data);
static void sen_get_fps_sc302iot(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data);
static void sen_get_speed_sc302iot(CTL_SEN_ID id, CTL_SENDRV_GET_SPEED_PARAM *data);
static void sen_get_mode_mipi_sc302iot(CTL_SENDRV_GET_MODE_MIPI_PARAM *data);
static void sen_get_modesel_sc302iot(CTL_SENDRV_GET_MODESEL_PARAM *data);
static UINT32 sen_calc_rowtime_sc302iot(CTL_SEN_ID id, CTL_SEN_MODE mode);
static UINT32 sen_calc_rowtime_step_sc302iot(CTL_SEN_ID id, CTL_SEN_MODE mode);
static void sen_get_rowtime_sc302iot(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data);
static void sen_set_cur_fps_sc302iot(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_cur_fps_sc302iot(CTL_SEN_ID id);
static void sen_set_chgmode_fps_sc302iot(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_chgmode_fps_sc302iot(CTL_SEN_ID id);
//=============================================================================
// global variable
//=============================================================================
...
static CTL_SEN_CMD sc302iot_mode_1[] = {
	//[paralist]
	{0x0103, 1, {0x01, 0x0}},
	{0x0100, 1, {0x00, 0x0}},
	{0x36e9, 1, {0x80, 0x0}},
...
static CTL_SEN_CMD sc302iot_mode_2[] = {
	//[paralist]
	{0x0103, 1, {0x01, 0x0}},
	{0x0100, 1, {0x00, 0x0}},
	{0x36e9, 1, {0x80, 0x0}},
...
static CTL_SEN_DRV_TAB sc302iot_sen_drv_tab = {
	sen_open_sc302iot,
	sen_close_sc302iot,
	sen_sleep_sc302iot,
	sen_wakeup_sc302iot,
	sen_write_reg_sc302iot,
	sen_read_reg_sc302iot,
	sen_chg_mode_sc302iot,
	sen_chg_fps_sc302iot,
	sen_set_info_sc302iot,
	sen_get_info_sc302iot,
};

static CTL_SEN_DRV_TAB *sen_get_drv_tab_sc302iot(void)
{
	return &sc302iot_sen_drv_tab;
}
...
static CTL_SEN_CMD sen_set_cmd_info_sc302iot(UINT32 addr, UINT32 data_length, UINT32 data0, UINT32 data1)
{
	CTL_SEN_CMD cmd;
...
#if defined(__KERNEL__)
static void sen_load_cfg_from_compatible_sc302iot(struct device_node *of_node)
{
...
static ER sen_open_sc302iot(CTL_SEN_ID id)
{
	...
	#if defined(__KERNEL__)
	sen_i2c_reg_cb(sen_load_cfg_from_compatible_sc302iot);
	#endif
...
static UINT32 sen_get_cmd_tab_sc302iot(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	switch (mode) {
	case CTL_SEN_MODE_1:
		*cmd_tab = sc302iot_mode_1;
		return sizeof(sc302iot_mode_1) / sizeof(CTL_SEN_CMD);

	case CTL_SEN_MODE_2:
		*cmd_tab = sc302iot_mode_2;
		return sizeof(sc302iot_mode_2) / sizeof(CTL_SEN_CMD);
...
static ER sen_chg_mode_sc302iot(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj)
{
	...
	if (is_fastboot[id]) {
		...
		sen_set_chgmode_fps_sc302iot(id, isp_builtin_get_chgmode_fps(id));
		sen_set_cur_fps_sc302iot(id, isp_builtin_get_chgmode_fps(id));
		sen_set_gain_sc302iot(id, &sensor_ctrl);
		sen_set_expt_sc302iot(id, &sensor_ctrl);
		#endif
	...
	cmd_num = sen_get_cmd_tab_sc302iot(chgmode_obj.mode, &p_cmd_list);
	if (p_cmd_list == NULL) {
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_sc302iot(id, chgmode_obj.frame_rate);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_sc302iot(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_sc302iot(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
	...
	for (idx = 0; idx < cmd_num; idx++) {
		if (p_cmd_list[idx].addr == SEN_CMD_DELAY) {
			...
			cmd = sen_set_cmd_info_sc302iot(0x320E, 1, (sensor_vd >> 8) & 0x7F, 0x00);
			rt |= sen_write_reg_sc302iot(id, &cmd);
			cmd = sen_set_cmd_info_sc302iot(0x320F, 1, sensor_vd & 0xFF, 0x00);
			rt |= sen_write_reg_sc302iot(id, &cmd);
			...
			sen_set_flip_sc302iot(id, &flip);
		} else {
			cmd = sen_set_cmd_info_sc302iot(p_cmd_list[idx].addr, p_cmd_list[idx].data_len, p_cmd_list[idx].data[0], p_cmd_list[idx].data[1]);
			rt |= sen_write_reg_sc302iot(id, &cmd);
		}
	}
...
static ER sen_chg_fps_sc302iot(CTL_SEN_ID id, UINT32 fps)
{
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_sc302iot(id, fps);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_sc302iot(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_sc302iot(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
...
static ER sen_set_info_sc302iot(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_SET_EXPT:
		sen_set_expt_sc302iot(id, data);
		break;
	case CTL_SENDRV_CFGID_SET_GAIN:
		sen_set_gain_sc302iot(id, data);
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		sen_set_flip_sc302iot(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE1:
		sen_set_preset_sc302iot(id, (ISP_SENSOR_PRESET_CTRL *)(data));
		break;
	default:
		return E_NOSPT;
	}
...
static ER sen_get_info_sc302iot(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_EXPT:
		sen_get_expt_sc302iot(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_GAIN:
		sen_get_gain_sc302iot(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_BASIC:
		sen_get_attr_basic_sc302iot((CTL_SENDRV_GET_ATTR_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_SIGNAL:
		sen_get_attr_signal_sc302iot((CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_CMDIF:
		rt = sen_get_attr_cmdif_sc302iot(id, (CTL_SENDRV_GET_ATTR_CMDIF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_IF:
		rt = sen_get_attr_if_sc302iot((CTL_SENDRV_GET_ATTR_IF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_FPS:
		sen_get_fps_sc302iot(id, (CTL_SENDRV_GET_FPS_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_SPEED:
		sen_get_speed_sc302iot(id, (CTL_SENDRV_GET_SPEED_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_BASIC:
		sen_get_mode_basic_sc302iot((CTL_SENDRV_GET_MODE_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_MIPI:
		sen_get_mode_mipi_sc302iot((CTL_SENDRV_GET_MODE_MIPI_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODESEL:
		sen_get_modesel_sc302iot((CTL_SENDRV_GET_MODESEL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_ROWTIME:
		sen_get_rowtime_sc302iot(id, (CTL_SENDRV_GET_MODE_ROWTIME_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		rt = sen_get_flip_sc302iot(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE2:
		sen_get_min_expt_sc302iot(id, data);
		break;
	default:
		rt = E_NOSPT;
	}
...
static UINT32 sen_calc_chgmode_vd_sc302iot(CTL_SEN_ID id, UINT32 fps)
{
	...
	sen_set_chgmode_fps_sc302iot(id, fps);
	sen_set_cur_fps_sc302iot(id, fps);
...
static void sen_set_gain_sc302iot(CTL_SEN_ID id, void *param)
{
	...
	//Set analog gain
	cmd = sen_set_cmd_info_sc302iot(0x3E09, 1, data1[0] & 0xFF, 0x0);
	rt |= sen_write_reg_sc302iot(id, &cmd);
	//Set digiral gain
	cmd = sen_set_cmd_info_sc302iot(0x3E06, 1, data2[0] & 0xFF, 0x0);
	rt |= sen_write_reg_sc302iot(id, &cmd);	
	cmd = sen_set_cmd_info_sc302iot(0x3E07, 1, data3[0] & 0xFF, 0x0);
	rt |= sen_write_reg_sc302iot(id, &cmd);

	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {   
		//Short exposure frame gain
		//Set analog gain
		cmd = sen_set_cmd_info_sc302iot(0x3E13, 1, data1[1] & 0xFF, 0x0);
		rt |= sen_write_reg_sc302iot(id, &cmd);
		//Set digital high gain
		cmd = sen_set_cmd_info_sc302iot(0x3E10, 1, data2[1] & 0xFF, 0x0);
		rt |= sen_write_reg_sc302iot(id, &cmd);
		cmd = sen_set_cmd_info_sc302iot(0x3E11, 1, data3[1] & 0xFF, 0x0);
		rt |= sen_write_reg_sc302iot(id, &cmd);		
	}
...
static void sen_set_expt_sc302iot(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		t_row = sen_calc_rowtime_sc302iot(id, cur_sen_mode[id]);
		if (0 == t_row) {
	...
	// Get fps
	chgmode_fps = sen_get_chgmode_fps_sc302iot(id);
	...
	sen_set_cur_fps_sc302iot(id, cur_fps);

	// Calculate new vd
	sensor_vd = sen_calc_exp_vd_sc302iot(id, cur_fps);
	...
	cmd = sen_set_cmd_info_sc302iot(0x320E, 1, (sensor_vd >> 8) & 0x7F, 0);
	rt |= sen_write_reg_sc302iot(id, &cmd);
	cmd = sen_set_cmd_info_sc302iot(0x320F, 1, sensor_vd & 0xFF, 0);
	rt |= sen_write_reg_sc302iot(id, &cmd);
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		...
		cmd = sen_set_cmd_info_sc302iot(0x3E00, 1, (line[0] >> 12) & 0x0F, 0);
		rt |= sen_write_reg_sc302iot(id, &cmd);
		cmd = sen_set_cmd_info_sc302iot(0x3E01, 1, (line[0] >> 4) & 0xFF , 0);
		rt |= sen_write_reg_sc302iot(id, &cmd);
		cmd = sen_set_cmd_info_sc302iot(0x3E02, 1, (line[0] & 0x0F) << 4, 0);
		rt |= sen_write_reg_sc302iot(id, &cmd);
	} else if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		//Check max exp line reg
		if (line[0] > (MAX_EXPOSURE_LINE)) {
		...
		cmd = sen_set_cmd_info_sc302iot(0x3E22, 1, (line[1] >> 12) & 0x0F, 0);
		rt |= sen_write_reg_sc302iot(id, &cmd);
		cmd = sen_set_cmd_info_sc302iot(0x3E04, 1, (line[1] >> 4) & 0xFF , 0);
		rt |= sen_write_reg_sc302iot(id, &cmd);
		cmd = sen_set_cmd_info_sc302iot(0x3E05, 1, (line[1] & 0x0F) << 4, 0);
		rt |= sen_write_reg_sc302iot(id, &cmd);
	}
...
static void sen_set_flip_sc302iot(CTL_SEN_ID id, CTL_SEN_FLIP *flip)
{
	...
	cmd = sen_set_cmd_info_sc302iot(0x3221, 1, 0x0, 0x0);
	rt |= sen_read_reg_sc302iot(id, &cmd);
...
static void sen_get_fps_sc302iot(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data)
{
	data->cur_fps = sen_get_cur_fps_sc302iot(id);
	data->chg_fps = sen_get_chgmode_fps_sc302iot(id);
}
...
static void sen_get_rowtime_sc302iot(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data)
{
	data->row_time_step = sen_calc_rowtime_step_sc302iot(id, data->mode);
	data->row_time = sen_calc_rowtime_sc302iot(id, data->mode) * (data->row_time_step);
}
...
#if defined(__FREERTOS)
void sen_get_i2c_id_sc302iot(CTL_SEN_ID id, UINT32 *i2c_id)
{
	*i2c_id = sen_i2c[id].id;
}

void sen_get_i2c_addr_sc302iot(CTL_SEN_ID id, UINT32 *i2c_addr)
{
	*i2c_addr = sen_i2c[id].addr;
}

int sen_init_sc302iot(SENSOR_DTSI_INFO *info)
{
	...
	sprintf(compatible, "nvt,sen_sc302iot");
	if (sen_common_check_compatible(compatible)) {
		...
		sprintf(node_path, "/sensor/sen_cfg/sen_sc302iot");
	...
	reg_obj.pwr_ctrl = sen_pwr_ctrl_sc302iot;
	reg_obj.det_plug_in = NULL;
	reg_obj.drv_tab = sen_get_drv_tab_sc302iot();
	rt = ctl_sen_reg_sendrv("nvt_sen_sc302iot", &reg_obj);
	if (rt != E_OK) {
...
void sen_exit_sc302iot(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_sc302iot");
}

#else
static int __init sen_init_sc302iot(void)
{
...
static void __exit sen_exit_sc302iot(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_sc302iot");
}

module_init(sen_init_sc302iot);
module_exit(sen_exit_sc302iot);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION(SEN_SC302IOT_MODULE_NAME);
MODULE_LICENSE("GPL");
#endif
