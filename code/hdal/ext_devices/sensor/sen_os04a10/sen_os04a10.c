...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_os04a10, 1, 51, 000, 00);
...
#define HCG_ENABLE                   1
#define HCG_LCG_RATIO                4
#define TOTAL_HCG_LCG_RATIO          16
...
static CTL_SENDRV_GET_MODE_BASIC_PARAM mode_basic_param[SEN_MAX_MODE] = {
	...
	},
	{
		...
		{0, 1720, 0, 2093},
		CTL_SEN_RATIO(16, 9),
		{1000, (248000 / (TOTAL_HCG_LCG_RATIO / HCG_LCG_RATIO))},
		100
	}
};
...
static CTL_SEN_CMD os04a10_mode_4[] = {
	//;------------------------------------
	//@@ OS04A10 2lane 2688x1520 LinearHCG 30fps
	//100 99 2688 1520
	{0x0100, 1, {0x00, 0x0}},	
	{0x0103, 1, {0x01, 0x0}},
	//;delay 1 ms
...
static CTL_SEN_CMD os04a10_mode_5[] = {
	{0x0100, 1, {0x00, 0x0}},
	{0x0103, 1, {0x01, 0x0}},
	//;delay 1 ms
	{SEN_CMD_DELAY, 1, {5, 0x0}},
	...
	{0x380c, 1, {0x06, 0x0}},
	{0x380d, 1, {0xb8, 0x0}},
	{0x380e, 1, {0x08, 0x0}},//06
	{0x380f, 1, {0x2D, 0x0}},//58
	{0x3811, 1, {0x09, 0x0}},//08
	{0x3813, 1, {0x09, 0x0}},//08
	{0x3814, 1, {0x01, 0x0}},
	{0x3815, 1, {0x01, 0x0}},
	{0x3816, 1, {0x01, 0x0}},
	{0x3817, 1, {0x01, 0x0}},
...
static UINT32 sen_get_cmd_tab_os04a10(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	...
	case CTL_SEN_MODE_4:
		*cmd_tab = os04a10_mode_4;
		return sizeof(os04a10_mode_4) / sizeof(CTL_SEN_CMD);
		
	case CTL_SEN_MODE_5:
		*cmd_tab = os04a10_mode_5;
		return sizeof(os04a10_mode_5) / sizeof(CTL_SEN_CMD);
				...
				case ISP_SENSOR_PRESET_DEFAULT:
					...
					if (mode_basic_param[cur_sen_mode[id]].mode_type == (UINT64)CTL_SEN_MODE_DCG_HDR) {
						if (mode_basic_param[cur_sen_mode[id]].frame_num == 2) {
							sensor_ctrl.gain_ratio[1] = sen_preset[id].gain_ratio >> 4;
						}
					}
...
static void sen_set_gain_os04a10(CTL_SEN_ID id, void *param)
{
	...
	if ((mode_basic_param[cur_sen_mode[id]].mode_type == (UINT64)CTL_SEN_MODE_LINEAR) || (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR)) {
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
			...
			if (0 == frame_cnt) {
				if (((TOTAL_HCG_LCG_RATIO / HCG_LCG_RATIO) * 1000) > ((sensor_ctrl->gain_ratio[0]) / HCG_LCG_RATIO)) {
					temp_gain[0] = TOTAL_HCG_LCG_RATIO * 1000 / HCG_LCG_RATIO;
					if (temp_gain[0] > (mode_basic_param[cur_sen_mode[id]].gain.max)) {
						temp_gain[0] = (mode_basic_param[cur_sen_mode[id]].gain.max);
					}
				} else {
					temp_gain[0] = sensor_ctrl->gain_ratio[0] / HCG_LCG_RATIO;
				}
			}
		}
...
//set expt to group 0 begin
		cmd = sen_set_cmd_info_os04a10(0x3501, 1, (exp_line[id][0] >> 8) & 0xFF, 0);
		rt |= sen_write_reg_os04a10(id, &cmd);
		cmd = sen_set_cmd_info_os04a10(0x3502, 1, exp_line[id][0] & 0xFF , 0);
		rt |= sen_write_reg_os04a10(id, &cmd);
...
//set expt to group 0 begin
		...
		cmd = sen_set_cmd_info_os04a10(0x3541, 1, (exp_line[id][1] >> 8) & 0xFF, 0);
		rt |= sen_write_reg_os04a10(id, &cmd);
		cmd = sen_set_cmd_info_os04a10(0x3542, 1, exp_line[id][1] & 0xFF , 0);
		rt |= sen_write_reg_os04a10(id, &cmd);
//set expt to group 0 end
#endif
		cmd = sen_set_cmd_info_os04a10(0x3508, 1, (data1[0] >> 4) & 0x0F, 0x0);
...
static void sen_get_modesel_os04a10(CTL_SENDRV_GET_MODESEL_PARAM *data)
{
	...
	if (data->data_lane == CTL_SEN_DATALANE_2) {
		...
		if (data->frame_num == 1) {
			...
			if (data->mode_type_sel == (UINT64)CTL_SEN_MODE_DCG_HDR) {
				if ((data->size.w <= 2688) && (data->size.h <= 1520)) {
					if (data->frame_rate <= 3000) {
...
static UINT32 sen_calc_rowtime_step_os04a10(CTL_SEN_ID id, CTL_SEN_MODE mode)
{
	...
	|| (mode_basic_param[mode].mode_type == (UINT64)CTL_SEN_MODE_DCG_HDR))	{
		div_step = 1;
	}
