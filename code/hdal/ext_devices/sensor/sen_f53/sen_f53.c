...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_f53, 1, 45, 000, 00);

//=============================================================================
// information
//=============================================================================
#define SEN_F53_MODULE_NAME     "sen_f53"
...
#define MAX_SHORT_EXPOSURE_LINE   0x88 //reg0x06 ==>(0x88 * 2) + 1 = 273; if 1:8 max short = 2400 / 9 = 266 ; 0x05 must < 0x06 - 5

#define SEN_I2C_ADDR 0x80 >> 1
#define SEN_I2C_COMPATIBLE "nvt,sen_f53"
...
//=============================================================================
static CTL_SEN_DRV_TAB *sen_get_drv_tab_f53(void);
static void sen_pwr_ctrl_f53(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb);
static ER sen_open_f53(CTL_SEN_ID id);
static ER sen_close_f53(CTL_SEN_ID id);
static ER sen_sleep_f53(CTL_SEN_ID id);
static ER sen_wakeup_f53(CTL_SEN_ID id);
static ER sen_write_reg_f53(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_read_reg_f53(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_chg_mode_f53(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj);
static ER sen_chg_fps_f53(CTL_SEN_ID id, UINT32 fps);
static ER sen_set_info_f53(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static ER sen_get_info_f53(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static UINT32 sen_calc_chgmode_vd_f53(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_calc_exp_vd_f53(CTL_SEN_ID id, UINT32 fps);
static void sen_set_gain_f53(CTL_SEN_ID id, void *param);
static void sen_set_expt_f53(CTL_SEN_ID id, void *param);
static void sen_set_preset_f53(CTL_SEN_ID id, ISP_SENSOR_PRESET_CTRL *ctrl);
static void sen_set_flip_f53(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
static ER sen_get_flip_f53(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
#if defined(__FREERTOS)
void sen_get_gain_f53(CTL_SEN_ID id, void *param);
void sen_get_expt_f53(CTL_SEN_ID id, void *param);
#else
static void sen_get_gain_f53(CTL_SEN_ID id, void *param);
static void sen_get_expt_f53(CTL_SEN_ID id, void *param);
#endif
static void sen_get_min_expt_f53(CTL_SEN_ID id, void *param);
static void sen_get_mode_basic_f53(CTL_SENDRV_GET_MODE_BASIC_PARAM *mode_basic);
static void sen_get_attr_basic_f53(CTL_SENDRV_GET_ATTR_BASIC_PARAM *data);
static void sen_get_attr_signal_f53(CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *data);
static ER sen_get_attr_cmdif_f53(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_CMDIF_PARAM *data);
static ER sen_get_attr_if_f53(CTL_SENDRV_GET_ATTR_IF_PARAM *data);
static void sen_get_fps_f53(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data);
static void sen_get_speed_f53(CTL_SEN_ID id, CTL_SENDRV_GET_SPEED_PARAM *data);
static void sen_get_mode_mipi_f53(CTL_SENDRV_GET_MODE_MIPI_PARAM *data);
static void sen_get_modesel_f53(CTL_SENDRV_GET_MODESEL_PARAM *data);
static UINT32 sen_calc_rowtime_f53(CTL_SEN_ID id, CTL_SEN_MODE mode);
static UINT32 sen_calc_rowtime_step_f53(CTL_SEN_ID id, CTL_SEN_MODE mode);
static void sen_get_rowtime_f53(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data);
static void sen_set_cur_fps_f53(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_cur_fps_f53(CTL_SEN_ID id);
static void sen_set_chgmode_fps_f53(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_chgmode_fps_f53(CTL_SEN_ID id);
static void sen_get_hdr_vd_diff_row_f53(CTL_SEN_ID id, CTL_SENDRV_GET_MFR_OUTPUT_TIMING_PARAM *data);
//static void sen_get_iadj_f53(CTL_SEN_ID id,  CTL_SENDRV_GET_MODE_MANUAL_IADJ *data);
//=============================================================================
// global variable
//=============================================================================
...
static CTL_SEN_DRV_TAB f53_sen_drv_tab = {
	sen_open_f53,
	sen_close_f53,
	sen_sleep_f53,
	sen_wakeup_f53,
...
static CTL_SEN_CMD sen_set_cmd_info_f53(UINT32 addr, UINT32 data_length, UINT32 data0, UINT32 data1)
{
	CTL_SEN_CMD cmd;
...
#if defined(__KERNEL__)
static void sen_load_cfg_from_compatible_f53(struct device_node *of_node)
{
...
static ER sen_open_f53(CTL_SEN_ID id)
{
	...
	#if defined(__KERNEL__)
	sen_i2c_reg_cb(sen_load_cfg_from_compatible_f53);
	#endif
...
static UINT32 sen_get_cmd_tab_f53(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	switch (mode) {
	case CTL_SEN_MODE_1:
		*cmd_tab = f53_mode_1;
		return sizeof(f53_mode_1) / sizeof(CTL_SEN_CMD);

	case CTL_SEN_MODE_2:
		*cmd_tab = f53_mode_2;
		return sizeof(f53_mode_2) / sizeof(CTL_SEN_CMD);
...
static ER sen_chg_mode_f53(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj)
{
	...
	if (is_fastboot[id]) {
		...
		sen_set_chgmode_fps_f53(id, isp_builtin_get_chgmode_fps(id));
		sen_set_cur_fps_f53(id, isp_builtin_get_chgmode_fps(id));
		sen_set_gain_f53(id, &sensor_ctrl);
		sen_set_expt_f53(id, &sensor_ctrl);
		#endif
	...
	cmd_num = sen_get_cmd_tab_f53(chgmode_obj.mode, &p_cmd_list);
	if (p_cmd_list == NULL) {
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_f53(id, chgmode_obj.frame_rate);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_f53(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_f53(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
	...
	for (idx = 0; idx < cmd_num; idx++) {
		if (p_cmd_list[idx].addr == SEN_CMD_DELAY) {
			...
			cmd = sen_set_cmd_info_f53(0x23, 1, (sensor_vd >> 8) & 0xFF, 0x00);
			rt |= sen_write_reg_f53(id, &cmd);
			cmd = sen_set_cmd_info_f53(0x22, 1, sensor_vd & 0xFF, 0x00);
			rt |= sen_write_reg_f53(id, &cmd);
			...
			sen_set_flip_f53(id, &flip);
		} else {
			cmd = sen_set_cmd_info_f53(p_cmd_list[idx].addr, p_cmd_list[idx].data_len, p_cmd_list[idx].data[0], p_cmd_list[idx].data[1]);
			rt |= sen_write_reg_f53(id, &cmd);
		}
	}
...
static ER sen_chg_fps_f53(CTL_SEN_ID id, UINT32 fps)
{
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_f53(id, fps);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_f53(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_f53(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
...
static ER sen_set_info_f53(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_SET_EXPT:
		sen_set_expt_f53(id, data);
		break;
	case CTL_SENDRV_CFGID_SET_GAIN:
		sen_set_gain_f53(id, data);
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		sen_set_flip_f53(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE1:
		sen_set_preset_f53(id, (ISP_SENSOR_PRESET_CTRL *)(data));
		break;
	default:
		return E_NOSPT;
	}
...
static ER sen_get_info_f53(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_EXPT:
		sen_get_expt_f53(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_GAIN:
		sen_get_gain_f53(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_BASIC:
		sen_get_attr_basic_f53((CTL_SENDRV_GET_ATTR_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_SIGNAL:
		sen_get_attr_signal_f53((CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_CMDIF:
		rt = sen_get_attr_cmdif_f53(id, (CTL_SENDRV_GET_ATTR_CMDIF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_IF:
		rt = sen_get_attr_if_f53((CTL_SENDRV_GET_ATTR_IF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_FPS:
		sen_get_fps_f53(id, (CTL_SENDRV_GET_FPS_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_SPEED:
		sen_get_speed_f53(id, (CTL_SENDRV_GET_SPEED_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_BASIC:
		sen_get_mode_basic_f53((CTL_SENDRV_GET_MODE_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_MIPI:
		sen_get_mode_mipi_f53((CTL_SENDRV_GET_MODE_MIPI_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODESEL:
		sen_get_modesel_f53((CTL_SENDRV_GET_MODESEL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_ROWTIME:
		sen_get_rowtime_f53(id, (CTL_SENDRV_GET_MODE_ROWTIME_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		rt = sen_get_flip_f53(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE2:
		sen_get_min_expt_f53(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_MFR_OUTPUT_TIMING:
		sen_get_hdr_vd_diff_row_f53(id, (CTL_SENDRV_GET_MFR_OUTPUT_TIMING_PARAM *)(data));
		break;		
/*
	case CTL_SENDRV_CFGID_GET_MODE_MIPI_MANUAL_IADJ:
		sen_get_iadj_f53(id, (CTL_SENDRV_GET_MODE_MANUAL_IADJ *)(data));
		break;
*/		
	default:
		  rt = E_NOSPT;
...
static UINT32 sen_calc_chgmode_vd_f53(CTL_SEN_ID id, UINT32 fps)
{
	...
	sen_set_chgmode_fps_f53(id, fps);
	sen_set_cur_fps_f53(id, fps);
...
static void sen_set_gain_f53(CTL_SEN_ID id, void *param)
{
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		cmd = sen_set_cmd_info_f53(0x00, 1, data1[0], 0); 
		rt |= sen_write_reg_f53(id, &cmd);			
	} else if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		//because 2 frame use the same gain,try set short frame value is better than long frame	value.
		cmd = sen_set_cmd_info_f53(0x00, 1, data1[1], 0); 
		rt |= sen_write_reg_f53(id, &cmd);
	}
...
static void sen_set_expt_f53(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		t_row = sen_calc_rowtime_f53(id, cur_sen_mode[id]);
		if (0 == t_row) {
	...
	// Write exposure line
	// Get fps
	chgmode_fps = sen_get_chgmode_fps_f53(id);
	...
	sen_set_cur_fps_f53(id, cur_fps);

	// Calculate new vd
	sensor_vd = sen_calc_exp_vd_f53(id, cur_fps);
	...
	cmd = sen_set_cmd_info_f53(0x23, 1, (sensor_vd >> 8) & 0xFF, 0);
	rt |= sen_write_reg_f53(id, &cmd);	
	cmd = sen_set_cmd_info_f53(0x22, 1, sensor_vd & 0xFF, 0); 
	rt |= sen_write_reg_f53(id, &cmd);
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		...
		cmd = sen_set_cmd_info_f53(0x02, 1, (line[0] >> 8) & 0xFF, 0);
		rt |= sen_write_reg_f53(id, &cmd); 	
		cmd = sen_set_cmd_info_f53(0x01, 1, (line[0] & 0xFF) , 0);
		rt |= sen_write_reg_f53(id, &cmd);
	} else if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		if (line[0] > MAX_EXPOSURE_LINE) {
		...
		cmd = sen_set_cmd_info_f53(0x08, 1, ((line[1]) >> 8) & 0x01 , 0);
		rt |= sen_write_reg_f53(id, &cmd);				
		cmd = sen_set_cmd_info_f53(0x05, 1, (line[1]) & 0xFF , 0);
		rt |= sen_write_reg_f53(id, &cmd);
	}
...
static void sen_set_flip_f53(CTL_SEN_ID id, CTL_SEN_FLIP *flip)
{
	...
	cmd = sen_set_cmd_info_f53(0x12, 1, 0x0, 0x0);
	rt |= sen_read_reg_f53(id, &cmd);
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		if (final_option & CTL_SEN_SUPPORT_PROPERTY_FLIP) {
			cmd = sen_set_cmd_info_f53(0x28, 1, 0x19, 0);
			rt |= sen_write_reg_f53(id, &cmd);
		} else {
			cmd = sen_set_cmd_info_f53(0x28, 1, 0x11, 0);
			rt |= sen_write_reg_f53(id, &cmd);
		}
	} else if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		if (final_option & CTL_SEN_SUPPORT_PROPERTY_FLIP) {
			cmd = sen_set_cmd_info_f53(0x28, 1, 0x29, 0);
			rt |= sen_write_reg_f53(id, &cmd);
		} else {
			cmd = sen_set_cmd_info_f53(0x28, 1, 0x21, 0);
			rt |= sen_write_reg_f53(id, &cmd);
		}		
	}
...
static void sen_get_hdr_vd_diff_row_f53(CTL_SEN_ID id, CTL_SENDRV_GET_MFR_OUTPUT_TIMING_PARAM *data)
{
	if (mode_basic_param[data->mode].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		if (((mode_basic_param[data->mode].frame_num) == data->num)  && ( mode_basic_param[data->mode].mode == (data->mode))) {
			if (2 == data->num) {
				data->diff_row[0] = MAX_SHORT_EXPOSURE_LINE * 2 + 1;
				data->diff_row_vd[0] = MAX_SHORT_EXPOSURE_LINE * 2 + 1;				
			}
		}
	}
}
...
static void sen_get_fps_f53(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data)
{
	data->cur_fps = sen_get_cur_fps_f53(id);
	data->chg_fps = sen_get_chgmode_fps_f53(id);
}
...
static void sen_get_rowtime_f53(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data)
{
	data->row_time_step = sen_calc_rowtime_step_f53(id, data->mode);	
	data->row_time = sen_calc_rowtime_f53(id, data->mode) * (data->row_time_step);	
}
...
#if defined(__FREERTOS)
void sen_get_i2c_id_f53(CTL_SEN_ID id, UINT32 *i2c_id)
{
	*i2c_id = sen_i2c[id].id;
}

void sen_get_i2c_addr_f53(CTL_SEN_ID id, UINT32 *i2c_addr)
{
	*i2c_addr = sen_i2c[id].addr;
}

int sen_init_f53(SENSOR_DTSI_INFO *info)
{
	...
	sprintf(compatible, "nvt,sen_f53");
	if (sen_common_check_compatible(compatible)) {
		...
		sprintf(node_path, "/sensor/sen_cfg/sen_f53");
	...
	reg_obj.pwr_ctrl = sen_pwr_ctrl_f53;
	reg_obj.det_plug_in = NULL;
	reg_obj.drv_tab = sen_get_drv_tab_f53();
	rt = ctl_sen_reg_sendrv("nvt_sen_f53", &reg_obj);
	if (rt != E_OK) {
...
void sen_exit_f53(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_f53");
}

#else
static int __init sen_init_f53(void)
{
...
static void __exit sen_exit_f53(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_f53");
}

module_init(sen_init_f53);
module_exit(sen_exit_f53);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION(SEN_F53_MODULE_NAME);
MODULE_LICENSE("GPL");
#endif
