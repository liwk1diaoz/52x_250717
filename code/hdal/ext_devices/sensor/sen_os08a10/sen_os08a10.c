...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_os08a10, 1, 47, 000, 00);
...
#define MIN_HDR_EXPOSURE_LINE       8
#define L2SOFFSET  4
#define BLACKLINE  8
#define ZEROLINE   2
...
static CTL_SEN_CMD os08a10_mode_1[] = {
	//@@ 0 2 Res 3844X2164 4lane MIPI1440Mbps Linear10 60fps MCLK24M
	{0x0100, 1, {0x00, 0x0}},
	{0x0103, 1, {0x01, 0x0}},
	{0x0303, 1, {0x01, 0x0}},
	{0x0305, 1, {0x5a, 0x0}},
	...
	{0x380c, 1, {0x08, 0x0}},//0x04
	{0x380d, 1, {0x18, 0x0}},//0x0c
	{0x380e, 1, {0x09, 0x0}},
	{0x380f, 1, {0x0a, 0x0}},
	{0x3813, 1, {0x10, 0x0}},
	{0x3814, 1, {0x01, 0x0}},
...
static CTL_SEN_CMD os08a10_mode_2[] = {
	//@@ 0 2 Res 3844X2164 4lane MIPI1440Mbps Linear10 60fps MCLK24M
	{0x0100, 1, {0x00, 0x0}},
	{0x0103, 1, {0x01, 0x0}},
	{0x0303, 1, {0x01, 0x0}},
...
static CTL_SEN_CMD os08a10_mode_3[] = {
	//@@ 2 2001 Res 3844X2164 4lane MIPI1440Mbps HDRVC10 30fps MCLK24M
	{0x0100, 1, {0x00, 0x0}},
	{0x0103, 1, {0x01, 0x0}},
	{0x0303, 1, {0x01, 0x0}},
	{0x0305, 1, {0x5a, 0x0}},
...
static UINT32 last_short_expline[CTL_SEN_ID_MAX] = {0};
static UINT32 last_sensor_vd[CTL_SEN_ID_MAX] = {0};
...
static UINT32 sen_get_cmd_tab_os08a10(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	...
	case CTL_SEN_MODE_2:
		*cmd_tab = os08a10_mode_2;
		return sizeof(os08a10_mode_2) / sizeof(CTL_SEN_CMD);

	case CTL_SEN_MODE_3:
		*cmd_tab = os08a10_mode_3;
		return sizeof(os08a10_mode_3) / sizeof(CTL_SEN_CMD);
...
static void sen_set_gain_os08a10(CTL_SEN_ID id, void *param)
{
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		// Write sensor gain
		//Set analog gain
		cmd = sen_set_cmd_info_os08a10(0x3508, 1, ((data1[0]) >> 8) & 0x3F, 0x0);
		rt |= sen_write_reg_os08a10(id, &cmd);
		...
		cmd = sen_set_cmd_info_os08a10(0x3509, 1, (data1[0])& 0xFF, 0x0);
		rt |= sen_write_reg_os08a10(id, &cmd);
		cmd = sen_set_cmd_info_os08a10(0x350C, 1, ((data1[1]) >> 8) & 0x3F, 0x0);
		rt |= sen_write_reg_os08a10(id, &cmd);
		cmd = sen_set_cmd_info_os08a10(0x350D, 1, (data1[1])& 0xFF, 0x0);
		rt |= sen_write_reg_os08a10(id, &cmd);	
		...
		cmd = sen_set_cmd_info_os08a10(0x350B, 1, (data2[0])& 0xFF, 0x0);
		rt |= sen_write_reg_os08a10(id, &cmd);
		cmd = sen_set_cmd_info_os08a10(0x350E, 1, ((data2[1]) >> 8) & 0x3F, 0x0);
		rt |= sen_write_reg_os08a10(id, &cmd);
		cmd = sen_set_cmd_info_os08a10(0x350F, 1, (data2[1])& 0xFF, 0x0);
		rt |= sen_write_reg_os08a10(id, &cmd);
	}
...
static void sen_set_expt_os08a10(CTL_SEN_ID id, void *param)
{
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {		
		...
		if (sensor_vd > (mode_basic_param[cur_sen_mode[id]].signal_info.vd_period)) {
			if (sensor_vd < last_sensor_vd[id]) { // vts and exp down
				cmd = sen_set_cmd_info_os08a10(0x380E, 1, ((last_sensor_vd[id]) >> 8) & 0xFF, 0);
				rt |= sen_write_reg_os08a10(id, &cmd);
				cmd = sen_set_cmd_info_os08a10(0x380F, 1, (last_sensor_vd[id]) & 0xFF, 0);
				rt |= sen_write_reg_os08a10(id, &cmd);
			} else {
				cmd = sen_set_cmd_info_os08a10(0x380E, 1, (sensor_vd >> 8) & 0xFF, 0);
				rt |= sen_write_reg_os08a10(id, &cmd);
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {					
		...
		if (0 != last_short_expline[id]) {
			if (((line[1]) + L2SOFFSET) > ((4 * ZEROLINE) + (2 * BLACKLINE))) {
				if ((line[1]) >= (MAX_VD_PERIOD + (last_short_expline[id]) - sensor_vd - 2)) {
					line[1] = MAX_VD_PERIOD + (last_short_expline[id]) - sensor_vd - 2 - 1;
				}  
			} else if (((last_short_expline[id]) + L2SOFFSET) <= ((4 * ZEROLINE) + (2 * BLACKLINE))) {
				if ((line[1]) >= (MAX_VD_PERIOD - L2SOFFSET - sensor_vd  + (4 * ZEROLINE) + (2 * BLACKLINE) - 2)) {
					line[1] = MAX_VD_PERIOD - L2SOFFSET - sensor_vd  + (4 * ZEROLINE) + (2 * BLACKLINE) - 2 - 1;
				} 				
			}
		}

		if (line[0] > (sensor_vd - line[1] - L2SOFFSET - 4)) {
			line[0] = sensor_vd - line[1] - L2SOFFSET - 4;
		}
		...
		cmd = sen_set_cmd_info_os08a10(0x3502, 1, line[0] & 0xFF , 0);
		rt |= sen_write_reg_os08a10(id, &cmd);
		cmd = sen_set_cmd_info_os08a10(0x3511, 1, (line[1] >> 8) & 0xFF, 0);
		rt |= sen_write_reg_os08a10(id, &cmd);
		cmd = sen_set_cmd_info_os08a10(0x3512, 1, line[1] & 0xFF , 0);
		rt |= sen_write_reg_os08a10(id, &cmd);	

		last_short_expline[id] = line[1];
		last_sensor_vd[id] = sensor_vd;
	}
