...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_f37, 1, 48, 000, 00);
...
static void sen_set_gain_f37(CTL_SEN_ID id, void *param)
{
	...
	CTL_SEN_CMD cmd;
	ER rt = E_OK;
	UINT16 a_gain = 0, i = 0, tbl_idx = NUM_OF_WDRGAINSET;
	...
	if(tbl_idx < 1){
		a_gain = gain_wdr_table[0].a_gain;
		//Write Sensor reg
		cmd = sen_set_cmd_info_f37(0x00, 1, a_gain, 0);
		rt |= sen_write_reg_f37(id, &cmd);
	} else {
		a_gain = gain_wdr_table[tbl_idx-1].a_gain;
		//Write Sensor reg
		cmd = sen_set_cmd_info_f37(0x00, 1, a_gain, 0);
		rt |= sen_write_reg_f37(id, &cmd);
	}
