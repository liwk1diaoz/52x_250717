...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_sc2210, 1, 51, 000, 00);

//=============================================================================
// information
//=============================================================================
#define SEN_SC2210_MODULE_NAME     "sen_sc2210"
...
#define SEN_I2C_COMPATIBLE "nvt,sen_sc2210"
...
//=============================================================================
static CTL_SEN_DRV_TAB *sen_get_drv_tab_sc2210(void);
static void sen_pwr_ctrl_sc2210(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb);
static ER sen_open_sc2210(CTL_SEN_ID id);
static ER sen_close_sc2210(CTL_SEN_ID id);
static ER sen_sleep_sc2210(CTL_SEN_ID id);
static ER sen_wakeup_sc2210(CTL_SEN_ID id);
static ER sen_write_reg_sc2210(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_read_reg_sc2210(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_chg_mode_sc2210(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj);
static ER sen_chg_fps_sc2210(CTL_SEN_ID id, UINT32 fps);
static ER sen_set_info_sc2210(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static ER sen_get_info_sc2210(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static UINT32 sen_calc_chgmode_vd_sc2210(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_calc_exp_vd_sc2210(CTL_SEN_ID id, UINT32 fps);
static void sen_set_gain_sc2210(CTL_SEN_ID id, void *param);
static void sen_set_expt_sc2210(CTL_SEN_ID id, void *param);
static void sen_set_preset_sc2210(CTL_SEN_ID id, ISP_SENSOR_PRESET_CTRL *ctrl);
static void sen_set_flip_sc2210(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
static ER sen_get_flip_sc2210(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
#if defined(__FREERTOS)
void sen_get_gain_sc2210(CTL_SEN_ID id, void *param);
void sen_get_expt_sc2210(CTL_SEN_ID id, void *param);
#else
static void sen_get_gain_sc2210(CTL_SEN_ID id, void *param);
static void sen_get_expt_sc2210(CTL_SEN_ID id, void *param);
#endif
static void sen_get_min_expt_sc2210(CTL_SEN_ID id, void *param);
static void sen_get_mode_basic_sc2210(CTL_SENDRV_GET_MODE_BASIC_PARAM *mode_basic);
static void sen_get_attr_basic_sc2210(CTL_SENDRV_GET_ATTR_BASIC_PARAM *data);
static void sen_get_attr_signal_sc2210(CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *data);
static ER sen_get_attr_cmdif_sc2210(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_CMDIF_PARAM *data);
static ER sen_get_attr_if_sc2210(CTL_SENDRV_GET_ATTR_IF_PARAM *data);
static void sen_get_fps_sc2210(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data);
static void sen_get_speed_sc2210(CTL_SEN_ID id, CTL_SENDRV_GET_SPEED_PARAM *data);
static void sen_get_mode_mipi_sc2210(CTL_SENDRV_GET_MODE_MIPI_PARAM *data);
static void sen_get_modesel_sc2210(CTL_SENDRV_GET_MODESEL_PARAM *data);
static UINT32 sen_calc_rowtime_sc2210(CTL_SEN_ID id, CTL_SEN_MODE mode);
static UINT32 sen_calc_rowtime_step_sc2210(CTL_SEN_ID id, CTL_SEN_MODE mode);
static void sen_get_rowtime_sc2210(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data);
static void sen_set_cur_fps_sc2210(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_cur_fps_sc2210(CTL_SEN_ID id);
static void sen_set_chgmode_fps_sc2210(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_chgmode_fps_sc2210(CTL_SEN_ID id);
//=============================================================================
// global variable
//=============================================================================
...
static CTL_SEN_CMD sc2210_mode_1[] = {
	//[paralist]
	{0x0103, 1, {0x01, 0x0}},
	{0x0100, 1, {0x00, 0x0}},
	{0x36e9, 1, {0x80, 0x0}},
...
static CTL_SEN_DRV_TAB sc2210_sen_drv_tab = {
	sen_open_sc2210,
	sen_close_sc2210,
	sen_sleep_sc2210,
	sen_wakeup_sc2210,
	sen_write_reg_sc2210,
	sen_read_reg_sc2210,
	sen_chg_mode_sc2210,
	sen_chg_fps_sc2210,
	sen_set_info_sc2210,
...
static CTL_SEN_CMD sen_set_cmd_info_sc2210(UINT32 addr, UINT32 data_length, UINT32 data0, UINT32 data1)
{
	CTL_SEN_CMD cmd;
...
#if defined(__KERNEL__)
static void sen_load_cfg_from_compatible_sc2210(struct device_node *of_node)
{
...
static ER sen_open_sc2210(CTL_SEN_ID id)
{
	...
	#if defined(__KERNEL__)
	sen_i2c_reg_cb(sen_load_cfg_from_compatible_sc2210);
	#endif
...
static UINT32 sen_get_cmd_tab_sc2210(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	switch (mode) {
	case CTL_SEN_MODE_1:
		*cmd_tab = sc2210_mode_1;
		return sizeof(sc2210_mode_1) / sizeof(CTL_SEN_CMD);
...
static ER sen_chg_mode_sc2210(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj)
{
	...
	if (is_fastboot[id]) {
		...
		sen_set_chgmode_fps_sc2210(id, isp_builtin_get_chgmode_fps(id));
		sen_set_cur_fps_sc2210(id, isp_builtin_get_chgmode_fps(id));
		sen_set_gain_sc2210(id, &sensor_ctrl);
		sen_set_expt_sc2210(id, &sensor_ctrl);
		#endif
	...
	cmd_num = sen_get_cmd_tab_sc2210(chgmode_obj.mode, &p_cmd_list);
	if (p_cmd_list == NULL) {
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_sc2210(id, chgmode_obj.frame_rate);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_sc2210(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_sc2210(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
	...
	for (idx = 0; idx < cmd_num; idx++) {
		if (p_cmd_list[idx].addr == SEN_CMD_DELAY) {
			...
			cmd = sen_set_cmd_info_sc2210(0x320E, 1, (sensor_vd >> 8) & 0xFF, 0x00);
			rt |= sen_write_reg_sc2210(id, &cmd);
			cmd = sen_set_cmd_info_sc2210(0x320F, 1, sensor_vd & 0xFF, 0x00);
			rt |= sen_write_reg_sc2210(id, &cmd);
			...
			sen_set_flip_sc2210(id, &flip);
		} else {
			cmd = sen_set_cmd_info_sc2210(p_cmd_list[idx].addr, p_cmd_list[idx].data_len, p_cmd_list[idx].data[0], p_cmd_list[idx].data[1]);
			rt |= sen_write_reg_sc2210(id, &cmd);
		}
	}
...
static ER sen_chg_fps_sc2210(CTL_SEN_ID id, UINT32 fps)
{
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_sc2210(id, fps);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_sc2210(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_sc2210(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
...
static ER sen_set_info_sc2210(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_SET_EXPT:
		sen_set_expt_sc2210(id, data);
		break;
	case CTL_SENDRV_CFGID_SET_GAIN:
		sen_set_gain_sc2210(id, data);
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		sen_set_flip_sc2210(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE1:
		sen_set_preset_sc2210(id, (ISP_SENSOR_PRESET_CTRL *)(data));
		break;
	default:
		return E_NOSPT;
	}
...
static ER sen_get_info_sc2210(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_EXPT:
		sen_get_expt_sc2210(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_GAIN:
		sen_get_gain_sc2210(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_BASIC:
		sen_get_attr_basic_sc2210((CTL_SENDRV_GET_ATTR_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_SIGNAL:
		sen_get_attr_signal_sc2210((CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_CMDIF:
		rt = sen_get_attr_cmdif_sc2210(id, (CTL_SENDRV_GET_ATTR_CMDIF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_IF:
		rt = sen_get_attr_if_sc2210((CTL_SENDRV_GET_ATTR_IF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_FPS:
		sen_get_fps_sc2210(id, (CTL_SENDRV_GET_FPS_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_SPEED:
		sen_get_speed_sc2210(id, (CTL_SENDRV_GET_SPEED_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_BASIC:
		sen_get_mode_basic_sc2210((CTL_SENDRV_GET_MODE_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_MIPI:
		sen_get_mode_mipi_sc2210((CTL_SENDRV_GET_MODE_MIPI_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODESEL:
		sen_get_modesel_sc2210((CTL_SENDRV_GET_MODESEL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_ROWTIME:
		sen_get_rowtime_sc2210(id, (CTL_SENDRV_GET_MODE_ROWTIME_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		rt = sen_get_flip_sc2210(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE2:
		sen_get_min_expt_sc2210(id, data);
		break;
	default:
		rt = E_NOSPT;
	}
...
static UINT32 sen_calc_chgmode_vd_sc2210(CTL_SEN_ID id, UINT32 fps)
{
	...
	sen_set_chgmode_fps_sc2210(id, fps);
	sen_set_cur_fps_sc2210(id, fps);
...
static void sen_set_gain_sc2210(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		if (54070 >= (sensor_ctrl->gain_ratio[frame_cnt])) {
			if (2000 > (sensor_ctrl->gain_ratio[frame_cnt])) {
				analog_gain_h = 0x3;
				analog_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 8 / 125;				
			} else if (3406 > (sensor_ctrl->gain_ratio[frame_cnt])) {
				analog_gain_h = 0x7;
				analog_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 8 / 250;						
			} else if (6812 > (sensor_ctrl->gain_ratio[frame_cnt])) {
				analog_gain_h = 0x23;
				analog_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 5 / 266;			
			} else if (13624 > (sensor_ctrl->gain_ratio[frame_cnt])) {
				analog_gain_h = 0x27;
				analog_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 1000 / 39 / 2729;								
			} else if (27248 > (sensor_ctrl->gain_ratio[frame_cnt])) {
				analog_gain_h = 0x2F;
				analog_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 8 / 1703;									
			} else {
				analog_gain_h = 0x3F;
				analog_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 5 / 16 / 133;									
			}
			
			if (0x07 == analog_gain_h) {
				if (0x6C < analog_gain_l) {
					analog_gain_l = 0x6C;
			...
			if (108140 > (sensor_ctrl->gain_ratio[frame_cnt])) {
				digital_gain_h = 0x0;  		
			} else if (216280 > (sensor_ctrl->gain_ratio[frame_cnt])) {
				digital_gain_h = 0x1;
			} else if (432560 > (sensor_ctrl->gain_ratio[frame_cnt])) {
				digital_gain_h = 0x3;
			} else if (865120 > (sensor_ctrl->gain_ratio[frame_cnt])) {
				digital_gain_h = 0x7;
			} else {
				digital_gain_h = 0xF;				
			}

			digital_gain_l = (sensor_ctrl->gain_ratio[frame_cnt]) * 128 / (digital_gain_h + 1) / 54070;
	...
	cmd = sen_set_cmd_info_sc2210(0x3E08, 1, data1[0] & 0xFF, 0x0);
	rt |= sen_write_reg_sc2210(id, &cmd);

	//Set analog low gain
	cmd = sen_set_cmd_info_sc2210(0x3E09, 1, data2[0] & 0xFF, 0x0);
	rt |= sen_write_reg_sc2210(id, &cmd);

	//Set digital high gain
	cmd = sen_set_cmd_info_sc2210(0x3E06, 1, (data3[0] >> 16) & 0xFF, 0x0);
	rt |= sen_write_reg_sc2210(id, &cmd);

	//Set digital high gain
	cmd = sen_set_cmd_info_sc2210(0x3E07, 1, data3[0] & 0xFF, 0x0);
	rt |= sen_write_reg_sc2210(id, &cmd);
...
static void sen_set_expt_sc2210(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		t_row = sen_calc_rowtime_sc2210(id, cur_sen_mode[id]);
		if (0 == t_row) {
	...
	// Get fps
	chgmode_fps = sen_get_chgmode_fps_sc2210(id);
	...
	sen_set_cur_fps_sc2210(id, cur_fps);

	// Calculate new vd
	sensor_vd = sen_calc_exp_vd_sc2210(id, cur_fps);
	...
	cmd = sen_set_cmd_info_sc2210(0x320E, 1, (sensor_vd >> 8) & 0xFF, 0);
	rt |= sen_write_reg_sc2210(id, &cmd);
	cmd = sen_set_cmd_info_sc2210(0x320F, 1, sensor_vd & 0xFF, 0);
	rt |= sen_write_reg_sc2210(id, &cmd);
	...
	cmd = sen_set_cmd_info_sc2210(0x3E00, 1, (line[0] >> 12) & 0x0F, 0);
	rt |= sen_write_reg_sc2210(id, &cmd);
	cmd = sen_set_cmd_info_sc2210(0x3E01, 1, (line[0] >> 4) & 0xFF , 0);
	rt |= sen_write_reg_sc2210(id, &cmd);
	cmd = sen_set_cmd_info_sc2210(0x3E02, 1, (line[0] & 0x0F) << 4, 0);
	rt |= sen_write_reg_sc2210(id, &cmd);
...
static void sen_set_flip_sc2210(CTL_SEN_ID id, CTL_SEN_FLIP *flip)
{
	...
	cmd = sen_set_cmd_info_sc2210(0x3221, 1, 0x0, 0x0);
	rt |= sen_read_reg_sc2210(id, &cmd);
...
static void sen_get_fps_sc2210(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data)
{
	data->cur_fps = sen_get_cur_fps_sc2210(id);
	data->chg_fps = sen_get_chgmode_fps_sc2210(id);
}
...
static void sen_get_rowtime_sc2210(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data)
{
	data->row_time_step = sen_calc_rowtime_step_sc2210(id, data->mode);
	data->row_time = sen_calc_rowtime_sc2210(id, data->mode) * (data->row_time_step);
}
...
#if defined(__FREERTOS)
void sen_get_i2c_id_sc2210(CTL_SEN_ID id, UINT32 *i2c_id)
{
	*i2c_id = sen_i2c[id].id;
}

void sen_get_i2c_addr_sc2210(CTL_SEN_ID id, UINT32 *i2c_addr)
{
	*i2c_addr = sen_i2c[id].addr;
}

int sen_init_sc2210(SENSOR_DTSI_INFO *info)
{
	...
	sprintf(compatible, "nvt,sen_sc2210");
	if (sen_common_check_compatible(compatible)) {
		...
		sprintf(node_path, "/sensor/sen_cfg/sen_sc2210");
	...
	reg_obj.pwr_ctrl = sen_pwr_ctrl_sc2210;
	reg_obj.det_plug_in = NULL;
	reg_obj.drv_tab = sen_get_drv_tab_sc2210();
	rt = ctl_sen_reg_sendrv("nvt_sen_sc2210", &reg_obj);
	if (rt != E_OK) {
...
void sen_exit_sc2210(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_sc2210");
}

#else
static int __init sen_init_sc2210(void)
{
...
static void __exit sen_exit_sc2210(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_sc2210");
}

module_init(sen_init_sc2210);
module_exit(sen_exit_sc2210);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION(SEN_SC2210_MODULE_NAME);
MODULE_LICENSE("GPL");
#endif
