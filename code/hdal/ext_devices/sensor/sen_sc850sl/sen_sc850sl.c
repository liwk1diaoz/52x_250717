...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_sc850sl, 1, 55, 000, 00);

//=============================================================================
// information
//=============================================================================
#define SEN_SC850SL_MODULE_NAME     "sen_sc850sl"
...
#define MAX_SHORT_EXPOSURE_LINE    266//0x10A 0x3e23 0x3e24

#define SEN_I2C_ADDR 0x60 >> 1
#define SEN_I2C_COMPATIBLE "nvt,sen_sc850sl"
...
//=============================================================================
static CTL_SEN_DRV_TAB *sen_get_drv_tab_sc850sl(void);
static void sen_pwr_ctrl_sc850sl(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb);
static ER sen_open_sc850sl(CTL_SEN_ID id);
static ER sen_close_sc850sl(CTL_SEN_ID id);
static ER sen_sleep_sc850sl(CTL_SEN_ID id);
static ER sen_wakeup_sc850sl(CTL_SEN_ID id);
static ER sen_write_reg_sc850sl(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_read_reg_sc850sl(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_chg_mode_sc850sl(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj);
static ER sen_chg_fps_sc850sl(CTL_SEN_ID id, UINT32 fps);
static ER sen_set_info_sc850sl(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static ER sen_get_info_sc850sl(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static UINT32 sen_calc_chgmode_vd_sc850sl(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_calc_exp_vd_sc850sl(CTL_SEN_ID id, UINT32 fps);
static void sen_set_gain_sc850sl(CTL_SEN_ID id, void *param);
static void sen_set_expt_sc850sl(CTL_SEN_ID id, void *param);
static void sen_set_preset_sc850sl(CTL_SEN_ID id, ISP_SENSOR_PRESET_CTRL *ctrl);
static void sen_set_flip_sc850sl(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
static ER sen_get_flip_sc850sl(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
#if defined(__FREERTOS)
void sen_get_gain_sc850sl(CTL_SEN_ID id, void *param);
void sen_get_expt_sc850sl(CTL_SEN_ID id, void *param);
#else
static void sen_get_gain_sc850sl(CTL_SEN_ID id, void *param);
static void sen_get_expt_sc850sl(CTL_SEN_ID id, void *param);
#endif
static void sen_get_min_expt_sc850sl(CTL_SEN_ID id, void *param);
static void sen_get_mode_basic_sc850sl(CTL_SENDRV_GET_MODE_BASIC_PARAM *mode_basic);
static void sen_get_attr_basic_sc850sl(CTL_SENDRV_GET_ATTR_BASIC_PARAM *data);
static void sen_get_attr_signal_sc850sl(CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *data);
static ER sen_get_attr_cmdif_sc850sl(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_CMDIF_PARAM *data);
static ER sen_get_attr_if_sc850sl(CTL_SENDRV_GET_ATTR_IF_PARAM *data);
static void sen_get_fps_sc850sl(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data);
static void sen_get_speed_sc850sl(CTL_SEN_ID id, CTL_SENDRV_GET_SPEED_PARAM *data);
static void sen_get_mode_mipi_sc850sl(CTL_SENDRV_GET_MODE_MIPI_PARAM *data);
static void sen_get_modesel_sc850sl(CTL_SENDRV_GET_MODESEL_PARAM *data);
static UINT32 sen_calc_rowtime_sc850sl(CTL_SEN_ID id, CTL_SEN_MODE mode);
static UINT32 sen_calc_rowtime_step_sc850sl(CTL_SEN_ID id, CTL_SEN_MODE mode);
static void sen_get_rowtime_sc850sl(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data);
static void sen_set_cur_fps_sc850sl(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_cur_fps_sc850sl(CTL_SEN_ID id);
static void sen_set_chgmode_fps_sc850sl(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_chgmode_fps_sc850sl(CTL_SEN_ID id);
//=============================================================================
// global variable
//=============================================================================
...
static CTL_SENDRV_GET_MODE_BASIC_PARAM mode_basic_param[SEN_MAX_MODE] = {
	...
	},
	{
		...
		CTL_SEN_FMT_POGRESSIVE,
		{3840, 2162},
		{{0, 2, 3840, 2160}, {0, 2, 3840, 2160}, {0, 0, 0, 0}, {0, 0, 0, 0}},
		{3840, 2160},
		{0, 550, 0, 4500},
		CTL_SEN_RATIO(16, 9),
		{1000, 396872},
...
static CTL_SEN_CMD sc850sl_mode_1[] = {
	//[paralist]
	{0x0103, 1, {0x01, 0x0}},
	{0x0100, 1, {0x00, 0x0}},
	{0x36e9, 1, {0x80, 0x0}},
...
static CTL_SEN_CMD sc850sl_mode_2[] = {
	//[paralist]
	{0x0103, 1, {0x01, 0x0}},
	{0x0100, 1, {0x00, 0x0}},
	{0x36e9, 1, {0x80, 0x0}},
...
static CTL_SEN_CMD sc850sl_mode_3[] = {
	//[paralist]
	{0x0103, 1, {0x01, 0x0}},
	{0x0100, 1, {0x00, 0x0}},
	{0x36e9, 1, {0x80, 0x0}},
...
static BOOL first_flag[CTL_SEN_ID_MAX] = {FALSE};  

static CTL_SEN_DRV_TAB sc850sl_sen_drv_tab = {
	sen_open_sc850sl,
	sen_close_sc850sl,
	sen_sleep_sc850sl,
	sen_wakeup_sc850sl,
	sen_write_reg_sc850sl,
	sen_read_reg_sc850sl,
	sen_chg_mode_sc850sl,
	sen_chg_fps_sc850sl,
	sen_set_info_sc850sl,
	sen_get_info_sc850sl,
};

static CTL_SEN_DRV_TAB *sen_get_drv_tab_sc850sl(void)
{
	return &sc850sl_sen_drv_tab;
}
...
static CTL_SEN_CMD sen_set_cmd_info_sc850sl(UINT32 addr, UINT32 data_length, UINT32 data0, UINT32 data1)
{
	CTL_SEN_CMD cmd;
...
#if defined(__KERNEL__)
static void sen_load_cfg_from_compatible_sc850sl(struct device_node *of_node)
{
...
static ER sen_open_sc850sl(CTL_SEN_ID id)
{
	...
	#if defined(__KERNEL__)
	sen_i2c_reg_cb(sen_load_cfg_from_compatible_sc850sl);
	#endif
...
static UINT32 sen_get_cmd_tab_sc850sl(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	switch (mode) {
	case CTL_SEN_MODE_1:
		*cmd_tab = sc850sl_mode_1;
		return sizeof(sc850sl_mode_1) / sizeof(CTL_SEN_CMD);

	case CTL_SEN_MODE_2:
		*cmd_tab = sc850sl_mode_2;
		return sizeof(sc850sl_mode_2) / sizeof(CTL_SEN_CMD);

	case CTL_SEN_MODE_3:
		*cmd_tab = sc850sl_mode_3;
		return sizeof(sc850sl_mode_3) / sizeof(CTL_SEN_CMD);
...
static ER sen_chg_mode_sc850sl(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj)
{
	...
	if (is_fastboot[id]) {
		...
		sen_set_chgmode_fps_sc850sl(id, isp_builtin_get_chgmode_fps(id));
		sen_set_cur_fps_sc850sl(id, isp_builtin_get_chgmode_fps(id));
		sen_set_gain_sc850sl(id, &sensor_ctrl);
		sen_set_expt_sc850sl(id, &sensor_ctrl);
		#endif
	...
	cmd_num = sen_get_cmd_tab_sc850sl(chgmode_obj.mode, &p_cmd_list);
	if (p_cmd_list == NULL) {
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_sc850sl(id, chgmode_obj.frame_rate);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_sc850sl(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_sc850sl(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
	...
	for (idx = 0; idx < cmd_num; idx++) {
		if (p_cmd_list[idx].addr == SEN_CMD_DELAY) {
			...
			cmd = sen_set_cmd_info_sc850sl(0x320E, 1, (sensor_vd >> 8) & 0x7F, 0x00);
			rt |= sen_write_reg_sc850sl(id, &cmd);
			cmd = sen_set_cmd_info_sc850sl(0x320F, 1, sensor_vd & 0xFF, 0x00);
			rt |= sen_write_reg_sc850sl(id, &cmd);
			if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
				cmd = sen_set_cmd_info_sc850sl(0x3230, 1, ((sensor_vd - 1) >> 8) & 0xFF , 0x0);
				rt |= sen_write_reg_sc850sl(id, &cmd);	
				cmd = sen_set_cmd_info_sc850sl(0x3231, 1, (sensor_vd - 1) & 0xFF, 0x0);
				rt |= sen_write_reg_sc850sl(id, &cmd);
			}				
			...
			sen_set_flip_sc850sl(id, &flip);
		} else {
			cmd = sen_set_cmd_info_sc850sl(p_cmd_list[idx].addr, p_cmd_list[idx].data_len, p_cmd_list[idx].data[0], p_cmd_list[idx].data[1]);
			rt |= sen_write_reg_sc850sl(id, &cmd);
...
static ER sen_chg_fps_sc850sl(CTL_SEN_ID id, UINT32 fps)
{
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_sc850sl(id, fps);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_sc850sl(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_sc850sl(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
...
static ER sen_set_info_sc850sl(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_SET_EXPT:
		sen_set_expt_sc850sl(id, data);
		break;
	case CTL_SENDRV_CFGID_SET_GAIN:
		sen_set_gain_sc850sl(id, data);
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		sen_set_flip_sc850sl(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE1:
		sen_set_preset_sc850sl(id, (ISP_SENSOR_PRESET_CTRL *)(data));
		break;
	default:
		return E_NOSPT;
	}
...
static ER sen_get_info_sc850sl(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_EXPT:
		sen_get_expt_sc850sl(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_GAIN:
		sen_get_gain_sc850sl(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_BASIC:
		sen_get_attr_basic_sc850sl((CTL_SENDRV_GET_ATTR_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_SIGNAL:
		sen_get_attr_signal_sc850sl((CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_CMDIF:
		rt = sen_get_attr_cmdif_sc850sl(id, (CTL_SENDRV_GET_ATTR_CMDIF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_IF:
		rt = sen_get_attr_if_sc850sl((CTL_SENDRV_GET_ATTR_IF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_FPS:
		sen_get_fps_sc850sl(id, (CTL_SENDRV_GET_FPS_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_SPEED:
		sen_get_speed_sc850sl(id, (CTL_SENDRV_GET_SPEED_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_BASIC:
		sen_get_mode_basic_sc850sl((CTL_SENDRV_GET_MODE_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_MIPI:
		sen_get_mode_mipi_sc850sl((CTL_SENDRV_GET_MODE_MIPI_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODESEL:
		sen_get_modesel_sc850sl((CTL_SENDRV_GET_MODESEL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_ROWTIME:
		sen_get_rowtime_sc850sl(id, (CTL_SENDRV_GET_MODE_ROWTIME_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		rt = sen_get_flip_sc850sl(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE2:
		sen_get_min_expt_sc850sl(id, data);
		break;
	default:
		rt = E_NOSPT;
	}
...
static UINT32 sen_calc_chgmode_vd_sc850sl(CTL_SEN_ID id, UINT32 fps)
{
	...
	sen_set_chgmode_fps_sc850sl(id, fps);
	sen_set_cur_fps_sc850sl(id, fps);
...
static void sen_set_gain_sc850sl(CTL_SEN_ID id, void *param)
{
	...
	CTL_SEN_CMD cmd;
	UINT32 a_gain_h = 0, a_gain_l = 0, d_gain = 0, temp_gain = 0;
	ER rt = E_OK;
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		d_gain = (sensor_ctrl->gain_ratio[frame_cnt]) / 49609; //max a gain = 49609 
		...
		temp_gain = (sensor_ctrl->gain_ratio[frame_cnt]) / d_gain;

		if (2000 > temp_gain) {
			a_gain_h = 0x03;
			a_gain_l = temp_gain * 64 / 1000;
		} else if (3125 > temp_gain) {
			a_gain_h= 0x07;
			a_gain_l = temp_gain * 322 / 10000;
			if (0x63 < a_gain_l) { 
				a_gain_l = 0x63;
			}	
		} else if (6250 > temp_gain) {
			a_gain_h = 0x23;
			a_gain_l = temp_gain * 206 / 10000;
			if (0x7F < a_gain_l) { 
				a_gain_l = 0x7F;
			}	
		} else if (12500 > temp_gain) {
			a_gain_h = 0x27;
			a_gain_l = temp_gain * 103 / 10000;
			if (0x7F < a_gain_l) { 
				a_gain_l = 0x7F;
			}		
		} else if (25000 > temp_gain) {
			a_gain_h = 0x2F;
			a_gain_l = temp_gain * 515 / 100000;
			if (0x7F < a_gain_l) { 
				a_gain_l = 0x7F;
			}			
		} else {
			a_gain_h = 0x3F;
			a_gain_l = temp_gain * 2564 / 1000000;
			if (0x7F < a_gain_l) { 
				a_gain_l = 0x7F;
			}
		}
		...
		data3[frame_cnt] = (d_gain - 1);
	}

	//Long exposure frame gain
	//Set analog high gain
	cmd = sen_set_cmd_info_sc850sl(0x3E08, 1, data1[0] & 0xFF, 0x0);
	rt |= sen_write_reg_sc850sl(id, &cmd);
	//Set analog low gain
	cmd = sen_set_cmd_info_sc850sl(0x3E09, 1, data2[0] & 0xFF, 0x0);
	rt |= sen_write_reg_sc850sl(id, &cmd);
	
	//Set digital gain
	cmd = sen_set_cmd_info_sc850sl(0x3E06, 1, data3[0] & 0xFF, 0x0);
	rt |= sen_write_reg_sc850sl(id, &cmd);

	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		//Short exposure frame gain
		//Set analog high gain
		cmd = sen_set_cmd_info_sc850sl(0x3E12, 1, data1[1] & 0xFF, 0x0);
		rt |= sen_write_reg_sc850sl(id, &cmd);
		//Set analog low gain
		cmd = sen_set_cmd_info_sc850sl(0x3E13, 1, data2[1] & 0xFF, 0x0);
		rt |= sen_write_reg_sc850sl(id, &cmd);
	
		//Set digital gain
		cmd = sen_set_cmd_info_sc850sl(0x3E10, 1, data3[1] & 0xFF, 0x0);
		rt |= sen_write_reg_sc850sl(id, &cmd);
	}

	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		if(first_flag[id]) {
			cmd = sen_set_cmd_info_sc850sl(0x3800, 1, 0x00, 0x0);
			rt |= sen_write_reg_sc850sl(id, &cmd);

			if (0x03 == (data1[0])) { // < 2x
				cmd = sen_set_cmd_info_sc850sl(0x363c, 1, 0x05, 0x0);
				rt |= sen_write_reg_sc850sl(id, &cmd);
			} else {
				cmd = sen_set_cmd_info_sc850sl(0x363c, 1, 0x07, 0x0);
				rt |= sen_write_reg_sc850sl(id, &cmd);
			}
			
			cmd = sen_set_cmd_info_sc850sl(0x3800, 1, 0x10, 0x0);
			rt |= sen_write_reg_sc850sl(id, &cmd);	
			cmd = sen_set_cmd_info_sc850sl(0x3800, 1, 0x60, 0x0);
			rt |= sen_write_reg_sc850sl(id, &cmd);
		}
	}
...
static void sen_set_expt_sc850sl(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		t_row = sen_calc_rowtime_sc850sl(id, cur_sen_mode[id]);
		if (0 == t_row) {
	...
	// Get fps
	chgmode_fps = sen_get_chgmode_fps_sc850sl(id);
	...
	sen_set_cur_fps_sc850sl(id, cur_fps);

	// Calculate new vd
	sensor_vd = sen_calc_exp_vd_sc850sl(id, cur_fps);
	...
	cmd = sen_set_cmd_info_sc850sl(0x320E, 1, (sensor_vd >> 8) & 0x7F, 0);
	rt |= sen_write_reg_sc850sl(id, &cmd);

	cmd = sen_set_cmd_info_sc850sl(0x320F, 1, sensor_vd & 0xFF, 0);
	rt |= sen_write_reg_sc850sl(id, &cmd);
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		...
		cmd = sen_set_cmd_info_sc850sl(0x3E00, 1, (line[0] >> 12) & 0x0F, 0);
		rt |= sen_write_reg_sc850sl(id, &cmd);
		cmd = sen_set_cmd_info_sc850sl(0x3E01, 1, (line[0] >> 4) & 0xFF , 0);
		rt |= sen_write_reg_sc850sl(id, &cmd);
		cmd = sen_set_cmd_info_sc850sl(0x3E02, 1, (line[0] & 0x0F) << 4, 0);
		rt |= sen_write_reg_sc850sl(id, &cmd);
		...
		max_short_expline = ((sensor_vd - 10) / 17) + 2;
		...
		//Check max exp line
		if ((line[0]) > (sensor_vd - max_short_expline - NON_HDR_LONG_EXPOSURE_LINE)) {
			line[0] = sensor_vd - max_short_expline - NON_HDR_LONG_EXPOSURE_LINE;
		}
		if (0 != (line[0] % 2)) {
			line[0] = (line[0]) / 2 * 2;
		...
		if ((line[1]) > (max_short_expline - NON_HDR_SHORT_EXPOSURE_LINE)) {
			line[1] = max_short_expline - NON_HDR_SHORT_EXPOSURE_LINE;
		}
		if (0 != (line[1] % 2)) {
			line[1] = (line[1]) / 2 * 2;
		...
		cmd = sen_set_cmd_info_sc850sl(0x3E22, 1, (line[1] >> 12) & 0x0F, 0);
		rt |= sen_write_reg_sc850sl(id, &cmd);
		cmd = sen_set_cmd_info_sc850sl(0x3E04, 1, (line[1] >> 4) & 0xFF , 0);
		rt |= sen_write_reg_sc850sl(id, &cmd);
		cmd = sen_set_cmd_info_sc850sl(0x3E05, 1, (line[1] & 0x0F) << 4, 0);
		rt |= sen_write_reg_sc850sl(id, &cmd);

		cmd = sen_set_cmd_info_sc850sl(0x3E23, 1, (max_short_expline >> 8) & 0xFF , 0);
		rt |= sen_write_reg_sc850sl(id, &cmd);
		cmd = sen_set_cmd_info_sc850sl(0x3E24, 1, max_short_expline & 0xFF, 0);
		rt |= sen_write_reg_sc850sl(id, &cmd);

		if(first_flag[id]) {
			cmd = sen_set_cmd_info_sc850sl(0x3800, 1, 0x01, 0x0);
			rt |= sen_write_reg_sc850sl(id, &cmd);	
			cmd = sen_set_cmd_info_sc850sl(0x3230, 1, ((sensor_vd - 1) >> 8) & 0xFF , 0x0);
			rt |= sen_write_reg_sc850sl(id, &cmd);	
			cmd = sen_set_cmd_info_sc850sl(0x3231, 1, (sensor_vd - 1) & 0xFF, 0x0);
			rt |= sen_write_reg_sc850sl(id, &cmd);		
			cmd = sen_set_cmd_info_sc850sl(0x3800, 1, 0x11, 0x0);
			rt |= sen_write_reg_sc850sl(id, &cmd);	
			cmd = sen_set_cmd_info_sc850sl(0x3800, 1, 0x61, 0x0);
			rt |= sen_write_reg_sc850sl(id, &cmd);
		}
	}
...
static void sen_set_flip_sc850sl(CTL_SEN_ID id, CTL_SEN_FLIP *flip)
{
	...
	cmd = sen_set_cmd_info_sc850sl(0x3221, 1, 0x0, 0x0);
	rt |= sen_read_reg_sc850sl(id, &cmd);
...
static void sen_get_fps_sc850sl(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data)
{
	data->cur_fps = sen_get_cur_fps_sc850sl(id);
	data->chg_fps = sen_get_chgmode_fps_sc850sl(id);
}
...
static UINT32 sen_calc_rowtime_sc850sl(CTL_SEN_ID id, CTL_SEN_MODE mode)
{
	...
	//Precision * 10
	row_time = 10 * 4 * (mode_basic_param[mode].signal_info.hd_period) / ((speed_param[mode].pclk) / 1000000);
...
static void sen_get_rowtime_sc850sl(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data)
{
	data->row_time_step = sen_calc_rowtime_step_sc850sl(id, data->mode);
	data->row_time = sen_calc_rowtime_sc850sl(id, data->mode) * (data->row_time_step);
}
...
#if defined(__FREERTOS)
void sen_get_i2c_id_sc850sl(CTL_SEN_ID id, UINT32 *i2c_id)
{
	*i2c_id = sen_i2c[id].id;
}

void sen_get_i2c_addr_sc850sl(CTL_SEN_ID id, UINT32 *i2c_addr)
{
	*i2c_addr = sen_i2c[id].addr;
}

int sen_init_sc850sl(SENSOR_DTSI_INFO *info)
{
	...
	sprintf(compatible, "nvt,sen_sc850sl");
	if (sen_common_check_compatible(compatible)) {
		...
		sprintf(node_path, "/sensor/sen_cfg/sen_sc850sl");
	...
	reg_obj.pwr_ctrl = sen_pwr_ctrl_sc850sl;
	reg_obj.det_plug_in = NULL;
	reg_obj.drv_tab = sen_get_drv_tab_sc850sl();
	rt = ctl_sen_reg_sendrv("nvt_sen_sc850sl", &reg_obj);
	if (rt != E_OK) {
...
void sen_exit_sc850sl(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_sc850sl");
}

#else
static int __init sen_init_sc850sl(void)
{
...
static void __exit sen_exit_sc850sl(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_sc850sl");
}

module_init(sen_init_sc850sl);
module_exit(sen_exit_sc850sl);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION(SEN_SC850SL_MODULE_NAME);
MODULE_LICENSE("GPL");
#endif
