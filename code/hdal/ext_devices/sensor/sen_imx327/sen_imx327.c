...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_imx327, 1, 50, 000, 00);

//=============================================================================
// information
//=============================================================================
#define SEN_IMX327_MODULE_NAME     "sen_imx327"
#define SEN_MAX_MODE               6
#define MAX_VD_PERIOD              0x3FFFF
#define BRL_ALL_PIXEL              1097
#define BRL_720P                   735
#define HCG_ENABLE                 99

#define SEN_I2C_ADDR 0x34>>1
#define SEN_I2C_COMPATIBLE "nvt,sen_imx327"
...
//=============================================================================
static CTL_SEN_DRV_TAB *sen_get_drv_tab_imx327(void);
static void sen_pwr_ctrl_imx327(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb);
static ER sen_open_imx327(CTL_SEN_ID id);
static ER sen_close_imx327(CTL_SEN_ID id);
static ER sen_sleep_imx327(CTL_SEN_ID id);
static ER sen_wakeup_imx327(CTL_SEN_ID id);
static ER sen_write_reg_imx327(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_read_reg_imx327(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_chg_mode_imx327(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj);
static ER sen_chg_fps_imx327(CTL_SEN_ID id, UINT32 fps);
static ER sen_set_info_imx327(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static ER sen_get_info_imx327(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static UINT32 sen_calc_chgmode_vd_imx327(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_calc_exp_vd_imx327(CTL_SEN_ID id, UINT32 fps);
static void sen_set_gain_imx327(CTL_SEN_ID id, void *param);
static void sen_set_expt_imx327(CTL_SEN_ID id, void *param);
static void sen_set_preset_imx327(CTL_SEN_ID id, ISP_SENSOR_PRESET_CTRL *ctrl);
static void sen_set_flip_imx327(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
static ER sen_get_flip_imx327(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
#if defined(__FREERTOS)
void sen_get_gain_imx327(CTL_SEN_ID id, void *param);
void sen_get_expt_imx327(CTL_SEN_ID id, void *param);
#else
static void sen_get_gain_imx327(CTL_SEN_ID id, void *param);
static void sen_get_expt_imx327(CTL_SEN_ID id, void *param);
#endif
static void sen_get_min_expt_imx327(CTL_SEN_ID id, void *param);
static void sen_get_mode_basic_imx327(CTL_SENDRV_GET_MODE_BASIC_PARAM *mode_basic);
static void sen_get_attr_basic_imx327(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_BASIC_PARAM *data);
static void sen_get_attr_signal_imx327(CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *data);
static ER sen_get_attr_cmdif_imx327(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_CMDIF_PARAM *data);
static ER sen_get_attr_if_imx327(CTL_SENDRV_GET_ATTR_IF_PARAM *data);
static void sen_get_fps_imx327(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data);
static void sen_get_speed_imx327(CTL_SEN_ID id, CTL_SENDRV_GET_SPEED_PARAM *data);
static void sen_get_mode_mipi_imx327(CTL_SENDRV_GET_MODE_MIPI_PARAM *data);
static void sen_get_modesel_imx327(CTL_SENDRV_GET_MODESEL_PARAM *data);
static UINT32 sen_calc_rowtime_imx327(CTL_SEN_ID id, CTL_SEN_MODE mode);
static UINT32 sen_calc_rowtime_step_imx327(CTL_SEN_ID id, CTL_SEN_MODE mode);
static void sen_get_rowtime_imx327(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data);
static void sen_set_cur_fps_imx327(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_cur_fps_imx327(CTL_SEN_ID id);
static void sen_set_chgmode_fps_imx327(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_chgmode_fps_imx327(CTL_SEN_ID id);
...
static CTL_SENDRV_GET_MODE_BASIC_PARAM mode_basic_param[SEN_MAX_MODE] = {
	...
	},
	{
		...
		CTL_SEN_FMT_POGRESSIVE,
		{1945, 1097},
		{{12, 8, 1920, 1080}, {12, 8, 1920, 1080}, {0, 0, 0, 0}, {0, 0, 0, 0}},
		{1920, 1080},
		{0, 2028, 0, 1220},
		CTL_SEN_RATIO(16, 9),
		{1000, 9285000},
	...
	},
	{
		...
		CTL_SEN_FMT_POGRESSIVE,
		{1305, 733},
		{{12, 8, 1280, 720}, {12, 8, 1280, 720}, {0, 0, 0, 0}, {0, 0, 0, 0}},
		{1280, 720},
		{0, 3300, 0, 750},
		CTL_SEN_RATIO(16, 9),
		{1000, 9285000},
...
static CTL_SEN_CMD imx327_mode_1[] = {
...
static CTL_SEN_CMD imx327_mode_2[] = {
	{0x3003, 1, {0x01, 0x0}}, // software reset
	{SEN_CMD_DELAY, 1, { 1, 0x0}},	
	{0x3000, 1, {0x01, 0x0}},
	{SEN_CMD_DELAY, 1, { 20, 0x0}},		
...
static CTL_SEN_CMD imx327_mode_3[] = {
	{0x3003, 1, {0x01, 0x0}}, // software reset
	{SEN_CMD_DELAY, 1, { 1, 0x0}},
	{0x3000, 1, {0x01, 0x0}},
	{SEN_CMD_DELAY, 1, { 20, 0x0}},
	...
	{0x3009, 1, {0x01, 0x0}},
	{0x300A, 1, {0x3C, 0x0}},
	{0x300C, 1, {0x11, 0x0}}, //hdr
	{0x3010, 1, {0x61, 0x0}}, //hdr	 L each gain
	{0x3011, 1, {0x02, 0x0}},
	{0x3016, 1, {0x09, 0x0}},
	{0x3018, 1, {0xC4, 0x0}},
	{0x3019, 1, {0x04, 0x0}},
	{0x301C, 1, {0xEC, 0x0}},
	{0x301D, 1, {0x07, 0x0}},
	{0x3020, 1, {0x02, 0x0}}, //hdr
	{0x3021, 1, {0x00, 0x0}}, //hdr
	{0x3024, 1, {0x53, 0x0}}, //hdr
	{0x3025, 1, {0x04, 0x0}}, //hdr
	{0x3029, 1, {0x00, 0x0}},
	{0x3030, 1, {0xE1, 0x0}}, //hdr
	{0x3031, 1, {0x00, 0x0}}, //hdr
	{0x303A, 1, {0x08, 0x0}}, //WIMWV_OB
	{0x303c, 1, {0x04, 0x0}}, //WINPV
	{0x303e, 1, {0x41, 0x0}}, //WINWV
	{0x303f, 1, {0x04, 0x0}}, //WINWV
	{0x3045, 1, {0x05, 0x0}}, //hdr
	{0x3046, 1, {0x00, 0x0}},
	{0x304B, 1, {0x0A, 0x0}},
	{0x305C, 1, {0x18, 0x0}},
	{0x305D, 1, {0x03, 0x0}},
	...
	{0x30D2, 1, {0x19, 0x0}},
	{0x30D7, 1, {0x03, 0x0}},
	{0x30F0, 1, {0xF0, 0x0}}, //hdr	 S each gain
	{0x3106, 1, {0x11, 0x0}}, //hdr
	{0x3129, 1, {0x1D, 0x0}},
	{0x313B, 1, {0x61, 0x0}},
	{0x315E, 1, {0x1A, 0x0}},
	{0x3164, 1, {0x1A, 0x0}},
	...
	{0x3407, 1, {0x03, 0x0}},
	{0x3414, 1, {0x00, 0x0}},
	{0x3415, 1, {0x00, 0x0}}, // hdr
	{0x3418, 1, {0x72, 0x0}}, // hdr
	{0x3419, 1, {0x09, 0x0}}, // hdr
	{0x3441, 1, {0x0A, 0x0}},
	{0x3442, 1, {0x0A, 0x0}},
	{0x3443, 1, {0x03, 0x0}},
	{0x3444, 1, {0x20, 0x0}},
	...
	{0x3454, 1, {0x17, 0x0}},
	{0x3455, 1, {0x00, 0x0}},
	{0x3472, 1, {0xA0, 0x0}}, // hdr
	{0x3473, 1, {0x07, 0x0}},
	{0x347B, 1, {0x23, 0x0}}, // hdr
	{0x3480, 1, {0x49, 0x0}},
	{SEN_CMD_SETVD, 1, {0x00, 0x0}},
	{SEN_CMD_PRESET, 1, {0x00, 0x0}},
	{SEN_CMD_DIRECTION, 1, {0x00, 0x0}},
...
static CTL_SEN_CMD imx327_mode_4[] = {
	{0x3003, 1, {0x01, 0x0}}, // software reset
	{SEN_CMD_DELAY, 1, { 1, 0x0}},	
	{0x3000, 1, {0x01, 0x0}},
	{SEN_CMD_DELAY, 1, { 20, 0x0}},		
	...
	{0x3020, 1, {0x02, 0x0}}, //hdr
	{0x3021, 1, {0x00, 0x0}}, //hdr
	{0x3024, 1, {0x1B, 0x0}}, //hdr
	{0x3025, 1, {0x05, 0x0}}, //hdr
	{0x3030, 1, {0x09, 0x0}}, //hdr
	{0x3031, 1, {0x00, 0x0}}, //hdr
	{0x3045, 1, {0x05, 0x0}}, //hdr
	{0x3046, 1, {0x01, 0x0}},
	{0x304B, 1, {0x0A, 0x0}},
	...
	{0x30D2, 1, {0x19, 0x0}},
	{0x30D7, 1, {0x03, 0x0}},
	{0x30F0, 1, {0x64, 0x0}}, //hdr	 S each gain
	{0x3106, 1, {0x11, 0x0}}, //hdr
	{0x3129, 1, {0x00, 0x0}},
	{0x313B, 1, {0x61, 0x0}},
	{0x315E, 1, {0x1A, 0x0}},
	...
	{0x3414, 1, {0x04, 0x0}},
	{0x3415, 1, {0x00, 0x0}}, // hdr
	{0x3418, 1, {0xC8, 0x0}}, // hdr
	{0x3419, 1, {0x05, 0x0}}, // hdr
	{0x3441, 1, {0x0C, 0x0}},
	{0x3442, 1, {0x0C, 0x0}},
	{0x3443, 1, {0x03, 0x0}},
	{0x3444, 1, {0x20, 0x0}},
...
static CTL_SEN_CMD imx327_mode_5[] = {
	{0x3003, 1, {0x01, 0x0}}, // software reset
	{SEN_CMD_DELAY, 1, { 1, 0x0}},	
	{0x3000, 1, {0x01, 0x0}},
	{SEN_CMD_DELAY, 1, { 20, 0x0}},		
...
static CTL_SEN_CMD imx327_mode_6[] = {
	{0x3003, 1, {0x01, 0x0}}, // software reset
	{SEN_CMD_DELAY, 1, { 1, 0x0}},
	{0x3000, 1, {0x01, 0x0}},
	{SEN_CMD_DELAY, 1, { 20, 0x0}},
...
static CTL_SEN_DRV_TAB imx327_sen_drv_tab = {
	sen_open_imx327,
	sen_close_imx327,
	sen_sleep_imx327,
	sen_wakeup_imx327,
	sen_write_reg_imx327,
	sen_read_reg_imx327,
	sen_chg_mode_imx327,
	sen_chg_fps_imx327,
	sen_set_info_imx327,
...
static CTL_SEN_CMD sen_set_cmd_info_imx327(UINT32 addr, UINT32 data_length, UINT32 data0, UINT32 data1)
{
	CTL_SEN_CMD cmd;
...
#if defined(__KERNEL__)
static void sen_load_cfg_from_compatible_imx327(struct device_node *of_node)
{
...
static ER sen_open_imx327(CTL_SEN_ID id)
{
	...
	#if defined(__KERNEL__)
	sen_i2c_reg_cb(sen_load_cfg_from_compatible_imx327);
	#endif
...
static UINT32 sen_get_cmd_tab_imx327(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	switch (mode) {
	case CTL_SEN_MODE_1:
		*cmd_tab = imx327_mode_1;
		return sizeof(imx327_mode_1) / sizeof(CTL_SEN_CMD);

	case CTL_SEN_MODE_2:
		*cmd_tab = imx327_mode_2;
		return sizeof(imx327_mode_2) / sizeof(CTL_SEN_CMD);

	case CTL_SEN_MODE_3:
		*cmd_tab = imx327_mode_3;
		return sizeof(imx327_mode_3) / sizeof(CTL_SEN_CMD);

	case CTL_SEN_MODE_4:
		*cmd_tab = imx327_mode_4;
		return sizeof(imx327_mode_4) / sizeof(CTL_SEN_CMD);
		
	case CTL_SEN_MODE_5:
		*cmd_tab = imx327_mode_5;
		return sizeof(imx327_mode_5) / sizeof(CTL_SEN_CMD);

	case CTL_SEN_MODE_6:
		*cmd_tab = imx327_mode_6;
		return sizeof(imx327_mode_6) / sizeof(CTL_SEN_CMD);		
...
static ER sen_chg_mode_imx327(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj)
{
	...
	if (is_fastboot[id]) {
		...
		sen_set_chgmode_fps_imx327(id, isp_builtin_get_chgmode_fps(id));
		sen_set_cur_fps_imx327(id, isp_builtin_get_chgmode_fps(id));
		sen_set_gain_imx327(id, &sensor_ctrl);
		sen_set_expt_imx327(id, &sensor_ctrl);
		#endif
	...
	cmd_num = sen_get_cmd_tab_imx327(chgmode_obj.mode, &p_cmd_list);
	if (p_cmd_list == NULL) {
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_imx327(id, chgmode_obj.frame_rate);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_imx327(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_imx327(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
	...
	for (idx = 0; idx < cmd_num; idx++) {
		if (p_cmd_list[idx].addr == SEN_CMD_DELAY) {
			...
			cmd = sen_set_cmd_info_imx327(0x3018, 1, sensor_vd & 0xFF, 0x00);
			rt |= sen_write_reg_imx327(id, &cmd);
			cmd = sen_set_cmd_info_imx327(0x3019, 1, (sensor_vd >> 8) & 0xFF, 0x00);
			rt |= sen_write_reg_imx327(id, &cmd);
			cmd = sen_set_cmd_info_imx327(0x301A, 1, (sensor_vd >> 16) & 0x03, 0x00);
			rt |= sen_write_reg_imx327(id, &cmd);
			...
			sen_set_flip_imx327(id, &flip);
		} else {
			cmd = sen_set_cmd_info_imx327(p_cmd_list[idx].addr, p_cmd_list[idx].data_len, p_cmd_list[idx].data[0], p_cmd_list[idx].data[1]);
			rt |= sen_write_reg_imx327(id, &cmd);
		}
	}
...
static ER sen_chg_fps_imx327(CTL_SEN_ID id, UINT32 fps)
{
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_imx327(id, fps);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_imx327(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_imx327(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
...
static ER sen_set_info_imx327(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_SET_EXPT:
		sen_set_expt_imx327(id, data);
		break;
	case CTL_SENDRV_CFGID_SET_GAIN:
		sen_set_gain_imx327(id, data);
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		sen_set_flip_imx327(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE1:
		sen_set_preset_imx327(id, (ISP_SENSOR_PRESET_CTRL *)(data));
		break;
	default:
		return E_NOSPT;
	}
...
static ER sen_get_info_imx327(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_EXPT:
		sen_get_expt_imx327(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_GAIN:
		sen_get_gain_imx327(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_BASIC:
		sen_get_attr_basic_imx327(id, (CTL_SENDRV_GET_ATTR_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_SIGNAL:
		sen_get_attr_signal_imx327((CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_CMDIF:
		rt = sen_get_attr_cmdif_imx327(id, (CTL_SENDRV_GET_ATTR_CMDIF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_IF:
		rt = sen_get_attr_if_imx327((CTL_SENDRV_GET_ATTR_IF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_FPS:
		sen_get_fps_imx327(id, (CTL_SENDRV_GET_FPS_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_SPEED:
		sen_get_speed_imx327(id, (CTL_SENDRV_GET_SPEED_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_BASIC:
		sen_get_mode_basic_imx327((CTL_SENDRV_GET_MODE_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_MIPI:
		sen_get_mode_mipi_imx327((CTL_SENDRV_GET_MODE_MIPI_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODESEL:
		sen_get_modesel_imx327((CTL_SENDRV_GET_MODESEL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_ROWTIME:
		sen_get_rowtime_imx327(id, (CTL_SENDRV_GET_MODE_ROWTIME_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		rt = sen_get_flip_imx327(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE2:
		sen_get_min_expt_imx327(id, data);
		break;	
	default:
		rt = E_NOSPT;
	}
...
static UINT32 sen_calc_chgmode_vd_imx327(CTL_SEN_ID id, UINT32 fps)
{
	...
	sen_set_chgmode_fps_imx327(id, fps);
	sen_set_cur_fps_imx327(id, fps);
...
static void sen_set_gain_imx327(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
			if (HCG_ENABLE == 0) {
				data2[frame_cnt] = 0;
			} else if (HCG_ENABLE == 1 || (sensor_ctrl->gain_ratio[0] >= 32000)) {
				temp_gain_ratio = sensor_ctrl->gain_ratio[frame_cnt] * 2 / 5;
				data2[frame_cnt] = 1;
			} else {
				data2[frame_cnt] = 0;
			}
			...
			if (HCG_ENABLE == 0) {
				data2[frame_cnt] = 0;
			} else if (HCG_ENABLE == 1 || ((sensor_ctrl->gain_ratio[0] >= 32000) && (sensor_ctrl->gain_ratio[1] >= 32000))) {
				temp_gain_ratio = sensor_ctrl->gain_ratio[frame_cnt] * 2 / 5;
				data2[frame_cnt] = 1;
			} else {
				data2[frame_cnt] = 0;
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		cmd = sen_set_cmd_info_imx327(0x3014, 1, data1[0] & 0xFF, 0x0);
		rt |= sen_write_reg_imx327(id, &cmd);

		if (pre_gain != data2[0]) {
			cmd = sen_set_cmd_info_imx327(0x3016, 1, 0x8, 0);
			rt |= sen_write_reg_imx327(id, &cmd);

			cmd = sen_set_cmd_info_imx327(0x3009, 1, 0x0, 0x0);
			rt |= sen_read_reg_imx327(id, &cmd);
			data = cmd.data[0];
			...
			cmd = sen_set_cmd_info_imx327(0x3009, 1, data, 0);
			rt |= sen_write_reg_imx327(id, &cmd);
			pre_gain = data2[0];
		} else {
			cmd = sen_set_cmd_info_imx327(0x3016, 1, 0x9, 0);
			rt |= sen_write_reg_imx327(id, &cmd);
		}
	} else if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR){
		if (mode_basic_param[cur_sen_mode[id]].frame_num == 2) {
			cmd = sen_set_cmd_info_imx327(0x3014, 1, data1[0] & 0xFF, 0x0);
			rt |= sen_write_reg_imx327(id, &cmd);
			cmd = sen_set_cmd_info_imx327(0x30F2, 1, data1[1] & 0xFF, 0x0);
			rt |= sen_write_reg_imx327(id, &cmd);
...
static ER sen_get_min_shr_imx327(CTL_SEN_MODE mode, UINT32 *min_shr, UINT32 *min_exp)
{
    ER rt = E_OK;
...
static void sen_set_expt_imx327(CTL_SEN_ID id, void *param)
{
	...
	UINT32 min_shr = 0, min_exp_line = 0, shs1 = 0;
	UINT32 shs2 = 0, y_out_size = 0, rhs1 = 0, t_row = 0, brl = 0, fsc = 0, temp_max_short_line = 0;
	UINT32  real_short_exp = 0;
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		t_row = sen_calc_rowtime_imx327(id, cur_sen_mode[id]);
		if (0 == t_row) {
			DBG_WRN("t_row  = 0, must >= 1 \r\n");
			t_row = 1;
		}
		line[frame_cnt] = (sensor_ctrl->exp_time[frame_cnt]) * 10 / t_row;
		sen_get_min_shr_imx327(cur_sen_mode[id], &min_shr, &min_exp_line);
	...
	// Write exposure line
	// Get fps
	chgmode_fps = sen_get_chgmode_fps_imx327(id);
	...
	sen_set_cur_fps_imx327(id, cur_fps);

	// Calculate new vd
	sensor_vd = sen_calc_exp_vd_imx327(id, cur_fps);
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		...
		if (line[0] > (MAX_VD_PERIOD - min_shr - 1)) {
			line[0] = (MAX_VD_PERIOD - min_shr - 1);
		}
		...
		//calculate shr
		if (line[0] > (sensor_vd - min_shr - 1)) {
			shs1 = min_shr;
		} else {
			shs1 = sensor_vd - line[0] - 1;
		}
		compensation_ratio[id][0] = 100 * temp_line[0] / (sensor_vd - shs1 - 1);
		...
		if (rhs1 > (temp_max_short_line + 1)) {
			rhs1 = temp_max_short_line + 1;
		}
		...
		if (shs2 > ((sensor_vd * 2) - 2)) {
		    shs2 = (sensor_vd * 2) - 2;
		}
		compensation_ratio[id][0] = 100 * temp_line[0] / (fsc - (shs2 + 1)) ;
		compensation_ratio[id][1] = 100 * temp_line[1] / (rhs1 - (shs1 + 1));	
	}

	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		cmd = sen_set_cmd_info_imx327(0x3020, 1, shs1 & 0xFF , 0);
		rt |= sen_write_reg_imx327(id, &cmd);
		cmd = sen_set_cmd_info_imx327(0x3021, 1, ( shs1 >> 8 ) & 0xFF, 0);
		rt |= sen_write_reg_imx327(id, &cmd);
		cmd = sen_set_cmd_info_imx327(0x3022, 1, ( shs1 >> 16 ) & 0x03 , 0);
		rt |= sen_write_reg_imx327(id, &cmd);
	} else if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		cmd = sen_set_cmd_info_imx327(0x3030, 1, rhs1 & 0xFF, 0x0);
		rt |= sen_write_reg_imx327(id, &cmd);
		cmd = sen_set_cmd_info_imx327(0x3031, 1, (rhs1 >> 8)& 0xFF, 0x0);
		rt |= sen_write_reg_imx327(id, &cmd);
		cmd = sen_set_cmd_info_imx327(0x3032, 1, (rhs1 >> 16)& 0x0F, 0x0);
		rt |= sen_write_reg_imx327(id, &cmd);
		cmd = sen_set_cmd_info_imx327(0x3020, 1, shs1 & 0xFF , 0);
		rt |= sen_write_reg_imx327(id, &cmd);
		cmd = sen_set_cmd_info_imx327(0x3021, 1, ( shs1 >> 8 ) & 0xFF, 0);
		...
		cmd = sen_set_cmd_info_imx327(0x3022, 1, ( shs1 >> 16 ) & 0x03 , 0);
		rt |= sen_write_reg_imx327(id, &cmd);
		cmd = sen_set_cmd_info_imx327(0x3024, 1, shs2 & 0xFF, 0x0);
		rt |= sen_write_reg_imx327(id, &cmd);
		cmd = sen_set_cmd_info_imx327(0x3025, 1, (shs2 >> 8)& 0xFF, 0x0);
		rt |= sen_write_reg_imx327(id, &cmd);
		cmd = sen_set_cmd_info_imx327(0x3026, 1, (shs2 >> 16)& 0x0F, 0x0);
		rt |= sen_write_reg_imx327(id, &cmd);
		cmd = sen_set_cmd_info_imx327(0x3418, 1, y_out_size & 0xFF, 0x0);
		rt |= sen_write_reg_imx327(id, &cmd);
		cmd = sen_set_cmd_info_imx327(0x3419, 1, (y_out_size >> 8)& 0x1F, 0x0);
		rt |= sen_write_reg_imx327(id, &cmd);
	}
...
static void sen_set_flip_imx327(CTL_SEN_ID id, CTL_SEN_FLIP *flip)
{
	...
	cmd = sen_set_cmd_info_imx327(0x3007, 1, 0x0, 0x0);
	rt |= sen_read_reg_imx327(id, &cmd);
...
static void sen_get_fps_imx327(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data)
{
	data->cur_fps = sen_get_cur_fps_imx327(id);
	data->chg_fps = sen_get_chgmode_fps_imx327(id);
}
...
static void sen_get_rowtime_imx327(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data)
{
	data->row_time_step = sen_calc_rowtime_step_imx327(id, data->mode);
	data->row_time = sen_calc_rowtime_imx327(id, data->mode)* (data->row_time_step);
}
...
#if defined(__FREERTOS)
void sen_get_i2c_id_imx327(CTL_SEN_ID id, UINT32 *i2c_id)
{
	*i2c_id = sen_i2c[id].id;
}

void sen_get_i2c_addr_imx327(CTL_SEN_ID id, UINT32 *i2c_addr)
{
	*i2c_addr = sen_i2c[id].addr;
}
int sen_init_imx327(SENSOR_DTSI_INFO *info)
{
	...
	sprintf(compatible, "nvt,sen_imx327");
	if (sen_common_check_compatible(compatible)) {
		...
		sprintf(node_path, "/sensor/sen_cfg/sen_imx327");
	...
	reg_obj.pwr_ctrl = sen_pwr_ctrl_imx327;
	reg_obj.det_plug_in = NULL;
	reg_obj.drv_tab = sen_get_drv_tab_imx327();
	rt = ctl_sen_reg_sendrv("nvt_sen_imx327", &reg_obj);
	if (rt != E_OK) {
...
void sen_exit_imx327(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_imx327");
}

#else
static int __init sen_init_imx327(void)
{
...
static void __exit sen_exit_imx327(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_imx327");
}

module_init(sen_init_imx327);
module_exit(sen_exit_imx327);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION(SEN_IMX327_MODULE_NAME);
MODULE_LICENSE("GPL");
#endif
