...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_imx335, 1, 51, 000, 00);
...
static CTL_SEN_CMD imx335_mode_1[] = {
	...
	{0x300C, 1, {0x42, 0x0}},
	{0x300D, 1, {0x2E, 0x0}},
	{0x3012, 1, {0x00, 0x0}},	//disable i2c timeout
	{0x3013, 1, {0x00, 0x0}},	//disable i2c timeout
	{0x3018, 1, {0x00, 0x0}},
	{0x3030, 1, {0x94, 0x0}},
	{0x3031, 1, {0x11, 0x0}},
	{0x3032, 1, {0x00, 0x0}},
...
static UINT32 last_compensation_ratio[CTL_SEN_ID_MAX][ISP_SEN_MFRAME_MAX_NUM] = {0};
...
static void sen_set_gain_imx335(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
			last_compensation_ratio[id][frame_cnt] = compensation_ratio[id][frame_cnt];	
		}
		if (100 <= (last_compensation_ratio[id][frame_cnt])) {
			sensor_ctrl->gain_ratio[frame_cnt] = (sensor_ctrl->gain_ratio[frame_cnt]) * (last_compensation_ratio[id][frame_cnt]) / 100;
		}
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		...
		rt |= sen_write_reg_imx335(id, &cmd);
		cmd = sen_set_cmd_info_imx335(0x30EB, 1, (data1[1] >> 8) & 0x07, 0x0);
		rt |= sen_write_reg_imx335(id, &cmd);
