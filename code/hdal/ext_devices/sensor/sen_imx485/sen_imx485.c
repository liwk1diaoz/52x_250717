...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_imx485, 1, 55, 000, 00);

//=============================================================================
// information
//=============================================================================
#define SEN_IMX485_MODULE_NAME     "sen_imx485"
#define SEN_MAX_MODE               2
#define MAX_VD_PERIOD              0xFFFFF
#define MAX_EXPOSURE_LINE          0xFFFFF
#define BRL_ALL_PIXEL              2210
#define HCG_ENABLE                 1

#define SEN_I2C_ADDR 0x34>>1
#define SEN_I2C_COMPATIBLE "nvt,sen_imx485"
...
//=============================================================================
static CTL_SEN_DRV_TAB *sen_get_drv_tab_imx485(void);
static void sen_pwr_ctrl_imx485(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb);
static ER sen_open_imx485(CTL_SEN_ID id);
static ER sen_close_imx485(CTL_SEN_ID id);
static ER sen_sleep_imx485(CTL_SEN_ID id);
static ER sen_wakeup_imx485(CTL_SEN_ID id);
static ER sen_write_reg_imx485(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_read_reg_imx485(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_chg_mode_imx485(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj);
static ER sen_chg_fps_imx485(CTL_SEN_ID id, UINT32 fps);
static ER sen_set_info_imx485(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static ER sen_get_info_imx485(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static UINT32 sen_calc_chgmode_vd_imx485(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_calc_exp_vd_imx485(CTL_SEN_ID id, UINT32 fps);
static void sen_set_gain_imx485(CTL_SEN_ID id, void *param);
static void sen_set_expt_imx485(CTL_SEN_ID id, void *param);
static void sen_set_preset_imx485(CTL_SEN_ID id, ISP_SENSOR_PRESET_CTRL *ctrl);
static void sen_set_flip_imx485(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
static ER sen_get_flip_imx485(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
#if defined(__FREERTOS)
void sen_get_gain_imx485(CTL_SEN_ID id, void *param);
void sen_get_expt_imx485(CTL_SEN_ID id, void *param);
#else
static void sen_get_gain_imx485(CTL_SEN_ID id, void *param);
static void sen_get_expt_imx485(CTL_SEN_ID id, void *param);
#endif
static void sen_get_min_expt_imx485(CTL_SEN_ID id, void *param);
static void sen_get_mode_basic_imx485(CTL_SENDRV_GET_MODE_BASIC_PARAM *mode_basic);
static void sen_get_attr_basic_imx485(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_BASIC_PARAM *data);
static void sen_get_attr_signal_imx485(CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *data);
static ER sen_get_attr_cmdif_imx485(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_CMDIF_PARAM *data);
static ER sen_get_attr_if_imx485(CTL_SENDRV_GET_ATTR_IF_PARAM *data);
static void sen_get_fps_imx485(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data);
static void sen_get_speed_imx485(CTL_SEN_ID id, CTL_SENDRV_GET_SPEED_PARAM *data);
static void sen_get_mode_mipi_imx485(CTL_SENDRV_GET_MODE_MIPI_PARAM *data);
static void sen_get_modesel_imx485(CTL_SENDRV_GET_MODESEL_PARAM *data);
static UINT32 sen_calc_rowtime_imx485(CTL_SEN_ID id, CTL_SEN_MODE mode);
static UINT32 sen_calc_rowtime_step_imx485(CTL_SEN_ID id, CTL_SEN_MODE mode);
static void sen_get_rowtime_imx485(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data);
static void sen_set_cur_fps_imx485(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_cur_fps_imx485(CTL_SEN_ID id);
static void sen_set_chgmode_fps_imx485(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_chgmode_fps_imx485(CTL_SEN_ID id);
static ER sen_get_min_shr_imx485(CTL_SEN_MODE mode, UINT32 *min_shr, UINT32 *min_exp);
//=============================================================================
// global variable
//=============================================================================
...
static CTL_SEN_CMD imx485_mode_1[] = {
...
static CTL_SEN_CMD imx485_mode_2[] = {
	{0x3000, 1, {0x01, 0x0}}, // standby        
	{0x3002, 1, {0x01, 0x0}}, //Master mode stop
	{0x3004, 1, {0x04, 0x0}}, 
	{0x3004, 1, {0x00, 0x0}}, 	
...
static CTL_SEN_DRV_TAB imx485_sen_drv_tab = {
	sen_open_imx485,
	sen_close_imx485,
	sen_sleep_imx485,
	sen_wakeup_imx485,
	sen_write_reg_imx485,
	sen_read_reg_imx485,
	sen_chg_mode_imx485,
	sen_chg_fps_imx485,
	sen_set_info_imx485,
...
static CTL_SEN_CMD sen_set_cmd_info_imx485(UINT32 addr, UINT32 data_length, UINT32 data0, UINT32 data1)
{
	CTL_SEN_CMD cmd;
...
#if defined(__KERNEL__)
static void sen_load_cfg_from_compatible_imx485(struct device_node *of_node)
{
...
static ER sen_open_imx485(CTL_SEN_ID id)
{
	...
	#if defined(__KERNEL__)
	sen_i2c_reg_cb(sen_load_cfg_from_compatible_imx485);
	#endif
...
static UINT32 sen_get_cmd_tab_imx485(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	switch (mode) {
	case CTL_SEN_MODE_1:
		*cmd_tab = imx485_mode_1;
		return sizeof(imx485_mode_1) / sizeof(CTL_SEN_CMD);
		
	case CTL_SEN_MODE_2:
		*cmd_tab = imx485_mode_2;
		return sizeof(imx485_mode_2) / sizeof(CTL_SEN_CMD);	
...
static ER sen_chg_mode_imx485(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj)
{
	...
	if (is_fastboot[id]) {
		...
		sen_set_chgmode_fps_imx485(id, isp_builtin_get_chgmode_fps(id));
		sen_set_cur_fps_imx485(id, isp_builtin_get_chgmode_fps(id));
		sen_set_gain_imx485(id, &sensor_ctrl);
		sen_set_expt_imx485(id, &sensor_ctrl);
		#endif
	...
	cmd_num = sen_get_cmd_tab_imx485(chgmode_obj.mode, &p_cmd_list);
	if (p_cmd_list == NULL) {
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_imx485(id, chgmode_obj.frame_rate);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_imx485(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_imx485(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
	...
	for (idx = 0; idx < cmd_num; idx++) {
		if (p_cmd_list[idx].addr == SEN_CMD_DELAY) {
			...
			cmd = sen_set_cmd_info_imx485(0x3024, 1, sensor_vd & 0xFF, 0x00);
			rt |= sen_write_reg_imx485(id, &cmd);
			cmd = sen_set_cmd_info_imx485(0x3025, 1, (sensor_vd >> 8) & 0xFF, 0x00);
			rt |= sen_write_reg_imx485(id, &cmd);
			cmd = sen_set_cmd_info_imx485(0x3026, 1, (sensor_vd >> 16) & 0x0F, 0x00);
			rt |= sen_write_reg_imx485(id, &cmd);
			...
			sen_set_flip_imx485(id, &flip);
		} else {
			cmd = sen_set_cmd_info_imx485(p_cmd_list[idx].addr, p_cmd_list[idx].data_len, p_cmd_list[idx].data[0], p_cmd_list[idx].data[1]);
			rt |= sen_write_reg_imx485(id, &cmd);
		}
	}
...
static ER sen_chg_fps_imx485(CTL_SEN_ID id, UINT32 fps)
{
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_imx485(id, fps);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_imx485(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_imx485(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
...
static ER sen_set_info_imx485(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_SET_EXPT:
		sen_set_expt_imx485(id, data);
		break;
	case CTL_SENDRV_CFGID_SET_GAIN:
		sen_set_gain_imx485(id, data);
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		sen_set_flip_imx485(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE1:
		sen_set_preset_imx485(id, (ISP_SENSOR_PRESET_CTRL *)(data));
		break;
	default:
		return E_NOSPT;
	}
...
static ER sen_get_info_imx485(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_EXPT:
		sen_get_expt_imx485(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_GAIN:
		sen_get_gain_imx485(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_BASIC:
		sen_get_attr_basic_imx485(id, (CTL_SENDRV_GET_ATTR_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_SIGNAL:
		sen_get_attr_signal_imx485((CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_CMDIF:
		rt = sen_get_attr_cmdif_imx485(id, (CTL_SENDRV_GET_ATTR_CMDIF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_IF:
		rt = sen_get_attr_if_imx485((CTL_SENDRV_GET_ATTR_IF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_FPS:
		sen_get_fps_imx485(id, (CTL_SENDRV_GET_FPS_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_SPEED:
		sen_get_speed_imx485(id, (CTL_SENDRV_GET_SPEED_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_BASIC:
		sen_get_mode_basic_imx485((CTL_SENDRV_GET_MODE_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_MIPI:
		sen_get_mode_mipi_imx485((CTL_SENDRV_GET_MODE_MIPI_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODESEL:
		sen_get_modesel_imx485((CTL_SENDRV_GET_MODESEL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_ROWTIME:
		sen_get_rowtime_imx485(id, (CTL_SENDRV_GET_MODE_ROWTIME_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		rt = sen_get_flip_imx485(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE2:
		sen_get_min_expt_imx485(id, data);
		break;
	default:
		rt = E_NOSPT;
	}
...
static UINT32 sen_calc_chgmode_vd_imx485(CTL_SEN_ID id, UINT32 fps)
{
	...
	sen_set_chgmode_fps_imx485(id, fps);
	sen_set_cur_fps_imx485(id, fps);
...
static void sen_set_gain_imx485(CTL_SEN_ID id, void *param)
{
	...
	static UINT32 delay_hcg_flag[CTL_SEN_ID_MAX][ISP_SEN_MFRAME_MAX_NUM] = {0};		
	UINT32 change_hcg_flag[CTL_SEN_ID_MAX][ISP_SEN_MFRAME_MAX_NUM] = {0};
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		if ((HCG_ENABLE) && (2600 <= (sensor_ctrl->gain_ratio[frame_cnt]))) {
			delay_hcg_flag[id][frame_cnt]++;
			if (2 <= (delay_hcg_flag[id][frame_cnt])) {
				temp_gain_ratio = sensor_ctrl->gain_ratio[frame_cnt] * 10 / 26 ;//8.3db = 2.6x				
				delay_hcg_flag[id][frame_cnt] = 2;
				change_hcg_flag[id][frame_cnt] = 1;
			} else {
				temp_gain_ratio = sensor_ctrl->gain_ratio[frame_cnt] * 10 / 26 ;//8.3db = 2.6x
				change_hcg_flag[id][frame_cnt] = 0;
			}
		} else {
			if (1 >= (delay_hcg_flag[id][frame_cnt])) {
				temp_gain_ratio = sensor_ctrl->gain_ratio[frame_cnt];
				delay_hcg_flag[id][frame_cnt] = 0;
				change_hcg_flag[id][frame_cnt] = 0;				
			} else {
				temp_gain_ratio = sensor_ctrl->gain_ratio[frame_cnt];
				delay_hcg_flag[id][frame_cnt]--;
	...
	cmd = sen_set_cmd_info_imx485(0x3001, 1, 0x01, 0x0);
	rt |= sen_write_reg_imx485(id, &cmd);

	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		cmd = sen_set_cmd_info_imx485(0x3200, 1, 0x01, 0x0);
		rt |= sen_write_reg_imx485(id, &cmd);				
		cmd = sen_set_cmd_info_imx485(0x3084, 1, data1[0] & 0xFF, 0x0);
		rt |= sen_write_reg_imx485(id, &cmd);
		cmd = sen_set_cmd_info_imx485(0x3085, 1, (data1[0] >> 8) & 0x07, 0x0);
		rt |= sen_write_reg_imx485(id, &cmd);

		if (change_hcg_flag[id][0]) {
			cmd = sen_set_cmd_info_imx485(0x3034, 1, 0x01, 0x0);
			rt |= sen_write_reg_imx485(id, &cmd);
		} else {
			cmd = sen_set_cmd_info_imx485(0x3034, 1, 0x00, 0x0);
			rt |= sen_write_reg_imx485(id, &cmd);				
		}		
	} else if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		if(!each_frame_gain_flag[id]) {
			cmd = sen_set_cmd_info_imx485(0x3200, 1, 0x0, 0x0);
			rt |= sen_read_reg_imx485(id, &cmd);
			data = cmd.data[0];
			data = data & (~0x01);
			cmd = sen_set_cmd_info_imx485(0x3200, 1, data, 0x0);
		  rt |= sen_write_reg_imx485(id, &cmd);						
		...
		cmd = sen_set_cmd_info_imx485(0x3085, 1, (data1[0] >> 8) & 0x07, 0x0);
		rt |= sen_write_reg_imx485(id, &cmd);
		cmd = sen_set_cmd_info_imx485(0x3086, 1, data1[1] & 0xFF, 0x0);
		rt |= sen_write_reg_imx485(id, &cmd);		
		cmd = sen_set_cmd_info_imx485(0x3087, 1, (data1[1] >> 8) & 0x07, 0x0);
		rt |= sen_write_reg_imx485(id, &cmd);
		...
		if (change_hcg_flag[id][1]) {
			cmd = sen_set_cmd_info_imx485(0x3035, 1, 0x01, 0x0);
			rt |= sen_write_reg_imx485(id, &cmd);
		} else {
			cmd = sen_set_cmd_info_imx485(0x3035, 1, 0x00, 0x0);
			rt |= sen_write_reg_imx485(id, &cmd);				
		}
	}

	cmd = sen_set_cmd_info_imx485(0x3001, 1, 0x00, 0x0);
	rt |= sen_write_reg_imx485(id, &cmd);		
...
static void sen_set_expt_imx485(CTL_SEN_ID id, void *param)
{
	...
	UINT32 shr0 = 0, rhs1 = 0, shr1 = 0, brl = 0, fcs = 0, temp_max_rhs1 = 0;
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		t_row = sen_calc_rowtime_imx485(id, cur_sen_mode[id]);
		if (0 == t_row) {
		...
		sen_get_min_shr_imx485(cur_sen_mode[id], &min_shr, &min_exp_line);
	...
	// Write exposure line
	// Get fps
	chgmode_fps = sen_get_chgmode_fps_imx485(id);
	...
	sen_set_cur_fps_imx485(id, cur_fps);

	// Calculate new vd
	sensor_vd = sen_calc_exp_vd_imx485(id, cur_fps);
	...
	// Set Vmax to sensor 
	cmd = sen_set_cmd_info_imx485(0x3024, 1, sensor_vd & 0xFF, 0);
	rt |= sen_write_reg_imx485(id, &cmd);
	cmd = sen_set_cmd_info_imx485(0x3025, 1, (sensor_vd >> 8) & 0xFF, 0);
	rt |= sen_write_reg_imx485(id, &cmd);
	cmd = sen_set_cmd_info_imx485(0x3026, 1, (sensor_vd >> 16 ) & 0x0F, 0);
	rt |= sen_write_reg_imx485(id, &cmd);
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		...
		cmd = sen_set_cmd_info_imx485(0x3050, 1, shr0 & 0xFF , 0);
		rt |= sen_write_reg_imx485(id, &cmd);
		cmd = sen_set_cmd_info_imx485(0x3051, 1, (shr0 >> 8) & 0xFF, 0);
		rt |= sen_write_reg_imx485(id, &cmd);
		cmd = sen_set_cmd_info_imx485(0x3052, 1, (shr0 >> 16) & 0x0F , 0);
		rt |= sen_write_reg_imx485(id, &cmd);
	} else if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		if ((mode_basic_param[cur_sen_mode[id]].frame_num) == 2) {
			brl = BRL_ALL_PIXEL;			
			fcs = sensor_vd * 2;
			temp_max_rhs1 = (100000000 / cur_fps ) * 10 / 17 / t_row; //max support 1:16	
			// NOTE: (FSC - BRL x 2) / 2 - 2 > (RHS1 - 3) / 2
			// NOTE: 4n+2, n = 0, 1, 2, ..., SHR1 + 8 <= RHS1 <= SHR0 - 9, RHS1 < BRL * 2
			rhs1 = (fcs - (brl * 2)) - 2;

			if (rhs1 > ((brl * 2) - 1)) {
				rhs1 = (brl * 2) - 1;
			} else if (rhs1 > (10 + temp_max_rhs1)) {
				rhs1 = 10 + temp_max_rhs1;//min shr1 = 10
			}

			if (rhs1 < (10 + 4)) {//min rhs1 = shr1+4 and should be 4n+2,so min rhs1 = 14
				rhs1 = 10 + 4;//min shr1 = 10, min rhs1 = shr1 + 4;
			}
			...
			// NOTE: 4n+2, n = 0, 1, 2, ..., 10 <= SHR1 <= RHS1 - 4
			if (line[1] > rhs1) {
				shr1 = 10;
			} else {
				shr1 = rhs1 - line[1];
			...
			// NOTE: 4n, n = 0, 1, 2, ..., RHS1 + 10 <= SHR0 <= FSC - 4
			if ((line[0]) > (fcs - (rhs1 + 10))) {
				shr0 = rhs1 + 10;
			} else {
				shr0 = fcs - line[0];
			}
			...
			cmd = sen_set_cmd_info_imx485(0x3052, 1, (shr0 >> 16) & 0x0F , 0);
			rt |= sen_write_reg_imx485(id, &cmd);
			cmd = sen_set_cmd_info_imx485(0x3060, 1, rhs1 & 0xFF, 0x0);
			rt |= sen_write_reg_imx485(id, &cmd);
			cmd = sen_set_cmd_info_imx485(0x3061, 1, (rhs1 >> 8)& 0xFF, 0x0);
			rt |= sen_write_reg_imx485(id, &cmd);
			cmd = sen_set_cmd_info_imx485(0x3062, 1, (rhs1 >> 16)& 0x0F, 0x0);
			rt |= sen_write_reg_imx485(id, &cmd);
			cmd = sen_set_cmd_info_imx485(0x3054, 1, shr1 & 0xFF, 0x0);
			rt |= sen_write_reg_imx485(id, &cmd);
			cmd = sen_set_cmd_info_imx485(0x3055, 1, (shr1 >> 8)& 0xFF, 0x0);
			rt |= sen_write_reg_imx485(id, &cmd);
			cmd = sen_set_cmd_info_imx485(0x3056, 1, (shr1 >> 16)& 0x0F, 0x0);
			rt |= sen_write_reg_imx485(id, &cmd);
		}
	}
...
static void sen_set_flip_imx485(CTL_SEN_ID id, CTL_SEN_FLIP *flip)
{
	...
	cmd = sen_set_cmd_info_imx485(0x3030, 1, 0x0, 0x0);
	rt |= sen_read_reg_imx485(id, &cmd);
...
static void sen_get_fps_imx485(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data)
{
	data->cur_fps = sen_get_cur_fps_imx485(id);
	data->chg_fps = sen_get_chgmode_fps_imx485(id);
}
...
static void sen_get_rowtime_imx485(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data)
{
	data->row_time_step = sen_calc_rowtime_step_imx485(id, data->mode);	
	data->row_time = sen_calc_rowtime_imx485(id, data->mode) * (data->row_time_step);
}
...
#if defined(__FREERTOS)
void sen_get_i2c_id_imx485(CTL_SEN_ID id, UINT32 *i2c_id)
{
	*i2c_id = sen_i2c[id].id;
}

void sen_get_i2c_addr_imx485(CTL_SEN_ID id, UINT32 *i2c_addr)
{
	*i2c_addr = sen_i2c[id].addr;
}
int sen_init_imx485(SENSOR_DTSI_INFO *info)
{
	...
	sprintf(compatible, "nvt,sen_imx485");
	if (sen_common_check_compatible(compatible)) {
		...
		sprintf(node_path, "/sensor/sen_cfg/sen_imx485");
	...
	reg_obj.pwr_ctrl = sen_pwr_ctrl_imx485;
	reg_obj.det_plug_in = NULL;
	reg_obj.drv_tab = sen_get_drv_tab_imx485();
	rt = ctl_sen_reg_sendrv("nvt_sen_imx485", &reg_obj);
	if (rt != E_OK) {
...
void sen_exit_imx485(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_imx485");
}

#else
static int __init sen_init_imx485(void)
{
...
static void __exit sen_exit_imx485(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_imx485");
}

module_init(sen_init_imx485);
module_exit(sen_exit_imx485);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION(SEN_IMX485_MODULE_NAME);
MODULE_LICENSE("GPL");
#endif
