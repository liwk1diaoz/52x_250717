...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_os04c10, 1, 53, 000, 00);
...
static void sen_get_hdr_rhs_os04c10(CTL_SEN_ID id, CTL_SENDRV_GET_MFR_OUTPUT_TIMING_PARAM *data);
//=============================================================================
// global variable
//=============================================================================
...
static ER sen_get_info_os04c10(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_MFR_OUTPUT_TIMING:
		sen_get_hdr_rhs_os04c10(id, (CTL_SENDRV_GET_MFR_OUTPUT_TIMING_PARAM *)(data));
		break;		
	default:
		rt = E_NOSPT;
	}
...
static void sen_set_gain_os04c10(CTL_SEN_ID id, void *param)
{
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		if (hcg_change_flag[id]) {
			conversion_gain = 0x25;			
		} else {
			conversion_gain = 0x00;
		}
	}
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {	
		...
		cmd = sen_set_cmd_info_os04c10(0x3880, 1, conversion_gain, 0x0);
		rt |= sen_write_reg_os04c10(id, &cmd);
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		...
		rt |= sen_write_reg_os04c10(id, &cmd);
		cmd = sen_set_cmd_info_os04c10(0x35A0, 1, conversion_gain_reg[8], 0x0);
		rt |= sen_write_reg_os04c10(id, &cmd);
...
static void sen_set_flip_os04c10(CTL_SEN_ID id, CTL_SEN_FLIP *flip)
{
	...
	cmd2 = sen_set_cmd_info_os04c10(0x3716, 1, 0x0, 0x0);
	rt |= sen_read_reg_os04c10(id, &cmd2);
	...
	cmd2 = sen_set_cmd_info_os04c10(0x3716, 1, cmd2.data[0], 0x0);
	rt |= sen_write_reg_os04c10(id, &cmd2);
...
static void sen_get_hdr_rhs_os04c10(CTL_SEN_ID id, CTL_SENDRV_GET_MFR_OUTPUT_TIMING_PARAM *data)
{
	UINT32 temp_max_short_line = 0, t_row = 0;
	...
	if (mode_basic_param[data->mode].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		if (((mode_basic_param[data->mode].frame_num) == data->num)  && ( mode_basic_param[data->mode].mode == (data->mode))) {
			if (2 == data->num) {
				t_row = sen_calc_rowtime_os04c10(id, data->mode);
				temp_max_short_line = (100000000 / (data->frame_rate)) * 10 / 17 / t_row - 1; //max support 1:16
				data->diff_row[0] = temp_max_short_line;
				data->diff_row_vd[0] = temp_max_short_line;
			}
		}
	}
}
