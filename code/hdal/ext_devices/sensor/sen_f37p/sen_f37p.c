...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_f37p, 1, 45, 000, 00);
...
#define MAX_SHORT_EXPOSURE_LINE   0x45 //reg0x06 ==>(0x45 * 2) + 1 = 139; if 1:16 max short = 2250 / 17 = 132 ; 0x05 must < 0x06 - 5
...
static CTL_SEN_CMD f37p_mode_1[] = {
	...
	{0x8A, 1, {0x44, 0x0}},
	{0x91, 1, {0x13, 0x0}},
	{0x94, 1, {0xE0, 0x0}},// for flip  no bad frame
	{0x9B, 1, {0x83, 0x0}},
	{0x9C, 1, {0xE1, 0x0}},
	{0xA4, 1, {0x80, 0x0}},
	{0xA6, 1, {0x22, 0x0}},
...
static CTL_SEN_CMD f37p_mode_2[] = {
	//[INI_Register]
	{0x12, 1, {0x48, 0x0}},
	{0x48, 1, {0x8A, 0x0}},
	{0x48, 1, {0x0A, 0x0}},
...
static UINT32 sen_get_cmd_tab_f37p(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	...
	case CTL_SEN_MODE_2:
		*cmd_tab = f37p_mode_2;
		return sizeof(f37p_mode_2) / sizeof(CTL_SEN_CMD);
...
static void sen_set_expt_f37p(CTL_SEN_ID id, void *param)
{
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		...
		cmd = sen_set_cmd_info_f37p(0x08, 1, ((line[1]) >> 8) & 0x01 , 0);
		rt |= sen_write_reg_f37p(id, &cmd);				
		cmd = sen_set_cmd_info_f37p(0x05, 1, (line[1]) & 0xFF , 0);
		rt |= sen_write_reg_f37p(id, &cmd);
	}
	if (rt != E_OK) {
		DBG_ERR("write register error %d \r\n", (INT)rt);
