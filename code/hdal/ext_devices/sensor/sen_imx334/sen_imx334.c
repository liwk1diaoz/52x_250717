...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_imx334, 1, 50, 000, 00);
...
static void sen_get_attr_basic_imx334(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_BASIC_PARAM *data);
...
static ER sen_get_info_imx334(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_ATTR_BASIC:
		sen_get_attr_basic_imx334(id, (CTL_SENDRV_GET_ATTR_BASIC_PARAM *)(data));
		break;
...
static void sen_set_gain_imx334(CTL_SEN_ID id, void *param)
{
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_STAGGER_HDR) {
		if ((mode_basic_param[cur_sen_mode[id]].frame_num) == 2) {
			if(!each_frame_gain_flag[id]) {
				cmd = sen_set_cmd_info_imx334(0x3200, 1, 0x0, 0x0);
				rt |= sen_read_reg_imx334(id, &cmd);
				data = cmd.data[0];
				data = data & (~0x01);
				cmd = sen_set_cmd_info_imx334(0x3200, 1, data, 0x0);
		  	rt |= sen_write_reg_imx334(id, &cmd);
				each_frame_gain_flag[id] = 1;
			}
			cmd = sen_set_cmd_info_imx334(0x3001, 1, 0x01, 0x0);
			rt |= sen_write_reg_imx334(id, &cmd);			
			...
			rt |= sen_write_reg_imx334(id, &cmd);
			cmd = sen_set_cmd_info_imx334(0x3001, 1, 0x00, 0x0);
			rt |= sen_write_reg_imx334(id, &cmd);			
		}
...
static void sen_set_expt_imx334(CTL_SEN_ID id, void *param)
{
	...
	UINT32 shr0 = 0, rhs1 = 0, shr1 = 0, fcs = 0;
	UINT32 real_l_exp = 0, real_s_exp = 0;
	UINT32 temp_max_s_line = 0;
	...
	cmd = sen_set_cmd_info_imx334(0x3030, 1, sensor_vd & 0xFF, 0);
	rt |= sen_write_reg_imx334(id, &cmd);
	cmd = sen_set_cmd_info_imx334(0x3031, 1, (sensor_vd >> 8) & 0xFF, 0);
	rt |= sen_write_reg_imx334(id, &cmd);	
	cmd = sen_set_cmd_info_imx334(0x3032, 1, (sensor_vd >> 16) & 0x0F, 0);
	rt |= sen_write_reg_imx334(id, &cmd);	
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		...
		if ((mode_basic_param[cur_sen_mode[id]].frame_num) == 2) {
			fcs = sensor_vd * 2;
			temp_max_s_line = (100000000 / cur_fps ) * 10 / 17 / t_row; //max support 1:16
			real_l_exp = line[0];
			real_s_exp = line[1];
			...
			if (!BEYOND_FSC) {
				...
				rhs1 = (BRL * 2) - 1;
				if (rhs1 > (9 +  temp_max_s_line)) {
					rhs1 = 9 + temp_max_s_line;//min shr1 = 9
				}
			}
