...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_gc3003, 1, 55, 000, 00);

//=============================================================================
// information
//=============================================================================
#define SEN_GC3003_MODULE_NAME     "sen_gc3003"
...
#define SEN_I2C_COMPATIBLE "nvt,sen_gc3003"
...
//=============================================================================
static CTL_SEN_DRV_TAB *sen_get_drv_tab_gc3003(void);
static void sen_pwr_ctrl_gc3003(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb);
static ER sen_open_gc3003(CTL_SEN_ID id);
static ER sen_close_gc3003(CTL_SEN_ID id);
static ER sen_sleep_gc3003(CTL_SEN_ID id);
static ER sen_wakeup_gc3003(CTL_SEN_ID id);
static ER sen_write_reg_gc3003(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_read_reg_gc3003(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_chg_mode_gc3003(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj);
static ER sen_chg_fps_gc3003(CTL_SEN_ID id, UINT32 fps);
static ER sen_set_info_gc3003(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static ER sen_get_info_gc3003(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static UINT32 sen_calc_chgmode_vd_gc3003(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_calc_exp_vd_gc3003(CTL_SEN_ID id, UINT32 fps);
static void sen_set_gain_gc3003(CTL_SEN_ID id, void *param);
static void sen_set_expt_gc3003(CTL_SEN_ID id, void *param);
static void sen_set_preset_gc3003(CTL_SEN_ID id, ISP_SENSOR_PRESET_CTRL *ctrl);
static void sen_set_flip_gc3003(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
static ER sen_get_flip_gc3003(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
#if defined(__FREERTOS)
void sen_get_gain_gc3003(CTL_SEN_ID id, void *param);
void sen_get_expt_gc3003(CTL_SEN_ID id, void *param);
#else
static void sen_get_gain_gc3003(CTL_SEN_ID id, void *param);
static void sen_get_expt_gc3003(CTL_SEN_ID id, void *param);
#endif
static void sen_get_min_expt_gc3003(CTL_SEN_ID id, void *param);
static void sen_get_mode_basic_gc3003(CTL_SENDRV_GET_MODE_BASIC_PARAM *mode_basic);
static void sen_get_attr_basic_gc3003(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_BASIC_PARAM *data);
static void sen_get_attr_signal_gc3003(CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *data);
static ER sen_get_attr_cmdif_gc3003(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_CMDIF_PARAM *data);
static ER sen_get_attr_if_gc3003(CTL_SENDRV_GET_ATTR_IF_PARAM *data);
static void sen_get_fps_gc3003(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data);
static void sen_get_speed_gc3003(CTL_SEN_ID id, CTL_SENDRV_GET_SPEED_PARAM *data);
static void sen_get_mode_mipi_gc3003(CTL_SENDRV_GET_MODE_MIPI_PARAM *data);
static void sen_get_modesel_gc3003(CTL_SENDRV_GET_MODESEL_PARAM *data);
static UINT32 sen_calc_rowtime_gc3003(CTL_SEN_ID id, CTL_SEN_MODE mode);
static UINT32 sen_calc_rowtime_step_gc3003(CTL_SEN_ID id, CTL_SEN_MODE mode);
static void sen_get_rowtime_gc3003(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data);
static void sen_set_cur_fps_gc3003(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_cur_fps_gc3003(CTL_SEN_ID id);
static void sen_set_chgmode_fps_gc3003(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_chgmode_fps_gc3003(CTL_SEN_ID id);
//=============================================================================
// global variable
//=============================================================================
...
static CTL_SEN_CMD gc3003_mode_1[] = {
	//mclk 27Mhz
	//mipi_data rate=630Mbps/lane
	//vts = 1338
	//window_size=2304x1298
	//row time=24.88us
	{0x03fe, 1, {0xf0, 0x0}},
	{0x03fe, 1, {0xf0, 0x0}},
	{0x03fe, 1, {0xf0, 0x0}},
...
{
	//00d1 00d0  00b8  00b9  0155  0080
	{0x00, 0x00, 0x01, 0x00, 0x04, 0x09,   1000},
	{0x0A, 0x00, 0x01, 0x0c, 0x04, 0x0b,   1187},
	{0x00, 0x01, 0x01, 0x1a, 0x04, 0x0d,   1406},
	{0x0A, 0x01, 0x01, 0x2a, 0x04, 0x0e,   1656},
	{0x20, 0x00, 0x02, 0x00, 0x04, 0x10,   2000},
	{0x25, 0x00, 0x02, 0x18, 0x04, 0x11,   2375},
	{0x20, 0x01, 0x02, 0x33, 0x04, 0x12,   2796},
	{0x25, 0x01, 0x03, 0x14, 0x04, 0x14,   3312},
	{0x30, 0x00, 0x04, 0x00, 0x04, 0x15,   4000},
	{0x32, 0x80, 0x04, 0x2f, 0x04, 0x16,   4734},
	{0x30, 0x01, 0x05, 0x26, 0x04, 0x18,   5593},
	{0x32, 0x81, 0x06, 0x29, 0x04, 0x19,   6640},
	{0x38, 0x00, 0x08, 0x00, 0x04, 0x1a,   8000},
	{0x39, 0x40, 0x09, 0x1f, 0x06, 0x1c,   9484},
	{0x38, 0x01, 0x0b, 0x0d, 0x06, 0x1e,  11187},
	{0x39, 0x41, 0x0d, 0x12, 0x06, 0x20,  13250},
	{0x30, 0x08, 0x10, 0x00, 0x06, 0x24,  16000},
	{0x32, 0x88, 0x12, 0x3e, 0x06, 0x26,  18968},
	{0x30, 0x09, 0x16, 0x1a, 0x06, 0x2a,  22406},
	{0x32, 0x89, 0x1a, 0x23, 0x06, 0x2d,  26546},
	{0x38, 0x08, 0x20, 0x00, 0x06, 0x31,  32000},
	{0x39, 0x48, 0x25, 0x3b, 0x09, 0x36,  37921},
	{0x38, 0x09, 0x2c, 0x33, 0x0b, 0x3a,  44765},
	{0x39, 0x49, 0x35, 0x06, 0x0d, 0x3c,  53015},
	{0x38, 0x0A, 0x3f, 0x3f, 0x0f, 0x3f,  64000},
};
...
static CTL_SEN_DRV_TAB gc3003_sen_drv_tab = {
	sen_open_gc3003,
	sen_close_gc3003,
	sen_sleep_gc3003,
	sen_wakeup_gc3003,
	sen_write_reg_gc3003,
	sen_read_reg_gc3003,
	sen_chg_mode_gc3003,
	sen_chg_fps_gc3003,
	sen_set_info_gc3003,
...
static CTL_SEN_CMD sen_set_cmd_info_gc3003(UINT32 addr, UINT32 data_length, UINT32 data0, UINT32 data1)
{
	CTL_SEN_CMD cmd;
...
#if defined(__KERNEL__)
static void sen_load_cfg_from_compatible_gc3003(struct device_node *of_node)
{
...
static ER sen_open_gc3003(CTL_SEN_ID id)
{
	...
	#if defined(__KERNEL__)
	sen_i2c_reg_cb(sen_load_cfg_from_compatible_gc3003);
	#endif
...
static UINT32 sen_get_cmd_tab_gc3003(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	switch (mode) {
	case CTL_SEN_MODE_1:
		*cmd_tab = gc3003_mode_1;
		return sizeof(gc3003_mode_1) / sizeof(CTL_SEN_CMD);
...
static ER sen_chg_mode_gc3003(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj)
{
	...
	if (is_fastboot[id]) {
		...
		sen_set_chgmode_fps_gc3003(id, isp_builtin_get_chgmode_fps(id));
		sen_set_cur_fps_gc3003(id, isp_builtin_get_chgmode_fps(id));
		sen_set_gain_gc3003(id, &sensor_ctrl);
		sen_set_expt_gc3003(id, &sensor_ctrl);
		#endif
	...
	cmd_num = sen_get_cmd_tab_gc3003(chgmode_obj.mode, &p_cmd_list);
	if (p_cmd_list == NULL) {
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_gc3003(id, chgmode_obj.frame_rate);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_gc3003(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_gc3003(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
	...
	for (idx = 0; idx < cmd_num; idx++) {
		if (p_cmd_list[idx].addr == SEN_CMD_DELAY) {
			...
			cmd = sen_set_cmd_info_gc3003(0x0D41, 1, (sensor_vd >> 8) & 0x3F, 0x00);
			rt |= sen_write_reg_gc3003(id, &cmd);
			cmd = sen_set_cmd_info_gc3003(0x0D42, 1, sensor_vd & 0xFF, 0x00);
			rt |= sen_write_reg_gc3003(id, &cmd);
			...
			sen_set_flip_gc3003(id, &flip);
		} else {
			cmd = sen_set_cmd_info_gc3003(p_cmd_list[idx].addr, p_cmd_list[idx].data_len, p_cmd_list[idx].data[0], p_cmd_list[idx].data[1]);
			rt |= sen_write_reg_gc3003(id, &cmd);
		}
	}
...
static ER sen_chg_fps_gc3003(CTL_SEN_ID id, UINT32 fps)
{
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_gc3003(id, fps);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_gc3003(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_gc3003(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
...
static ER sen_set_info_gc3003(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_SET_EXPT:
		sen_set_expt_gc3003(id, data);
		break;
	case CTL_SENDRV_CFGID_SET_GAIN:
		sen_set_gain_gc3003(id, data);
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		sen_set_flip_gc3003(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE1:
		sen_set_preset_gc3003(id, (ISP_SENSOR_PRESET_CTRL *)(data));
		break;
	default:
		return E_NOSPT;
	}
...
static ER sen_get_info_gc3003(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_EXPT:
		sen_get_expt_gc3003(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_GAIN:
		sen_get_gain_gc3003(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_BASIC:
		sen_get_attr_basic_gc3003(id, (CTL_SENDRV_GET_ATTR_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_SIGNAL:
		sen_get_attr_signal_gc3003((CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_CMDIF:
		rt = sen_get_attr_cmdif_gc3003(id, (CTL_SENDRV_GET_ATTR_CMDIF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_IF:
		rt = sen_get_attr_if_gc3003((CTL_SENDRV_GET_ATTR_IF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_FPS:
		sen_get_fps_gc3003(id, (CTL_SENDRV_GET_FPS_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_SPEED:
		sen_get_speed_gc3003(id, (CTL_SENDRV_GET_SPEED_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_BASIC:
		sen_get_mode_basic_gc3003((CTL_SENDRV_GET_MODE_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_MIPI:
		sen_get_mode_mipi_gc3003((CTL_SENDRV_GET_MODE_MIPI_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODESEL:
		sen_get_modesel_gc3003((CTL_SENDRV_GET_MODESEL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_ROWTIME:
		sen_get_rowtime_gc3003(id, (CTL_SENDRV_GET_MODE_ROWTIME_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		rt = sen_get_flip_gc3003(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE2:
		sen_get_min_expt_gc3003(id, data);
		break;
	default:
		rt = E_NOSPT;
	}
...
static UINT32 sen_calc_chgmode_vd_gc3003(CTL_SEN_ID id, UINT32 fps)
{
	...
	sen_set_chgmode_fps_gc3003(id, fps);
	sen_set_cur_fps_gc3003(id, fps);
...
static void sen_set_gain_gc3003(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {	
		...
		if (tbl_idx < 1) {
			...
			data1[frame_cnt] = (gain_table[0].gain_reg0 << 16) | (gain_table[0].gain_reg1 << 8) | (gain_table[0].gain_reg2);
			data2[frame_cnt] = (gain_table[0].gain_reg3 << 16) | (gain_table[0].gain_reg4 << 8) | (gain_table[0].gain_reg5);
			data3[frame_cnt] = decimal;
		} else {
			...
			data1[frame_cnt] = (gain_table[tbl_idx-1].gain_reg0 << 16) | (gain_table[tbl_idx-1].gain_reg1 << 8) | (gain_table[tbl_idx-1].gain_reg2);
			data2[frame_cnt] = (gain_table[tbl_idx-1].gain_reg3 << 16) | (gain_table[tbl_idx-1].gain_reg4 << 8) | (gain_table[tbl_idx-1].gain_reg5);
			data3[frame_cnt] = decimal;
		}
	}

	cmd = sen_set_cmd_info_gc3003(0x00D1, 1, (data1[0] >> 16) & 0xFF, 0x0);
	rt |= sen_write_reg_gc3003(id, &cmd);
	cmd = sen_set_cmd_info_gc3003(0x00D0, 1, (data1[0] >> 8) & 0xFF, 0x0);
	rt |= sen_write_reg_gc3003(id, &cmd);
	cmd = sen_set_cmd_info_gc3003(0x00B8, 1, data1[0] & 0xFF, 0x0);
	rt |= sen_write_reg_gc3003(id, &cmd);
	cmd = sen_set_cmd_info_gc3003(0x00B9, 1, (data2[0] >> 16) & 0xFF, 0x0);
	rt |= sen_write_reg_gc3003(id, &cmd);
	cmd = sen_set_cmd_info_gc3003(0x0155, 1, (data2[0] >> 8) & 0xFF, 0x0);
	rt |= sen_write_reg_gc3003(id, &cmd);
	cmd = sen_set_cmd_info_gc3003(0x0080, 1, data2[0] & 0xFF, 0x0);
	rt |= sen_write_reg_gc3003(id, &cmd);

	cmd = sen_set_cmd_info_gc3003(0x00B1, 1, (data3[0] >> 6) & 0x0F, 0x0);
	rt |= sen_write_reg_gc3003(id, &cmd);
	cmd = sen_set_cmd_info_gc3003(0x00B2, 1, (data3[0] & 0x3F) << 2, 0x0);
	rt |= sen_write_reg_gc3003(id, &cmd);	
...
static void sen_set_expt_gc3003(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		t_row = sen_calc_rowtime_gc3003(id, cur_sen_mode[id]);
		if (0 == t_row) {
	...
	// Get fps
	chgmode_fps = sen_get_chgmode_fps_gc3003(id);
	...
	sen_set_cur_fps_gc3003(id, cur_fps);

	// Calculate new vd
	sensor_vd = sen_calc_exp_vd_gc3003(id, cur_fps);
	...
	cmd = sen_set_cmd_info_gc3003(0x0D41, 1, (sensor_vd >> 8) & 0x3F, 0);
	rt |= sen_write_reg_gc3003(id, &cmd);

	cmd = sen_set_cmd_info_gc3003(0x0D42, 1, sensor_vd & 0xFF, 0);
	rt |= sen_write_reg_gc3003(id, &cmd);
	...
	cmd = sen_set_cmd_info_gc3003(0x0D03, 1, (line[0] >> 8) & 0x3F , 0);
	rt |= sen_write_reg_gc3003(id, &cmd);
	cmd = sen_set_cmd_info_gc3003(0x0D04, 1, line[0] & 0xFF, 0);
	rt |= sen_write_reg_gc3003(id, &cmd);
...
static void sen_set_flip_gc3003(CTL_SEN_ID id, CTL_SEN_FLIP *flip)
{
	...
	cmd = sen_set_cmd_info_gc3003(0x0015, 1, 0x0, 0x0);
	rt |= sen_read_reg_gc3003(id, &cmd);
	temp_reg1 = cmd.data[0];

	cmd = sen_set_cmd_info_gc3003(0x0D15, 1, 0x0, 0x0);
	rt |= sen_read_reg_gc3003(id, &cmd);
	...
	cmd = sen_set_cmd_info_gc3003(0x0015, 1, temp_reg1, 0x0);
	rt |= sen_write_reg_gc3003(id, &cmd);

	cmd = sen_set_cmd_info_gc3003(0x0D15, 1, temp_reg2, 0x0);
	rt |= sen_write_reg_gc3003(id, &cmd);
...
static void sen_get_fps_gc3003(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data)
{
	data->cur_fps = sen_get_cur_fps_gc3003(id);
	data->chg_fps = sen_get_chgmode_fps_gc3003(id);
}
...
static void sen_get_rowtime_gc3003(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data)
{
	data->row_time_step = sen_calc_rowtime_step_gc3003(id, data->mode);
	data->row_time = sen_calc_rowtime_gc3003(id, data->mode) * (data->row_time_step);
}
...
#if defined(__FREERTOS)
void sen_get_i2c_id_gc3003(CTL_SEN_ID id, UINT32 *i2c_id)
{
	*i2c_id = sen_i2c[id].id;
}

void sen_get_i2c_addr_gc3003(CTL_SEN_ID id, UINT32 *i2c_addr)
{
	*i2c_addr = sen_i2c[id].addr;
}

int sen_init_gc3003(SENSOR_DTSI_INFO *info)
{
	...
	sprintf(compatible, "nvt,sen_gc3003");
	if (sen_common_check_compatible(compatible)) {
		...
		sprintf(node_path, "/sensor/sen_cfg/sen_gc3003");
	...
	reg_obj.pwr_ctrl = sen_pwr_ctrl_gc3003;
	reg_obj.det_plug_in = NULL;
	reg_obj.drv_tab = sen_get_drv_tab_gc3003();
	rt = ctl_sen_reg_sendrv("nvt_sen_gc3003", &reg_obj);
	if (rt != E_OK) {
...
void sen_exit_gc3003(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_gc3003");
}

#else
static int __init sen_init_gc3003(void)
{
...
static void __exit sen_exit_gc3003(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_gc3003");
}

module_init(sen_init_gc3003);
module_exit(sen_exit_gc3003);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION(SEN_GC3003_MODULE_NAME);
MODULE_LICENSE("GPL");
#endif
