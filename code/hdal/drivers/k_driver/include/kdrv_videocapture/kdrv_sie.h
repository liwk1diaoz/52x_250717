...
#define KDRV_SIE_MAX_ENG_NT98515		3	//maximum support engine number for 98515
#define KDRV_SIE_MAX_ENG_NT98528		5	//maximum support engine number for 98528
...
#define KDRV_SIE_LA_HIST_BIN			128
...
typedef enum {
	...
	KDRV_SIE_MCLKSRC_480,      ///< SIE M-clock source as 480MHz			//680/520/528
	KDRV_SIE_MCLKSRC_PLL4,     ///< not support 							//680
	KDRV_SIE_MCLKSRC_PLL5,     ///< SIE M-clock source as PLL5 (sensor 1)	//680/520/528
	KDRV_SIE_MCLKSRC_PLL10,    ///< SIE M-clock source as PLL10
	KDRV_SIE_MCLKSRC_PLL12,    ///< SIE M-clock source as PLL12(sensor 2)	//520/528
	KDRV_SIE_MCLKSRC_PLL18,    ///< SIE M-clock source as PLL18(sensor 3)	//528
...
typedef enum {
	...
	KDRV_SIE_INT_DRAM_IO_ERR    = 0x00000040, 	///< interrupt: in/out err
...
typedef struct {
	UINT32 src_addr;
	KDRV_SIE_CA_RST *ca_rst;
} KDRV_SIE_CA_RST_INFO;
...
typedef struct {
	UINT32 src_addr;
	KDRV_SIE_LA_RST *la_rst;
} KDRV_SIE_LA_RST_INFO;
...
typedef struct {
	...
	USIZE ca_win_max_num;		//ca window maximum number
	USIZE la_win_max_num;		//la window maximum number
	USIZE yout_win_max_num;		//y out window maximum number
	UINT32 la_hist_bin_num;		//la histogram maximum bin number
	USIZE pat_gen_src_win_align; // for src width/height
	USIZE act_win_align;		//active size w/h size align
	USIZE crp_win_align;		//crop size w/h size align
	USIZE scale_out_align;		//scale out size w/h size align
	USIZE ca_crp_win_align;		//ca crop window size w/h size align
	USIZE la_crp_win_align;		//la crop window size w/h size align
	UINT32 out_lofs_align[KDRV_SIE_CH_MAX];	//output lineoofset align
	UINT32 support_func;		//reference to KDRV_SIE_FUNC_SUPPORT
	UINT32 ring_buf_len_max;	//ring buffer length maximum
} KDRV_SIE_LIMIT;

/**
    KDRV RHE all IQ function enable/disable operation for IPL
*/
...
typedef enum {
	...
	KDRV_SIE_ITEM_DATA_FMT,		///< data_type: [Set/Get] KDRV_SIE_DATAFORMAT
	KDRV_SIE_ITEM_YUV_ORDER,	///< data_type: [Set/Get] KDRV_SIE_YUV_ORDER
	KDRV_SIE_ITEM_SIGNAL,		///< data_type: [Set/Get] KDRV_SIE_SIGNAL
	KDRV_SIE_ITEM_FLIP,			///< data_type: [Set/Get] CTL_SIE_FLIP_TYPE
	...
	KDRV_SIE_ITEM_OUTPUT_MODE,  ///< data_type: [Set/Get] KDRV_SIE_OUTPUT_MODE
