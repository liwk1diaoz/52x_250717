...
typedef struct {
	...
	UINT32 src_frame_rate;		///< [r/w] input frame rate when skip frame enable
	UINT32 encode_frame_rate;	///< [r/w] target frame rate when skip frame enable
} KDRV_VDOENC_CBR;
...
typedef struct {
	...
	UINT8 avc_inter_luma_coeff_cost_th;		// [r/w] default: 5, range: 0~15, //INTER Y coefficient cost threshold to eliminate coefficient
	UINT8 avc_inter_chroma_coeff_cost_th;	// [r/w] default: 5, range: 0~15, //INTER C coefficient cost threshold to eliminate coefficient
} KDRV_VDOENC_RDO_264;
...
typedef struct {
	...
	KDRV_VDOENC_FRM_CROP_INFO frm_crop_info; ///< frame crop information
} KDRV_VDOENC_INIT;
...
typedef struct {
	...
	UINT32 evbr_still_flag;                             ///< [r] evbr state
	UINT32 re_trigger;
...
/*!
 * @fn INT32 kdrv_videoenc_open(UINT32 chip, UINT32 engine)
