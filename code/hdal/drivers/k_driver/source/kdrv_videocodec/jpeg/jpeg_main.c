...
//=============================================================================
//static int nvt_jpg_open(struct inode *inode, struct file *file);
//static int nvt_jpg_release(struct inode *inode, struct file *file);
//static long nvt_jpg_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
//static int nvt_jpg_probe(struct platform_device *pdev);
//static int nvt_jpg_suspend(struct platform_device *pdev, pm_message_t state);
//static int nvt_jpg_resume(struct platform_device *pdev);
//static int nvt_jpg_remove(struct platform_device *pdev);
...
static long nvt_jpg_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	return nvt_jpg_drv_ioctl(MINOR(inode->i_rdev), &pdrv->module_info, cmd, arg);
}

struct file_operations nvt_jpg_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_jpg_open,
	.release = nvt_jpg_release,
	.unlocked_ioctl = nvt_jpg_ioctl,
	.llseek  = no_llseek,
};
...
static int nvt_jpg_probe(struct platform_device *pdev)
{
	...
	cdev_init(&pdrv_info->cdev, &nvt_jpg_fops);
...
static struct platform_driver kdrv_jpg_driver = {
	.driver = {
		.name   = "kdrv_jpg",
		.owner = THIS_MODULE,
	...
	//.probe      = nvt_jpg_probe,
	//.remove     = nvt_jpg_remove,
	//.suspend = nvt_jpg_suspend,
	//.resume = nvt_jpg_resume
};
...
int __init nvt_jpg_module_init(void)
{
	...
	ret = platform_driver_register(&kdrv_jpg_driver);
...
void __exit nvt_jpg_module_exit(void)
{
	...
	platform_driver_unregister(&kdrv_jpg_driver);
}
