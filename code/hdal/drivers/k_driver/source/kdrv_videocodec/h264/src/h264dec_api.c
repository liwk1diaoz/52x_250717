...
INT32 h264Dec_prepareOnePicture(PH264DEC_INFO pH264DecInfo, H26XDEC_VAR *pH264DecVar)
{
	...
	UINT32 uiHwHeaderNum, uiHwHeaderAddr, uiHwHeaderSize[32];
	UINT32 uiBsSize = (pH264DecInfo->uiCurBsSize == 0) ? pH264DecInfo->uiBSSize : pH264DecInfo->uiCurBsSize;
	UINT32 uiIdx = 0;

	h26x_cache_clean(pH264DecInfo->uiBSAddr, pH264DecInfo->uiBSSize);
...
static UINT32 query_dec_mem_size(UINT32 uiWidth, UINT32 uiHeight)
{
	...
	return SIZE_4X(uiSizeContext + uiSizeSideInfo*2 + uiSizeColMVs + uiSizeBSDMA + uiSizeAPB + uiSizeDummyWT + 128);
}
...
UINT32 h264Dec_getResYAddr2(H26XDEC_VAR *pH264DecVar)
{
	H264DEC_CTX *pContext = pH264DecVar->pContext;

	return pContext->sH264VirDecAddr.uiResYAddr2;
}
