...
#if 0
	save_to_reg(&pH264RegSet->FUNC_CFG[1], (pH264DecSeqCfg->uiPicWidth <= 2048) , 3, 1); //flexible search range enable, flxsr_en
#else
    save_to_reg(&pH264RegSet->FUNC_CFG[1], (pH264DecSeqCfg->uiPicWidth <= 4096)&&(pH264DecSeqCfg->uiPicWidth >2176) , 3, 1); //flexible search range enable, flxsr_en
    save_to_reg(&pH264RegSet->FUNC_CFG[1], (pH264DecSeqCfg->uiPicWidth <= 2176) , 22, 1); //flexible search range enable, flxsr_en
#endif
...
void H264Dec_modifyFrmIdx(H264DecHdrObj *pH264DecHdrObj, H26XDecAddr *pH264VirDecAddr)
{
	...
	{
		//pH264VirDecAddr->uiResYAddr = pH264VirDecAddr->uiRefAndRecYAddr[FRM_IDX_LT_0];
