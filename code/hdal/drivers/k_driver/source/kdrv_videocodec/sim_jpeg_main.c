...
//#include "jpgheader.h"
#include "jpeg/include/jpeg_file.h"
#include "jpeg/include/jpeg_emu.h"
//#include "jpeg_enc.h"
#include "../include/jpeg.h"
#include <mach/fmem.h>
//#include "imgtrans/grph.h"
//[temporary mark]#include "../../kdrv_gfx2d/kdrv_grph/include/grph_compatible.h"
//#include "../../../../../../linux-driver/include/imgtrans/grph.h"
#include "../include/jpg_enc.h"
#include "../include/jpg_header.h"
#include "jpeg/hal/jpg_int.h"
...
extern INT32 kdrv_videoenc_set(UINT32 id, KDRV_VDOENC_SET_PARAM_ID parm_id, VOID *param);
extern INT32 kdrv_videoenc_get(UINT32 id, KDRV_VDOENC_GET_PARAM_ID parm_id, VOID *param);
...
//printk(("(6) ========== EncBuf.uiTempYBuf: 0x%x",EncBuf.uiTempYBuf));
	...
	UINT32 freq2;
       ...
       kdrv_videoenc_set(0, VDOENC_SET_JPEG_FREQ, &freq);

	   kdrv_videoenc_get(0, VDOENC_GET_JPEG_FREQ, &freq2);
	   printk("VDOENC_GET_JPEG_FREQ = %d\r\n", (int)(freq2));

#if 1//KDRV_JPG
//jpeg_open();
//kdrv_videoencjpg_open(0,KDRV_VIDEOCDC_ENGINE_JPEG);
kdrv_videoenc_open(0,KDRV_VIDEOCDC_ENGINE_JPEG);
			...
			enc_obj.bs_start_addr = EncBuf.uiBSBuf;


   			//if (kdrv_videoencjpg_trigger(0, &enc_obj, &callback, &i) != 0)
   			//if (kdrv_videoencjpg_trigger(0, &enc_obj, NULL, &i) != 0)
   			if (kdrv_videoenc_trigger(KDRV_DEV_ID(0, KDRV_VIDEOCDC_ENGINE_JPEG, 0), &enc_obj, NULL, &i) != 0)
				printk("encode error\r\n");

			jpeg_hdr_size = enc_obj.svc_hdr_size;
//jpeg_close();
							  ...
							  UINT32 uiDstLineOffset)
{
	...
	for (i = 0; i < uiHeight; i++) {
		for (j = 0; j < uiWidth; j++) {
			...
			if (uiSrc != uiDst) {
				printk("HW: 0x%.2X, Pattern: 0x%.2X\r\n", uiDst, uiSrc);
				printk("U Byte %dx%d (%dx%d) compare error!\r\n", (int)(j), (int)(i), (int)(uiWidth), (int)(uiHeight));
			...
			if (uiSrc != uiDst) {
				printk("HW: 0x %X, Pattern: 0x %X\r\n", (unsigned int)(uiDst), (unsigned int)(uiSrc));
				printk("V Byte %dx%d (%dx%d) compare error!\r\n", (int)(j), (int)(i), (int)(uiWidth), (int)(uiHeight));
...
static BOOL emu_jpegReadSrcBSFile(void)
{
	...
	if (file_hdl != NULL) {
		// Read header
		uiFileSize = pDecRegSet->JPEGDecRegData.HEADER_LEN+pDecRegSet->JPEGDecRegData.BS_LEN;
...
//    case EMU_JPEG_MCU_100:
	...
	if (pDecRegSet->JPEGDecRegData.CROP_EN == TRUE) {
		printk("Crop size: %dx%d (%dx%d MCU)\r\n",
...
//printk("\r\nuiUVExtraSize = 0x%x\r\n", (unsigned int)(uiUVExtraSize));
	...
	// Calculate available address for next buffer
	uiTempAddr += pDecRegSet->JPEGDecRegData.HEADER_LEN+pDecRegSet->JPEGDecRegData.BS_LEN;
	...
	UINT32 freq2;
       ...
       kdrv_videodec_set(0, VDODEC_SET_JPEG_FREQ, &freq);

	   kdrv_videodec_get(0, VDODEC_GET_JPEG_FREQ, &freq2);
	   printk("VDODEC_GET_JPEG_FREQ = %d\r\n", (int)(freq2));


  //jpeg_open();
//   kdrv_videoencjpg_open(0,KDRV_VIDEOCDC_ENGINE_JPEG);
     kdrv_videodec_open(0,KDRV_VIDEOCDC_ENGINE_JPEG);
            //enc_info.encode_width=uiYWidth;
            //enc_info.encode_height=uiYHeight;
            //enc_info.in_fmt=KDRV_JPEGYUV_FORMAT_420;
            //enc_info.retstart_interval = pEncRegSet->JPEGEncRegData.RSTR_NUM;
...
printk("uiDecYLineOffset= %d \r\n", (int)(uiDecYLineOffset));
printk("uiDecUVLineOffset = %d  \r\n", (int)(uiDecUVLineOffset));
			...
			dec_obj.bs_size= uiBSLen;
   			...
   			if (kdrv_videodec_trigger(KDRV_DEV_ID(0, KDRV_VIDEOCDC_ENGINE_JPEG, 0), &dec_obj, NULL, &i) != 0)
				printk("decode error\r\n");
...
//frammap_buf_t      buf_info = {0};
	...
	frm_get_buf_ddr(DDR_ID0, &buf_info);
#endif
	printk("buf_info.va_addr = 0x%08x, buf_info.phy_addr = 0x%08x\r\n", (UINT32)buf_info.vaddr, buf_info.paddr);

	//emu_jpeg_encode((UINT32)buf_info.vaddr, buf_info.size);
	//frm_free_buf_ddr((void *)buf_info.va_addr);
}

void sim_kdrv_jpgdec(void)
{
#if 0//KDRV_JPG_EMU
	frammap_buf_t      buf_info = {0};
...
//frammap_buf_t      buf_info = {0};
	...
	//emu_jpeg_decode((UINT32)buf_info.vaddr, buf_info.size);
	//frm_free_buf_ddr((void *)buf_info.va_addr);
}