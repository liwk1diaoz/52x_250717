...
void H265Dec_initRegSet(H26XRegSet *pH265RegSet, H26XDecAddr *pH265VirDecAddr)
{
	...
	pH265RegSet->NAL_LEN_OUT_ADDR = h26x_getPhyAddr(pH265VirDecAddr->uiDummyWTAddr);
	...
	if (h26x_getChipId() == 98528) {
		save_to_reg(&pH265RegSet->TIMEOUT_CNT_MAX, 0x70800, 0, 24);
	} else {
...
void H265Dec_prepareRegSet(H26XRegSet *pH265RegSet, H26XDecAddr *pH265VirDecAddr, H265DEC_CTX *pContext)
{
	...
	if (pps->tiles_enabled_flag) {
		if (h26x_getChipId() == 98520) {
			if (pps->num_tile_columns_minus1 > 1) {
				if (pH265DecSeqCfg->uiTileWidth[0] > H26X_FIRST_TILE_MAX_W_SR36 || pH265DecSeqCfg->uiTileWidth[1] > H26X_MIDDLE_TILE_MAX_W_SR36) {
					save_to_reg(&pH265RegSet->FUNC_CFG[1], 0, 3, 1);
				} else {
					save_to_reg(&pH265RegSet->FUNC_CFG[1], 1, 3, 1);
				}
			} else {
				if (pH265DecSeqCfg->uiTileWidth[0] > H26X_FIRST_TILE_MAX_W_SR36) {
					save_to_reg(&pH265RegSet->FUNC_CFG[1], 0, 3, 1);
				} else {
					save_to_reg(&pH265RegSet->FUNC_CFG[1], 1, 3, 1);
				}
			...
			if (pps->num_tile_columns_minus1 > 1) {
				if (pH265DecSeqCfg->uiTileWidth[0] > H26X_FIRST_TILE_MAX_W_SR36_528 || pH265DecSeqCfg->uiTileWidth[1] > H26X_MIDDLE_TILE_MAX_W_SR36_528) {
					save_to_reg(&pH265RegSet->FUNC_CFG[1], 0, 3, 1);
				} else {
					if (pH265DecSeqCfg->uiTileWidth[0] > H26X_FIRST_TILE_MAX_W_SR52_528 || pH265DecSeqCfg->uiTileWidth[1] > H26X_MIDDLE_TILE_MAX_W_SR52_528) {
						save_to_reg(&pH265RegSet->FUNC_CFG[1], 1, 3, 1);
					} else {
						save_to_reg(&pH265RegSet->FUNC_CFG[1], 1, 22, 1);
					}
				}
			} else {
				if (pH265DecSeqCfg->uiTileWidth[0] > H26X_FIRST_TILE_MAX_W_SR36_528) {
					save_to_reg(&pH265RegSet->FUNC_CFG[1], 0, 3, 1);
				} else {
					if (pH265DecSeqCfg->uiTileWidth[0] > H26X_FIRST_TILE_MAX_W_SR52_528) {
						save_to_reg(&pH265RegSet->FUNC_CFG[1], 1, 3, 1);
					} else {
						save_to_reg(&pH265RegSet->FUNC_CFG[1], 1, 22, 1);
					}
		...
		if (h26x_getChipId() == 98520) {
			if (pH265DecSeqCfg->uiPicWidth <= H26X_MAX_W_WITHOUT_TILE_V36)
				save_to_reg(&pH265RegSet->FUNC_CFG[1], 0, 3, 1);
			else
				save_to_reg(&pH265RegSet->FUNC_CFG[1], 1, 3, 1);
		} else {
			if (pH265DecSeqCfg->uiPicWidth <= H26X_MAX_W_WITHOUT_TILE_V36_528)
				save_to_reg(&pH265RegSet->FUNC_CFG[1], 1, 22, 1);
			else if (pH265DecSeqCfg->uiPicWidth <= H26X_MAX_W_WITHOUT_TILE_V28_528)
				save_to_reg(&pH265RegSet->FUNC_CFG[1], 0, 3, 1);
			else
				save_to_reg(&pH265RegSet->FUNC_CFG[1], 1, 3, 1);
		}
	...
	if (sps->max_transform_hierarchy_depth_intra == 0) {
		save_to_reg(&pH265RegSet->FUNC_CFG[1], 1, 31, 1);
	} else {
		save_to_reg(&pH265RegSet->FUNC_CFG[1], 0, 31, 1);
	}
	...
	save_to_reg(&pH265RegSet->AEAD_CFG, sps->log2_max_pic_order_cnt_lsb_minus4,  28, 4);
