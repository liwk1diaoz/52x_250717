...
static INT32 h265Dec_initInternalMemory(H265DEC_INIT *pH265DecInit, H26XDEC_VAR *pH265DecVar)
{
	...
	//pH265VirDecAddr->uiFrmBufNum = uiFrmBufNum; //hk: move after H265DecSeqHeader to get svc & lt info
	...
	SetMemoryAddr(&pH265VirDecAddr->uiDummyWTAddr, &uiVirMemAddr, uiSizeDummyWT);
...
//determine uiFrmBufNum
{
	H265DecSpsObj *pH265DecSpsObj = &pH265DecHdrObj->sH265DecSpsObj;
	u32 uiNumStRps = pH265DecSpsObj->num_short_term_ref_pic_sets;
	u32 iRefFrmNum = pH265DecSpsObj->sps_max_dec_pic_buffering_minus1[0] + 1 ;

	if(pH265DecSpsObj->long_term_ref_pics_present_flag) {
		uiNumStRps--;
		iRefFrmNum--;
	}

	if( uiNumStRps == 2 && iRefFrmNum == 2 ){
		pH265DecSpsObj->ucSVCLayer = 1;
	}
	else if( uiNumStRps == 4 && iRefFrmNum == 3 ){
		pH265DecSpsObj->ucSVCLayer = 2;
	}
	else if( uiNumStRps == 1 && iRefFrmNum == 2 ){
		pH265DecSpsObj->ucSVCLayer = 0;
	}
	else{
		DBG_ERR("uiNumStRps %d, iRefFrmNum %d error!\r\n",(unsigned int)uiNumStRps,(unsigned int)iRefFrmNum);
	}

	switch (pH265DecSpsObj->ucSVCLayer) { //svc
    case 0:
        pH265VirDecAddr->uiFrmBufNum = 2;
        break;
    case 1:
      	pH265VirDecAddr->uiFrmBufNum = 2;
        break;
    case 2:
        pH265VirDecAddr->uiFrmBufNum = 3;
        break;
    default:
        pH265VirDecAddr->uiFrmBufNum = 2;
	}

	if(pH265DecSpsObj->long_term_ref_pics_present_flag) { //ltr
		pH265VirDecAddr->uiFrmBufNum++ ;
	}

	//DBG_DUMP("uiFrmBufNum=%d\n",(unsigned int)pH265VirDecAddr->uiFrmBufNum);
}
...
INT32 h265Dec_prepareOnePicture(H265DEC_INFO *pH265DecInfo, H26XDEC_VAR *pH265DecVar)
{
	...
	UINT32 uiIdx;
#if (NEW_SVC == 1)
    int RecIsLTSTREC[FRM_IDX_MAX];
	for (uiIdx = 0; uiIdx < FRM_IDX_MAX; uiIdx++) {
        RecIsLTSTREC[uiIdx] = 0;
	}
#endif
	h26x_cache_clean(pH265DecInfo->uiBSAddr, pH265DecInfo->uiBSSize);
...

	{
		...
		if(sps->ucSVCLayer==0 && sps->long_term_ref_pics_present_flag==0) //normal
			pH265VirDecAddr->uiResYAddr = (pH265DecPicCfg->uiPicDecNum==0) ? 0 : pH265VirDecAddr->uiRefAndRecYAddr[pH265VirDecAddr->uiRecIdx];
		...
		//for (j = 0; j < FRM_IDX_MAX; j++) {
		for (i = 0; i < 2; i++) {
			for (j = 0; j < (int)uiFrmBufNum; j++) {
				if (PocStCurrBefore[i] != -1 &&
					pH265VirDecAddr->iRefAndRecPOC[j] == PocStCurrBefore[i]) {
					RecIsLTSTREC[j] |= 1<<0;
				}
			}
		}
		for (i = 0; i < (int)slice->num_long_term_ref_pics; i++) {
			for (j = 0; j < (int)uiFrmBufNum; j++) {
				if (slice->pocLt[i] != -1 &&
					slice->pocLt[i] == pH265VirDecAddr->iRefAndRecPOC[j]) {
					RecIsLTSTREC[j] |= 1<<1;
				}
			}
		}
		...
		if(sps->ucSVCLayer>0 || sps->long_term_ref_pics_present_flag) //svc or ltr
		...
		DBG_IND("refPoc=%d Poc=%d ref = %d, rec = %d TLid=%d\r\n",
		...
		for (i = 0; i < (int)uiFrmBufNum; i++) {
			if (pH265VirDecAddr->uiRefAndRecYAddr[i]) {
				DBG_IND("Frame buffer[%d] POC=%d, 0x%08x, lt = %d\r\n", (int)(i), (int)(pH265VirDecAddr->iRefAndRecPOC[i]),(int)h26x_getPhyAddr(pH265VirDecAddr->uiRefAndRecYAddr[i]),(int)pH265VirDecAddr->uiRefAndRecIsLT[i]);
			} else {
				DBG_IND("Frame buffer[%d] POC=%d, 0x%08x, lt = %d\r\n", (int)(i), (int)(pH265VirDecAddr->iRefAndRecPOC[i]),(int)(pH265VirDecAddr->uiRefAndRecYAddr[i]),(int)pH265VirDecAddr->uiRefAndRecIsLT[i]);
			}
		}
...
INT32 h265Dec_setNextBsBuf(H26XDEC_VAR *pH265DecVar, UINT32 uiBsAddr, UINT32 uiBsSize, UINT32 uiTotalBsSize)
{
	...
	//h26x_cache_invalidate(uiBsAddr, uiBsSize);
...
static UINT32 query_dec_mem_size(UINT32 uiWidth, UINT32 uiHeight)
{
	...
	return SIZE_4X(uiSizeContext/* + uiSizeSideInfo*2*/ + uiSizeColMVs*FRM_IDX_MAX + uiSizeBSDMA + uiSizeAPB + uiSizeDummyWT + 128);
}
