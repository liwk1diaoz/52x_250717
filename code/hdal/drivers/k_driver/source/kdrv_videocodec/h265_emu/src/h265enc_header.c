...
static void writeVUI(bstream *pH265Bs, H265EncSeqCfg *pSeqCfg, H265EncPicCfg *pPicCfg)
{
	...
	if (video_signal_type_presetn_flag) {
		UINT32 colour_description_present_flag = 1;
...
static void profile_tier_level(bstream *pH265Bs, int profilePresentFlag, int maxNumSubLayersMinus1, UINT8 ucLevelIdc)
{
	...
	HEVC_TIER_TYPE general_tier_flag = HEVC_TIER_HIGH;
...
void h265Enc_encSeqHeader(H265ENC_CTX *pVdoCtx, H26XCOMN_CTX *pComnCtx)
{
	...
	init_pack_bitstream(pH265Bs, (UINT8 *)ucH265EncNalBuf, H265ENC_HEADER_MAXSIZE);
	...
	if (uiTotalBytes > H265ENC_HEADER_MAXSIZE) {
		DBG_ERR("Header Len Error :%d, Addr:0x%08x, max :%d\r\n", uiTotalBytes, (UINT32)pHeaderAddr, H265ENC_HEADER_MAXSIZE);
	}
...
void h265Enc_encSliceHeader(H265ENC_CTX *pVdoCtx, H26XFUNC_CTX *pFuncCtx, H26XCOMN_CTX *pComnCtx,BOOL bGetSeqHdrEn)
{
	H265EncSeqCfg *pSeqCfg = &pVdoCtx->stSeqCfg;
	H265EncPicCfg *pPicCfg = &pVdoCtx->stPicCfg;
	bstream sH265Bs;
	...
	UINT32 si;
	INT32 maxSliceSegmentAddress = pSeqCfg->usLcuWidth * pSeqCfg->usLcuHeight;
    ...
    uiSliceRowNum = (pFuncCtx->stSliceSplit.stCfg.bEnable)? pFuncCtx->stSliceSplit.stCfg.uiSliceRowNum: pSeqCfg->usLcuHeight;
	...
	#if 0
    init_pack_bitstream(bs, (UINT8 *)(pComnCtx->stVaAddr.uiPicHdr), H265ENC_HEADER_MAXSIZE);
	if (bGetSeqHdrEn)
	{
	...
	init_pack_bitstream(bs, (UINT8 *)(pComnCtx->stVaAddr.uiPicHdr + uiPicInfoLen), (H265ENC_HEADER_MAXSIZE - uiPicInfoLen));
	#endif
	for (si = 0; si < uiNaluNum; si++) {
        ...
        if (si) {
            if (pSeqCfg->bTileEn) {
                if (pFuncCtx->stSliceSplit.stCfg.bEnable) {
		...
		if (!pPicCfg->uiDepSlcEn) {
			...
			if (pSeqCfg->ilf_attr.db_control_present) { // reference cmodel's writing
				...
				if (deblocking_filter_override_flag) {
					put_bits(bs, (pPicCfg->uiFrameSkip) ? 1 : slice_deblocking_filter_disabled_flag, 1); // slice_deblocking_filter_disabled_flag
	...
	h26x_flushCache(pComnCtx->stVaAddr.uiBsdma, ((uiNaluNum+1)*2 + 1)*4);
}
