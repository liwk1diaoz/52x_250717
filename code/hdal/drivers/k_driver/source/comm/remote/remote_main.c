...
module_param_named(remote_debug_level, remote_debug_level, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(remote_debug_level, "Debug message level");
#endif

//=============================================================================
// Global variable
//=============================================================================
static struct of_device_id remote_match_table[] = {
...
//=============================================================================
static int nvt_remote_open(struct inode *inode, struct file *file);
static int nvt_remote_release(struct inode *inode, struct file *file);
static long nvt_remote_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int nvt_remote_probe(struct platform_device * pdev);
static int nvt_remote_suspend( struct platform_device * pdev, pm_message_t state);
static int nvt_remote_resume( struct platform_device * pdev);
static int nvt_remote_remove(struct platform_device *pdev);
int __init nvt_remote_module_init(void);
void __exit nvt_remote_module_exit(void);
...
static int nvt_remote_open(struct inode *inode, struct file *file)
{
	REMOTE_DRV_INFO *pdrv_info;

	pdrv_info = container_of(inode->i_cdev, REMOTE_DRV_INFO, cdev);
	file->private_data = pdrv_info;

	if (nvt_remote_drv_open(&pdrv_info->module_info, MINOR(inode->i_rdev))) {
...
static int nvt_remote_release(struct inode *inode, struct file *file)
{
	...
	nvt_remote_drv_release(&pdrv_info->module_info, MINOR(inode->i_rdev));
    return 0;
}
...
{
	struct inode *inode;
	PREMOTE_DRV_INFO pdrv;

	inode = file_inode(filp);
	pdrv = filp->private_data;

	return nvt_remote_drv_ioctl(MINOR(inode->i_rdev), &pdrv->module_info, cmd, arg);
}

struct file_operations nvt_remote_fops = {
    .owner   = THIS_MODULE,
    .open    = nvt_remote_open,
    .release = nvt_remote_release,
    .unlocked_ioctl = nvt_remote_ioctl,
    .llseek  = no_llseek,
};
...
static int nvt_remote_probe(struct platform_device * pdev)
{
	...
	match = of_match_device(remote_match_table, &pdev->dev);
	if (!match){
        nvt_dbg(ERR, "Platform device not found \n");
		return -EINVAL;
	}

	pdrv_info = kzalloc(sizeof(REMOTE_DRV_INFO), GFP_KERNEL);
    if (!pdrv_info) {
	...
	cdev_init(&pdrv_info->cdev, &nvt_remote_fops);
    ...
    pdrv_info->pmodule_class = class_create(THIS_MODULE, "nvt_remote");
	...
	ret = nvt_remote_proc_init(pdrv_info);
	if (ret) {
		nvt_dbg(ERR, "failed in creating proc.\n");
		goto FAIL_DEV;
	}

	ret = nvt_remote_drv_init(&pdrv_info->module_info);
...
FAIL_DRV_INIT:
	nvt_remote_proc_remove(pdrv_info);
...
static int nvt_remote_remove(struct platform_device *pdev)
{
	...
	nvt_remote_drv_remove(&pdrv_info->module_info);
...
static int nvt_remote_suspend( struct platform_device * pdev, pm_message_t state)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_remote_drv_suspend(&pdrv_info->module_info);
...
static int nvt_remote_resume( struct platform_device * pdev)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_remote_drv_resume(&pdrv_info->module_info);

    nvt_dbg(IND, "finished\n");
	return 0;
}

static struct platform_driver nvt_remote_driver = {
    .driver = {
		        .name   = "nvt_remote",
				.owner = THIS_MODULE,
				.of_match_table = remote_match_table,
		      },
    .probe      = nvt_remote_probe,
    .remove     = nvt_remote_remove,
	.suspend = nvt_remote_suspend,
	.resume = nvt_remote_resume
};
...
int __init nvt_remote_module_init(void)
{
    ...
    nvt_dbg(WRN, "\n");
	ret = platform_driver_register(&nvt_remote_driver);
...
{
	nvt_dbg(WRN, "\n");
	platform_driver_unregister(&nvt_remote_driver);
}

module_init(nvt_remote_module_init);
module_exit(nvt_remote_module_exit);
