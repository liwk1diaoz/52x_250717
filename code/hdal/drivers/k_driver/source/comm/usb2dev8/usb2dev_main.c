...
//=============================================================================
#ifdef DEBUG
unsigned int usb2dev_debug_level = NVT_DBG_WRN;//NVT_DBG_WRN
...
static int nvt_usb2dev_probe(struct platform_device *pdev)
{
	...
	for (ucloop = 0 ; ucloop < MODULE_CLK_NUM; ucloop++) {
		...
		pdrv_info->module_info.pclk[ucloop] = clk_get(&pdev->dev, "f0600000.usb20");
	...
	if (of_property_read_u32(pdev->dev.of_node, "tx_swing", &get_tx_swing)) {
		pdrv_info->module_info.u_tx_swing = -1;
	} else {
		pdrv_info->module_info.u_tx_swing = get_tx_swing;
	}
