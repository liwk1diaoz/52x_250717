...
static USB_DEVICE_DESC usbioc_dev_desc;
static UINT8 usbioc_devquali_desc[USB_DESC_MAX_SIZE];
static UINT8 usbioc_cfg_desc_hs[USB_DESC_MAX_SIZE];
static UINT8 usbioc_cfg_desc_fs[USB_DESC_MAX_SIZE];
static UINT8 usbioc_cfg_desc_hs_oth[USB_DESC_MAX_SIZE];
static UINT8 usbioc_cfg_desc_fs_oth[USB_DESC_MAX_SIZE];
static UINT8 usbioc_str_desc0[USB_DESC_MAX_SIZE];
static UINT8 usbioc_str_desc1[USB_DESC_MAX_SIZE];
static UINT8 usbioc_str_desc2[USB_DESC_MAX_SIZE];
static UINT8 usbioc_str_desc3[USB_DESC_MAX_SIZE];
static UINT8 usbioc_str_desc4[USB_DESC_MAX_SIZE];
static UINT8 usbioc_str_desc5[USB_DESC_MAX_SIZE];
static UINT8 usbioc_str_desc6[USB_DESC_MAX_SIZE];
static UINT8 usbioc_str_desc7[USB_DESC_MAX_SIZE];
...
int nvt_usb2dev_drv_init(USB2DEV_MODULE_INFO *pmodule_info)
{
	...
	if (pmodule_info->vbus_gpio_pin > 0)
		usb_detect_vbusgpio(pmodule_info->vbus_gpio_pin, pmodule_info->vbus_gpio_pol);

	if (pmodule_info->u_tx_swing > 0)
		usb_set_txswing(pmodule_info->u_tx_swing);
...
void usb2dev_ioc_read_queue_done_cb(UINT32 event)
{
	FLGPTN uiFlag = 0;
	...
	// fill event exported here
	usbioc_event.evt_type = USB2DEV_EVT_TYPE_RQDONE_CB;
	usbioc_event.event = event;
...
int nvt_usb2dev_drv_ioctl(unsigned char uc_if, USB2DEV_MODULE_INFO *pmodule_info, unsigned int ui_cmd, unsigned long ul_arg)
{
			...
			if (!ret) {
				if ((data_rw.epn <= (USB_EP_MAX -1))) {
					if (data_rw.epn == USB_EP0) {
						...
						if(dma_len <= 64) {
							usb2dev_read_endpoint(data_rw.epn, buf64, &dma_len);
							ret = copy_to_user((void __user *)data_rw.buffer, buf64, dma_len);
							if (ret != 0)
								DBG_ERR("copy_to_user\n");
			...
			if (!ret) {
				if ((data_rw.epn <= (USB_EP_MAX -1))) {
					if (data_rw.epn == USB_EP0) {
						...
						if(dma_len <= 64) {
							ret = copy_from_user(buf64, (void __user *)data_rw.buffer,  dma_len);
							if (ret != 0)
								DBG_ERR("copy_from_user\n");

							usb2dev_write_endpoint(data_rw.epn, buf64, &dma_len);
	...
	case USB2DEV_IOC_SET_EP_RQ: {
		USB2DEV_DATA_RW data_rw;
		UINT32 dma_len = 0, queue = 0xff;
		...
		if (!ret) {
			...
			if (!ret) {
				queue = usb2dev_set_ep_read_queue(data_rw.epn, data_rw.buffer, &dma_len);

				ret = copy_to_user((void __user *)data_rw.dma_length, &queue, sizeof(UINT32));
				if (ret != 0)
					DBG_ERR("copy_to_user\n");
			}
		}
