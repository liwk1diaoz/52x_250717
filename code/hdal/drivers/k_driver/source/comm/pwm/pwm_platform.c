...
static BOOL v_pwm_clk_is_en[PWM_PWMCH_BITS + PWM_CCNTCH_BITS] = {0};
...
void pwm_platform_disable_clk(UINT32 id)
{
	...
	if (v_pwm_clk_is_en[id]) {
		clk_disable(v_pwm_clk_en[id]);
		clk_unprepare(v_pwm_clk_en[id]);
		v_pwm_clk_is_en[id] = 0;

	} else {
		nvt_dbg(WRN, "pwm#%d clk is not enable\r\n", (int)id);
	}
#endif
}
