...
UINT32 affine_platform_spin_lock(AFFINE_ID id)
{
	...
	loc_cpu();
#elif defined(__FREERTOS)
	vk_spin_lock_irqsave(&v_affine_spin_locks[id], flags);
#else
	spin_lock_irqsave(&v_affine_spin_locks[id], flags);
#endif
...
void affine_platform_register(void)
{
	if(kdrv_gfx_if_register_affine_open(kdrv_affine_open) != 0){
        nvt_dbg(ERR, "failed to register open to gfx.\n");
    }
    if(kdrv_gfx_if_register_affine_trigger(kdrv_affine_trigger) != 0){
        nvt_dbg(ERR, "failed to register trigger to gfx.\n");
    }
    if(kdrv_gfx_if_register_affine_close(kdrv_affine_close) != 0){
        nvt_dbg(ERR, "failed to register close to gfx.\n");
    }
...
#endif
	rel_flg(FLG_ID_AFFINE);
	// only rtos has FLG_ID_AFFINE_IST
	if (FLG_ID_AFFINE_IST) {
		rel_flg(FLG_ID_AFFINE_IST);
	}
	SEM_DESTROY(SEMID_AFFINE);
	SEM_DESTROY(SEMID_AFFINE_OPEN_CLOSE);
