...
void grph_platform_clk_set_freq(GRPH_ID id, UINT32 freq)
{
...
#else	// else of #if defined(_BSP_NA51055_)
	// NA51000
	const UINT32 v_clk_rate[2][4] = {
		{
	...
	if (freq < 240) {
		...
		printk("%d: input frequency %d round to 380MHz\r\n", id, freq);
		parent_clk = clk_get(NULL, "pll6");
	} else if (freq == 420) {
		parent_clk = clk_get(NULL, "pll13");
	} else if (freq < 480) {
		printk("%d: input frequency %d round to 420MHz\r\n", id, freq);
		parent_clk = clk_get(NULL, "pll13");
	} else if (freq == 480) {
...
void grph_platform_set_ist_event(GRPH_ID id)
{
#if defined(__FREERTOS)
...
void grph_platform_ist(void)
{
	...
	while (1) {
		...
		if (ptn & v_flag_ptn[0]) {
			grph_isr_bottom(GRPH_ID_1, 0);
		}
		if (ptn & v_flag_ptn[1]) {
			grph_isr_bottom(GRPH_ID_2, 0);
		}
	}
