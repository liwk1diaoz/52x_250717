...
{
	_dai_enableclk_platform();
}

void dai_disableclk(void)
{
	_dai_disableclk_platform();
}

void dai_setclkrate(unsigned long clkrate)
{
	_dai_setclkrate_platform(clkrate);
}
#else
void dai_enableclk(void)
{
...
void dai_set_i2s_config(DAI_I2SCONFIG_ID config_id, UINT32 config_value)
{
	...
	T_DAI_I2S_TDM_ORDER2_REG reg_i2s_order_2;
	unsigned long flag;
	UINT32 		ret = 0;
	...
	reg_i2s_order_2.reg = DAI_GETREG(DAI_I2S_TDM_ORDER2_REG_OFS);
	...
	case DAI_I2SCONFIG_ID_DATA_ORDER: {
			if (config_value == DAI_I2S_DATAORDER_TYPE1) {
				switch ((32 << reg_i2s_ctrl.bit.CKRATIO) / (16 << reg_i2s_ctrl.bit.CHANNEL_LEN)) {
						...
						case 16:{
								reg_i2s_order.bit.I2S_TDM_ORDER 	= 0xECA86420;
								reg_i2s_order_2.bit.I2S_TDM_ORDER2  = 0xFDB97531;
							}break;
				...
				switch ((32 << reg_i2s_ctrl.bit.CKRATIO) / (16 << reg_i2s_ctrl.bit.CHANNEL_LEN)) {
						...
						case 16:{
								reg_i2s_order.bit.I2S_TDM_ORDER 	= 0xBA983210;
								reg_i2s_order_2.bit.I2S_TDM_ORDER2  = 0xFEDC7654;
							}break;
	...
	DAI_SETREG(DAI_I2S_TDM_ORDER2_REG_OFS, reg_i2s_order_2.reg);
	unl_cpu(flag);
}
...
void dai_dbg_msg_enable(void)
{
	DAI_DBG_MSG = !DAI_DBG_MSG;
	DBG_WRN("DAI_DBG_MSG = [%d]\r\n", DAI_DBG_MSG);
}
