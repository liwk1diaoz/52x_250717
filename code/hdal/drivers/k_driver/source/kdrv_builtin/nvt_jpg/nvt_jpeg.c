...
static UINT8                jpeg_enc_busy = 0;
static UINT8                jpeg_dec_busy = 0;
static UINT8                jpeg_driver_opened = 0;
#if defined(__FREERTOS)
#if defined(_BSP_NA51055_)
...
ER jpeg_set_config(JPEG_CONFIG_ID cfg_id, UINT32 cfg)
{
	...
	case JPEG_CONFIG_ID_FREQ:
		if (cfg < 240) {
			...
			jpeg_clock = 480;//PLL_CLKSEL_JPEG_PLL6;
		}
		else {
...
//#endif
	...
	if(jpeg_enc_busy || jpeg_dec_busy)
	{
		JPGDBG_ERR("JPEG function still busy !!\r\n");
		return E_OK;
	}

	if(!jpeg_driver_opened) {
		...
		//JPGDBG_ERR("jpeg_driver_opened  ========= !!\r\n");
		jpeg_driver_opened = 1;
...
ER jpeg_close(void)
{
	...
	if (ctrl_reg.bit.jpg_act == 1) {
		JPGDBG_ERR("JPEG engine still running !!\r\n");
		return E_SYS;
	}
...
void jpeg_set_reset(void)
{
	...
	map_addr = (UINT32)ioremap_nocache(0xF0020080, 0x80);
	if (0 == map_addr) {
		JPGDBG_ERR("ioremap() failed\r\n");
	} else {
		reg = *(UINT32 *) map_addr;
		*(UINT32 *)map_addr = reg&0xFBFFFFFF;
		printk("JPEG_INT_BUFEND! bitstream buffer full ! !\r\n");
		*(UINT32 *)map_addr = reg|0x4000000;
		//printk("Reset 0x%x = 0x%x\r\n",map_addr,*(UINT32 *)map_addr);
		iounmap((volatile void __iomem *)map_addr);
	}
#endif	
}
...
ER jpeg_set_newimgdma(BOOL en)
{
	...
	if ((en == TRUE)&&(jpeg_yuvfmt != JPEG_YUV_FORMAT_411)) {
...
ER jpeg_add_queue(UINT32 id, JPEG_CODEC_MODE codec_mode, void *p_param, KDRV_CALLBACK_FUNC *p_cb_func, VOID *p_user_data)
{
	...
	UINT32 spin_flags;
	jpeg_platform_Queue_sem_wait();
	if (1) {
		//loc_cpu(flag);
		// Enter critical section
		spin_flags = jpeg_platform_spin_lock();
		//printk("[ID] = %d, Para = %x\r\n", id, (unsigned int)p_param);
		...
		if (codec_mode == JPEG_CODEC_MODE_DEC) {
			...
			//Keep bsize in jpegdec_Cfg.headerlen to do error handle (buf size check)
			jpegdec_Cfg.headerlen = p_dec_param->bs_size;
			jpghdr_errcode = jpg_parse_header(&jpegdec_Cfg, DEC_PRIMARY, NULL);

			if (jpghdr_errcode != JPG_HEADER_ER_OK) {
				//printk("Dec error 0x%x\r\n",jpghdr_errcode);
			...
			p_enc_param->motion_ratio = (unsigned int)p_user_data; //Keep userdata@motion_ratio
		}
		...
		if (p_trig->is_busy == TRUE) {
			...
			//printk("[Q  ID] = 0x%x\r\n", (unsigned int)(p_enc_param->temproal_id ));
			//printk("[Q Data] = %x\r\n", (unsigned int)p_user_data);
			if (codec_mode == JPEG_CODEC_MODE_ENC) {
				element.jpeg_enc_param = (KDRV_VDOENC_PARAM *) p_param;
				element.jpeg_enc_param->motion_ratio = (unsigned int)p_user_data;
			...
			//printk("[Q  param] = %x\r\n", (unsigned int)element.jpeg_enc_param);
			...
			if (kdrv_jpeg_queue_add_p(p_trig->p_queue, &element) == -3/*KDRV_RPC_ER_QUEUE_FULL*/) {
				//unl_cpu(flag);
				jpeg_platform_spin_unlock(spin_flags);
				JPGDBG_ERR("JPEG Queue full !\r\n");
				ret = -3;	/*KDRV_RPC_ER_QUEUE_FULL*/;
				goto exit_add_queue;
			}
			//unl_cpu(flag);
			jpeg_platform_spin_unlock(spin_flags);
			...
			if (codec_mode == JPEG_CODEC_MODE_DEC) {
				p_trig->jpeg_dec_param = (KDRV_VDODEC_PARAM *)p_param;
				p_trig->jpeg_enc_param = NULL;
			} else {
				p_trig->jpeg_dec_param = NULL;
				p_trig->jpeg_enc_param = (KDRV_VDOENC_PARAM *)p_param;
				//printk("[Add  param] = %x\r\n", (unsigned int)p_trig->jpeg_enc_param);
		...
		if (is_block_mode) {
			...
			while (1) {
				...
				if (uiIntSts & JPEG_INT_FRAMEEND) {
					...
					JPGDBG_ERR("JPEG_INT_BUFEND! bitstream buffer full !\r\n");
					jpeg_set_reset();
					ret = -1;
					break;
				} else {
					JPGDBG_ERR("[jpeg_get_status] uiIntSts = 0x%x\r\n", (unsigned int)(uiIntSts));
					ret = -1;
					break;
				}
			}
			...
			// Clear interrupt status
			jpeg_intsts = 0;
			//jpeg_platform_Queue_sem_signal();
		}
	}
exit_add_queue:
	jpeg_platform_Queue_sem_signal();
