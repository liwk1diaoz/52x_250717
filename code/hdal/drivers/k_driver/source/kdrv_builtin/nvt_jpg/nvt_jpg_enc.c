...
#ifdef __KERNEL__
//proc hw util
extern void mjd_mark_engine_start(void);
extern void mjd_mark_engine_show(void);
#endif

extern JPEGENC_RESULT_INFO   g_jpegenc_info;
...
ER jpg_enc_one_img(PJPG_ENC_CFG p_jpg_param, UINT32 quality)
{
	...
	UINT8  QTableY[DQT_SIZE]= {0};
	UINT8  QTableUV[DQT_SIZE]= {0};
	...
	//if ((bs_size > g_brcParam.UBoundByte) || (bs_size < g_brcParam.LBoundByte))
	{
		...
		*p_qf = jpgbrc_predict_quality(&brcInfo);
        ...
        if(bs_size > brcInfo.pbrc_param->ubound_byte)
       	{
       		if (g_uiBrcStdQTableQuality[id] > BRC2_STEP_Q_QUALITY)
	            g_uiBrcStdQTableQuality[id]= g_uiBrcStdQTableQuality[id]-BRC2_STEP_Q_QUALITY;
			else
				g_uiBrcStdQTableQuality[id] = 1;
...
UINT32 jpg_async_enc_setheader(PJPG_ENC_CFG p_jpg_param, UINT8 *p_q_tbl_y, UINT8 *p_q_tbl_uv)
{
	...
	if (p_jpg_param->header_form == JPG_HEADER_MOV) {
	...
	else if (p_jpg_param->header_form == JPG_HEADER_JFIF) {
		memcpy((void *)JPG_BS_Buf_Addr, (void *) &JPGJFIFHeader, JPG_JFIF_HEADER_SIZE);
		
		//Set JPG header image size.
		((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_sof.img_width[0] = hi_byte(img_width);
		((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_sof.img_width[1] = lo_byte(img_width);
		((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_sof.img_height[0] = hi_byte(img_height);
		((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_sof.img_height[1] = lo_byte(img_height);
		q_tbl_y  = (UINT8 *)(((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_dqt.q_tbl_0th.q_data);
		q_tbl_uv = (UINT8 *)(((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_dqt.q_tbl_1th.q_data);
		g_header.size = sizeof(JPG_JFIF_HEADER) - sizeof(TAG_DRI);

		//insert DRI jpeg tag info when restart interval enabled
	    if (p_jpg_param->interval > 0)
	    {
			sos_mark = (UINT8 *)(&((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_sos);
			...
			//TAG_DRI size = 6
			pBuf = JPG_BS_Buf_Addr+JPG_JFIF_HEADER_SIZE-8;
			//TAG_SOS size = 14
		...
		if (yuv_fmt == JPG_FMT_YUV100) {
            //only 1 channel
			((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_sos.tag_length[1] = 0x8;
			((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_sos.component_nums = 0x1;
			//sos_mark = (UINT8 *)(&((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_sos);

			pBuf = JPG_BS_Buf_Addr+JPG_JFIF_HEADER_SIZE-7;
			memcpy((UINT8 *)pBuf, (UINT8 *)(pBuf+4), 3);
			//DBG_INFO("g_header.size -= 6 \r\n");
			g_header.size -= 4;

			//dht_mark = (UINT8 *)(&((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_dht);
			//pBuf = dht_mark-6
			pBuf = JPG_BS_Buf_Addr+JPG_JFIF_HEADER_SIZE-sizeof(TAG_SOS)-sizeof(TAG_DHT)-6;
	...
	if (yuv_fmt == JPG_FMT_YUV422 || yuv_fmt == JPG_FMT_YUV211) {
		...
		if (p_jpg_param->header_form == JPG_HEADER_MOV) {
			...
			((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_sof.component1.sample_hv_rate = 0x22;   // YUV422-> 21
		} else {
		...
		if (p_jpg_param->header_form == JPG_HEADER_JFIF) {
			((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_sof.tag_length[1] = 0xB;
			((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_sof.component_nums = 1;
			((JPG_JFIF_HEADER *)(JPG_BS_Buf_Addr))->mark_sof.component1.sample_hv_rate = 0x11;   // YUV100-> 11
		}
		else {
...
//#endif
		...
		if (p_enc_param->jpeg_jfif_en)
			jpgCmpParam.header_form = JPG_HEADER_JFIF;
		else
			jpgCmpParam.header_form = JPG_HEADER_STD;
...
//		jpg_async_enc_open(); // need this ??
		...
		g_jpegenc_info.bs_addr = p_enc_param->bs_addr_1;
		g_jpegenc_info.bs_hdr_size = p_enc_param->svc_hdr_size;
		g_jpegenc_info.user_data = p_enc_param->motion_ratio;
		g_jpegenc_info.job_id = p_enc_param->temproal_id;
		g_jpegenc_info.enc_param = (UINT32)p_enc_param;
