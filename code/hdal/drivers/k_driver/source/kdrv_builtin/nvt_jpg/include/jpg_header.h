...
#define JPG_JFIF_HEADER_SIZE	(sizeof(JPG_JFIF_HEADER) - 6)
//@}
...
/**
    JFIF tag
*/
typedef _PACKED_BEGIN struct _TAG_JFIF {
	UINT8           tag_id[2];       ///< tag ID
	UINT8           tag_length[2];   ///< tag length
	UINT8           identifier[5];   ///< JFIF identifier
	UINT8           version[2];      ///< JFIG version
	UINT8           density;         ///< JFIF density units
	UINT8           x_den[2];        ///< horizontal pixel density
	UINT8           y_den[2];        ///< vertical pixel density
	UINT8           x_thumb;         ///< Horizontal pixel count of the following embedded RGB thumbnail
	UINT8           y_thumb;         ///< Vertical pixel count of the following embedded RGB thumbnail
} _PACKED_END TAG_JFIF;    // Start of Scan
...
/**
    JPEG JFIF header
*/
typedef _PACKED_BEGIN struct _JPG_JFIF_HEADER {
	UINT16		  soi;				///< SOI marker
	TAG_JFIF      mark_jfif;         ///< JFIF APP0 marker
	TAG_DQT 	  mark_dqt; 		 ///< DQT marker
	TAG_SOF 	  mark_sof; 		 ///< SOF marker
	TAG_DHT 	  mark_dht; 		 ///< DHT marker
	TAG_SOS 	  mark_sos; 		 ///< SOS marker
	TAG_DRI 	  mark_dri; 		 ///< DRI marker
} _PACKED_END JPG_JFIF_HEADER, *PJPG_JFIF_HEADER;
