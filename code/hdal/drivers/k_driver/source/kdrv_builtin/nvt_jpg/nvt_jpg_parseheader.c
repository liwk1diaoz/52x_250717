...
//#NT#2010/06/03#Scottie -end
	...
	//jdcfg_p->pExifInfo = (PEXIFHeaderInfo)&EXIFData;
       ...
       //jdcfg_p->headerlen to keep bs_size
	bs_size = jdcfg_p->headerlen;
...
//#NT#2008/12/03#Scottie -end
	...
	while (GetCurByte(&buf_p) == 0xFF) {
		...
		if((headerlen+ms_len)>bs_size)
		{
			DBG_ERR("headerlen+ms_len>bs_size !\r\n");
			return (JPG_HEADER_ER_MARKER);
		}
...
//#NT#2010/06/03#Scottie -end
		...
		case 0xC0:      // SOF marker for Baseline DCT
			...
			if(jdcfg_p->numcomp > 3){
				DBG_ERR("numcomp = %d  over 3 planes \r\n",jdcfg_p->numcomp);
				return (JPG_HEADER_ER_SOF_NC);
			}
