...
INT32 g_backbone_out_indexs[3][2] = {{112, 113}, {126, 127}, {152, 155}}; // process index for pdcnn
...
static ER _kflow_ai_builtin_get_fastboot_dt(KFLOW_AI_BUILTIN_MAP_MEM_PARM *p_mem)
{
	...
	p_mem->kerl_parm.va = nvtmpp_buitin_sys_pa2va(p_mem->kerl_parm.pa);
	...
	p_mem->user_model.va = nvtmpp_buitin_sys_pa2va(p_mem->user_model.pa);
	...
	p_mem->pd_buff.va = nvtmpp_buitin_sys_pa2va(p_mem->pd_buff.pa);
	...
	p_mem->rslt_buff.va = nvtmpp_buitin_sys_pa2va(p_mem->rslt_buff.pa);
	...
	p_mem->io_buff.va = nvtmpp_buitin_sys_pa2va(p_mem->io_buff.pa);
...
void _kflow_ai_builtin_trig(KFLOW_AI_BUILTIN_FMD_INFO *p_info, UINT32 reserved)
{
	...
	if (p_info->out_img[KDRV_IPP_BUILTIN_PATH_ID_3].addr[0] == 0 || size == 0) {
		printk("invalid input, addr(0x%x) size(0x%x)\r\n",
