...
UINT32  nvtmpp_sys_va2pa(UINT32 virt_addr)
{
	if (!mmz_configs.is_dynamic_map) {
		...
		UINT32 pa;
		pa = vos_cpu_get_phy_addr((u32)virt_addr);
...
/* return  > 0 for success, 0 for fail*/
UINT32  nvtmpp_buitin_sys_pa2va(UINT32 phys_addr)
{
	UINT32 virt_addr = 0;
	UINT32 i;
...
static int nvtmpp_parse_mmz_dt_p(NVTMPP_MMZ_INFO_S *mmz, NVTMPP_MMZ_CONFIG *mmz_cfg)
{
	...
	if (dt_node) {
		if (of_property_read_u32_array(dt_node, "max_pools_cnt", (UINT32 *)&mmz->max_pools_cnt, 1)) {
			mmz->max_pools_cnt = 0;
		}
		if (of_property_read_u32_array(dt_node, "max_yuv_lineoffset", (UINT32 *)&mmz->max_yuv_lineoffset, 1)) {
			mmz->max_yuv_lineoffset = 0;
		}
		if (of_property_read_u32_array(dt_node, "dynamic_map", (UINT32 *)&mmz_cfg->is_dynamic_map, 1)) {
			mmz_cfg->is_dynamic_map = 0;
		}
		if (of_property_read_u32_array(dt_node, "vcap_fixed_map_poolcnt", (UINT32 *)&mmz_cfg->vcap_fixed_map_poolcnt, 1)) {
			mmz_cfg->vcap_fixed_map_poolcnt = 0;
		}
		if (of_property_read_u32_array(dt_node, "dyanmic_map_blk_size_threshold", (UINT32 *)&mmz_cfg->dyanmic_map_blk_size_threshold, 1)) {
...
int nvtmpp_parse_fastboot_mem_dt(void)
{
	...
	for (i = 0 ; i < p_mem->vcap_blk_cnt; i++) {
		...
		if (!nvtmpp_is_dynamic_map()) {
			p_mem->vcap_blk[i].addr = nvtmpp_buitin_sys_pa2va(p_mem->vcap_blk[i].pa);
		} else {
			ioremap_cache(p_mem->vcap_blk[i].pa, p_mem->vcap_blk[i].size);
		}
	...
	for (i = 0 ; i < p_mem->vprc_blk_cnt[0]; i++) {
		...
		if (!nvtmpp_is_dynamic_map()) {
			p_mem->vprc_blk[0][i].addr = nvtmpp_buitin_sys_pa2va(p_mem->vprc_blk[0][i].pa);
		} else {
			ioremap_cache(p_mem->vprc_blk[0][i].pa, p_mem->vprc_blk[0][i].size);
		}
	...
	for (path_id = 1; path_id < FBOOT_VPRC_MAX_PATH; path_id++) {
		...
		for (i = 0 ; i < p_mem->vprc_blk_cnt[path_id]; i++) {
			...
			if (!nvtmpp_is_dynamic_map()) {
				p_mem->vprc_blk[path_id][i].addr = nvtmpp_buitin_sys_pa2va(p_mem->vprc_blk[path_id][i].pa);
			} else {
				ioremap_cache(p_mem->vprc_blk[path_id][i].pa, p_mem->vprc_blk[path_id][i].size);
			}
	...
	for (pool_id = 0; pool_id < FBOOT_MISC_CPOOL_MAX; pool_id++) {
		...
		for (i = 0 ; i < p_mem->misc_cpool[pool_id].blk_cnt; i++) {
			...
			if (of_property_read_u32_array(dt_node, node_name, (UINT32 *)&p_mem->misc_cpool[pool_id].blk[i], 2)) {
				DBG_ERR("Failed to read %s\r\n", node_name);
				return -1;
			}
			if (!nvtmpp_is_dynamic_map()) {
				...
				ioremap_cache(p_mem->misc_cpool[pool_id].blk[i].pa, p_mem->misc_cpool[pool_id].blk[i].size);
			}
	...
	for (i = 0; i < FBOOT_POOL_CNT; i++) {
		...
		if (!ret) {
			if (!nvtmpp_is_dynamic_map()) {
				...
				ioremap_cache(p_mem->pv_pools[i].pa, p_mem->pv_pools[i].size);
			}
		} else {
