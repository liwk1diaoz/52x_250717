...
ER cnn_set_eltmode(BOOL cnn_id, CNN_ELT_MODE_TYPE mode)
{
	...
	case CNN_ELT_MAX:
		...
		break;
	default:
		nvt_dbg(ERR, "Unknown eltwise mode: %d!\r\n", mode);
		return E_PAR;
	}
	CNN_SETDATA(ofs, reg1.reg, base_addr);
	return E_OK;
...
ER cnn_set_actmode(BOOL cnn_id, CNN_ACT_MODE_TYPE mode)
{
	...
	case CNN_ACT_LUT:
		...
		break;
	default:
		nvt_dbg(ERR, "Unknown act mode: %d!\r\n", mode);
		return E_PAR;
	}
	CNN_SETDATA(ofs, reg1.reg, base_addr);
	return E_OK;
...
ER cnn_set_poolmode(BOOL cnn_id, CNN_POOLING_TYPE mode)
{
	...
	case CNN_POOLING_GLOBAL:
		...
		break;
	default:
		nvt_dbg(ERR, "Unknown pool mode: %d!\r\n", mode);
		return E_PAR;
	}
	CNN_SETDATA(ofs, reg1.reg, base_addr);
	return E_OK;
...
ER cnn_set_lutmode(BOOL cnn_id, CNN_LUT_MODE_TYPE mode)
{
	...
	case CNN_LUT_SIGMOID:
		...
		break;
	default:
		nvt_dbg(ERR,  "Unknown lut mode: %d!\r\n", mode);
		return E_PAR;
	}
	CNN_SETDATA(ofs, reg1.reg, base_addr);
	return E_OK;
...
ER cnn_set_intype(BOOL cnn_id, CNN_IO_TYPE in_type)
{
	...
	if ((cnn_get_engmode(cnn_id) == CNN_CONV) && (cnn_get_kerl_en(cnn_id) & CNN_CONV_KERL_EN)) {
		if ((cnn_get_io_enable(cnn_id) & CNN_IN_ISIMAGE_EN) && ((in_type == CNN_UINT16) || (in_type == CNN_INT16))) {
			nvt_dbg(ERR,  "CNN input type must be 8 bits when CNN image mode!\r\n");
			if (in_type == CNN_UINT16) {
				nvt_dbg(ERR, "CNN input type set CNN_UINT8!\r\n");
			}
			else {
				nvt_dbg(ERR, "CNN input type set CNN_INT8!\r\n");
			}
			return E_PAR;
		}
	}
	...
	if ((cnn_get_engmode(cnn_id) == CNN_DECONV) || (cnn_get_engmode(cnn_id) == CNN_SCALEUP)) {
		...
		if (((intype == CNN_UINT8)||(intype == CNN_UINT16)) && ((out_type == CNN_INT8)||(out_type == CNN_INT16))) {
			if (out_type == CNN_INT8) {
				nvt_dbg(ERR, "CNN for deconv or scale-up, out0_type must the same signedness as the input ,set CNN_UINT8!\r\n");
			}
			else {
				nvt_dbg(ERR, "CNN for deconv or scale-up, out0_type must the same signedness as the input ,set CNN_UINT16!\r\n");
			}
			return E_PAR;
		}
		else if (((intype == CNN_INT8)||(intype == CNN_INT16)) && ((out_type == CNN_UINT8)||(out_type == CNN_UINT16))) {
			if (out_type == CNN_UINT8) {
				nvt_dbg(ERR, "CNN for deconv or scale-up, out0_type must the same signedness as the input ,set CNN_INT8!\r\n");
			}
			else {
				nvt_dbg(ERR, "CNN for deconv or scale-up, out0_type must the same signedness as the input ,set CNN_INT16!\r\n");
			}
			return E_PAR;
		}
	}
...
ER cnn_set_out0ofs(BOOL cnn_id, CNN_OUT_SIZE *p_parm)
{
	...
	if ((((cnn_get_io_enable(cnn_id) & CNN_OUT0_EN) != CNN_OUT0_EN) || (cnn_get_out0mode(cnn_id) == CNN_OUT0_INTERMEDIATE))
		&& (p_parm->out0_ofs > 0)) {
			nvt_dbg(WRN, "CNN: valid only when OUT0_EN= 1 and OUT0_MODE = ReLU0 or ReLU1.\r\n");
	} else if (p_parm->out0_ofs > CNN_CONV_OUT0OFFSET_MAX) {
		nvt_dbg(ERR, "CNN: For OUT0_MODE equals to relu0 or relu1, OUT0_OFFSET must be smaller than %d.\r\n", CNN_CONV_OUT0OFFSET_MAX);
		return E_PAR;
	}
...
ER cnn_set_out0mode(BOOL cnn_id, CNN_OUT0_TYPE mode)
{
	...
	if ((cnn_get_engmode(cnn_id) == CNN_DECONV)||(cnn_get_engmode(cnn_id) == CNN_SCALEUP)) {
		if (mode != CNN_OUT0_RELU0) {
			nvt_dbg(ERR, "CNN for CNN_DECONV or CNN_SCALEUP, output 0 mode must set as 0!\r\n");
			return E_PAR;
		}
	}
	switch (mode) {
	...
	case CNN_OUT0_INTERMEDIATE:
		...
		break;
	default:
		nvt_dbg(ERR, "Unknown output 0 mode: %d!\r\n", mode);
		return E_PAR;
	}
	...
	UINT32 addr4)
{
	...
	if ((in_bitdepth == 1) && ((addr0 & 0x1) != 0)) {
		nvt_dbg(ERR, "CNN: For 16-b input, addr0 must be 2-byte aligned.\r\n");
		return E_PAR;
	}
	//DRAM_SAI4
	//CONV: convolution weight
	if ((cnn_get_engmode(cnn_id) == CNN_CONV) && (cnn_get_kerl_en(cnn_id) & CNN_CONV_KERL_EN) &&  ((addr1 & 0x3) != 0)) {
		nvt_dbg(ERR, "CNN: For CNN_CONV, addr4 must be word-aligned.\r\n");
		return E_PAR;
	}
	//DRAM_SAI5
	...
	if ((cnn_get_engmode(cnn_id) == CNN_CONV) && (cnn_get_kerl_en(cnn_id) & CNN_CONV_KERL_EN) && (cnn_get_kerl_en(cnn_id) & CNN_FCD_QUANTI_EN) && ((addr2 & 0x3) != 0)) {
		nvt_dbg(ERR, "CNN: For CNN_CONV and CNN_FCD_QUANTI_EN, addr5 must be word-aligned.\r\n");
		return E_PAR;
	...
	if ((cnn_get_engmode(cnn_id) == CNN_CONV) && (cnn_get_kerl_en(cnn_id) & CNN_CONV_KERL_EN) && ((addr3 & 0x3) != 0)) {
		nvt_dbg(ERR, "CNN: For CNN_CONV, addr6 must be word-aligned.\r\n");
		return E_PAR;
	...
	if ((cnn_get_engmode(cnn_id) == CNN_CONV) && (cnn_get_kerl_en(cnn_id) & CNN_CONV_KERL_EN) && (cnn_get_io_enable(cnn_id) & CNN_IN_ISCHANNELSTRIPE_EN) && ((addr4 & 0x3) != 0)) {
		nvt_dbg(ERR, "CNN: For CNN_CONV and CNN_IN_ISCHANNELSTRIPE_EN, addr7 must be word-aligned.\r\n");
		return E_PAR;
	...
	//If ELTWISE_INPUT_BIT_DEPTH =1 : Byte-aligned, Otherwise : 2-Byte-aligned.
		...
		nvt_dbg(ERR, "CNN: For ELTWISE 8-b elt input, addr7 must be 2-byte aligned.\r\n");
		return E_PAR;
	}
...
UINT32 cnn_get_dmain_addr(BOOL cnn_id, CNN_IN_BUFID buf_id)
{
	...
	case CNN_IN7_BUF:
		...
		//break;
	default:
		nvt_dbg(ERR, "Error input biffer ID: %d\r\n", buf_id);
		return E_CTX;
	}
...
ER cnn_set_dma_lofs(BOOL cnn_id, CNN_DMAIO_LOFS *p_lofs)
{
	...
	if (cnn_get_engmode(cnn_id) == CNN_CONV) {
		if (cnn_get_kerl_en(cnn_id) & CNN_CONV_KERL_EN) {
			...
			if ((cnn_get_io_enable(cnn_id) & CNN_IN_ISHSTRIPE_EN) && (in_bitdepth == 1) && ((p_lofs->in0_lofs & 0x1) != 0)) {
				nvt_dbg(ERR, "CNN: For 16-b input OUT0_BITDEPTH=1 , CNN_DRAMOFSI0 is lineoffset, must be 2-byte aligned.\r\n");
				return E_PAR;
			}
		}
		else {
			...
			//Batch offset
				...
				if ((insize.batch > 1) && ((cnn_get_io_enable(cnn_id) & CNN_IN_ISIMAGE_EN) == 0)  && ((p_lofs->in0_lofs & 0x3) != 0)) {
					nvt_dbg(ERR, "CNN: For multiple batch and CNN_IN_ISIMAGE_EN=0 , CNN_DRAMOFSI0 is batchoffset, word aligned will be more efficient.\r\n");
				}
			}
		}
	}
	...
	if ((cnn_get_engmode(cnn_id) == CNN_CONV) && ((cnn_get_io_enable(cnn_id) & CNN_IN_ISHSTRIPE_EN) || (cnn_get_io_enable(cnn_id) & CNN_IN_ISVSTRIPE_EN))) {
		if ((in_bitdepth == 1) && ((p_lofs->in1_lofs & 0x1) != 0)) {
			nvt_dbg(ERR, "CNN: For 16-b input OUT0_BITDEPTH=1 , CNN_DRAMOFSI1 is channel offset, must be 2-byte aligned.\r\n");
			return E_PAR;
		}
	}
	//	DRAM_ OFSI2
	...
	if (cnn_get_engmode(cnn_id) == CNN_CONV) {
		if ( cnn_get_kerl_en(cnn_id) & CNN_ELTWISE_KERL_EN) {
			...
			if ((cnn_get_io_enable(cnn_id) & CNN_IN_ISHSTRIPE_EN) && (elt_bitdepth == 1) && ((p_lofs->in2_lofs & 0x1) != 0)) {
				nvt_dbg(ERR, "CNN: For 16-b input ELT_BITDEPTH=1 , CNN_DRAMOFSI2 is lineoffset, must be 2-byte aligned.\r\n");
				return E_PAR;
			}
			//Batch offset
			else {
				if ((insize.batch > 1) && ((cnn_get_io_enable(cnn_id) & CNN_IN_ISIMAGE_EN) == 0)  && (cnn_get_io_enable(cnn_id) & CNN_IN_ISHSTRIPE_EN) && ((p_lofs->in2_lofs & 0x3) != 0)) {
					nvt_dbg(ERR, "CNN: For feat and CNN_IN_ISHSTRIPE_EN=1 , CNN_DRAMOFSI2 is batch offset, word aligned will be more efficient.\r\n");
				}
			}
		}
	}
	...
	if (cnn_get_kerl_en(cnn_id) & CNN_ELTWISE_KERL_EN) {
		if ((cnn_get_engmode(cnn_id) == CNN_CONV) && ((cnn_get_io_enable(cnn_id) & CNN_IN_ISHSTRIPE_EN) || (cnn_get_io_enable(cnn_id) & CNN_IN_ISVSTRIPE_EN))) {
			if ((elt_bitdepth == 1) && ((p_lofs->in3_lofs & 0x1) != 0)) {
				nvt_dbg(ERR, "CNN: For 16-b input OUT0_BITDEPTH=1 , CNN_DRAMOFSI3 is channel offset, must be 2-byte aligned.\r\n");
				return E_PAR;
			}
		}
	}
	...
	if (cnn_get_engmode(cnn_id) == CNN_CONV && (cnn_get_io_enable(cnn_id) & CNN_OUT0_EN)) {
		if (cnn_get_kerl_en(cnn_id) & CNN_CONV_KERL_EN) {
			...
			if ((cnn_get_io_enable(cnn_id) & CNN_IN_ISHSTRIPE_EN) && (out0_bitdepth == 1) && ((p_lofs->out0_lofs & 0x1) != 0)) {
				nvt_dbg(ERR, "CNN: For 16-b output CNN_IN_ISHSTRIPE_EN=1 , CNN_DRAMOFSO0 is lineoffset, must be 2-byte aligned.\r\n");
				return E_PAR;
			}
		}
		else {
			...
			//Batch offset
				...
				if ((insize.batch > 1) && ((p_lofs->out0_lofs & 0x3) != 0)) {
					nvt_dbg(ERR, "CNN: For multiple batch , CNN_DRAMOFSO0 is batch offset, word aligned will be more efficient.\r\n");
				}
			}
		}
	}
	...
	if ((cnn_get_io_enable(cnn_id) & CNN_OUT0_EN) && ((cnn_get_io_enable(cnn_id) & CNN_IN_ISHSTRIPE_EN) || (cnn_get_io_enable(cnn_id) & CNN_IN_ISVSTRIPE_EN))) {
		if ((out0_bitdepth == 1) && ((p_lofs->out1_lofs & 0x1) != 0)) {
			nvt_dbg(ERR, "CNN: For 16-b out0_bitdepth=1 , CNN_DRAMOFSO1 is channel offset, must be 2-byte aligned.\r\n");
			return E_PAR;
		}
	}
	//DRAM_OFSO2
	...
	if (cnn_get_engmode(cnn_id) == CNN_CONV && (cnn_get_io_enable(cnn_id) & CNN_OUT1_EN)) {
		if (cnn_get_kerl_en(cnn_id) & CNN_CONV_KERL_EN) {
			...
			if ((cnn_get_io_enable(cnn_id) & CNN_IN_ISHSTRIPE_EN) && (out1_bitdepth == 1) && ((p_lofs->out2_lofs & 0x1) != 0)) {
				nvt_dbg(ERR, "CNN: For 16-b output CNN_IN_ISHSTRIPE_EN=1 , CNN_DRAMOFSO2 is lineoffset, must be 2-byte aligned.\r\n");
				return E_PAR;
			}
		}
		else {
	...
	if ((cnn_get_io_enable(cnn_id) & CNN_OUT1_EN) && ((cnn_get_io_enable(cnn_id) & CNN_IN_ISHSTRIPE_EN) || (cnn_get_io_enable(cnn_id) & CNN_IN_ISVSTRIPE_EN))) {
		if ((out1_bitdepth == 1) && ((p_lofs->out3_lofs & 0x1) != 0)) {
			nvt_dbg(ERR, "CNN: For 16-b out1_bitdepth=1 , DRAMOFSO3 is channel offset, must be 2-byte aligned.\r\n");
			return E_PAR;
		}
	}
...
ER cnn_set_dmaout0_addr(BOOL cnn_id, UINT32 addr0)
{
	...
	if (cnn_get_io_enable(cnn_id) & CNN_OUT0_EN) {
		if ((out0_bitdepth == 1) && ((addr0 & 0x1) != 0)) {
			nvt_dbg(ERR, "CNN: For out0_bitdepth = 1, addr0 must be 2-byte aligned.\r\n");
			return E_PAR;
		}
	}
...
ER cnn_set_dmaout1_addr(BOOL cnn_id, UINT32 addr0)
{
	...
	if (cnn_get_io_enable(cnn_id) & CNN_OUT1_EN) {
		if ((out1_bitdepth == 1) && ((addr0 & 0x1) != 0)) {
			nvt_dbg(ERR, "CNN: For out1_bitdepth = 1, addr0 must be 2-byte aligned.\r\n");
			return E_PAR;
		}
	}
...
ER cnn_set_insize(BOOL cnn_id, CNN_IN_SIZE *p_size)
{
	...
	if (eng_mode == CNN_CONV) {
		...
		if (channel < CNN_CHANNEL_MIN) {
			nvt_dbg(ERR, "CNN: For conv mode, Input channel must be >= %d : %d -> %d\r\n", CNN_CHANNEL_MIN, (int)channel, CNN_CHANNEL_MIN);
			return E_PAR;
		...
		if (width > CNN_WIDTH_MAX) {
			nvt_dbg(ERR, "CNN: Input width must be <= %d : %d -> %d\r\n", CNN_WIDTH_MAX, (int)width, CNN_WIDTH_MAX);
			return E_PAR;
		...
		if (height > CNN_HEIGHT_MAX) {
			nvt_dbg(ERR, "CNN: Input height must be <= %d : %d -> %d\r\n", CNN_HEIGHT_MAX, (int)height, CNN_HEIGHT_MAX);
			return E_PAR;
		...
		if ((cnn_get_kerl_en(cnn_id) & CNN_CONV_KERL_EN)) {
			...
			if (conv_para.conv_kersize == CNN_CONV_KERSZ_11_11) {
				...
				if ((width + 1*(conv_para.is_left_pad + conv_para.is_right_pad)) < CNN_CONV_K3_WIDTH_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 3X3 img mode, Input width + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K3_WIDTH_MIN, (int)width, CNN_CONV_K3_WIDTH_MIN);
					return E_PAR;
				}
				if ((height+ 1*(conv_para.is_top_pad + conv_para.is_bot_pad)) < CNN_CONV_K3_HEIGHT_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 3X3 img mode, Input height + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K3_HEIGHT_MIN, (int)height, CNN_CONV_K3_HEIGHT_MIN);
					return E_PAR;
				}
			} else if (conv_para.conv_kersize == CNN_CONV_KERSZ_1_1) {
				if (width < CNN_CONV_K1_WIDTH_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 1X1 feat mode, Input width + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K1_WIDTH_MIN, (int)width, CNN_CONV_K1_WIDTH_MIN);
					return E_PAR;
				}
				if (height < CNN_CONV_K1_HEIGHT_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 1X1 feat mode, Input height + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K1_HEIGHT_MIN, (int)height, CNN_CONV_K1_HEIGHT_MIN);
					return E_PAR;
				}
			} else if (conv_para.conv_kersize == CNN_CONV_KERSZ_7_1) {
				if ((width + 3*(conv_para.is_left_pad + conv_para.is_right_pad)) < CNN_CONV_K7_WIDTH_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 7X1 feat mode, Input width + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K7_WIDTH_MIN, (int)width, CNN_CONV_K7_WIDTH_MIN);
					return E_PAR;
				}
				if (height < CNN_CONV_K1_HEIGHT_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 7X1 feat mode, Input height + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K1_HEIGHT_MIN, (int)height, CNN_CONV_K1_HEIGHT_MIN);
					return E_PAR;
				}
			} else if (conv_para.conv_kersize == CNN_CONV_KERSZ_1_7) {
				if (width < CNN_CONV_K1_WIDTH_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 1X7 feat mode, Input width + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K1_WIDTH_MIN, (int)width, CNN_CONV_K1_WIDTH_MIN);
					return E_PAR;
				}
				if ((height+ 3*(conv_para.is_top_pad + conv_para.is_bot_pad)) < CNN_CONV_K7_HEIGHT_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 1X7 feat mode, Input height + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K7_HEIGHT_MIN, (int)height, CNN_CONV_K7_HEIGHT_MIN);
					return E_PAR;
				}
			} else if (conv_para.conv_kersize == CNN_CONV_KERSZ_5_1) {
				if ((width + 2*(conv_para.is_left_pad + conv_para.is_right_pad)) < CNN_CONV_K5_WIDTH_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 5X1 feat mode, Input width + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K5_WIDTH_MIN, (int)width, CNN_CONV_K5_WIDTH_MIN);
					return E_PAR;
				}
				if (height < CNN_CONV_K1_HEIGHT_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 5x1 feat mode, Input height + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K1_HEIGHT_MIN, (int)height, CNN_CONV_K1_HEIGHT_MIN);
					return E_PAR;
				}
			} else if (conv_para.conv_kersize == CNN_CONV_KERSZ_1_5) {
				if (width < CNN_CONV_K1_WIDTH_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 1X5 feat mode, Input width + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K1_WIDTH_MIN, (int)width, CNN_CONV_K1_WIDTH_MIN);
					return E_PAR;
				}
				if ((height+ 2*(conv_para.is_top_pad + conv_para.is_bot_pad)) < CNN_CONV_K5_HEIGHT_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 1X5 feat mode, Input height + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K5_HEIGHT_MIN, (int)height, CNN_CONV_K5_HEIGHT_MIN);
					return E_PAR;
				}
			} else if (conv_para.conv_kersize == CNN_CONV_KERSZ_3_1) {
				if ((width + 1*(conv_para.is_left_pad + conv_para.is_right_pad)) < CNN_CONV_K3_WIDTH_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 3X1 feat mode, Input width + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K3_WIDTH_MIN, (int)width, CNN_CONV_K3_WIDTH_MIN);
					return E_PAR;
				}
				if (height < CNN_CONV_K1_HEIGHT_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 3X1 feat mode, Input height + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K1_HEIGHT_MIN, (int)height, CNN_CONV_K1_HEIGHT_MIN);
					return E_PAR;
				}
			} else if (conv_para.conv_kersize == CNN_CONV_KERSZ_1_3) {
				if (width < CNN_CONV_K1_WIDTH_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 1X3 feat mode, Input width + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K1_WIDTH_MIN, (int)width, CNN_CONV_K1_WIDTH_MIN);
					return E_PAR;
				}
				if ((height+ 1*(conv_para.is_top_pad + conv_para.is_bot_pad)) < CNN_CONV_K3_HEIGHT_MIN) {
					nvt_dbg(ERR, "CNN: for kernel 1X3 feat mode, Input height + pad must be >= %d : %d -> %d\r\n", CNN_CONV_K3_HEIGHT_MIN, (int)height, CNN_CONV_K3_HEIGHT_MIN);
					return E_PAR;
				}
			...
			if (cnn_get_io_enable(cnn_id) & CNN_IN_ISIMAGE_EN) {
				...
				if (batch > CNN_CONV_BATCH_MIN) {
					nvt_dbg(ERR, "CNN: For image mode, CNN_BATCHNUM must be 1 : %d -> 1\r\n", (int)batch);
					return E_PAR;
				}
			} else {//feature mode.
				//channel
				...
				if (channel > CNN_CHANNEL_MAX) {
					nvt_dbg(ERR, "CNN: For feat mode, Input channel must be <= %d : %d -> %d\r\n", CNN_CHANNEL_MAX, (int)channel, CNN_CHANNEL_MAX);
					return E_PAR;
				}
				//batch
				if (((cnn_get_io_enable(cnn_id) & CNN_IN_ISHSTRIPE_EN) || (cnn_get_io_enable(cnn_id) & CNN_IN_ISVSTRIPE_EN)) && (batch > CNN_CONV_BATCH_MIN)) {
					nvt_dbg(ERR, "CNN: For multiple-stripe(h-streip = 1 or v-stripe = 1), CNN_BATCHNUM must be 1 : %d -> 1\r\n", (int)batch);
					return E_PAR;
				}
				if (((cnn_get_io_enable(cnn_id) & CNN_IN_ISHSTRIPE_EN) || (cnn_get_io_enable(cnn_id) & CNN_IN_ISVSTRIPE_EN)) && (batch > CNN_CONV_BATCH_MIN)) {
					nvt_dbg(ERR, "CNN: For multiple-stripe(h-streip = 1 or v-stripe = 1), CNN_BATCHNUM must be 1 : %d -> 1\r\n", (int)batch);
				...
				if (batch > CNN_CONV_BATCH_MAX) {
					nvt_dbg(ERR, "CNN: For convolution, input batch must be <= %d : %d -> %d\r\n", CNN_CONV_BATCH_MAX, (int)batch, CNN_CONV_BATCH_MAX);
					return E_PAR;
		...
		if ((cnn_get_kerl_en(cnn_id) & CNN_ELTWISE_KERL_EN)) {
			//channel
			if (channel < CNN_CHANNEL_MIN) {
				nvt_dbg(ERR, "CNN: For eltwise on, Input channel must be >= %d : %d -> %d\r\n", CNN_CHANNEL_MIN, (int)channel, CNN_CHANNEL_MIN);
				return E_PAR;
			}
			if (channel > CNN_CHANNEL_MAX) {
				nvt_dbg(ERR, "CNN: For eltwise on, Input channel must be <= %d : %d -> %d\r\n", CNN_CHANNEL_MAX, (int)channel, CNN_CHANNEL_MAX);
			...
			if (width > CNN_ELT_WIDTH_MAX) {
				nvt_dbg(ERR, "CNN: For eltwise on, Input width must be <= %d : %d -> %d\r\n", CNN_ELT_WIDTH_MAX, (int)width, CNN_ELT_WIDTH_MAX);
				return E_PAR;
			...
			if (height > CNN_ELT_HEIGHT_MAX) {
				nvt_dbg(ERR, "CNN: For eltwise on, Input height must be <= %d : %d -> %d\r\n", CNN_ELT_HEIGHT_MAX, (int)height, CNN_ELT_HEIGHT_MAX);
				return E_PAR;
		...
		if ((cnn_get_kerl_en(cnn_id) & CNN_POOLING_KERL_EN)) {
			//channel
			if (channel < CNN_CHANNEL_MIN) {
				nvt_dbg(ERR, "CNN: For pool on, Input channel must be >= %d : %d -> %d\r\n", CNN_CHANNEL_MIN, (int)channel, CNN_CHANNEL_MIN);
				return E_PAR;
			}
			if (channel > CNN_CHANNEL_MAX) {
				nvt_dbg(ERR, "CNN: For pool on, Input channel must be <= %d : %d -> %d\r\n", CNN_CHANNEL_MAX, (int)channel, CNN_CHANNEL_MAX);
			...
			if (cnn_get_poolmode(cnn_id) == CNN_POOLING_LOCAL) {
				...
				if(local_pool_ker_sz < 2){//pool_ker_sz = 2x2 or 3x3
					...
					if (width > CNN_POOL_WIDTH_MAX) {
						...
						nvt_dbg(ERR, "CNN: For pool on(local pool), Input width + pad must be >= pool_ker_w %d : %d -> %d\r\n", pool_ker_w, (int)width, pool_ker_w);
						return E_PAR;
					}
					...
					if (height > CNN_POOL_HEIGHT_MAX) {
						...
						nvt_dbg(ERR, "CNN: For pool on(local pool), Input height + pad must be >= pool_ker_h %d : %d -> %d\r\n", pool_ker_h, (int)height, pool_ker_h);
						return E_PAR;
					}
				}
			} else {
		...
		//channel
		if (channel < CNN_CHANNEL_MIN) {
			nvt_dbg(ERR, "CNN: For deconv, Input channel must be >= %d : %d -> %d\r\n", CNN_CHANNEL_MIN, (int)channel, CNN_CHANNEL_MIN);
			return E_PAR;
		}
		if (channel > CNN_CHANNEL_MAX) {
			nvt_dbg(ERR, "CNN: For deconv, Input channel must be <= %d : %d -> %d\r\n", CNN_CHANNEL_MAX, (int)channel, CNN_CHANNEL_MAX);
		...
		if (width > CNN_DECONV_WIDTH_MAX) {
			nvt_dbg(ERR, "CNN: For deconv, Input width must be <= %d : %d -> %d\r\n", CNN_DECONV_WIDTH_MAX, (int)width, CNN_DECONV_WIDTH_MAX);
			return E_PAR;
		...
		//batch
		if (batch > CNN_CONV_BATCH_MAX) {
			nvt_dbg(ERR, "CNN: For deconv, input batch must be <= %d : %d -> %d\r\n", CNN_CONV_BATCH_MAX, (int)batch, CNN_CONV_BATCH_MAX);
			return E_PAR;
		} else if (batch < CNN_CONV_BATCH_MIN) {
			nvt_dbg(ERR, "CNN: For deconv, input batch must be >= %d : %d -> %d\r\n", CNN_CONV_BATCH_MIN, (int)batch, CNN_CONV_BATCH_MIN);
			return E_PAR;
		...
		//channel
		if (channel < CNN_CHANNEL_MIN) {
			nvt_dbg(ERR, "CNN: For scale-up, Input channel must be >= %d : %d -> %d\r\n", CNN_CHANNEL_MIN, (int)channel, CNN_CHANNEL_MIN);
			return E_PAR;
		}
		if (channel > CNN_CHANNEL_MAX) {
			nvt_dbg(ERR, "CNN: For scale-up, Input channel must be <= %d : %d -> %d\r\n", CNN_CHANNEL_MAX, (int)channel, CNN_CHANNEL_MAX);
		...
		if (width > CNN_SCALE_WIDTH_MAX) {
			nvt_dbg(ERR, "CNN: For scale-up, Input width must be <= %d : %d -> %d\r\n", CNN_SCALE_WIDTH_MAX, (int)width, CNN_SCALE_WIDTH_MAX);
			return E_PAR;
		...
		if (height > CNN_SCALE_HEIGHT_MAX) {
			nvt_dbg(ERR, "CNN: For scale-up, Input height must be <= %d : %d -> %d\r\n", CNN_SCALE_HEIGHT_MAX, (int)height, CNN_SCALE_HEIGHT_MAX);
			return E_PAR;
		...
		//batch
		if (batch > CNN_CONV_BATCH_MAX) {
			nvt_dbg(ERR, "CNN: For scale-up, input batch must be <= %d : %d -> %d\r\n", CNN_CONV_BATCH_MAX, (int)batch, CNN_CONV_BATCH_MAX);
			return E_PAR;
		} else if (batch < CNN_CONV_BATCH_MIN) {
			nvt_dbg(ERR, "CNN: For scale-up, input batch must be >= %d : %d -> %d\r\n", CNN_CONV_BATCH_MIN, (int)batch, CNN_CONV_BATCH_MIN);
			return E_PAR;
...
ER cnn_set_convparm(BOOL cnn_id, CNN_CONVKERL_PARM *p_parm)
{
	...
	if ((cnn_get_engmode(cnn_id) == CNN_CONV) && (cnn_get_kerl_en(cnn_id) & CNN_CONV_KERL_EN)) {
		...
		if ((p_parm->conv_kersize == CNN_CONV_KERSZ_11_11)
			&& ((cnn_get_io_enable(cnn_id) & CNN_IN_ISIMAGE_EN) != CNN_IN_ISIMAGE_EN)) {
				nvt_dbg(ERR, "CNN: IN_ISIMG needs to be 1 for conv kernel 11x11.\r\n");
				return E_PAR;
		}

		if ((p_parm->conv_kersize == CNN_CONV_KERSZ_11_11) && (p_parm->conv_stride != 0) && (p_parm->conv_stride != 1) && (p_parm->conv_stride != 2)) {
			nvt_dbg(ERR, "CNN: CONV_STRIDE can only be 0 or 1 or 2 for conv kernel 11x11.\r\n");
			return E_PAR;
		}
	...
	case CNN_CONV_KERSZ_9_9:
		...
		break;
	default:
		nvt_dbg(ERR, "In convolution, unknown kersel: %d\r\n", (int)p_parm->conv_kersize);
		return E_PAR;
	}
	...
	case CNN_CONV_KER_STRIDE_4:
		...
		break;
	default:
		nvt_dbg(ERR, "In convolution, unknown stride: %d\r\n", (int)p_parm->conv_stride);
		return E_PAR;
	}
...
ER cnn_set_fcd_en(BOOL cnn_id, CNN_FCD_PARM *p_parm)
{
	...
	if ((p_parm->fcd_vlc_en == FALSE) && (p_parm->fcd_quanti_kmean_en == TRUE)) {
		nvt_dbg(ERR,"When VLC_EN disable, QUANTIZATION_KMEANS_EN not allow to be enable !\r\n");
		return E_PAR;
	}
...
CNN_FCD_PARM cnn_get_fcd_parm(BOOL cnn_id)
{
	...
	if (er_code != E_OK) {
		nvt_dbg(ERR, "cnn_get_fcd_en fail...\r\n");
	}

	er_code = cnn_get_fcd_encbitlength(cnn_id, &parm);
	if (er_code != E_OK) {
		nvt_dbg(ERR, "cnn_get_fcd_encbitlength fail...\r\n");
	}
	if ((cnn_get_engmode(cnn_id) == CNN_CONV) && (cnn_get_kerl_en(cnn_id) & CNN_CONV_KERL_EN) && (cnn_get_kerl_en(cnn_id) & CNN_FCD_VLC_EN)) {
		...
		if (er_code != E_OK) {
			nvt_dbg(ERR, "cnn_get_fcd_vlc_table_code fail...\r\n");
		}
		er_code = cnn_get_fcd_vlc_table_ofs(cnn_id, &parm);
		if (er_code != E_OK) {
			nvt_dbg(ERR, "cnn_get_fcd_vlc_table_ofs fail...\r\n");
		}
	}
...
ER cnn_set_bnscale_parm(BOOL cnn_id, CNN_BNSCALE_PARM *p_parm)
{
	...
	if (p_parm->bn_shf_mean > CNN_BN_SHIFTM_MAX) {
		nvt_dbg(ERR, "CNN: CNN_BATCHNORM_SHIFT_M must be [0, %d].\r\n", CNN_BN_SHIFTM_MAX);
		return E_PAR;
	}
	if (p_parm->scale_shf_bias > CNN_SCALE_SHIFTB_MAX) {
		nvt_dbg(ERR, "CNN: CNN_SCALE_SHIFT_B must be [0, %d].\r\n", CNN_SCALE_SHIFTB_MAX);
		return E_PAR;
	}
	if (p_parm->scale_shf_alpha > SCALE_SHIFT_ALPHA_MAX) {
		nvt_dbg(ERR, "CNN: SCALE_SHIFT_ALPHA must be [0, %d].\r\n", SCALE_SHIFT_ALPHA_MAX);
		return E_PAR;
	}
...
ER cnn_set_elt_parm(BOOL cnn_id, CNN_ELTWISE_PARM *p_parm)
{
	...
	if (elt_coef0 > CNN_ELT_COEF0_MAX) {
		nvt_dbg(ERR,"In Eltwise, coeff 0 %d is clamp %d!\r\n", (int)elt_coef0, CNN_ELT_COEF0_MAX);
		return E_PAR;
	...
	if (elt_coef1 > CNN_ELT_COEF1_MAX) {
		nvt_dbg(ERR,"In Eltwise, coeff 1 %d is clamp %d!\r\n", (int)p_parm->elt_coef1, CNN_ELT_COEF1_MAX);
		return E_PAR;
...
ER cnn_set_prerelu_parm(BOOL cnn_id, CNN_RELU_IN *p_parm)
{
	...
	if (leaky_val > CNN_RELU_LEAKY_VAL_MAX) {
		nvt_dbg(ERR, "In ReLU, leaky value %d is clamp %d!\r\n", (int)leaky_val, CNN_RELU_LEAKY_VAL_MAX);
		return E_PAR;
	...
	if (p_parm->leaky_shf > CNN_RELU_LEAKY_SHF_MAX) {
		nvt_dbg(ERR, "In ReLU, leaky shift %d is clamp %d!\r\n", (int)p_parm->leaky_shf, CNN_RELU_LEAKY_SHF_MAX);
		return E_PAR;
...
ER cnn_set_deconvparm(BOOL cnn_id, CNN_DECONV_PARM *p_parm)
{
	...
	if (p_parm->is_top_pad > CNN_DECONV_PADNUM_MAX) {
		nvt_dbg(ERR, "CNN: CNN_DECONV_PADNUM_MAX must not over %d.\r\n", CNN_DECONV_PADNUM_MAX);
		return E_PAR;
	} 
	...
	if (p_parm->deconv_padval > CNN_DECONV_PADVAL_MAX) {
		nvt_dbg(ERR, "CNN: DECONV_PAD_VAL must not over %d.\r\n", CNN_DECONV_PADVAL_MAX);
		return E_PAR;
	} else if (p_parm->deconv_padval < CNN_DECONV_PADVAL_MIN) {
		nvt_dbg(ERR, "CNN: DECONV_PAD_VAL must not less than %d.\r\n", CNN_DECONV_PADVAL_MIN);
		return E_PAR;
	}
	...
	case CNN_DECONV_KER_STRIDE_8:
		...
		break;
	default:
		nvt_dbg(ERR, "In deconvolution, unknown stride: %d\r\n", p_parm->deconv_stride);
		return E_CTX;
	}
...
ER cnn_set_scaleupparm(BOOL cnn_id, CNN_SCALEUP_PARM *p_parm)
{
	...
	case CNN_SCALEUP_8:
		...
		break;
	default:
		nvt_dbg(ERR, "In scaleup, unknown stride: %d\r\n", p_parm->scaleup_rate);
		return E_PAR;
	}
...
ER cnn_set_poolkertype(BOOL cnn_id, CNN_POOL_KER type)
{
	...
	case CNN_POOL_KER_AVG:
		...
		break;
	default:
		nvt_dbg(ERR, "In local/global pooling, unknown kernel type: %d\r\n", type);
		return E_PAR;
	}
	CNN_SETDATA(ofs, reg1.reg, base_addr);
	return E_OK;
...
ER cnn_set_poolshift(BOOL cnn_id, BOOL pool_shf_dir, UINT32 pool_shf)
{
	...
	if (pool_shf > CNN_POOL_SHF_MAX) {
		nvt_dbg(ERR, "CNN: CNN_POOLSHF_MAX must not over %d.\r\n", CNN_POOL_SHF_MAX);
		return E_PAR;
	} 
...
ER cnn_set_localpool_stride(BOOL cnn_id, CNN_POOL_KER_STRIDE stride)
{
	...
	case CNN_POOL_KER_STRIDE_2:
		...
		break;
	default:
		nvt_dbg(ERR, "In local pooling, unknown stride: %d\r\n", stride);
		return E_PAR;
	}
	CNN_SETDATA(ofs, reg1.reg, base_addr);
	return E_OK;
...
ER cnn_set_localpool_kersize(BOOL cnn_id, CNN_POOL_KER_SIZE kersize)
{
	...
	case CNN_POOL_KERSZ_5:
		...
		break;
	default:
		nvt_dbg(ERR, "In local pooling, unknown kernel size: %d\r\n", kersize);
		return E_PAR;
	}
	CNN_SETDATA(ofs, reg1.reg, base_addr);
	return E_OK;
...
ER cnn_set_localpool_parm(BOOL cnn_id, CNN_LOCAL_POOL_PARM *p_parm)
{
	...
	if (er_return != E_OK) {
		nvt_dbg(ERR, "cnn_set_localpool_kersize fail...\r\n");
		return E_PAR;
	}

	er_return = cnn_set_poolkertype(cnn_id, p_parm->ker_type);
	if (er_return != E_OK) {
		nvt_dbg(ERR, "cnn_set_poolshift fail...\r\n");
		return E_PAR;
	}
	...
	if (er_return != E_OK) {
		nvt_dbg(ERR, "cnn_set_localpool_stride fail...\r\n");
		return E_PAR;
	}
...
CNN_LOCAL_POOL_PARM cnn_get_localpool_parm(BOOL cnn_id)
{
	...
	if (cnn_get_localpool_pad(cnn_id, &parm.is_top_pad, &parm.is_left_pad, &parm.is_bot_pad, &parm.is_right_pad) != E_OK) {
		nvt_dbg(ERR, "cnn_get_localpool_pad fail...\r\n");
	}
...
ER cnn_set_globalpool_avg(BOOL cnn_id, UINT32 mul, UINT32 shf)
{
	...
	if (mul > CNN_POOL_AVE_MUL_MAX) {
		nvt_dbg(ERR, "In global pooling, pool_ave_mul %d is clamp %d!\r\n", (int)mul, CNN_POOL_AVE_MUL_MAX);
		return E_PAR;
	} 

	if (shf > CNN_POOL_AVE_SHF_MAX) {
		nvt_dbg(ERR, "In global pooling, pool_ave_shift %d is clamp %d!\r\n", (int)shf, CNN_POOL_AVE_SHF_MAX);
		return E_PAR;
...
ER cnn_set_globalpool_parm(BOOL cnn_id, CNN_GLOBAL_POOL_PARM *p_parm)
{
	...
	nvt_dbg(ERR, "CNN mode should be global pooling mode\r\n");
	return E_CTX;
	}*/
	...
	if (p_parm->ker_type == CNN_POOL_KER_AVG) {
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR, "cnn_set_globalpool_avg fail...\r\n");
			return E_PAR;
		}
	}
...
CNN_GLOBAL_POOL_PARM cnn_get_globalpool_parm(BOOL cnn_id)
{
	...
	if (cnn_get_globalpool_avg(cnn_id, &parm.avg_mul, &parm.avg_shf) != E_OK) {
		nvt_dbg(ERR, "cnn_get_globalpool_avg fail...\r\n");
	}
	cnn_get_poolshift(cnn_id, &parm.pool_shf_dir, &parm.pool_shf);
	cnn_get_poolsignedness(cnn_id, &parm.pool_signedness);
...
ER cnn_set_dmaio_addr(BOOL cnn_id, CNN_DMAIO_ADDR dma_addr)
{
	...
	case CNN_CONV:
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR, "cnn_set_dmain_addr fail...\r\n");
			return E_PAR;
		}

#if 0

		nvt_dbg(IND, "addr0:0x%08X, phy_addr0:0x%08X\n", addr0, phy_addr0);
		nvt_dbg(IND, "addr4:0x%08X, phy_addr4:0x%08X\n", addr4, phy_addr4);
		nvt_dbg(IND, "addr5:0x%08X, phy_addr5:0x%08X\n", addr5, phy_addr5);
		nvt_dbg(IND, "addr6:0x%08X, phy_addr6:0x%08X\n", addr6, phy_addr6);
		nvt_dbg(IND, "addr7:0x%08X, phy_addr7:0x%08X\n", addr7, phy_addr7);
#endif
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR, "cnn_set_dmaout0_addr fail...\r\n");
			return E_PAR;
		}
		//nvt_dbg(IND, "addr0:0x%08X, size0:%d\n", CNN_dram_out_addr0, CNN_dram_out_size0);
		//nvt_dbg(IND, "addr1:0x%08X, size1:%d\n", CNN_dram_out_addr1, CNN_dram_out_size1);
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR, "cnn_set_dmaout1_addr fail...\r\n");
			return E_PAR;
		}
		//nvt_dbg(INFO, "Out1 addr9:%08lx phy_addr9:%08lx\r\n", addr9, phy_addr9);

#if 0
		nvt_dbg(IND, "addr8:0x%08X, phy_addr8:0x%08X\n", addr8, phy_addr8);
		nvt_dbg(IND, "addr9:0x%08X, phy_addr9:0x%08X\n", addr9, phy_addr9);
#endif
		break;
	...
	case CNN_LRN:
		...
		break;
	default:
		nvt_dbg(ERR, "Error unknown CNN engine mode: %d\r\n", cnn_get_engmode(cnn_id));
		return E_CTX;
	}
