...
VOID cnn_isr(BOOL cnn_id)
{
	...
	if (cnn_int_status == 0) {
		//nvt_dbg(ERR, "cnn_int_status isr err\r\n");
		return;
	}
...
ER cnn_chk_state_machine(BOOL cnn_id, UINT32 operation, BOOL update)
{
	...
	case CNN_ENGINE_IDLE:
		...
		case CNN_OP_ATTACH:
			...
			er_return = E_OK;
			//nvt_dbg(ERR, "cnn_chk_state_machine cnn_status:%d\n", cnn_status);
			break;
		case CNN_OP_DETACH:
			if (update) {
				cnn_status = CNN_ENGINE_IDLE;
...
ER cnn_change_interrupt(BOOL cnn_id, UINT32 int_en)
{
	...
	//nvt_dbg(ERR, "cnn_interrupt: cnn_chk_state_machine\n");
	er_return = cnn_chk_state_machine(cnn_id, CNN_OP_CHGINT, FALSE);
	if (er_return != E_OK) {
		return er_return;
	}
	cnn_enable_int(cnn_id, ENABLE, int_en);
	//nvt_dbg(ERR, "cnn_interrupt_ed: cnn_chk_state_machine\n");
...
ER cnn_open(BOOL cnn_id, CNN_OPENOBJ *p_obj_cb)
{
	...
	//nvt_dbg(ERR, "cnn_open: cnn_chk_state_machine\n");
	// Check state-machine
	...
	//nvt_dbg(ERR, "cnn_open_end: cnn_chk_state_machine\n");
...
ER cnn_close(BOOL cnn_id)
{
	...
	//nvt_dbg(ERR, "cnn_close: cnn_chk_state_machine\n");
	// Check state-machine
	...
	// update state-machine
	//nvt_dbg(ERR, "cnn_close_ed: cnn_chk_state_machine\n");
...
ER cnn_pause(BOOL cnn_id)
{
	...
	PROF_END("kdrv_cnn");
	//printk("cnn cycle = %d\n", cnn_getcycle(cnn_id));
	//nvt_dbg(ERR, "cnn_pause: cnn_chk_state_machine\n");
	...
	//nvt_dbg(ERR, "cnn_pause_ed: cnn_chk_state_machine\n");
...
ER cnn_set_para2eng(BOOL cnn_id, CNN_OPMODE mode, CNN_PARM *p_parms)
{
	...
	case CNN_OPMODE_CONV:
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR, "cnn_set_engmode fail...\r\n");
			return er_return;
		}
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR, "cnn_set_convparm fail...\r\n");
			return er_return;
		}
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR, "cnn_set_fcd_parm fail...\r\n");
			return er_return;
		}
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR,  "cnn_set_bnscale_parm fail...\r\n");
			return er_return;
		}
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR,  "cnn_set_elt_parm fail...\r\n");
			return er_return;
		}
		...
		if (cnn_get_poolmode(cnn_id) == CNN_POOLING_LOCAL) {
			...
			if (er_return != E_OK) {
				nvt_dbg(ERR, "cnn_set_localpool_parm fail...\r\n");
				return er_return;
			}
		} else {
			er_return = cnn_set_globalpool_parm(cnn_id, &p_parms->global_pool);
			if (er_return != E_OK) {
				nvt_dbg(ERR, "cnn_set_globalpool_parm fail...\r\n");
				return er_return;
			}
		}
		break;
	...
	case CNN_OPMODE_DECONV:
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR,"cnn_set_deconvparm fail...\r\n");
			return er_return;
		}
		break;
	...
	case CNN_OPMODE_SACLEUP:
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR,"cnn_set_scaleupparm fail...\r\n");
			return er_return;
		}
		break;
	...
	case CNN_OPMODE_LRN:
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR,"cnn_set_lrnparm fail...\r\n");
			return er_return;
		}
		break;
	...
	case CNN_OPMODE_USERDEFINE:
		...
		if (cnn_get_engmode(cnn_id) == CNN_CONV) {
			...
			if (er_return != E_OK) {
				nvt_dbg(ERR,"cnn_set_pool_parm fail...\r\n");
				return er_return;
			}
		}
...
{
	ER er_return;
	//nvt_dbg(ERR, "cnn_setmode: cnn_chk_state_machine\n");
	er_return = cnn_chk_state_machine(cnn_id, CNN_OP_SET2PAUSE, FALSE);
	if (er_return != E_OK) {
		return er_return;
	}
	...
	//nvt_dbg(ERR, "cnn_set_mode_ed: cnn_chk_state_machine\n");
	er_return = cnn_chk_state_machine(cnn_id, CNN_OP_SET2PAUSE, TRUE);
...
BOOL cnn_eng_valid(UINT32 cnn_id) 
{
	...
	if (cnn_id == 0) {
		...
		if (efuse_check_available(__xstring(__section_name__)) != TRUE) {
			nvt_dbg(ERR, "not support CNN2!\r\n");
			is_valid = FALSE;
		} else {
			is_valid = TRUE;
		}
	} else {
		nvt_dbg(ERR, "engine id is invalid!\r\n");
	}
...
VOID cnn_reset_status(VOID)
{
	...
	if (cnn_lock_status1 != NO_TASK_LOCKED) {
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR, "cnn_reset_status: Error at cnn_close().\r\n");
            goto err_exit;
		}
	}
	if (cnn_lock_status2 != NO_TASK_LOCKED) {
...
VOID cnn_dma_abort(UINT32 cnn_id)
{
    ...
    if (is_dma_disable == 1) {
        nvt_dbg(IND, "CNN_DMA_ABORT: it has been in dma_disable state.\r\n");
        return;
    }
