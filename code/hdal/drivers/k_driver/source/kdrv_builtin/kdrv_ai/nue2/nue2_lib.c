...
UINT64 nue2_dump_total_cycle(UINT32 debug_layer)
{
    ...
    if(debug_layer==1) {
    	...
    	for (i = 0; i < nue2_cnt; i++) {
    		printk("item %d nue2 cycle = %d\n", i, (unsigned int)nue2_dbg_cnt[i]);
    	}
    }
    //#else
        //printk("NUE2_CYCLE_LAYER is disable now.\n");
    #endif
...
ER nue2_open(NUE2_OPENOBJ *p_obj_cb)
{
    ...
    if (is_dma_disable == 1) {
        nvt_dbg(IND, "NUE2_OPEN: it has been in dma_disable state.\r\n");
    } else {
		// SW reset
		nue2_clr(0);
		nue2_clr(1);
...
VOID nue2_reset_status(VOID)
{
	...
	if (g_nue2_lock_status != NO_TASK_LOCKED) {
		...
		if (er_return != E_OK) {
			nvt_dbg(ERR, "nue2_reset_status: Error at nue2_close().\r\n");
			goto err_exit;
		}
	}
	nue2_status = NUE2_ENGINE_IDLE;
...
VOID nue2_dma_abort(VOID)
{
	...
	is_dma_disable = nue2_get_dma_disable();
	if (is_dma_disable == 1) {
		nvt_dbg(IND, "NUE2_DMA_ABORT: it has been in dma_disable state.\r\n");
		return;
	}
	nue2_set_dma_disable(1);
