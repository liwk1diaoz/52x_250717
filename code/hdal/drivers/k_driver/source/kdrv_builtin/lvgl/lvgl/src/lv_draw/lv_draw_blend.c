...
#elif LV_USE_GPU_NVT_DMA2D
	#include "../lv_gpu/lv_gpu_nvt_dma2d.h"
#endif
 ...
 **********************/
	...
	#define DISP_BUF_FIRST_ALPHA(X) disp_buf_first[X].ext_ch.alpha
	#define DISP_BUF_LAST_ALPHA last_res_color.ext_ch.alpha
#else
	#define DISP_BUF_FIRST_ALPHA(X) disp_buf_first[X].ch.alpha
	#define DISP_BUF_LAST_ALPHA last_res_color.ch.alpha
#endif
...
#define FILL_NORMAL_MASK_PX_SCR_TRANSP(out_x,  color)                                               \
    if(*mask_tmp_x) {          \
        if(*mask_tmp_x == LV_OPA_COVER) disp_buf_first[out_x] = color;                                 \
        else if(disp->driver.screen_transp) lv_color_mix_with_alpha(disp_buf_first[out_x], DISP_BUF_FIRST_ALPHA(out_x),              \
                                                                        color, *mask_tmp_x, &disp_buf_first[out_x], &DISP_BUF_FIRST_ALPHA(out_x));           \
        else disp_buf_first[out_x] = lv_color_mix(color, disp_buf_first[out_x], *mask_tmp_x);            \
    }                                                                                                      \
    mask_tmp_x++;
...
#define MAP_NORMAL_MASK_PX_SCR_TRANSP(x)                        \
    if(*mask_tmp_x) {          \
        if(*mask_tmp_x == LV_OPA_COVER) disp_buf_first[x] = map_buf_first[x];                                 \
        else if(disp->driver.screen_transp) lv_color_mix_with_alpha(disp_buf_first[x], DISP_BUF_FIRST_ALPHA(x),              \
                                                                        map_buf_first[x], *mask_tmp_x, &disp_buf_first[x], &DISP_BUF_FIRST_ALPHA(x));                  \
        else disp_buf_first[x] = lv_color_mix(map_buf_first[x], disp_buf_first[x], *mask_tmp_x);            \
    }                                                                                               \
    mask_tmp_x++;
...
#elif LV_USE_GPU_NVT_DMA2D && (LV_COLOR_DEPTH == 32) && 0

            if( (draw_area_h >= 2) &&
            	(draw_area_w >= 2) &&
            	(lv_area_get_size(draw_area) >= LV_GPU_NVT_FILL_WITH_ALPHA_SIZE_LIMIT)){

				lv_gpu_nvt_dma2d_fill_with_alpha(
						disp_buf,
						disp_w,
                ...
                for(x = 0; x < draw_area_w; x++) {
                    if(last_dest_color.full != disp_buf_first[x].full) {
                        ...
                        if(disp->driver.screen_transp) {
                            lv_color_mix_with_alpha(disp_buf_first[x], DISP_BUF_FIRST_ALPHA(x), color, opa, &last_res_color,
                                                    &DISP_BUF_LAST_ALPHA);
                        }
                        else
#endif
...
#if LV_COLOR_DEPTH == 24
        last_dest_color.ext_ch.alpha = disp_buf_first[0].ext_ch.alpha;
        last_res_color.ext_ch.alpha = disp_buf_first[0].ext_ch.alpha;
#endif
...
#if LV_COLOR_SCREEN_TRANSP
                            if(disp->driver.screen_transp) {
                                lv_color_mix_with_alpha(disp_buf_first[x], DISP_BUF_FIRST_ALPHA(x), color, opa_tmp, &last_res_color,
                                                        &DISP_BUF_LAST_ALPHA);
                            }
                            else
#endif
...
#if LV_COLOR_DEPTH == 24
                            last_dest_color.ext_ch.alpha = disp_buf_first[x].ext_ch.alpha;
#endif
                        }
                        disp_buf_first[x] = last_res_color;
                    }
...
#if LV_COLOR_DEPTH == 24
        last_dest_color.ext_ch.alpha = disp_buf_tmp[0].ext_ch.alpha;
        last_res_color.ext_ch.alpha = disp_buf_tmp[0].ext_ch.alpha;
#endif
...
#if LV_COLOR_DEPTH == 24
                    last_dest_color.ext_ch.alpha = disp_buf_tmp[x].ext_ch.alpha;
#endif
                }
                disp_buf_tmp[x] = last_res_color;
            }
                                             ...
                                             const lv_opa_t * mask, lv_draw_mask_res_t mask_res)
{
    ...
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
        ...
        if(opa > LV_OPA_MAX) {
            ...
            if(	(draw_area_w >= 2) &&
            	(draw_area_h >= 2) &&
            	(lv_area_get_size(draw_area) >= LV_GPU_NVT_COPY_SIZE_LIMIT)) {

            	lv_gpu_nvt_dma2d_copy_src_memcpy(
            			map_buf_first,
						map_w,
						draw_area_h,
						disp_buf,
            ...
            if(	(draw_area_w >= 2) &&
            	(draw_area_h >= 2) &&
            	lv_area_get_size(draw_area) >= LV_GPU_NVT_BLEND_SIZE_LIMIT) {

            	lv_gpu_nvt_dma2d_blend_src_memcpy(
            			map_buf_first,
						map_w,
						draw_area_h,
						disp_buf,
...
#if LV_COLOR_SCREEN_TRANSP
                    if(disp->driver.screen_transp) {
                        lv_color_mix_with_alpha(disp_buf_first[x], DISP_BUF_FIRST_ALPHA(x), map_buf_first[x], opa, &disp_buf_first[x],
                                                &DISP_BUF_FIRST_ALPHA(x));
                    }
                    else
#endif
                    {
...
#if LV_COLOR_SCREEN_TRANSP
                        if(disp->driver.screen_transp) {
                            lv_color_mix_with_alpha(disp_buf_first[x], DISP_BUF_FIRST_ALPHA(x), map_buf_first[x], opa_tmp, &disp_buf_first[x],
                                                    &DISP_BUF_FIRST_ALPHA(x));
                        }
                        else
#endif
...
#elif LV_COLOR_DEPTH == 16 || LV_COLOR_DEPTH == 24
