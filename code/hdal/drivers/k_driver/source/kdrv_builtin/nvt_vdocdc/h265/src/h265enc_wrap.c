...
void h265Enc_wrapSeqCfg(H26XRegSet *pRegSet, H265ENC_CTX *pVdoCtx, UINT32 uiPaBsdmaAddr)
{
	...
	save_to_reg(&pRegSet->FUNC_CFG[1], 6, 16,3);    // intra_direct_mode_wt
	...
	if (pSeqCfg->bTileEn) {
		if (pSeqCfg->bD2dEn) {
			if (h26x_getChipId() == 98520) {
				save_to_reg(&pRegSet->FUNC_CFG[1], 0, 3, 1);
			} else {
				save_to_reg(&pRegSet->FUNC_CFG[1], 1, 3, 1);
			}
		} else {
			if (h26x_getChipId() == 98520) {
				if (pSeqCfg->eQLevel == HEVC_QUALITY_MAIN) {
					save_to_reg(&pRegSet->FUNC_CFG[1], 1, 3, 1);
				} else {
					save_to_reg(&pRegSet->FUNC_CFG[1], 0, 3, 1);
				}
				...
				if (pSeqCfg->eQLevel == HEVC_QUALITY_MAIN) {
					save_to_reg(&pRegSet->FUNC_CFG[1], 1, 22, 1);
				} else {
					save_to_reg(&pRegSet->FUNC_CFG[1], 0, 3, 1);
				}
			}
	...
	save_to_reg(&pRegSet->ILF_CFG[0], pSeqCfg->ilf_attr.llf_bata_offset_div2, 4, 4);
	save_to_reg(&pRegSet->ILF_CFG[0], pSeqCfg->ilf_attr.llf_alpha_c0_offset_div2, 8, 4);
...
#if FIX_POINT_LAMBDA
static INT32 set_lambda(H26XENC_VAR *pVar, H26XRegSet *pRegSet)
{
	...
	if ((INT32)pPicCfg->ucSliceQP < 0 || (INT32)pPicCfg->ucSliceQP > 51) {
		DBG_ERR("wrong slice qp value:%d\r\n", (int)pPicCfg->ucSliceQP);
		return H26XENC_FAIL;
	} else {
		// Cb
...
static void h265Enc_dumpPhyAddr(H265ENC_INFO *pInfo, H26XENC_VAR *pVar)
{
	H265ENC_CTX   *pVdoCtx  = (H265ENC_CTX *)pVar->pVdoCtx;
	...
	if(pPicCfg->uiRecExtraEnable & 0x1) {
		for (i = 0; i < pSeqCfg->ucTileNum-1; i++) {
			DBG_DUMP("TILE_EXTRA_RD_Y_ADDR[%d] : %08x\r\n", (int)i, (int)h26x_getPhyAddr(pAddr->uiTileExtraY[i][pVdoCtx->eRefIdx]));
			DBG_DUMP("TILE_EXTRA_RD_C_ADDR[%d] : %08x\r\n", (int)i, (int)h26x_getPhyAddr(pAddr->uiTileExtraC[i][pVdoCtx->eRefIdx]));
		}
		DBG_DUMP("SIDE_INFO_RD_ADDR_0 : %08x\r\n", (int)h26x_getPhyAddr(pAddr->uiSideInfo[1][pVdoCtx->eRefIdx]));
	}
	if (pPicCfg->uiRecExtraEnable & 0x2){
		for (i = 0; i < pSeqCfg->ucTileNum-1; i++) {
			DBG_DUMP("TILE_EXTRA_WR_Y_ADDR[%d] : %08x\r\n", (int)i, (int)h26x_getPhyAddr(pAddr->uiTileExtraY[i][pVdoCtx->eRecIdx]));
			DBG_DUMP("TILE_EXTRA_WR_C_ADDR[%d] : %08x\r\n", (int)i, (int)h26x_getPhyAddr(pAddr->uiTileExtraC[i][pVdoCtx->eRecIdx]));
		}
		DBG_DUMP("SIDE_INFO_WR_ADDR_0 : %08x\r\n", (int)h26x_getPhyAddr(pAddr->uiSideInfo[1][pVdoCtx->eRecIdx]));
	}
...
INT32 h265Enc_wrapPicCfg(H26XRegSet *pRegSet, H265ENC_INFO *pInfo, H26XENC_VAR *pVar)
{
	...
	INT32 dime_dist_scale = 1;
	UINT32 uiSideInfoSize = 0;
	UINT8 i;

	#if VDOCDC_DUMP_PA_ADDR
	h265Enc_dumpPhyAddr(pInfo, pVar);
	#endif

	// for CIM coverity
	if (pVdoCtx->eRecIdx >= FRM_IDX_MAX || pVdoCtx->eRefIdx >= FRM_IDX_MAX ) {
		DBG_ERR("wrong rec/ref index:%d,%d\r\n", (int)pVdoCtx->eRecIdx, (int)pVdoCtx->eRefIdx);
		return H26XENC_FAIL;
	} else {
		// address and lineoffset set by each picture //
		...
		*/
		...
		{
			for (i = 0; i < pSeqCfg->ucTileNum-1; i++) {
			    save_to_reg(&pRegSetTile->TILE_EXTRA_RD_Y_ADDR[i], h26x_getPhyAddr(pAddr->uiTileExtraY[i][pVdoCtx->eRefIdx]), 0, 32);
			    save_to_reg(&pRegSetTile->TILE_EXTRA_RD_C_ADDR[i], h26x_getPhyAddr(pAddr->uiTileExtraC[i][pVdoCtx->eRefIdx]), 0, 32);
			}
		...
		if (pPicCfg->uiRecExtraEnable & 0x2){
			for (i = 0; i < pSeqCfg->ucTileNum-1; i++) {
			    save_to_reg(&pRegSetTile->TILE_EXTRA_WR_Y_ADDR[i], h26x_getPhyAddr(pAddr->uiTileExtraY[i][pVdoCtx->eRecIdx]), 0, 32);
			    save_to_reg(&pRegSetTile->TILE_EXTRA_WR_C_ADDR[i], h26x_getPhyAddr(pAddr->uiTileExtraC[i][pVdoCtx->eRecIdx]), 0, 32);
			}
		...
		for (i = 0; i < pSeqCfg->ucTileNum-1; i++) {
			uiSideInfoSize += pVdoCtx->uiSizeSideInfo[i];
 			save_to_reg(&pRegSetTile->TILE_SIDE_INFO_WR_ADDR[i], (pRegSet->SIDE_INFO_WR_ADDR_0 + uiSideInfoSize), 0, 32);
			save_to_reg(&pRegSetTile->TILE_SIDE_INFO_RD_ADDR[i], (pRegSet->SIDE_INFO_RD_ADDR_0 + uiSideInfoSize), 0, 32);
		}
		...
		save_to_reg(&pRegSet->ILF_CFG[0], (pPicCfg->uiFrameSkip) ? 0 : ((pVdoCtx->eRecIdx == FRM_IDX_NON_REF) ? 0 :pSeqCfg->ilf_attr.ilf_across_tile_en), 12, 1);
		...
		if (pSeqCfg->bTileEn)
			save_to_reg(&pRegSet->PIC_CFG, pFuncCtx->stSliceSplit.stExe.uiSliceRowNum, 16, 16);
		if(1){
			save_to_reg(&pRegSet->PIC_CFG, (pVdoCtx->eRecIdx == pVdoCtx->eRefIdx), 8, 1);
		}else{
			save_to_reg(&pRegSet->PIC_CFG, 0, 8, 1);
		}
		save_to_reg(&pRegSet->PIC_CFG, (pPicCfg->prev_frm_is_skip[pVdoCtx->eRefIdx]), 9, 1);
		...
		save_to_reg(&pRegSet->OSG_CFG[0x494/4  + 16 * 1],   1,          	0, 1); //overlap 0:64x64, 1:16x16
		if (h26x_getChipId() == 98528)
...
void h265Enc_wrapRdo(H26XRegSet *pRegSet, H265ENC_CTX *pVdoCtx)
{
	...
	save_to_reg(&pRegSet->RDO_CFG[3], pRdoCfg->stRdoCfg.cost_bias_skip , 16 ,5);
	save_to_reg(&pRegSet->RDO_CFG[3], pRdoCfg->stRdoCfg.cost_bias_merge , 24 ,5);

	save_to_reg(&pRegSet->RDO_CFG_4, pRdoCfg->stRdoCfg.global_motion_penalty_I32, 0, 4);
	save_to_reg(&pRegSet->RDO_CFG_4, pRdoCfg->stRdoCfg.global_motion_penalty_I16, 4, 4);
	save_to_reg(&pRegSet->RDO_CFG_4, pRdoCfg->stRdoCfg.global_motion_penalty_I08, 8, 4);
	...
	save_to_reg(&pRegSet->IME_CFG, /*(amvp_mode == 6)*/0, 0, 1);
	save_to_reg(&pRegSet->IME_CFG, pRdoCfg->stRdoCfg.ime_scale_64c, 1, 5);
	save_to_reg(&pRegSet->IME_CFG, pRdoCfg->stRdoCfg.ime_scale_64p, 6, 5);
	save_to_reg(&pRegSet->IME_CFG, pRdoCfg->stRdoCfg.ime_scale_32c, 11, 5);
	save_to_reg(&pRegSet->IME_CFG, pRdoCfg->stRdoCfg.ime_scale_32p, 16, 5);
	save_to_reg(&pRegSet->IME_CFG, pRdoCfg->stRdoCfg.ime_scale_16c, 21, 5);
}
...
void h265Enc_wrapRdoCfg(H26XRegSet *pRegSet, H265ENC_CTX *pVdoCtx)
{
	...
	save_to_reg(&pRegSet->RDO_CFG_4, pRdoCfg->global_motion_penalty_I32, 12, 4);
	save_to_reg(&pRegSet->RDO_CFG_4, pRdoCfg->global_motion_penalty_I16, 16, 4);
	save_to_reg(&pRegSet->RDO_CFG_4, pRdoCfg->global_motion_penalty_I08, 20, 4);

	save_to_reg(&pRegSet->IME_CFG, pRdoCfg->ime_scale_64c, 1, 5);
	save_to_reg(&pRegSet->IME_CFG, pRdoCfg->ime_scale_64p, 6, 5);
	save_to_reg(&pRegSet->IME_CFG, pRdoCfg->ime_scale_32c, 11, 5);
	save_to_reg(&pRegSet->IME_CFG, pRdoCfg->ime_scale_32p, 16, 5);
	save_to_reg(&pRegSet->IME_CFG, pRdoCfg->ime_scale_16c, 21, 5);
}
