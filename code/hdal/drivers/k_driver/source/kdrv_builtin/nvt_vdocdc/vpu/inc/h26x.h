...
#define H26X_REG_PLUG_IN_FINISH_OFFSET	0x74C
#define H26X_REG_PLUG_IN_2_START_OFFSET	0x750 //due to apb_wr_reg_burst need < 256, so split two parts
#define H26X_REG_PLUG_IN_2_FINISH_OFFSET 0x9B4
#define H26X_REG_PLUG_IN_3_START_OFFSET	0x350 //due to apb_wr_reg_burst need < 256, so split two parts
#define H26X_REG_PLUG_IN_3_FINISH_OFFSET  0x3FC
...
typedef struct _H26XRegSet{
	...
	UINT32 JND_CFG;					// 0x22C
	UINT32 MOTION_BIT_ADDR[3]; 		// 0x230 ~ 0x238
	UINT32 DBG_CFG;					// 0x23C
	UINT32 DBG_REPORT[3];			// 0x240 ~ 0x248
	UINT32 RES_24C;					// 0x24C
	UINT32 CHK_REPORT[64];			// 0x250 ~ 0x34C
	UINT32 TILE_CFG[27];            // 0x350 ~ 0x3B8
	UINT32 RRC_REF_FRM_COEFF4;      // 0x3BC
	UINT32 SRAQ_CFG_17;				// 0x3C0
	UINT32 RMD_1;					// 0x3C4
	UINT32 JND_CFG_1;				// 0x3C8
	UINT32 BGR_CFG[4];				// 0x3CC ~ 0x3D8
	UINT32 RRC_CFG_11[6];				// 0x3DC ~ 0x3F0
	UINT32 RES_3F4_3FC[3];				// 0x3F4 ~ 0x3FC
	UINT32 OSG_CFG[310];				// 0x400 ~ 0x8D4
	UINT32 NAL_REPORT[2];				// 0x8D8 ~ 0x8DC
	UINT32 RES_8E0_8FC[8];				// 0x8D4 ~ 0x8FC
	UINT32 SDE_CFG[46];				// 0x900 ~ 0x9B4
	UINT32 RES_9B8_9BC[2];				// 0x9B8 ~ 0x9BC
	UINT32 CHK_REPORT_2[64];			// 0x9C0 ~ 0xABC
	UINT32 TNR_CFG[19];				// 0xAC0 ~ 0xB08
	UINT32 RES_B0C[5];				// 0xB0C ~ 0xB1C
	UINT32 LMTBL_CFG[26];				// 0xB20 ~ 0xB84
	UINT32 SLMTBL_CFG[18];				// 0xB88 ~ 0xBCC
}H26XRegSet;
...
extern void h26x_getEncReport(UINT32 uiEncReport[53]);
...
extern void h26x_enableClk(void);
extern void h26x_disableClk(void);
extern H26xStatus h26x_lock(void);
extern H26xStatus h26x_unlock(void);
