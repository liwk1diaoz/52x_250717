...
typedef struct _H264ENC_INIT_{
	...
	BOOL			bGDRIFrm;			///< GDR in I frame enable
	H26XEncFrmCropInfo frm_crop_info; ///< frame crop information
} H264ENC_INIT;
...
extern INT32 h264Enc_getResult(H26XENC_VAR *pVar, unsigned int enc_mode, H26XEncResultCfg *pResult, UINT32 uiHwInt, UINT32 retri, H264ENC_INFO *pInfo, UINT32 bsout_buf_addr_2);
...
extern int h264Enc_getMaxDecFrameBuf(void);
extern int h264Enc_setMaxDecFrameBuf(int value);
#endif
#if H264_FRO_IP_SYNC
extern int h264Enc_setFroSync(int value);
extern int h264Enc_getFroSync(void);
#endif
...
extern void h264Enc_prepareSpecialIFrame(H26XENC_VAR *pVar, UINT32 bs_buf_size);

// emulation only //
extern void h264Enc_setPicQP(UINT8 ucQP, H26XENC_VAR *pVar);

extern int h264Enc_setSEIBsChksumen(H26XENC_VAR *pVar, UINT32 enable);
#endif
