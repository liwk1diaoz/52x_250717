...
static void encode_sps_frm_cropping(bstream *bs, H264EncSeqCfg *pSeqCfg)
{
	...
	UINT32 crop_width = (pSeqCfg->usMbWidth*16 - (pSeqCfg->frm_crop_info.frm_crop_enable? pSeqCfg->frm_crop_info.display_w: pSeqCfg->uiDisplayWidth))/2;
	UINT32 crop_bot = (pSeqCfg->usMbHeight*16 - (pSeqCfg->frm_crop_info.frm_crop_enable? pSeqCfg->frm_crop_info.display_h: pSeqCfg->usHeight))/2;
...
static void encode_sps_vui(bstream *bs, H264EncSeqCfg *pSeqCfg)
{
	...
	if (pSeqCfg->ucColourPrimaries == 2 && pSeqCfg->ucTransferCharacteristics == 2 && pSeqCfg->ucMatrixCoef == 2) {
		put_bits(bs, 0, 1); /* colour_description_present_flag */
	} else {
	...
	if (0 == gH264MaxDecFrameBuf) {
		...
		/* bitstream_restriction_flag */
		put_bits(bs, 1, 1);
		put_bits(bs, 0, 1);			/* motion_vectors_over_pic_boundaries_flag */
		write_uvlc_codeword(bs, 0);	/* max_bytes_pre_pic_denom */
		write_uvlc_codeword(bs, 0);	/* max_bits_per_mb_denom */
		write_uvlc_codeword(bs, 16);	/* log2_max_mv_length_horizontal */
		write_uvlc_codeword(bs, 16);	/* log2_max_mv_length_vertical */
		write_uvlc_codeword(bs, 0);	/* num_recorder_frames */
		write_uvlc_codeword(bs, gH264MaxDecFrameBuf);	/* max_dec_frame_buffering */
	}
}
...
static void encode_pps(bstream *bs, H264EncSeqCfg *pSeqCfg, H264EncPicCfg *pPicCfg)
{
	...
	write_signed_uvlc_codeword(bs, pSeqCfg->cChrmQPIdx);	/* chroma_qp_idx_offset */
	...
	if (pSeqCfg->eProfile == PROFILE_HIGH)
	{
		...
		write_signed_uvlc_codeword(bs, pSeqCfg->cSecChrmQPIdx);		/* (sign)second_croma_qp_index_offset */
	}
	write_rbsp_trailing_bits(bs);
}
...
#if 1
    int reorder_flag = 0;
    if ((pSeqCfg->uiLTRInterval && ((pPicCfg->sPoc % pSeqCfg->uiLTRInterval) == 0)) ||
        ((pSeqCfg->ucSVCLayer == 2) && ((pPicCfg->sRefPoc % 4) == 0))) {
        reorder_flag = 1;
    }
    if (0 == reorder_flag) {
		put_bits(bs, 0, 1);
...
void h264Enc_encSliceHeader(H264ENC_CTX *pVdoCtx, H26XFUNC_CTX *pFuncCtx, H26XCOMN_CTX *pComnCtx, BOOL bGetSeqHdrEn, BOOL bSpecialP)
{
	H264EncSeqCfg *pSeqCfg = &pVdoCtx->stSeqCfg;
	H264EncPicCfg *pPicCfg = &pVdoCtx->stPicCfg;
	...
	//special p frame, recover to I_SLICE at the buttom
	if (bSpecialP == 1) pPicCfg->ePicType = P_SLICE;
	...
	for (i = 0; i < bsdma_addr[0]; i++) {
		UINT32 bit_res, uiSliceInfoLen;
		UINT8  bs_buf[32], *addr;

		uiSliceInfoLen = 0;
		addr = (UINT8 *)(pComnCtx->stVaAddr.uiPicHdr + uiPicInfoLen + pPicCfg->uiPicHdrLen);

		//write_slice_nal_header(bs, nal_ref_idc, nal_unit_type);
		uiSliceInfoLen += write_nal_header(addr, nal_ref_idc, nal_unit_type);

		init_pack_bitstream(bs,(void *)bs_buf, 32);

		write_uvlc_codeword(bs, (i * pFuncCtx->stSliceSplit.stExe.uiSliceRowNum * pSeqCfg->usMbWidth)); // first_mb_in_slice //
		...
		if (bs_byte_length(bs) > 32) {
			DBG_ERR("fail, slice header size is over buffer size:bs_buf\r\n");
		}
		//init_pack_bitstream(bs, (UINT8 *)(pComnCtx->stVaAddr.uiPicHdr + uiPicInfoLen), (pComnCtx->uiPicHdrBufSize - uiPicInfoLen));
		uiSliceInfoLen += rbspToebsp(addr + uiSliceInfoLen, bs_buf, bs_byte_length(bs));
		//pVdoCtx->uiSPSHdrLen = pVdoCtx->uiSeqHdrLen;
