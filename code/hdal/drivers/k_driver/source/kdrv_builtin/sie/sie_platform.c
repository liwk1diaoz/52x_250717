...
static SIE_CLKSRC_SEL g_sieClkSrc[MODULE_CLK_NUM]={SIE_CLKSRC_480,SIE_CLKSRC_480,SIE_CLKSRC_480,SIE_CLKSRC_480,SIE_CLKSRC_480};
static UINT32 g_sieClkRate[MODULE_CLK_NUM]={240000000,240000000,240000000,240000000,240000000};
...
static SEM_HANDLE SEMID_SIE4;
static ID     FLG_ID_SIE4;
static SEM_HANDLE SEMID_SIE5;
static ID     FLG_ID_SIE5;
...
void sie_platform_enable_clk(SIE_ENGINE_ID id)
{
	...
	case SIE_ENGINE_ID_4:
		pll_enableClock(SIE4_CLK);
		break;

	case SIE_ENGINE_ID_5:
		pll_enableClock(SIE5_CLK);
		break;
...
void sie_platform_disable_clk(SIE_ENGINE_ID id)
{
	...
	case SIE_ENGINE_ID_4:
		pll_disableClock(SIE4_CLK);
		sie_getClock(SIE_ENGINE_ID_4,&clksrc, &clkrate);
		break;

	case SIE_ENGINE_ID_5:
		pll_disableClock(SIE5_CLK);
		sie_getClock(SIE_ENGINE_ID_5,&clksrc, &clkrate);
		break;
...
ER sie_platform_sem_wait(SIE_ENGINE_ID id)
{
	...
	case SIE_ENGINE_ID_4:
		erReturn = SEM_WAIT(SEMID_SIE4);
		break;

	case SIE_ENGINE_ID_5:
		erReturn = SEM_WAIT(SEMID_SIE5);
		break;
...
ER sie_platform_sem_signal(SIE_ENGINE_ID id)
{
	...
	case SIE_ENGINE_ID_4:
		SEM_SIGNAL(SEMID_SIE4);
		break;

	case SIE_ENGINE_ID_5:
		SEM_SIGNAL(SEMID_SIE5);
		break;
...
ER sie_platform_flg_clear(SIE_ENGINE_ID id, FLGPTN flg)
{
	...
	case SIE_ENGINE_ID_4:
		vos_flag_clr(FLG_ID_SIE4, flg);
		break;

	case SIE_ENGINE_ID_5:
		vos_flag_clr(FLG_ID_SIE5, flg);
		break;
...
ER sie_platform_flg_set(SIE_ENGINE_ID id, FLGPTN flg)
{
	...
	case SIE_ENGINE_ID_4:
		vos_flag_iset(FLG_ID_SIE4, flg);
		break;

	case SIE_ENGINE_ID_5:
		vos_flag_iset(FLG_ID_SIE5, flg);
		break;
...
ER sie_platform_flg_wait(SIE_ENGINE_ID id, PFLGPTN p_flgptn, FLGPTN flg)
{
	...
	case SIE_ENGINE_ID_1:
		if (vos_flag_wait_timeout(p_flgptn, FLG_ID_SIE, flg, TWF_CLR|TWF_ORW, vos_util_msec_to_tick(SIE_DRVWAIT_FLG_TIMEOUT_MS)) != 0) {
			nvt_dbg(ERR,"ERR id: %d, wait flag timeout (0x%08x,0x%08x)\r\n",id,(int)p_flgptn,(int)flg);
			wai_rt = E_SYS;
		}		
		break;
	...
	case SIE_ENGINE_ID_4:
		if (vos_flag_wait_timeout(p_flgptn, FLG_ID_SIE4, flg, TWF_CLR|TWF_ORW, vos_util_msec_to_tick(SIE_DRVWAIT_FLG_TIMEOUT_MS)) != 0) {
			nvt_dbg(ERR,"ERR id: %d, wait flag timeout (0x%08x,0x%08x)\r\n",id,(int)p_flgptn,(int)flg);
			wai_rt = E_SYS;
		}
		break;
	...
	case SIE_ENGINE_ID_5:
		if (vos_flag_wait_timeout(p_flgptn, FLG_ID_SIE5, flg, TWF_CLR|TWF_ORW, vos_util_msec_to_tick(SIE_DRVWAIT_FLG_TIMEOUT_MS)) != 0) {
			nvt_dbg(ERR,"ERR id: %d, wait flag timeout (0x%08x,0x%08x)\r\n",id,(int)p_flgptn,(int)flg);
			wai_rt = E_SYS;
		}
		break;
...
irqreturn_t sie4_platform_isr(int irq, void *devid)
{
	sie_isr(SIE_ENGINE_ID_4);
	return IRQ_HANDLED;
}
irqreturn_t sie5_platform_isr(int irq, void *devid)
{
	sie_isr(SIE_ENGINE_ID_5);
	return IRQ_HANDLED;
}
void sie_platform_create_resource(void)
{
	...
	vos_flag_create(&FLG_ID_SIE4, NULL, "sie4");
	vos_sem_create(&SEMID_SIE4, 1, "SEM_ID_SIE4");

	vos_flag_create(&FLG_ID_SIE5, NULL, "sie5");
	vos_sem_create(&SEMID_SIE5, 1, "SEM_ID_SIE5");
	...
	_SIE_REG_BASE_ADDR_SET[SIE_ENGINE_ID_4] = 0xF0D40000;
	_SIE_REG_BASE_ADDR_SET[SIE_ENGINE_ID_5] = 0xF0D80000;
	...
	request_irq(INT_ID_SIE4, sie4_platform_isr, IRQF_TRIGGER_HIGH, "sie4", 0);
	request_irq(INT_ID_SIE5, sie5_platform_isr, IRQF_TRIGGER_HIGH, "sie5", 0);
}
#else
void sie_platform_create_resource(SIE_MODULE_INFO *pmodule_info)
{
	...
	_SIE_REG_BASE_ADDR_SET[SIE_ENGINE_ID_4] = (UINT32)pmodule_info->io_addr[SIE_ENGINE_ID_4];
	_SIE_REG_BASE_ADDR_SET[SIE_ENGINE_ID_5] = (UINT32)pmodule_info->io_addr[SIE_ENGINE_ID_5];
	...
	sie_clk[SIE_ENGINE_ID_4] = clk_get(NULL, "f0d40000.sie");//
	sie_clk[SIE_ENGINE_ID_5] = clk_get(NULL, "f0d80000.sie");//
...
void sie_platform_release_resource(void)
{
	...
	vos_sem_destroy(FLG_ID_SIE4);
	vos_sem_destroy(SEMID_SIE4);

	vos_sem_destroy(FLG_ID_SIE5);
	vos_sem_destroy(SEMID_SIE5);
}
...
ER sie1_setClock(SIE_CLKSRC_SEL ClkSrc, UINT32 uiClkRate)
{
	...
	pll_setClockRate(PLL_CLKSEL_SIE_CLKSRC, PLL_CLKSEL_SIE_CLKSRC_PLL10);
	g_sieClkSrc[SIE_ENGINE_ID_1] = SIE_CLKSRC_PLL10;
	g_sieClkRate[SIE_ENGINE_ID_1] = pll_get_pll_freq(PLL_ID_10)>>1;
	pll_setPLLEn(PLL_ID_10, TRUE);
	...
	if (sie_power_saving_mode==0) {
	...
	}else if ((sie_power_saving_mode > 0) && (sie_power_saving_clkrate == 0)) {
		// error, use default
		parent_clk = clk_get(NULL, "pll10");
		sie_src_freq = clk_get_rate(parent_clk);
		g_sieClkSrc[SIE_ENGINE_ID_1] = SIE_CLKSRC_PLL10;
		...
		if (sie_power_saving_mode== 1) {
			...
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_1] = SIE_CLKSRC_PLL10;
			g_sieClkRate[SIE_ENGINE_ID_1] = sie_power_saving_clkrate;
			...
			g_sieClkRate[SIE_ENGINE_ID_1] = sie_power_saving_clkrate;
		}else if (sie_power_saving_mode == 3) {
			// clk src = 320MHz
			parent_clk = clk_get(NULL, "fix320m");
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_1] = SIE_CLKSRC_320;
			g_sieClkRate[SIE_ENGINE_ID_1] = sie_power_saving_clkrate;
		}else if (sie_power_saving_mode == 4) {
			// clk src = 192MHz
			parent_clk = clk_get(NULL, "fix192m");
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_1] = SIE_CLKSRC_192;
			g_sieClkRate[SIE_ENGINE_ID_1] = sie_power_saving_clkrate;
		}
	}
	#if 0
...
{
#if defined(__FREERTOS)
	pll_setClockRate(PLL_CLKSEL_SIE2_CLKSRC, PLL_CLKSEL_SIE2_CLKSRC_PLL10);
	g_sieClkSrc[SIE_ENGINE_ID_2] = SIE_CLKSRC_PLL10;
	g_sieClkRate[SIE_ENGINE_ID_2] = pll_get_pll_freq(PLL_ID_10)>>1;
	pll_setPLLEn(PLL_ID_10, TRUE);
	...
	if (sie_power_saving_mode==0) {
		...
		if (sie_power_saving_mode== 1) {
			...
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_2] = SIE_CLKSRC_PLL10;
			g_sieClkRate[SIE_ENGINE_ID_2] = sie_power_saving_clkrate;
		}else if (sie_power_saving_mode == 2) {
			// clk src = 480MHz
			parent_clk = clk_get(NULL, "fix480m");
			sie_src_freq = clk_get_rate(parent_clk);
			...
			parent_clk = clk_get(NULL, "fix192m");
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_2] = SIE_CLKSRC_192;
			g_sieClkRate[SIE_ENGINE_ID_2] = sie_power_saving_clkrate;
		}
	}
...
{
#if defined(__FREERTOS)
	pll_setClockRate(PLL_CLKSEL_SIE3_CLKSRC, PLL_CLKSEL_SIE3_CLKSRC_PLL10);
	g_sieClkSrc[SIE_ENGINE_ID_3] = SIE_CLKSRC_PLL10;
	g_sieClkRate[SIE_ENGINE_ID_3] = pll_get_pll_freq(PLL_ID_10)>>1;
	pll_setPLLEn(PLL_ID_10, TRUE);
	...
	if (sie_power_saving_mode==0) {
		...
		if (sie_power_saving_mode== 1) {
			...
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_3] = SIE_CLKSRC_PLL10;
			g_sieClkRate[SIE_ENGINE_ID_3] = sie_power_saving_clkrate;
		}else if (sie_power_saving_mode == 2) {
			// clk src = 480MHz
			parent_clk = clk_get(NULL, "fix480m");
			sie_src_freq = clk_get_rate(parent_clk);
			...
			parent_clk = clk_get(NULL, "fix192m");
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_3] = SIE_CLKSRC_192;
			g_sieClkRate[SIE_ENGINE_ID_3] = sie_power_saving_clkrate;
		}
	}
...
ER sie4_setClock(SIE_CLKSRC_SEL ClkSrc, UINT32 uiClkRate)
{
#if defined(__FREERTOS)
	pll_setClockRate(PLL_CLKSEL_SIE4_CLKSRC, PLL_CLKSEL_SIE4_CLKSRC_PLL10);
	g_sieClkSrc[SIE_ENGINE_ID_4] = SIE_CLKSRC_PLL10;
	g_sieClkRate[SIE_ENGINE_ID_4] = pll_get_pll_freq(PLL_ID_10)>>1;
	pll_setPLLEn(PLL_ID_10, TRUE);

	//nvt_dbg(ERR, "SIE ClkSrc = %d, ClkRate = %d\r\n",g_sieClkSrc[SIE_ENGINE_ID_1],g_sieClkRate[SIE_ENGINE_ID_1]);

	// Enable PLL clock source
	if (g_sieClkRate[SIE_ENGINE_ID_4] != 0) {
		pll_setClockFreq(SIE4CLK_FREQ, g_sieClkRate[SIE_ENGINE_ID_4]);
	}
#else
	struct clk *parent_clk=NULL;
	UINT32 sie_src_freq;
	...
	if (sie_power_saving_mode==0) {
		...
		g_sieClkSrc[SIE_ENGINE_ID_4] = SIE_CLKSRC_PLL10;
		if (sie_src_freq==SIE_MAX_CLK_FREQ)
			g_sieClkRate[SIE_ENGINE_ID_4] = sie_src_freq;
		else if (sie_src_freq==(2*SIE_MAX_CLK_FREQ))
			g_sieClkRate[SIE_ENGINE_ID_4] = sie_src_freq>>1;
		...
		if (sie_power_saving_mode== 1) {
			...
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_4] = SIE_CLKSRC_PLL10;
			g_sieClkRate[SIE_ENGINE_ID_4] = sie_power_saving_clkrate;
		}else if (sie_power_saving_mode == 2) {
			// clk src = 480MHz
			parent_clk = clk_get(NULL, "fix480m");
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_4] = SIE_CLKSRC_480;
			g_sieClkRate[SIE_ENGINE_ID_4] = sie_power_saving_clkrate;
		}else if (sie_power_saving_mode == 3) {
			// clk src = 320MHz
			parent_clk = clk_get(NULL, "fix320m");
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_4] = SIE_CLKSRC_320;
			g_sieClkRate[SIE_ENGINE_ID_4] = sie_power_saving_clkrate;
		}else if (sie_power_saving_mode == 4) {
			// clk src = 192MHz
			parent_clk = clk_get(NULL, "fix192m");
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_4] = SIE_CLKSRC_192;
			g_sieClkRate[SIE_ENGINE_ID_4] = sie_power_saving_clkrate;
		}
	}
		

	//nvt_dbg(ERR, "SIE4 ClkSrc = %d, ClkRate = %d\r\n",g_sieClkSrc[SIE_ENGINE_ID_4],g_sieClkRate[SIE_ENGINE_ID_4]);

	if (!IS_ERR(parent_clk)) {
		clk_set_parent(sie_clk[SIE_ENGINE_ID_4], parent_clk);
	}

	clk_put(parent_clk);

	if (g_sieClkRate[SIE_ENGINE_ID_4] != 0) {
		clk_set_rate(sie_clk[SIE_ENGINE_ID_4], g_sieClkRate[SIE_ENGINE_ID_4]);
	}
#endif

	return 0;
}
ER sie5_setClock(SIE_CLKSRC_SEL ClkSrc, UINT32 uiClkRate)
{
#if defined(__FREERTOS)
	pll_setClockRate(PLL_CLKSEL_SIE5_CLKSRC, PLL_CLKSEL_SIE5_CLKSRC_PLL10);
	g_sieClkSrc[SIE_ENGINE_ID_5] = SIE_CLKSRC_PLL10;
	g_sieClkRate[SIE_ENGINE_ID_5] = pll_get_pll_freq(PLL_ID_10)>>1;
	pll_setPLLEn(PLL_ID_10, TRUE);

	//nvt_dbg(ERR, "SIE ClkSrc = %d, ClkRate = %d\r\n",g_sieClkSrc[SIE_ENGINE_ID_1],g_sieClkRate[SIE_ENGINE_ID_1]);

	// Enable PLL clock source
	if (g_sieClkRate[SIE_ENGINE_ID_5] != 0) {
		pll_setClockFreq(SIE5CLK_FREQ, g_sieClkRate[SIE_ENGINE_ID_5]);
	}
#else
	struct clk *parent_clk=NULL;
	UINT32 sie_src_freq;
	...
	if (sie_power_saving_mode==0) {
		...
		g_sieClkSrc[SIE_ENGINE_ID_5] = SIE_CLKSRC_PLL10;
		if (sie_src_freq==SIE_MAX_CLK_FREQ)
			g_sieClkRate[SIE_ENGINE_ID_5] = sie_src_freq;
		else if (sie_src_freq==(2*SIE_MAX_CLK_FREQ))
			g_sieClkRate[SIE_ENGINE_ID_5] = sie_src_freq>>1;
		...
		if (sie_power_saving_mode== 1) {
			...
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_5] = SIE_CLKSRC_PLL10;
			g_sieClkRate[SIE_ENGINE_ID_5] = sie_power_saving_clkrate;
		}else if (sie_power_saving_mode == 2) {
			// clk src = 480MHz
			parent_clk = clk_get(NULL, "fix480m");
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_5] = SIE_CLKSRC_480;
			g_sieClkRate[SIE_ENGINE_ID_5] = sie_power_saving_clkrate;
		}else if (sie_power_saving_mode == 3) {
			// clk src = 320MHz
			parent_clk = clk_get(NULL, "fix320m");
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_5] = SIE_CLKSRC_320;
			g_sieClkRate[SIE_ENGINE_ID_5] = sie_power_saving_clkrate;
		}else if (sie_power_saving_mode == 4) {
			// clk src = 192MHz
			parent_clk = clk_get(NULL, "fix192m");
			sie_src_freq = clk_get_rate(parent_clk);
			g_sieClkSrc[SIE_ENGINE_ID_5] = SIE_CLKSRC_192;
			g_sieClkRate[SIE_ENGINE_ID_5] = sie_power_saving_clkrate;
		}
	}
		

	//nvt_dbg(ERR, "SIE5 ClkSrc = %d, ClkRate = %d\r\n",g_sieClkSrc[SIE_ENGINE_ID_5],g_sieClkRate[SIE_ENGINE_ID_5]);

	if (!IS_ERR(parent_clk)) {
		clk_set_parent(sie_clk[SIE_ENGINE_ID_5], parent_clk);
	}

	clk_put(parent_clk);

	if (g_sieClkRate[SIE_ENGINE_ID_5] != 0) {
		clk_set_rate(sie_clk[SIE_ENGINE_ID_5], g_sieClkRate[SIE_ENGINE_ID_5]);
	}
#endif
...
ER sie_setClock(SIE_ENGINE_ID id, SIE_CLKSRC_SEL ClkSrc, UINT32 uiClkRate)
{
	...
	case SIE_ENGINE_ID_4:
		sie4_setClock(ClkSrc, uiClkRate);
		break;

	case SIE_ENGINE_ID_5:
		sie5_setClock(ClkSrc, uiClkRate);
		break;
...
ER sie4_setPxClock(SIE_PXCLKSRC PxClkSel)
{
#if defined(__FREERTOS)
	#if defined(_BSP_NA51055_)
	switch (PxClkSel) {
	...
	case SIE_PXCLKSRC_PAD:
		pll_setClockRate(PLL_CLKSEL_SIE4_PXCLKSRC, PLL_CLKSEL_SIE4_PXCLKSRC_PXCLKPAD);
		pll_setClockRate(PLL_CLKSEL_SIE4_IO_PXCLKSRC, PLL_CLKSEL_SIE4_IO_PXCLKSRC_PXCLKPAD);
		pll_enableClock(SIE4_PXCLK);
		break;

	case SIE_PXCLKSRC_PAD_AHD:
		pll_setClockRate(PLL_CLKSEL_SIE4_PXCLKSRC, PLL_CLKSEL_SIE4_PXCLKSRC_PXCLKPAD);
		pll_setClockRate(PLL_CLKSEL_SIE4_IO_PXCLKSRC, PLL_CLKSEL_SIE4_IO_PXCLKSRC_PXCLKPAD2);
		pll_enableClock(SIE4_PXCLK);
		break;

	case SIE_PXCLKSRC_MCLK:
		pll_setClockRate(PLL_CLKSEL_SIE4_PXCLKSRC, PLL_CLKSEL_SIE4_PXCLKSRC_MCLK);
		pll_enableClock(SIE4_PXCLK);
		break;

	default:
		nvt_dbg(ERR,"SIE4 PX_SRC %d out of range !!\r\n", PxClkSel);
		break;
	}
	#endif
#else
	...
	case SIE_PXCLKSRC_PAD:
		parent_clk = clk_get(NULL, "sie4_io_pxclk");
		parent_clk2 = clk_get(NULL, "sie1_pxclkpad");
		break;
	...
	if (!IS_ERR(parent_clk)) {
		clk_set_parent(sie_pxclk[SIE_ENGINE_ID_4], parent_clk);
	}
	...
	if (parent_clk2 != NULL) {
		if (!IS_ERR(parent_clk2)) {
			clk_set_parent(sie_io_pxclk[SIE_ENGINE_ID_4], parent_clk2);
		}
		//clk_put(parent_clk2);
	}
#endif
...
ER sie5_setPxClock(SIE_PXCLKSRC PxClkSel)
{
#if defined(__FREERTOS)
	#if defined(_BSP_NA51055_)
	switch (PxClkSel) {
	...
	default:
		nvt_dbg(ERR,"SIE5 PX_SRC %d out of range !!\r\n", PxClkSel);
		break;
	}
	#endif
#else
...
ER sie_setPxClock(SIE_ENGINE_ID id, SIE_PXCLKSRC PxClkSel)
{
	...
	case SIE_ENGINE_ID_4:
		sie4_setPxClock(PxClkSel);
		break;

	case SIE_ENGINE_ID_5:
		sie5_setPxClock(PxClkSel);
		break;
...
static SIE_REG_INFO *reg_log[MODULE_CLK_NUM]={NULL,NULL,NULL,NULL,NULL};
static UINT32 reg_log_cnt[MODULE_CLK_NUM]={0};
static UINT32 b_reg_log_en[MODULE_CLK_NUM] = {DISABLE,DISABLE,DISABLE,DISABLE,DISABLE};
