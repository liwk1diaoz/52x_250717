...
#define IQ_DPC_NODE_PATH_2              "/fastboot/iq_dpc"
#define IQ_SHADING_NODE_PATH_2          "/fastboot/iq_shading"
#define IQ_LDC_NODE_PATH_2              "/fastboot/iq_ldc"
...
#define IQ_SHDR_DPC_NODE_PATH_2         "/fastboot/iq_dpc_shdr_1"
#define IQ_SHDR_SHADING_NODE_PATH_2     "/fastboot/iq_shading_shdr_1"
#define IQ_SHDR_LDC_NODE_PATH_2         "/fastboot/iq_ldc_shdr_1"

#define IQ_IR_FRONT_NODE_PATH_1         "/fastboot/front_iq_ir_0"
...
#define IQ_IR_FRONT_NODE_PATH_2         "/fastboot/front_iq_ir_1"
#define IQ_IR_NODE_PATH_2               "/fastboot/iq_ir_1"
#define IQ_IR_DPC_NODE_PATH_2           "/fastboot/iq_dpc_ir_1"
#define IQ_IR_SHADING_NODE_PATH_2       "/fastboot/iq_shading_ir_1"
#define IQ_IR_LDC_NODE_PATH_2           "/fastboot/iq_ldc_ir_1"
...
static SIE_CA_WIN_INFO sie_ca_win_info[ISP_BUILTIN_ID_MAX_NUM] = {{32, 32, 10, 8}, {32, 32, 10, 8}, {32, 32, 10, 8}, {32, 32, 10, 8}, {32, 32, 10, 8}};
static SIE_LA_WIN_INFO sie_la_win_info[ISP_BUILTIN_ID_MAX_NUM] = {{32, 32, 10, 8}, {32, 32, 10, 8}, {32, 32, 10, 8}, {32, 32, 10, 8}, {32, 32, 10, 8}};
static BOOL isp_builtin_task_open = FALSE;
static BOOL isp_builtin_direct_mode = TRUE;
static UINT32 isp_sensor_path_2 = 0xFFFFFFFF;
...
static ISP_BUILTIN_DTSI isp_builtin_dtsi[ISP_BUILTIN_DTSI_MAX] = {
	{ IQ_FRONT_NODE_PATH_1, IQ_NODE_PATH_1, IQ_DPC_NODE_PATH_1, IQ_SHADING_NODE_PATH_1, IQ_LDC_NODE_PATH_1, AE_NODE_PATH_1, AWB_NODE_PATH_1 },
	{ IQ_FRONT_NODE_PATH_2, IQ_NODE_PATH_2, IQ_DPC_NODE_PATH_2, IQ_SHADING_NODE_PATH_2, IQ_LDC_NODE_PATH_2, AE_NODE_PATH_2, AWB_NODE_PATH_2 }
};
...
static void isp_builtin_get_win_info(UINT32 id)
{
	...
	sie_getLaWin(id, &sie_la_win_info[id]);
	sie_getCaWin(id, &sie_ca_win_info[id]);

	if (shdr_enable[id]) {
		for (i = ISP_BUILTIN_ID_MAX_NUM - 1; i >= 0; i--) {
			if ((0x1 << i) & shdr_id_mask[id]) {
				break;
			}
		}

		sie_getLaWin(i, &sie_la_win_info[i]);
		sie_getCaWin(i, &sie_ca_win_info[i]);
	}
}
...
static void isp_builtin_cb_enc(UINT32 id, UINT32 status)
{
	...
	if (status == BUILTIN_ISP_EVENT_ENC_TNR) {
...
ISP_BUILTIN_LA_RSLT *isp_builtin_get_la(UINT32 id)
{
	...
	if (addr3 != 0x0) {
		...
		sie_get_histo(id, la_rslt->histogram);
	} else {
...
{
	//return ae_builtin_get_compensation_ratio(UINT32 id);
...
ER isp_builtin_init(void)
{
	...
	if (sensor_name_1 != 0xFFFFFFFF) {
		...
		kdrv_bridge_get_tag(AE_STITCH_MODE, &ae_stitch_mode[id]);
		kdrv_bridge_get_tag(AWB_STITCH_MODE, &awb_stitch_mode[id]);

		printk("isp bridge 1/3, id %d, name 0x%X, fps %d, expt %d, gain %d, expt max %d, i2c 0x%X 0x%X \r\n", id,
																											sensor_name[id],
																											...
																											sensor_i2c_id[id],
																											sensor_i2c_addr[id]);
		printk("isp bridge 2/3, dgain %d, cgain %d %d %d,  shdr 0x%X, 0x%X, 0x%X \r\n", dgain[id],
																						cgain_bridge.r,
																						cgain_bridge.g,
																						cgain_bridge.b,
																						shdr_enable[id],
		...
		printk("isp bridge 3/3, ui %d %d %d %d %d %d, night %d, stitch %d %d \r\n", nr_lv[id],
																					_3dnr_lv[id],
																					sharpness_lv[id],
																					saturation_lv[id],
																					contrast_lv[id] ,
	...
	if (sensor_name_2 != 0xFFFFFFFF) {
		...
		kdrv_bridge_get_tag(AE_STITCH_MODE_2, &ae_stitch_mode[id]);
		kdrv_bridge_get_tag(AWB_STITCH_MODE_2, &awb_stitch_mode[id]);
	...
	for (id = 0; id < ISP_BUILTIN_ID_MAX_NUM; id++) {
		if ((id == 0) || (id == isp_sensor_path_2)) {
			...
			if (shdr_enable[id]) {
				printk("id %d, alloc ca & la buffer \r\n", shdr_map_path[id]);
				isp_builtin_ca_rslt[shdr_map_path[id]] = kzalloc(sizeof(ISP_BUILTIN_CA_RSLT), GFP_KERNEL);
				if (isp_builtin_ca_rslt[shdr_map_path[id]] == NULL) {
					printk("allocate isp_builtin_ca_rslt fail!\n");
					return -E_SYS;
				}
				isp_builtin_la_rslt[shdr_map_path[id]] = kzalloc(sizeof(ISP_BUILTIN_LA_RSLT), GFP_KERNEL);
				if (isp_builtin_la_rslt[shdr_map_path[id]] == NULL) {
					printk("allocate isp_builtin_ca_rslt fail!\n");
					return -E_SYS;
				}
			}
			...
			isp_builtin_defog_stcs[id] = kzalloc(sizeof(ISP_BUILTIN_DEFOG_STCS), GFP_KERNEL);
			if (isp_builtin_defog_stcs[id] == NULL) {
				printk("allocate isp_builtin_defog_stcs fail!\n");
				return -E_SYS;
			}
			isp_builtin_ipe_subout[id] = kzalloc(sizeof(ISP_BUILTIN_IPE_SUBOUT_BUF), GFP_KERNEL);
			if (isp_builtin_ipe_subout[id] == NULL) {
				printk("allocate isp_builtin_ipe_subout fail!\n");
				return -E_SYS;
			}
		}
	}
...
ER isp_builtin_uninit(void)
{
	...
	for (id = 0; id < ISP_BUILTIN_ID_MAX_NUM; id++) {
		if (sensor_valid[id]) {
			...
			if (isp_builtin_defog_stcs[id]) {
				kfree(isp_builtin_defog_stcs[id]);
				isp_builtin_defog_stcs[id] = NULL;
			}
			if (isp_builtin_ipe_subout[id]) {
				kfree(isp_builtin_ipe_subout[id]);
				isp_builtin_ipe_subout[id] = NULL;
			}
		}
		#if (_2A_BUILTIN_NVT)
