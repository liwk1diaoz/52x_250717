...
irqreturn_t i2c4_isr(int irq, void *devid);

static ID SEMID_I2C4;
static ID FLG_ID_I2C4;



// Module related definition -begin
// This is I2C4 moudle
#define MODULE_REG_BASE         IOADDR_I2C4_REG_BASE
#define MODULE_FLAG_ID          FLG_ID_I2C4
#define MODULE_FLAG_PTN         FLGPTN_I2C4
#define MODULE_SEM_ID           SEMID_I2C4



#define MODULE_CLK_ID           I2C4_CLK
#if (I2C_HANDLE_PINMUX == ENABLE)
#define MODULE_PIN1ST_ID        PINMUX_FUNC_ID_I2C4_1ST
#define MODULE_PIN2ND_ID        PINMUX_FUNC_ID_I2C4_2ND
#endif

#define module_unlock           i2c4_unlock
#define module_getConfig        i2c4_getConfig
...
/*
    Interrupt Service Routine of I2C4
...
/**
    ...
    @return Open status
        ...
        - @b E_OK       : Open I2C4 driver OK, available session is returned in pSession.
*/
ER i2c4_open(PI2C_SESSION pSession)
{
	RCW_DEF(I2C_CTRL_REG);
	UINT32  i;
	unsigned long      flags;
	...
	// All sessions are available
		...
		vos_flag_create(&MODULE_FLAG_ID, NULL, "FLG_ID_I2C4");
		vos_sem_create(&MODULE_SEM_ID, 1, "SEMID_I2C4");

		// clear the interrupt flag
		clr_flg(MODULE_FLAG_ID, MODULE_FLAG_PTN);

		// Enable interrupt
		request_irq(INT_ID_I2C4, i2c4_isr ,IRQF_TRIGGER_HIGH, "i2c4", 0);
...
/**
    Close I2C4 driver for specific session

    Close I2C4 driver for specific session.

    @param[in] Session      I2C4 session

    @return Close status.
        - @b E_SYS      : The session is not opened yet
        - @b E_OK       : Session is closed
*/
ER i2c4_close(I2C_SESSION Session)
{
	unsigned long      flags;
...
/**
    Check if I2C4 driver is opened or not for specific session
    ...
    @return
        - @b TRUE   : I2C4 session is opened
        - @b FALSE  : I2C4 session is not opened
*/

BOOL i2c4_isOpened(I2C_SESSION Session)
{
...
/**
    ...
    Lock I2C4 session. You should lock I2C4 session before sending / receving any data,
    and unlock I2C4 session after data is transferred.
    ...
    @return Lock status.
        ...
        - @b E_SYS      : I2C4 session is not opened
        - @b E_OK       : I2C4 session is locked
*/
ER i2c4_lock(I2C_SESSION Session)
{
	ER  erReturn;
	RCW_DEF(I2C_CTRL_REG);
...
/**
    ...
    Unlock I2C4 session. You should unlock I2C4 session and let other task can use I2C4.
    ...
    @return Unlock status.
        ...
        - @b E_SYS      : I2C4 session is not opened or not locked
        - @b E_OK       : I2C4 session is unlocked
*/
ER i2c4_unlock(I2C_SESSION Session)
{
	RCW_DEF(I2C_CTRL_REG);
	RCW_DEF(I2C_STS_REG);
	FLGPTN  FlagPtn;
...
/**
    Check if I2C4 Session is locked or not.
...
*/
BOOL i2c4_islocked(I2C_SESSION Session)
{
...
/**
    ...
    Configuration for specific I2C4 session.

    @param[in] Session      I2C4 session that will be configured
    @param[in] ConfigID     Configuration ID
    @param[in] uiConfig     Configuration value

    @return void
*/
void i2c4_setConfig(I2C_SESSION Session, I2C_CONFIG_ID ConfigID, UINT32 uiConfig)
{
	// Check session
...
/**
    Get I2C4 configuration

    Get I2C4 configuration for specific session.

    @param[in] Session      I2C4 session that will be probed
    @param[in] ConfigID     Configuration ID

    @return Configuration value.
*/
UINT32 i2c4_getConfig(I2C_SESSION Session, I2C_CONFIG_ID ConfigID)
{
	// Check session
	if (Session >= I2C_TOTAL_SESSION) {
		DBG_ERR("Invalid session!\r\n");
...
/**
    Transmit I2C4 data in PIO mode

    Transmit I2C4 data in PIO mode for I2C_SESx that are locked.
    ...
    @return Transmit status, one of the following:
        - @b I2C_STS_INVALID_PARAM      : Byte count is not 1 ~ 8, no START or I2C4 is not locked
...
*/
I2C_STS i2c4_transmit(PI2C_DATA pData)
{
	RCW_DEF(I2C_STS_REG);
	RCW_DEF(I2C_CTRL_REG);
	UINT32      i;
...
/**
    Receive I2C4 data in PIO mode

    Receive I2C4 data in PIO mode for I2C_SESx that are locked.

    @param[in] pData    PIO data to received, including response NACK condition

    @return Transmit status, one of the following:
        - @b I2C_STS_INVALID_PARAM      : Byte count is not 1 ~ 8 or I2C4 is not locked
        - @b I2C_STS_BUS_NOT_AVAILABLE  : Bus is busy
        - @b I2C_STS_AL                 : Arbitration lost
        - @b I2C_STS_OK                 : Data is received, could be OR with I2C_STS_STOP.
        - @b I2C_STS_STOP               : Slave mode only, receive STOP condition from master.
*/
I2C_STS i2c4_receive(PI2C_DATA pData)
{
	RCW_DEF(I2C_STS_REG);
	RCW_DEF(I2C_CTRL_REG);
	UINT32      i;
...
#if (MODULE_SUPPORT_DMA == ENABLE)
/**
    Transmit I2C4 data in DMA mode

    Transmit I2C4 data in DMA mode for I2C_SESx that are locked. Please note that DMA mode
    ...
    @return Transmit status, one of the following:
        - @b I2C_STS_INVALID_PARAM      : Address or size is not word alignment, size over limitation or I2C4 is not locked
        - @b I2C_STS_BUS_NOT_AVAILABLE  : Bus is busy
        - @b I2C_STS_NACK               : Receive NACK
        - @b I2C_STS_AL                 : Arbitration lost
        - @b I2C_STS_OK                 : Data is transmitted
*/
I2C_STS i2c4_transmitDMA(UINT32 uiAddr, UINT32 uiSize)
{
	RCW_DEF(I2C_STS_REG);
	RCW_DEF(I2C_CONFIG_REG);
	RCW_DEF(I2C_CTRL_REG);
...
/**
    ...
    @return Transmit status, one of the following:
        ...
        - @b I2C_STS_OK                 : Data is transmitted
*/
I2C_STS i2c4_transmitDMA_En(UINT32 uiAddr, UINT32 uiSize)
{
	RCW_DEF(I2C_STS_REG);
	RCW_DEF(I2C_CONFIG_REG);
	RCW_DEF(I2C_CTRL_REG);
...
/**
    ...
    @return Transmit status, one of the following:
        ...
        - @b I2C_STS_OK                 : Data is transmitted
*/
I2C_STS i2c4_transmitDMA_Wait(UINT32 uiAddr, UINT32 uiSize)
{
	RCW_DEF(I2C_STS_REG);
//    RCW_DEF(I2C_CONFIG_REG);
//    RCW_DEF(I2C_CTRL_REG);
...
/**
    Receive I2C4 data in DMA mode

    Receive I2C4 data in DMA mode for I2C_SESx that are locked. Please note that DMA mode
...
*/
I2C_STS i2c4_receiveDMA(UINT32 uiAddr, UINT32 uiSize)
{
	RCW_DEF(I2C_STS_REG);
	RCW_DEF(I2C_CONFIG_REG);
	RCW_DEF(I2C_CTRL_REG);
...
*/
I2C_STS i2c4_waitSARMatch(I2C_SAR_LEN SARLen)
{
	UINT32      i;
	FLGPTN      FlagPtn;
	unsigned long      flags;
...
void i2c4_autoRst_En(I2C_SESSION *i2c_session)
{
	...
	pll_enable_system_reset(I2C4_RSTN);
	pll_disable_system_reset(I2C4_RSTN);

	i2c4_close(*i2c_session);
	i2c4_open(i2c_session);
...
BOOL    i2c4Test_compareRegDefaultValue(void);
void    i2c4Test_getClockPeriod(I2C_SESSION Session, double *dHigh, double *dLow);
...
/*
    (Verification code) Compare I2C4 register default value
...
/*
    (Verification code) Get I2C4 bus clock high / low period
...
/*
    (Verification code) Get I2C4 bus VD Delay
    ...
    @return void
*/
void i2c4Test_setVDDelay(I2C_SESSION Session, UINT32 uiDelay)
{
	// Configuration of this session is modified
	uiI2CSesModified |= (I2C_SES_MODIFIED << Session);
