...
irqreturn_t i2c_isr(int irq, void *devid);

static ID SEMID_I2C;
static ID FLG_ID_I2C;



// Module related definition -begin
// This is I2C moudle
#define MODULE_REG_BASE         IOADDR_I2C_REG_BASE
#define MODULE_FLAG_ID          FLG_ID_I2C
#define MODULE_FLAG_PTN         FLGPTN_I2C
#define MODULE_SEM_ID           SEMID_I2C



#define MODULE_CLK_ID           I2C_CLK
#if (I2C_HANDLE_PINMUX == ENABLE)
#define MODULE_PIN1ST_ID        PINMUX_FUNC_ID_I2C1_1ST
#define MODULE_PIN2ND_ID        PINMUX_FUNC_ID_I2C1_2ND
#endif

#define module_unlock           i2c_unlock
#define module_getConfig        i2c_getConfig
...
/**
    ...
    @return Open status
        ...
        - @b E_OK       : Open I2C driver OK, available session is returned in pSession.
*/
ER i2c_open(PI2C_SESSION pSession)
{
	...
	// All sessions are available
		...
		vos_flag_create(&MODULE_FLAG_ID, NULL, "FLG_ID_I2C");
		vos_sem_create(&MODULE_SEM_ID, 1, "SEMID_I2C");

		// clear the interrupt flag
		clr_flg(MODULE_FLAG_ID, MODULE_FLAG_PTN);

		// Enable interrupt
		request_irq(INT_ID_I2C, i2c_isr ,IRQF_TRIGGER_HIGH, "i2c", 0);
	...
	// PAD: disable pull up
	//pad_setPullUpDown(PAD_PIN_SGPIO7, PAD_NONE);
	//pad_setPullUpDown(PAD_PIN_SGPIO8, PAD_NONE);
...
/**
    Close I2C driver for specific session
...
/**
    Check if I2C driver is opened or not for specific session
    ...
    @return
        - @b TRUE   : I2C session is opened
        - @b FALSE  : I2C session is not opened
*/

BOOL i2c_isOpened(I2C_SESSION Session)
{
...
/**
    ...
    Lock I2C session. You should lock I2C session before sending / receving any data,
    and unlock I2C session after data is transferred.
    ...
    @return Lock status.
        ...
        - @b E_SYS      : I2C session is not opened
        - @b E_OK       : I2C session is locked
*/
...
/**
    ...
    Unlock I2C session. You should unlock I2C session and let other task can use I2C.
    ...
    @return Unlock status.
        ...
        - @b E_SYS      : I2C session is not opened or not locked
        - @b E_OK       : I2C session is unlocked
*/
...
/**
    Check if I2C Session is locked or not.
...
*/
BOOL i2c_islocked(I2C_SESSION Session)
{
...
/**
    ...
    Configuration for specific I2C session.

    @param[in] Session      I2C session that will be configured
    @param[in] ConfigID     Configuration ID
    @param[in] uiConfig     Configuration value

    @return void
*/
void i2c_setConfig(I2C_SESSION Session, I2C_CONFIG_ID ConfigID, UINT32 uiConfig)
{
	// Check session
...
/**
    ...
    Get I2C configuration for specific session.

    @param[in] Session      I2C session that will be probed
    @param[in] ConfigID     Configuration ID

    @return Configuration value.
*/
UINT32 i2c_getConfig(I2C_SESSION Session, I2C_CONFIG_ID ConfigID)
{
	// Check session
	if (Session >= I2C_TOTAL_SESSION) {
		DBG_ERR("Invalid session!\r\n");
...
/**
    Transmit I2C data in PIO mode

    Transmit I2C data in PIO mode for I2C_SESx that are locked.
    ...
    @return Transmit status, one of the following:
        - @b I2C_STS_INVALID_PARAM      : Byte count is not 1 ~ 8, no START or I2C is not locked
...
/**
    Receive I2C data in PIO mode

    Receive I2C data in PIO mode for I2C_SESx that are locked.

    @param[in] pData    PIO data to received, including response NACK condition

    @return Transmit status, one of the following:
        - @b I2C_STS_INVALID_PARAM      : Byte count is not 1 ~ 8 or I2C is not locked
        - @b I2C_STS_BUS_NOT_AVAILABLE  : Bus is busy
        - @b I2C_STS_AL                 : Arbitration lost
...
*/
I2C_STS i2c_transmitDMA(UINT32 uiAddr, UINT32 uiSize)
{
	RCW_DEF(I2C_STS_REG);
	RCW_DEF(I2C_CONFIG_REG);
	RCW_DEF(I2C_CTRL_REG);
...
/**
    ...
    @return Transmit status, one of the following:
        ...
        - @b I2C_STS_OK                 : Data is transmitted
*/
I2C_STS i2c_transmitDMA_En(UINT32 uiAddr, UINT32 uiSize)
{
	RCW_DEF(I2C_STS_REG);
	RCW_DEF(I2C_CONFIG_REG);
	RCW_DEF(I2C_CTRL_REG);
...
/**
    ...
    @return Transmit status, one of the following:
        ...
        - @b I2C_STS_OK                 : Data is transmitted
*/
I2C_STS i2c_transmitDMA_Wait(UINT32 uiAddr, UINT32 uiSize)
{
	RCW_DEF(I2C_STS_REG);
//    RCW_DEF(I2C_CONFIG_REG);
//    RCW_DEF(I2C_CTRL_REG);
...
/**
    Receive I2C data in DMA mode

    Receive I2C data in DMA mode for I2C_SESx that are locked. Please note that DMA mode
...
*/
I2C_STS i2c_receiveDMA(UINT32 uiAddr, UINT32 uiSize)
{
	RCW_DEF(I2C_STS_REG);
	RCW_DEF(I2C_CONFIG_REG);
	RCW_DEF(I2C_CTRL_REG);
...
*/
I2C_STS i2c_waitSARMatch(I2C_SAR_LEN SARLen)
{
	UINT32      i;
	FLGPTN      FlagPtn;
	unsigned long      flags;
...
void i2c_autoRst_En(I2C_SESSION *i2c_session)
{
	...
	pll_enable_system_reset(I2C_RSTN);
	pll_disable_system_reset(I2C_RSTN);

	i2c_close(*i2c_session);
	i2c_open(i2c_session);
...
BOOL    i2cTest_compareRegDefaultValue(void);
void    i2cTest_getClockPeriod(I2C_SESSION Session, double *dHigh, double *dLow);
...
/*
    (Verification code) Compare I2C register default value
...
/*
    (Verification code) Get I2C bus clock high / low period
...
/*
    (Verification code) Get I2C bus VD Delay
    ...
    @return void
*/
void i2cTest_setVDDelay(I2C_SESSION Session, UINT32 uiDelay)
{
	// Configuration of this session is modified
	uiI2CSesModified |= (I2C_SES_MODIFIED << Session);
