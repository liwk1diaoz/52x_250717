...
// Internal function declaration for StandardDeviceRequest528[]
static void ReqGetStatus528(void);
static void ReqClrFeature528(void);
static void ReqSetFeature528(void);
static void ReqGetDescriptor528(void);
static void ReqSetAddress528(void);
static void ReqGetConfig528(void);
static void ReqSetConfig528(void);
static void ReqGetInterface528(void);
static void ReqSetInterface528(void);
static void ReqSynchFrame528(void);

//
// Internal static functions
//
static void usb528_clrEPFIFOMap(void);
static void usb528_clrAllEP(void);
static BOOL usb528_isFIFOEnabled(UINT32 FIFOn);
static void usb528_clrEPStall(USB_EP EPn);
static void usb528_rstEPDataToggle(USB_EP EPn);


// processing of USB setup transaction TYPE = Standard
_ALIGNED(4) void (*StandardDeviceRequest528[])(void) = {
	ReqGetStatus528,           //0
	ReqClrFeature528,          //1
	ReqReserved528,            //2
	ReqSetFeature528,          //3
	ReqReserved528,            //4
	ReqSetAddress528,          //5
	ReqGetDescriptor528,       //6
	ReqReserved528,            //7
	ReqGetConfig528,           //8
	ReqSetConfig528,           //9
	ReqGetInterface528,        //10
	ReqSetInterface528,        //11
	ReqSynchFrame528,          //12
	ReqReserved528,            //13
	ReqReserved528,            //14
	ReqReserved528             //15
...
static void ReqGetStatus528(void)
{
	...
	if (bRecipient == USB_RECIPIENT_DEVICE) {
		...
		case USB_EP7:
		case USB_EP8: {
				FIFOn = usb528_getFifoFromEp(bIndex & USB_EP_ADDRESS_MASK);

				if ((FIFOn != USB_FIFO_NOT_USE) && usb528_isFIFOEnabled(FIFOn)) {

					if (((USB_EP_IN_ADDRESS & bIndex)  && (usb528_getEpDirection(bIndex & USB_EP_ADDRESS_MASK) == EP_DIR_IN)) ||
						((!(USB_EP_IN_ADDRESS & bIndex)) && (usb528_getEpDirection(bIndex & USB_EP_ADDRESS_MASK) == EP_DIR_OUT))) {
						if (usb528_EPStallStatus(bIndex & USB_EP_ADDRESS_MASK) == FALSE) {
	...
	DMALen = 2;
	usb2dev528_write_endpoint(USB_EP0, uRecipientStatus, &DMALen);
...
static void ReqClrFeature528(void)
{
	...
	if ((bRecipient == USB_RECIPIENT_DEVICE) && (FeatureSelect == USB_FEATURE_REMOTE_WAKEUP)) {
		...
		case USB_EP8: {
				...
				if ((FIFOn != USB_FIFO_NOT_USE) && usb528_isFIFOEnabled(FIFOn)) {
					if (((USB_EP_IN_ADDRESS & bIndex)  && (usb528_getEpDirection(bIndex & USB_EP_ADDRESS_MASK) == EP_DIR_IN)) ||
						...
						// clear EP Stall
						usb528_clrEPStall(USB_EP_ADDRESS_MASK & bIndex);

						// reset toggle sequence for IN EP. spec page 256.
						usb528_rstEPDataToggle(USB_EP_ADDRESS_MASK & bIndex);
		...
		usb2dev528_set_ep_stall(USB_EP0);
...
static void ReqSetFeature528(void)
{
	...
	if ((bRecipient == USB_RECIPIENT_DEVICE) && (FeatureSelect == USB_FEATURE_REMOTE_WAKEUP)) {
		...
		case USB_EP7:
		case USB_EP8: {
				FIFOn = usb528_getFifoFromEp(USB_EP_ADDRESS_MASK & bIndex);
				if ((FIFOn != USB_FIFO_NOT_USE) && usb528_isFIFOEnabled(FIFOn)) {
					if (((USB_EP_IN_ADDRESS & bIndex)  && (usb528_getEpDirection(bIndex & USB_EP_ADDRESS_MASK) == EP_DIR_IN)) ||
						((!(USB_EP_IN_ADDRESS & bIndex)) && (usb528_getEpDirection(bIndex & USB_EP_ADDRESS_MASK) == EP_DIR_OUT))) {
						usb2dev528_set_ep_stall(USB_EP_ADDRESS_MASK & bIndex);
		...
		case 0x4: {
				usb2dev528_set_ep0_done();     // special case: follow the test sequence
				USB_DELAY_MS(100);
				...
				TstPktLen = 53;
				usb2dev528_write_endpoint(USB_EP0, TstPkt, &TstPktLen);
				...
				usb528_writePhyReg(0x14, 0x2A);
				USB_DELAY_MS(500);
				usb528_writePhyReg(0x14, 0x2B);
				USB_DELAY_MS(500);
				#endif
				DBG_ERR("TEST_PKY\r\n");
			}
...
static void ReqGetDescriptor528(void)
{
	...
	if (DescType == USB_DEVICE_DESCRIPTOR_TYPE) {
		...
		pDataAddr = (unsigned long *)pUSBMng->p_dev_desc;
	}
...
static void ReqGetConfig528(void)
{
	...
	DMALen = 1;
	usb2dev528_write_endpoint(USB_EP0, pTmp, &DMALen);
...
static void ReqSynchFrame528(void)
{
	...
	case USB_EP6:
	case USB_EP7:	{
			FIFOn           = usb528_getFifoFromEp(USB_EP_ADDRESS_MASK & pDevReq->w_index);
...
*/
BOOL usb528_EPStallStatus(USB_EP EPn)
{
	T_USB_DEVMAXPS_INEP_REG   devEPMaxPS;
	T_USB_DEVMAXPS_OUTEP_REG   devEPMaxPSOut;
