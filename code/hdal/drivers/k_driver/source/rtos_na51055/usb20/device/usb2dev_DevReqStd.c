...
static void ReqSynchFrame(void)
{
	...
	case USB_EP15: {
			...
			FIFOCfgReg.reg  = USB_GETREG(USB_DEVBUFCFG0_REG_OFS + ((FIFOn >> 2) << 2));
...
static void usb_clrAllEP(void)
{
	...
	for (EPn = USB_EP1; EPn <= USB_EP15; EPn++) {
...
static BOOL usb_isFIFOEnabled(UINT32 FIFOn)
{
	T_USB_DEVBUFCFG0_REG FIFOCfg0Reg;
	T_USB_DEVBUFCFG1_REG FIFOCfg1Reg;

	FIFOCfg0Reg.reg = USB_GETREG(USB_DEVBUFCFG0_REG_OFS);
	FIFOCfg1Reg.reg = USB_GETREG(USB_DEVBUFCFG1_REG_OFS);

	switch (FIFOn) {
	case 0:
		return (BOOL)(FIFOCfg0Reg.bit.BUF0_EN);
	case 1:
		return (BOOL)(FIFOCfg0Reg.bit.BUF1_EN);
	case 2:
		return (BOOL)(FIFOCfg0Reg.bit.BUF2_EN);
	case 3:
		return (BOOL)(FIFOCfg0Reg.bit.BUF3_EN);
	case 4:
		return (BOOL)(FIFOCfg1Reg.bit.BUF4_EN);
	case 5:
		return (BOOL)(FIFOCfg1Reg.bit.BUF5_EN);
	case 6:
		return (BOOL)(FIFOCfg1Reg.bit.BUF6_EN);
	case 7:
		return (BOOL)(FIFOCfg1Reg.bit.BUF7_EN);
	default:
