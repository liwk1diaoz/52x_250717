...
static BOOL usb_chkDataContact(void)
{
	...
	PhyReg0B.reg = USB_GETREG(USB_PHYCTRL0B_REG_OFS);
	PhyReg0B.bit.DATARPDM = 1;
	PhyReg0B.bit.DATARPDP = 1;
	USB_SETREG(USB_PHYCTRL0B_REG_OFS, PhyReg0B.reg);

	// Enable DM Pull-Down resist
	PhyReg0C.reg = USB_GETREG(USB_PHYCTRL0C_REG_OFS);
	PhyReg0C.bit.DATARPDM = 1;
	USB_SETREG(USB_PHYCTRL0C_REG_OFS, PhyReg0C.reg);
#if 0
...
static ER usb_powerdown_callback(CLK_PDN_MODE Mode, BOOL bEnter)
{
	...
	if((Mode == CLK_PDN_MODE_SLEEP1) || (Mode <= CLK_PDN_MODE_SLEEP2) || (Mode <= CLK_PDN_MODE_SLEEP3)) {
		...
		if(bEnter) {
			...
			OUTW(0xc06010D0, 0xFF);
			OUTW(0xc06010D4, 0x34);
			OUTW(0xc0601008, 0x14);
			OUTW(0xc0601000, 0x88);
			OUTW(0xc0601030, 0x35);
			OUTW(0xc060102c, 0xFF);
		} else {
			pll_enableSystemReset(USB_RSTN);
			pll_disableSystemReset(USB_RSTN);
...
USB_CHARGER_STS usb2dev_check_charger(UINT32 uiDebounce)
{
	...
	if(nvt_get_chip_id() == CHIP_NA51084) {
		return usb2dev528_check_charger(uiDebounce);
	}
...
void usb2dev_power_on_init(BOOL bHighSpeed)
{
	...
	if(nvt_get_chip_id() == CHIP_NA51084) {
		usb2dev528_power_on_init(bHighSpeed);
		return;
	}
	...
	*/
	{
		UINT16 	SWCTRL=6,SQSEL=4,RESINT=0x8;
		UINT32  temp;
...
BOOL usb2dev_state_change(void)
{
	...
	if(nvt_get_chip_id() == CHIP_NA51084) {
		return usb2dev528_state_change();
	}
...
{
	if(nvt_get_chip_id() == CHIP_NA51084) {
		usb2dev528_set_standby_wakeup(fpIsr);
		return;
	}
...
{
	if(nvt_get_chip_id() == CHIP_NA51084) {
		return usb2dev528_get_controller_state();
	}
...
BOOL usb2dev_check_configured(void)
{
	...
	if(nvt_get_chip_id() == CHIP_NA51084) {
		return usb2dev528_check_configured();
	}
...
ER usb2dev_set_config(USB_CONFIG_ID configID, UINT32 configContext)
{
	...
	if(nvt_get_chip_id() == CHIP_NA51084) {
		return usb2dev528_set_config(configID, configContext);
	}
...
{
	if(nvt_get_chip_id() == CHIP_NA51084) {
		return usb2dev528_get_config(configID);
	}
	...
	case USB_CONFIG_ID_SQUELCH_LEVEL: {
			...
			phySquelch.reg = USB_GETREG(USB_PHYCTRL05_REG_OFS);
	...
	case USB_CONFIG_ID_HS_SWING: {
			...
			phySwingReg.reg = USB_GETREG(USB_PHYCTRL06_REG_OFS);
	...
	case USB_CONFIG_ID_REF_CURRENT: {
			...
			phyRefAdjReg.reg = USB_GETREG(USB_PHYCTRL3A_REG_OFS);
...
{
	if(nvt_get_chip_id() == CHIP_NA51084) {
		return usb2dev528_set_callback(callBackID, pCallBack);
	}
...
BOOL usb2dev_is_highspeed_enabled(void)
{
	...
	if(nvt_get_chip_id() == CHIP_NA51084) {
		return usb2dev528_is_highspeed_enabled();
	}
...
{
	if(nvt_get_chip_id() == CHIP_NA51084) {
		usb2dev528_init_management(pMng);
		return;
	}
...
{
	if(nvt_get_chip_id() == CHIP_NA51084) {
		return usb2dev528_set_management(pMng);
	}
...
{
	if(nvt_get_chip_id() == CHIP_NA51084) {
		return usb2dev528_get_management(pMng);
	}
...
ER usb_openDebounceTest(UINT32 uiDebounce)
{
	...
	devMainCtl.bit.RESET_DEBOUNCE_INTERVAL = uiDebounce;
	USB_SETREG(USB_DEVMAINCTRL_REG_OFS, devMainCtl.reg);
...
ER usb_writePhyReg(UINT32 uiOffset, UINT32 uiValue)
{
	...
	I2cData.VersionInfo     = DRV_VER_98520;
...
#else
	// Real board
	USB_SETREG(USB_PHY_BASE_OFS + uiOffset * 4, uiValue);
...
#else
	// Real board
	*puiValue = USB_GETREG(USB_PHY_BASE_OFS + uiOffset * 4);
#endif
#endif
