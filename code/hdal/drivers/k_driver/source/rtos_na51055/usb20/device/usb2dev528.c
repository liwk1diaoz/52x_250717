...
#include "../../../rtos_na51055/i2c/i2c.h"
// PinmuxCfg.c
...
//
static ER                   usb528_openInternal(BOOL bChkCharger, UINT32 uiDebounce);
static BOOL                 usb528_chkDataContact(void);
#if 1
static BOOL                 usb_chkPCbyLineSts(void);
#endif
#if _USB2DEV_TODO_
static ER 					usb528_powerdown_callback(CLK_PDN_MODE Mode, BOOL bEnter);
#endif
#if (_EMULATION_ == ENABLE)
extern ER                   usb528_openDebounceTest(UINT32 uiDebounce);
extern UINT32               usb528_readIntGrp2(void);
extern UINT32               usb528_getLineStatus(void);
extern void                 usb528_testDataContact(void);
extern void                 usb528_testChargingPort(void);
extern void                 usb528_testChargingPortSecondary(void);
#endif
...
/*
    ...
    @return void
*/
irqreturn_t usb528_isr(int irq, void *devid)
{
	REGVALUE                    devIntGrp, devIntGrpMsk;
	//T_USB_GLOBALINTMASK_REG     globalIntEn;
	#if _EMULATION_
	T_USB_GLOBALINTSTATUS_REG   globalIntSts;
	T_USB_HCMISC_REG            hcMiscReg, bakHcMiscReg;

	if (gUsbCtrlMode == USB_CTRL_MODE_HOST) {
		// In USB host mode, host phy suspend may be set after suspend device
		// Always enable it to ensure interrupt status can be correctly read
		hcMiscReg.reg                   = USB_GETREG(USB_HCMISC_REG_OFS);
		bakHcMiscReg.reg                = hcMiscReg.reg;
		hcMiscReg.bit.HostPhy_Suspend   = 0;
		USB_SETREG(USB_HCMISC_REG_OFS, hcMiscReg.reg);
	}
	#endif

	#if _EMULATION_
	globalIntSts.reg = USB_GETREG(USB_GLOBALINTSTATUS_REG_OFS);
	//globalIntEn.reg  = ~(USB_GETREG(USB_GLOBALINTMASK_REG_OFS));

	if (globalIntSts.reg == 0) {
		if (gUsbCtrlMode == USB_CTRL_MODE_HOST) {
			// Restore host phy suspend if nothing happened
			USB_SETREG(USB_HCMISC_REG_OFS, bakHcMiscReg.reg);
		}
		return IRQ_HANDLED;
	}
	#endif
	...
	gUSBCause = devIntGrp & devIntGrpMsk;
...
/*
    ...
    @return void
*/
static void usb528_attach(void)
{
//#if _USE_USB_CH2_
	//pll_enable_bus_clock(FOTG210_1_CLK);
	//pll_enable_apb_clock(FOTG210PHY_1_PCLK);
//#else
	//pll_enable_bus_clock(FOTG210_0_CLK);
	//pll_enable_apb_clock(FOTG210PHY_0_PCLK);
//#endif
}
...
/*
    ...
    @return void
*/
static void usb528_detach(void)
{
//#if _USE_USB_CH2_
	//pll_disable_bus_clock(FOTG210_1_CLK);
	//pll_disable_apb_clock(FOTG210PHY_1_PCLK);
//#else
	//pll_disable_bus_clock(FOTG210_0_CLK);
	//pll_disable_apb_clock(FOTG210PHY_0_PCLK);
//#endif
}
...
#else
	// 660 bug: Use half-speed to patch UVC header error due to sram rw at the same time.
...
static ER usb528_openInternal(BOOL bChkCharger, UINT32 uiDebounce)
{
	...
	//{
		...
		usb2dev528_power_on_init(bUsbHighSpeed);
		usb528_setControllerState(USB_CONTROLLER_STATE_NONE);
	}
	...
	usb2dev528_set_vbusi(TRUE);
	if (bChkCharger == FALSE) {
		// start running control & setup task for isr
		USBCtlStartTsk_ID = vos_task_create(USB528CtlStartTsk,  0, "USB528CtlStartTsk",   TSK_PRI_USBCTRL,	TSK_STKSIZE_USBCTRL);
		vos_task_resume(USBCtlStartTsk_ID);
		USBSetupHalTsk_ID = vos_task_create(USB528SetupHalTsk,  0, "USB528SetupHalTsk",   TSK_PRI_USBSETUP,	TSK_STKSIZE_USBSETUP);
	...
	request_irq(INT_ID_USB, usb528_isr, IRQF_TRIGGER_HIGH, 0, 0);
...
/*
    ...
    @return void
*/
void usb528_setControllerState(USB_CONTROLLER_STATE eUSBCtrlState)
{
...
static void usb528_enableHighSpeed(BOOL bHighSpeed)
{
	T_USB_DEVMAINCTRL_REG devMainCtl;
...
static void usb528_showDescriptor(UINT8 *pDesc, CHAR *pName)
{
	UINT32 i,len,mark=0;
...
BOOL usb528_debugDump(CHAR *strCmd);
BOOL usb528_debugDump(CHAR *strCmd)
{
	...
	if(RegTop.bit.ID) {
		DBG_DUMP("DEVICE.(%s) in %s\r\n",devPhyTest.bit.UNPLUG?"DISABLE":"ENABLE",usb2dev528_is_highspeed_enabled()?"HS":"FS");
		...
		usb528_showDescriptor((UINT8 *)gUSBManage.p_dev_desc, 		"DEVICE");
		usb528_showDescriptor((UINT8 *)gUSBManage.p_config_desc_hs, 	"HS_CONFIG");
		usb528_showDescriptor((UINT8 *)gUSBManage.p_config_desc_fs, 	"FS_CONFIG");
	} else {
...
static void usb528_installCmd(void)
{
#if _USB2DEV_TODO_
	static BOOL bInstall = FALSE;
	static SXCMD_BEGIN(usb, "usb debug message dump")
	SXCMD_ITEM("usb2",  usb528_debugDump,  "dump usb 2.0 info")
	SXCMD_END()
...
/**
    ...
    @return
        ...
        - @b E_SYS: USB has been unplug, so ignore this request...
*/
ER usb2dev528_open(void)
{
	usb528_installCmd();

	return usb528_openInternal(FALSE, 0);
}
...
*/
USB_CHARGER_STS usb2dev528_check_charger(UINT32 uiDebounce)
{
#if 1
	T_USB_DEVDMACTRL1_REG  devDMACtl1;
	T_USB_DEVDMACTRL1_REG  devDMACtl1_bak;
	...
	if ((!bStandardCharger) && (usb528_chkDataContact() == FALSE) && gpio_getIntStatus(GPIO_INT_USBPLUGIN)) {
...
/**
    ...
    @return void
*/
void usb2dev528_power_on_init(BOOL bHighSpeed)
{
	T_USB_DEVDMACTRL1_REG       devDMACtl1;
	...
	if (!rtos_init) {
		rtos_init = 1;
		cre_flg(&FLG_ID_USB,            NULL,   "FLG_ID_USB");
		cre_flg(&FLG_ID_USBTSK,         NULL,   "FLG_ID_USBTSK");
...
#if (_FPGA_EMULATION_ == 1)
	/* Enable i2c pinmux*/
	*(UINT32*) 0xF0010014 |= 0x1;
	*(UINT32*) 0xF00100B0 &= ~0x180;
	...
	// switch to page-0
	usb528_writePhyReg(0xFF, 0x00);

	// Switch TC gpio to usb mode.
	usb528_writePhyReg(0xBF, 0x0A);
	...
	usb528_enableHighSpeed(bHighSpeed);
	...
	#if !_FPGA_EMULATION_
	/*
		NT96510 bug patch for no TRIM default value
	*/
	usb528_writePhyReg(0x51, 0x20);
	usb528_writePhyReg(0x50, 0x30);
	...
	*/
	{
		...
		usb528_readPhyReg(0x06, &temp);
		temp &= ~(0x7<<1);
		temp |= (SWCTRL<<1);
		usb528_writePhyReg(0x06, temp);

		usb528_readPhyReg(0x05, &temp);
		temp &= ~(0x7<<2);
		temp |= (SQSEL<<2);
		usb528_writePhyReg(0x05, temp);

		usb528_writePhyReg(0x52, 0x60+RESINT);
	}
	usb528_writePhyReg(0x51, 0x00);
	#endif
	...
	#if _USB2DEV_TODO_
	clk_set_callback(CLK_CALLBACK_USB_PWRDN, usb528_powerdown_callback);
	#endif
...
/**
    ...
    @return
        ...
        - @b TRUE: Plug State.
*/
BOOL usb2dev528_state_change(void)
{
	if (!gUSBOpen) {
		if (gpio_getIntStatus(GPIO_INT_USBPLUGIN))          // VBUSI GPIO is HIGH
		{
			...
			if (bStandardCharger) {                         // If standard charger, also check DCD
				...
				bContatct = usb528_chkDataContact();
...
ER usb2dev528_close(void)
{
	...
	usb528_abortDMA(USB_EP_NOT_USE, USB_DMACH_ALL);

	wai_flg(&uiFlag, FLG_ID_USBTSK, FLGUSB_SETUPTSK_IDLE, TWF_ORW);
	//ter_tsk(USBSetupHalTsk_ID);
	vos_task_destroy(USBSetupHalTsk_ID);

	wai_flg(&uiFlag, FLG_ID_USBTSK, FLGUSB_CTLTSK_IDLE, TWF_ORW);
	//ter_tsk(USBCtlStartTsk_ID);
	vos_task_destroy(USBCtlStartTsk_ID);
	...
	if (gpio_getIntStatus(GPIO_INT_USBPLUGIN) == 0) {
		...
		devMainCtl.bit.SFRST = 0; // bit[4]
		USB_SETREG(USB_DEVMAINCTRL_REG_OFS, devMainCtl.reg);
	} else {                                    // USB cable connect with host
...
/**
    ...
    @return void
*/
void usb2dev528_set_vbusi(BOOL bPlug)
{
	T_USB_PHYTOP_REG   RegTop;
	unsigned long      flags;
...
/**
    ...
    @return void
*/
void usb2dev528_set_standby_wakeup(DRV_CB fpIsr)
{
...
/**
    ...
    @return    USB controller state
        ...
        - @b USB_CONTROLLER_STATE_SUSPEND: USB is in suspend state
*/
USB_CONTROLLER_STATE usb2dev528_get_controller_state(void)
{
...
*/
BOOL usb2dev528_check_configured(void)
{
	T_USB_DEVADDR_REG devAddr;
...
*/
ER usb2dev528_set_config(USB_CONFIG_ID configID, UINT32 configContext)
{
	ER erReturn = E_OK;
...
*/
UINT32 usb2dev528_get_config(USB_CONFIG_ID configID)
{
	switch (configID) {
	case USB_CONFIG_ID_STANDARD_CHARGER: {
...
*/
ER usb2dev528_set_callback(USB_CALLBACK callBackID, USB_GENERIC_CB pCallBack)
{
	switch (callBackID) {
...
*/
BOOL usb2dev528_is_highspeed_enabled(void)
{
	T_USB_DEVMAINCTRL_REG devMainCtl;
...
/**
    ...
    @return void
*/
void usb2dev528_init_management(USB_MNG *pMng)
{
	if (pMng) {
...
*/
ER usb2dev528_set_management(USB_MNG *pMng)
{
	if (!pMng) {
		DBG_ERR("NULL PTR!\r\n");
		return E_PAR;
...
/**
    ...
    @return
     ...
     - @b E_CTX: No Init. Please apply usb_initManagement() to pMng first.
*/
ER usb2dev528_get_management(USB_MNG *pMng)
{
	if (!pMng) {
		DBG_ERR("NULL PTR!\r\n");
		return E_PAR;
...
/*
    ...
    @return
        ...
        - @b Else: fail
*/
ER usb528_openDebounceTest(UINT32 uiDebounce)
{
	T_USB_DEVDMACTRL1_REG   devDMACtl1;
	T_USB_DEVDMACTRL1_REG   devDMACtl1_bak;
	T_USB_OTGCTRLSTATUS_REG otgCtrlSts;
...
*/
UINT32 usb528_readIntGrp2(void)
{
...
/*
    ...
    @return line status
*/
UINT32 usb528_getLineStatus(void)
{
	T_USB_HCPORTSTACTRL_REG portStatusReg;
...
/*
    ...
    @return void
*/
void usb528_testDataContact(void)
{
	T_USB_DEVDMACTRL1_REG  devDMACtl1;
	//T_USB_DEVDMACTRL1_REG  devDMACtl1_bak;
...
/*
    ...
    @return void
*/
void usb528_testChargingPort(void)
{
#if 0
	T_USB_CHARGER_CONTROL_REG chargerCtrlReg;
	T_USB_BUS_STATUS_REG busStsReg;
...
/*
    ...
    @return void
*/
void usb528_testChargingPortSecondary(void)
{
#if 0
	T_USB_CHARGER_CONTROL_REG chargerCtrlReg;
	T_USB_BUS_STATUS_REG busStsReg;
...
*/
void usb528_setID(BOOL bDevice)
{
	T_USB_PHYTOP_REG RegTop;
	unsigned long    flags;
...
/*
    ...
    @return
        - @b E_OK: success
*/
ER usb528_writePhyReg(UINT32 uiOffset, UINT32 uiValue)
{
#ifdef _FPGA_EMULATION_
#if (_FPGA_EMULATION_ == 1)
	I2C_DATA    I2cData;
...
/*
    ...
    @return
        - @b E_OK: success
*/
ER usb528_readPhyReg(UINT32 uiOffset, UINT32 *puiValue)
{
#ifdef _FPGA_EMULATION_
#if (_FPGA_EMULATION_ == 1)
	I2C_DATA    I2cData;
