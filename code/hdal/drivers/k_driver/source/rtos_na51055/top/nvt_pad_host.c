...
#define PAD_GETREG(ofs)         INW(TOP_PAD_REG_BASE + (ofs))
#define PAD_SETREG(ofs,value)   OUTW(TOP_PAD_REG_BASE + (ofs), value)

static VK_DEFINE_SPINLOCK(pad_spinlock);
#define loc_cpu(flags) vk_spin_lock_irqsave(&pad_spinlock, flags)
#define unl_cpu(flags) vk_spin_unlock_irqrestore(&pad_spinlock, flags)
