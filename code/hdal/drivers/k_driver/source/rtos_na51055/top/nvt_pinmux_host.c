...
#define DRV_VERSION "1.00.10"

static VK_DEFINE_SPINLOCK(top_spinlock);
#define loc_cpu(flags) vk_spin_lock_irqsave(&top_spinlock, flags)
#define unl_cpu(flags) vk_spin_unlock_irqrestore(&top_spinlock, flags)
...
void pinmux_parsing(struct nvt_pinctrl_info *info)
{
	...
	//printf("[pinmux_Parsing LCD ========]: 0x%lx\r\n", disp_pinmux_config[PINMUX_FUNC_ID_LCD]);
...
ER pinmux_init(struct nvt_pinctrl_info *info)
{
	...
	for (i = 0; i < PIN_FUNC_MAX; i++) {
		if (info->top_pinmux[i].pin_function != i) {
			printf("top_config[%ld].pinFunction context error\n", i);
			/*Leave critical section*/
			unl_cpu(flags);
			return E_CTX;
		}
		...
		if (err != E_OK) {
			printf("top_config[%ld].config config error\n", i);
			/*Leave critical section*/
			unl_cpu(flags);
			return err;
		}
...
static int pinmux_config_sdio(uint32_t config)
{
	if (config == PIN_SDIO_CFG_NONE) {
		...
		if ((top_reg7.bit.PWM0 == PWM_ENUM_PWM_2ND) ||
			...
			printf("SDIO conflict with PWM0_2 ~PWM5_2 \r\n");
			return E_PAR;
		}
...
static int pinmux_config_sdio2(uint32_t config)
{
	if (config == PIN_SDIO_CFG_NONE) {
		...
		if (top_reg5.bit.I2C2 == I2C_ENUM_I2C_2ND) {
			printf("SDIO2 conflict with I2C2_2\r\n");
			return E_PAR;
		}

		if (top_reg9.bit.UART3 == UART3_ENUM_2ND_PINMUX) {
			printf("SDIO2 conflict with UART3_2\r\n");
			return E_PAR;
		}

		if (top_reg5.bit.SPI == SPI_1ST_PINMUX) {
			printf("SDIO2 conflict with SPI_1 \r\n");
			return E_PAR;
		}
		...
		if ((top_reg7.bit.PWM6 == PWM_ENUM_PWM_2ND) ||
			...
			printf("SDIO2 conflict with PWM6_2 ~PWM11_2 \r\n");
			return E_PAR;
		}
	...
	if (config == PIN_SDIO_CFG_NONE) {
		...
		if (config & PIN_SDIO_CFG_2ND_PINMUX) {
			...
			if (top_reg9.bit.UART3 == UART3_ENUM_5TH_PINMUX) {
				printf("SDIO3 conflict with UART3_5\r\n");
				return E_PAR;
			}

			if(top_reg6.bit.AUDIO == AUDIO_ENUM_I2S_2ND_PINMUX) {
				printf("SDIO3 conflict with I2S_2\r\n");
				return E_PAR;
			}

			if(top_reg6.bit.SP2_CLK == SP2_CLK_SEL_ENUM_SP2_2_CLK) {
				printf("SDIO3 conflict with SP_CLK2_2\r\n");
				return E_PAR;
			}
			...
			if (config & PIN_SDIO_CFG_8BITS) {
				if (nvt_get_chip_id() == CHIP_NA51084) {
					if (top_reg9.bit.UART4 == UART4_ENUM_2ND_PINMUX) {
						printf("SDIO3 8-bit conflict with UART4_2\r\n");
						return E_PAR;
					}

					if(top_reg5.bit.I2C4 == I2C_ENUM_I2C_2ND) {
						printf("SDIO3 8-bit conflict with I2C4_2\r\n");
						return E_PAR;
					}

					if(top_reg4.bit.SPI4 == SPI4_2ND_PINMUX) {
						printf("SDIO3 8-bit conflict with SPI4_2\r\n");
						return E_PAR;
					}
				}
				...
				if ((top_reg7.bit.PWM0 == PWM_ENUM_PWM_4TH) ||
					...
					printf("SDIO3 8-bit conflict with PWM0_4 ~PWM3_4 \r\n");
					return E_PAR;
				}
...
static int pinmux_config_nand(uint32_t config)
{
	if (config == PIN_NAND_CFG_NONE) {
		...
		if (top_reg1.bit.SDIO3_EN == SDIO3_EN) {
			printf("NAND conflict with SDIO3\r\n");
			return E_OBJ;
		}

		if (config & PIN_NAND_CFG_2CS) {
			printf("Not support PIN_NAND_CFG_2CS\r\n");
			return E_OBJ;
		}
...
static int pinmux_config_sensor(uint32_t config)
{
	...
	if (config == PIN_SENSOR_CFG_NONE) {
		...
		tmp = config & (PIN_SENSOR_CFG_12BITS|PIN_SENSOR_CFG_MIPI|PIN_SENSOR_CFG_LVDS| PIN_SENSOR_CFG_CCIR8BITS | PIN_SENSOR_CFG_CCIR16BITS | PIN_SENSOR_CFG_12BITS_2ND);
		switch (tmp) {
		...
		default:
			printf("sensor no bus width assigned: 0x%lx\r\n", config);
			return E_PAR;
		}
		...
		if (config & PIN_SENSOR_CFG_MCLK_2ND) {
			...
			printf("Not support PIN_SENSOR_CFG_MCLK_2ND\r\n");
			return E_PAR;
		}
		...
		if (config & PIN_SENSOR_CFG_MCLK_3RD) {
			...
			//mclk_sensor[2] = 1;
			//52x compatible - donot support this function
			printf("Not support PIN_SENSOR_CFG_MCLK_3RD\r\n");
			return E_PAR;
		}
		...
		if (config & (PIN_SENSOR_CFG_SPCLK | PIN_SENSOR_CFG_SPCLK_2ND |PIN_SENSOR_CFG_SPCLK_3RD)) {
			if (config & PIN_SENSOR_CFG_SPCLK_2ND) {
				...
				if(top_reg7.bit.PWM4 == PWM_ENUM_PWM_4TH) {
					printf("SP_CLK_3 conflict with PWM4_4\r\n");
					return E_OBJ;
				}
				...
				//check PICNT3_1
				if (top_reg7.bit.PI_CNT3 == PICNT_ENUM_PICNT) {
					printf("SP_CLK_1 conflict with PICNT3_1\r\n");
					return E_OBJ;
				}

				if (top_reg5.bit.SPI3 == SPI3_1ST_PINMUX) {
					printf("SP_CLK_1 conflict with SPI3_1 \r\n");
					return E_OBJ;
				}

				if(top_reg9.bit.UART2 == UART2_ENUM_1ST_PINMUX) {
					printf("SP_CLK_1 conflict with UART2_1 \r\n");
					return E_OBJ;
				}
		...
		if (config & (PIN_SENSOR_CFG_SP2CLK | PIN_SENSOR_CFG_SP2CLK_2ND|PIN_SENSOR_CFG_SP2CLK_3RD)) {
			if (config & PIN_SENSOR_CFG_SP2CLK_2ND) {
				...
				if (top_reg1.bit.SDIO3_EN == SDIO3_EN) {
					printf("SP_CLK2 conflict with SDIO3\r\n");
					return E_OBJ;
				}
				...
				if(top_reg7.bit.PWM5 == PWM_ENUM_PWM_4TH) {
					printf("SP_CLK2_3 conflict with PWM5_4\r\n");
					return E_OBJ;
                }
				...
				if (top_reg7.bit.PI_CNT == PICNT_ENUM_PICNT2) {
					printf("SP_CLK2 conflict with PICNT1_2\r\n");
					return E_OBJ;
				}

				if (top_reg5.bit.SPI3 == SPI3_1ST_PINMUX) {
					printf("SP_CLK2 conflict with SPI3_1 \r\n");
					return E_OBJ;
				}

				if(top_reg9.bit.UART2 == UART2_ENUM_1ST_PINMUX) {
					printf("SP_CLK2 conflict with UART2_1 \r\n");
					return E_OBJ;
				}
...
static int pinmux_config_sensor2(uint32_t config)
{
	...
	if (config == PIN_SENSOR2_CFG_NONE) {
		...
		default:
			printf("sensor2 no bus width assigned: 0x%lx\r\n", config);
			break;
		}
...
static int pinmux_config_mipi_lvds(uint32_t config)
{
	if (config == PIN_MIPI_LVDS_CFG_NONE) {
		...
		if (config & PIN_MIPI_LVDS_CFG_DAT0) {
			if (top_reg3.bit.SENSOR == SENSOR_ENUM_12BITS_1ST) {
				printf("MIPI D0 conflict with SENSOR 12 or SENSOR2 10 bits\r\n");
				return E_OBJ;
			}
	...
	if (config == PIN_I2C_CFG_NONE) {
		...
		if (config & PIN_I2C_CFG_CH1) {
			if (top_reg5.bit.SIFCH0 == SIFCH_ENUM_SIF) {
				printf("I2C_1 conflict with SIF_CH0\r\n");
				return E_OBJ;
			}
			...
			if ((top_reg7.bit.PWM6 == PWM_ENUM_PWM_3RD) ||
				...
				printf("I2C_1 conflict with PWM6_3/PWM7_3 \r\n");
				return E_OBJ;
			}
		...
		if (config & PIN_I2C_CFG_CH1_2ND_PINMUX) {
			...
			//if (nvt_get_chip_id() == CHIP_NA51084) {
			//52x compatible - donot support this function
			printf("not support I2C_2\r\n");
			return E_OBJ;
			//}

			//if(top_reg3.bit.SEN_MCLK == SENMCLK_SEL_ENUM_MCLK_2ND) {
			//	printf("I2C_2 conflict with SN_MCLK_2 \r\n");
			//	return E_OBJ;
			//}
		...
		if (config & PIN_I2C_CFG_CH2) {
			if (top_reg5.bit.SPI == SPI_2ND_PINMUX) {
				printf("I2C2_1 conflict with SPI_2\r\n");
				return E_OBJ;
			}

			if (top_reg5.bit.SIFCH1 == SIFCH_ENUM_SIF) {
				printf("I2C2_1 conflict with SIF_CH1\r\n");
				return E_OBJ;
			}
		...
		if (config & PIN_I2C_CFG_CH2_2ND_PINMUX) {
			if (top_reg5.bit.SPI == SPI_1ST_PINMUX) {
				printf("I2C2_2 conflict with SPI_1\r\n");
				return E_OBJ;
			}

			if (top_reg1.bit.SDIO2_EXIST == SDIO2_EXIST_EN) {
				printf("I2C2_2 conflict with SDIO2\r\n");
				return E_OBJ;
			}
			...
			if ((top_reg7.bit.PWM6 == PWM_ENUM_PWM_2ND) ||
				...
				printf("I2C2_2 conflict with PWM6_2/PWM7_2 \r\n");
				return E_OBJ;
			}
		...
		if (config & PIN_I2C_CFG_CH3_2ND_PINMUX) {
			if (top_reg1.bit.SDIO_EXIST == SDIO_EXIST_EN) {
				printf("I2C3_2 conflict with to SDIO\r\n");
				return E_OBJ;
			}

			if (top_reg6.bit.DIGITAL_MIC == DMIC_ENUM_DMIC_3RD_PINMUX) {
				printf("I2C3_2 conflict with DMCLK_3 \r\n");
				return E_OBJ;
			}

			if (top_reg5.bit.SDP == SDP_1ST_PINMUX) {
				printf("I2C3_2 conflict with SDP_1 \r\n");
				return E_OBJ;
			}
			...
			if ((top_reg7.bit.PWM4 == PWM_ENUM_PWM_2ND) ||
				...
				printf("I2C3_2 conflict with PWM4_2/PWM5_2 \r\n");
				return E_OBJ;
			}
		...
		if (nvt_get_chip_id() == CHIP_NA51084) {
			...
			if (config & PIN_I2C_CFG_CH4) {
				...
				if(top_reg7.bit.PI_CNT == PICNT_ENUM_PICNT) {
					printf("I2C4_1 conflict with PICNT_1\r\n");
					return E_PAR;
				}
			...
			if (config & PIN_I2C_CFG_CH4_2ND_PINMUX) {
				...
				if (top_reg9.bit.UART4_CTSRTS == UART_CTSRTS_PINMUX) {
					printf("I2C4_2 conflict with UART4_2 CTSRTS\r\n");
					return E_PAR;
				}

				if(top_reg1.bit.SDIO3_BUS_WIDTH == SDIO3_BUS_WIDTH_8BITS) {
					printf("I2C4_2 conflict with SDIO3 8bit\r\n");
					return E_PAR;
				}

				if(top_reg4.bit.SPI4 == SPI4_2ND_PINMUX) {
					printf("I2C4_2 conflict with SPI4_2\r\n");
					return E_PAR;
				}
				...
				if (	(top_reg7.bit.PWM2 == PWM_ENUM_PWM_4TH) ||
					...
					printf("I2C4_2 conflict with PWM2_4 ~PWM3_4 \r\n");
					return E_PAR;
				}
			...
			if (config & PIN_I2C_CFG_CH5_2ND_PINMUX) {
				...
				if(top_reg9.bit.UART5_CTSRTS != UART_CTSRTS_GPIO) {
					printf("I2C5_2 conflict with UART5_2 CTSRTS \r\n");
					return E_PAR;
				}
	...
	if (config == PIN_SIF_CFG_NONE) {
		...
		if (config & PIN_SIF_CFG_CH0) {
			if (top_reg5.bit.I2C == I2C_ENUM_I2C) {
				printf("SIF_CH0 conflict with I2C_1\r\n");
				return E_OBJ;
			}

			if (top_reg1.bit.EXTROM_EXIST == EXTROM_EXIST_EN) {
				printf("SIF_CH0 conflict with BMC\r\n");
				return E_OBJ;
			}

			if (top_reg9.bit.UART3 == UART3_ENUM_4TH_PINMUX) {
				printf("SIF_CH0 conflict with UART3_4\r\n");
				return E_OBJ;
			}
			...
			if ((top_reg7.bit.PWM5 == PWM_ENUM_PWM_3RD) ||
				...
				printf("SIF_CH0 conflict with PWM5_3/PWM6_3/PWM7_3 \r\n");
				return E_OBJ;
			}
		...
		if (config & (PIN_SIF_CFG_CH1|PIN_SIF_CFG_CH1_2ND_PINMUX)) {
			if (top_reg5.bit.SPI == SPI_2ND_PINMUX) {
				printf("SIF_CH1 conflict with SPI_2\r\n");
				return E_OBJ;
			}

			if (top_reg5.bit.I2C2 == I2C_ENUM_I2C) {
				printf("SIF_CH1 conflict with I2C2_1\r\n");
				return E_OBJ;
			}
		...
		if (config & (PIN_SIF_CFG_CH2|PIN_SIF_CFG_CH2_2ND_PINMUX|PIN_SIF_CFG_CH2_3RD_PINMUX)) {
			if (config & PIN_SIF_CFG_CH2) {
				if (top_reg3.bit.SP_CLK == SP_CLK_SEL_ENUM_SP_2_CLK) {
					printf("SIF_CH2_1 conflict with SP_CLK_2\r\n");
					return E_OBJ;
				}
				...
				if (top_reg5.bit.SPI3 == SPI3_1ST_PINMUX) {
					printf("SIF_CH2_2 conflict with SPI3_1\r\n");
					return E_OBJ;
				}
				...
				if ((top_reg6.bit.PWM8 == PWM_ENUM_PWM_4TH) ||
					...
					printf("SIF_CH2_3 conflict with PWM8_4 ~PWM10_4 \r\n");
					return E_OBJ;
				}

				if (top_reg6.bit.DIGITAL_MIC == DMIC_ENUM_DMIC_2ND_PINMUX) {
					printf("SIF_CH2_3 conflict with DMCLK_2 \r\n");
					return E_OBJ;
				}
		...
		if (config & PIN_SIF_CFG_CH3) {
			if (top_reg5.bit.SPI2 == SPI2_1ST_PINMUX) {
				printf("SIF_CH3 conflict with SPI2_1\r\n");
				return E_OBJ;
			}
			...
			if(top_reg7.bit.PI_CNT2 == PICNT_ENUM_PICNT) {
				printf("SIF_CH3 conflict with PICNT2_1\r\n");
				return E_OBJ;
			}
	...
	if (config == PIN_UART_CFG_NONE) {
		...
		if (config & PIN_UART_CFG_CH2) {
			if (config & PIN_UART_CFG_CH2_2ND) {
				if (top_reg1.bit.SDIO_EXIST == SDIO_EXIST_EN) {
					printf("UART2 conflict with to SDIO: 0x%lx\r\n", config);
					return E_OBJ;
				}

				if (top_reg5.bit.SPI3 == SPI3_2ND_PINMUX) {
					printf("UART2 conflict with to SPI3_2: 0x%lx\r\n", config);
					return E_OBJ;
				}
				...
				if (nvt_get_chip_id() == CHIP_NA51084) {
					printf("CHIP_NA51084 not support UART2_3\r\n");
					return E_OBJ;
				}

				if(top_reg5.bit.SPI2 == SPI2_2ND_PINMUX) {
					printf("UART2_3 conflict with SPI2_2\r\n");
					return E_OBJ;
				}
				...
				if (top_reg6.bit.AUDIO == AUDIO_ENUM_I2S) {
					printf("UART2 conflict with to AUDIO: 0x%lx\r\n", config);
					return E_OBJ;
				}

				if (top_reg5.bit.SPI3 == SPI3_1ST_PINMUX) {
					printf("UART2 conflict with SPI3_1\r\n");
					return E_OBJ;
				}

				if (top_reg5.bit.SIFCH2 == SIFCH_ENUM_SIF_2ND) {
					printf("UART2 conflict with SIF_CH2_2\r\n");
					return E_OBJ;
				}

				if (config & PIN_UART_CFG_CH2_CTSRTS) {
					if (top_reg7.bit.PI_CNT2 == PICNT_ENUM_PICNT) {
						printf("UART2 conflict with PICNT2_1: 0x%lx\r\n", config);
						return E_OBJ;
					}
		...
		if (config & PIN_UART_CFG_CH3) {
			if (config & PIN_UART_CFG_CH3_2ND) {
				if (top_reg6.bit.PWM8 == PWM_ENUM_PWM) {
					printf("UART3_2 conflict with to PWM8: 0x%lx\r\n", config);
					return E_OBJ;
				}

				if (top_reg6.bit.PWM9 == PWM_ENUM_PWM) {
					printf("UART3_2 conflict with to PWM9: 0x%lx\r\n", config);
					return E_OBJ;
				}

				if (top_reg5.bit.SPI2 == SPI2_2ND_PINMUX) {
					printf("UART3_2 conflict with to SPI2_2: 0x%lx\r\n", config);
					return E_OBJ;
				}
				...
				if (config & PIN_UART_CFG_CH3_CTSRTS) {
					if (top_reg6.bit.PWM10 == PWM_ENUM_PWM) {
						printf("UART3_2 conflict with to PWM10: 0x%lx\r\n", config);
						return E_OBJ;
					}
					if (top_reg6.bit.PWM11 == PWM_ENUM_PWM) {
						printf("UART3_2 conflict with to PWM11: 0x%lx\r\n", config);
						return E_OBJ;
					}
				...
				if (nvt_get_chip_id() == CHIP_NA51084) {
					printf("CHIP_NA51084 not support UART3_3\r\n");
					return E_OBJ;
				}
				...
				if (top_reg5.bit.SPI == SPI_2ND_PINMUX) {
					printf("UART3 conflict with to SPI_2: 0x%lx\r\n", config);
					return E_OBJ;
				}

				if (top_reg5.bit.SIFCH3 == SIFCH_ENUM_SIF) {
					printf("UART3 conflict with to SIF3: 0x%lx\r\n", config);
					return E_OBJ;
				}

				if (top_reg6.bit.SP2_CLK == SP2_CLK_SEL_ENUM_SP2_2_CLK) {
					printf("UART3 conflict with to SP_CLK2_2: 0x%lx\r\n", config);
					return E_OBJ;
				}
				if (config & PIN_UART_CFG_CH3_CTSRTS) {
					top_reg_pgpio0.bit.PGPIO_15 = GPIO_ID_EMUM_FUNC;
		...
		if (config & PIN_UART_CFG_CH4) {
			if (config & PIN_UART_CFG_CH4_2ND) {
                if (config & PIN_UART_CFG_CH4_CTSRTS) {
					if(top_reg5.bit.I2C4 == I2C_ENUM_I2C_2ND) {
						printf("UART4_2 RTSCTS conflict with I2C4_2\r\n");
						return E_PAR;
					}
					...
					if (	(top_reg7.bit.PWM2 == PWM_ENUM_PWM_4TH) ||
						...
						printf("UART4_2 RTSCTS conflict with PWM2_4 ~PWM3_4 \r\n");
						return E_PAR;
					}
				...
				if (config & PIN_UART_CFG_CH4_DIROE) {
					...
					if (top_reg7.bit.PWM2 == PWM_ENUM_PWM_4TH) {
						...
						printf("UART4_2 DIROE conflict with PWM2_4 \r\n");
						return E_PAR;
					}

					if(top_reg5.bit.I2C4 == I2C_ENUM_I2C_2ND) {
						printf("UART4_2 DIROE conflict with I2C4_2\r\n");
						return E_PAR;
					}
				...
				if(top_reg1.bit.SDIO3_BUS_WIDTH == SDIO3_BUS_WIDTH_8BITS) {
					printf("UART4_2 conflict with SDIO3 8bit\r\n");
					return E_PAR;
				}

				if(top_reg4.bit.SPI4 == SPI4_2ND_PINMUX) {
					printf("UART4_2 conflict with SPI4_2\r\n");
					return E_PAR;
				}
				...
				if (	(top_reg7.bit.PWM0 == PWM_ENUM_PWM_4TH) ||
					...
					printf("UART4_2 conflict with PWM0_4 ~PWM1_4 \r\n");
					return E_PAR;
				}
			...
			if (config & PIN_UART_CFG_CH5_2ND) {
                if (config & PIN_UART_CFG_CH5_CTSRTS) {
					if(top_reg4.bit.I2C5 == I2C_ENUM_I2C_2ND) {
						printf("UART5_2 CTSRTS conflict with I2C5_2\r\n");
						return E_PAR;
					}
					top_reg_dgpio0.bit.DGPIO_17 = GPIO_ID_EMUM_FUNC;
					top_reg_dgpio0.bit.DGPIO_18 = GPIO_ID_EMUM_FUNC;
				...
				if (config & PIN_UART_CFG_CH5_DIROE) {
					if(top_reg4.bit.I2C5 == I2C_ENUM_I2C_2ND) {
						printf("UART5_2 DIROE conflict with I2C5_2\r\n");
						return E_PAR;
					}
					top_reg_dgpio0.bit.DGPIO_17 = GPIO_ID_EMUM_FUNC;
        ...
        if (config & PIN_UART_CFG_CH6) {
			if (config & PIN_UART_CFG_CH6_2ND) {
                if (config & PIN_UART_CFG_CH6_CTSRTS) {
					printf("CHIP_NA51084 not support UART6_2 flow control\r\n");
				}
	...
	if (config == PIN_SPI_CFG_NONE) {
		...
		if (config & (PIN_SPI_CFG_CH1 | PIN_SPI_CFG_CH1_2ND_PINMUX | PIN_SPI_CFG_CH1_3RD_PINMUX|PIN_SPI_CFG_CH1_2BITS | PIN_SPI_CFG_CH1_4BITS | PIN_SPI_CFG_CH1_DI_ONLY)) {
			if (config & PIN_SPI_CFG_CH1_2ND_PINMUX) {
				...
				if (top_reg5.bit.I2C2 == I2C_ENUM_I2C) {
					printf("SPI_2 conflict with I2C2_1\r\n");
					return E_OBJ;
				}

				if (top_reg5.bit.SIFCH1 == SIFCH_ENUM_SIF) {
					printf("SPI_2 conflict with SIF_CH1\r\n");
					return E_OBJ;
				}
				...
				if (top_reg5.bit.I2C2 == I2C_ENUM_I2C_2ND) {
					printf("SPI_1 conflict with I2C2_2\r\n");
					return E_OBJ;
				}

				if ((top_reg9.bit.UART3 == UART3_ENUM_2ND_PINMUX) &&
					(top_reg9.bit.UART3_CTSRTS == UART_CTSRTS_PINMUX)){
					printf("SPI_1 conflict with UART3_2 flow control\r\n");

					return E_OBJ;
				}

				if (top_reg1.bit.SDIO2_EXIST == SDIO2_EXIST_EN) {
					printf("SPI_1 conflict with SDIO2\r\n");
					return E_OBJ;
				}
				...
				if ((top_reg7.bit.PWM6 == PWM_ENUM_PWM_2ND) ||
					...
					printf("SPI_1 conflict with PWM6_2/PWM7_2/PWM8_2/PWM9_2 \r\n");
					return E_OBJ;
				}
		...
		if (config & (PIN_SPI_CFG_CH2 | PIN_SPI_CFG_CH2_2ND_PINMUX | PIN_SPI_CFG_CH2_2BITS)) {
			if (config & PIN_SPI_CFG_CH2_2ND_PINMUX) {
				...
				if (nvt_get_chip_id() == CHIP_NA51084) {
					printf("CHIP_NA51084 not support SPI2_2\r\n");
					return E_OBJ;
				}
				...
				if ((top_reg7.bit.PWM0 == PWM_ENUM_PWM) ||
					...
					printf("SPI2_2 conflict with PWM0_1 ~PWM2_1 \r\n");
					return E_OBJ;
				}

				if (top_reg9.bit.UART2 == UART2_ENUM_3RD_PINMUX) {
					printf("SPI2_2 conflict with UART2_3\r\n");
					return E_OBJ;
				}

				if (config & PIN_SPI_CFG_CH2_2BITS) {
					if (top_reg7.bit.PWM3 == PWM_ENUM_PWM){
						printf("SPI2_2 conflict with PWM3_1 \r\n");
						return E_OBJ;
					}
					top_reg_pgpio0.bit.PGPIO_3 = GPIO_ID_EMUM_FUNC;
				...
				if (top_reg7.bit.PI_CNT2 == PICNT_ENUM_PICNT) {
					printf("SPI2_1 conflict with PI_CNT2_1\r\n");
					return E_OBJ;
				}

				if (top_reg5.bit.SIFCH3 == SIFCH_ENUM_SIF) {
					printf("SPI2_1 conflict with SIF_CH3\r\n");
					return E_OBJ;
				}

				if (top_reg9.bit.UART3 == UART3_ENUM_1ST_PINMUX) {
					printf("SPI2_1 conflict with UART3_1\r\n");
					return E_OBJ;
				}
		...
		if (config & (PIN_SPI_CFG_CH3 | PIN_SPI_CFG_CH3_2ND_PINMUX | PIN_SPI_CFG_CH3_3RD_PINMUX |PIN_SPI_CFG_CH3_2BITS | PIN_SPI_CFG_CH3_RDY)) {
			if (config & PIN_SPI_CFG_CH3_2ND_PINMUX) {
				if (top_reg1.bit.SDIO_EXIST == SDIO_EXIST_EN) {
					printf("SPI3_2 conflict with to SDIO\r\n");
					return E_OBJ;
				}

				if (top_reg5.bit.I2C3 == I2C_ENUM_I2C_2ND) {
					printf("SPI3_2 conflict with I2C3_2\r\n");
					return E_OBJ;
				}

				if (top_reg5.bit.SDP == SDP_1ST_PINMUX) {
					printf("SPI3_2 conflict with SDP_1 \r\n");
					return E_OBJ;
				}

				if (top_reg6.bit.DIGITAL_MIC == DMIC_ENUM_DMIC_3RD_PINMUX) {
					printf("SPI3_2 conflict with DMCLK_3 \r\n");
					return E_OBJ;
				}
				...
				if (config & PIN_SPI_CFG_CH3_2BITS) {
					if (top_reg6.bit.REMOTE == REMOTE_ENUM_REMOTE_2ND) {
						printf("SPI3_2 conflict with remote_in_2 \r\n");
						return E_OBJ;
					}
					top_reg_cgpio0.bit.CGPIO_14 = GPIO_ID_EMUM_FUNC;
				...
				if (nvt_get_chip_id() == CHIP_NA51084) {
					printf("CHIP_NA51084 not support SPI3_3\r\n");
					return E_OBJ;
				}

				if (top_reg9.bit.UART3 == UART3_ENUM_3RD_PINMUX) {
					printf("SPI3_3 conflict with UART3_3\r\n");
					return E_OBJ;
				}

				if(top_reg5.bit.SDP == SDP_2ND_PINMUX){
					printf("SPI3_3 conflict with SDP_2 \r\n");
					return E_OBJ;
				}

				if (config & PIN_SPI_CFG_CH3_RDY) {
					//52x compatible - donot support this function
					printf("SPI3_3 not support RDY\r\n");
					return E_OBJ;
					//top_reg_pgpio0.bit.PGPIO_8 = GPIO_ID_EMUM_FUNC;
				...
				if (top_reg9.bit.UART2 == UART2_ENUM_1ST_PINMUX) {
					printf("SPI3_1 conflict with UART2 1st\r\n");
					return E_OBJ;
				} else if (top_reg6.bit.AUDIO == AUDIO_ENUM_I2S) {
					printf("SPI3_1 conflict with I2S_1\r\n");
					return E_OBJ;
				}
        ...
        if (nvt_get_chip_id() == CHIP_NA51084) {
			...
			if (config & (PIN_SPI_CFG_CH4 | PIN_SPI_CFG_CH4_2ND_PINMUX | PIN_SPI_CFG_CH4_2BITS | PIN_SPI_CFG_CH4_RDY)) {
				printf("Not support SPI4\r\n");
			}

			if (config & (PIN_SPI_CFG_CH5 | PIN_SPI_CFG_CH5_2ND_PINMUX | PIN_SPI_CFG_CH5_2BITS)) {
				printf("Not support SPI5\r\n");
			}
       	}
	}
...
static int pinmux_config_sdp(uint32_t config)
{
	if (config == PIN_SDP_CFG_NONE) {
 		...
 		if (top_reg1.bit.SDIO_EXIST == SDIO_EXIST_EN) {
			printf("SDP_1 conflict with to SDIO\r\n");
			return E_OBJ;
		}

		if (top_reg9.bit.UART2 == UART2_ENUM_2ND_PINMUX) {
			printf("SDP_1 conflict with UART2_2\r\n");
			return E_OBJ;
		}

		if (top_reg5.bit.I2C3 == I2C_ENUM_I2C_2ND) {
			printf("SDP_1 conflict with I2C3_2\r\n");
			return E_OBJ;
		}

		if (top_reg5.bit.SPI3 == SPI3_2ND_PINMUX) {
			printf("SDP_1 conflict with SPI3_2 \r\n");
			return E_OBJ;
		}

		if (top_reg6.bit.REMOTE == REMOTE_ENUM_REMOTE_2ND) {
			printf("SDP_1 conflict with remote_in_2 \r\n");
			return E_OBJ;
		}

		if (top_reg6.bit.DIGITAL_MIC == DMIC_ENUM_DMIC_3RD_PINMUX) {
			printf("SDP_1 conflict with DMCLK_3 \r\n");
			return E_OBJ;
		}
		...
		if ((top_reg7.bit.PWM0 == PWM_ENUM_PWM_2ND) ||
			...
			printf("SDP_1 conflict with PWM0_2/PWM1_2/PWM2_2/PWM4_2/PWM5_2 \r\n");
			return E_OBJ;
		}
		...
		if (top_reg5.bit.SPI3 == SPI3_3RD_PINMUX) {
			printf("SDP_2 conflict with SPI3_3 \r\n");
			return E_OBJ;
		}
		...
		if ((top_reg7.bit.PWM4 == PWM_ENUM_PWM) ||
			...
			printf("SDP_2 conflict with PWM4_1 ~PWM8_1 \r\n");
			return E_OBJ;
		}

		if (top_reg9.bit.UART3 == UART3_ENUM_3RD_PINMUX) {
			printf("SDP_2 conflict with UART3_3\r\n");
			return E_OBJ;
		}

		if (top_reg6.bit.DIGITAL_MIC == DMIC_ENUM_DMIC) {
			printf("SDP_2 conflict with DMCLK_1 \r\n");
			return E_OBJ;
		}
...
static int pinmux_config_remote(uint32_t config)
{
	if (config == PIN_REMOTE_CFG_NONE) {
		...
		if (top_reg5.bit.SPI3 == SPI3_1ST_PINMUX) {
			printf("Remote_1 conflict with SPI3_1 \r\n");
			return E_OBJ;
		}

		if(top_reg9.bit.UART2 == UART2_ENUM_1ST_PINMUX) {
			printf("Remote_1 conflict with UART2_1 \r\n");
			return E_OBJ;
		}

		if (top_reg7.bit.PI_CNT3 == PICNT_ENUM_PICNT) {
			printf("Remote_1 conflict with PICNT3_1\r\n");
			return E_OBJ;
		}

		if (top_reg3.bit.SP_CLK ==SP_CLK_SEL_ENUM_SPCLK) {
			printf("Remote_1 conflict with SP_CLK_1\n");
			return E_OBJ;
		}

		if (top_reg6.bit.AUDIO ==AUDIO_ENUM_I2S) {
			printf("Remote_1 conflict with I2S_1\n");
			return E_OBJ;
		}
		top_reg_pgpio0.bit.PGPIO_17 = GPIO_ID_EMUM_FUNC;
		top_reg6.bit.REMOTE = REMOTE_ENUM_REMOTE_1ST;
 		...
 		if (top_reg1.bit.SDIO_EXIST == SDIO_EXIST_EN) {
			printf("Remote_2 conflict with to SDIO\r\n");
			return E_OBJ;
		}

		if (top_reg9.bit.UART2 == UART2_ENUM_2ND_PINMUX) {
			printf("Remote_2 conflict with UART2_2\r\n");
			return E_OBJ;
		}

		if (top_reg5.bit.SPI3 == SPI3_2ND_PINMUX) {
			printf("Remote_2 conflict with SPI3_2 \r\n");
			return E_OBJ;
		}

		if (top_reg7.bit.PWM1 == PWM_ENUM_PWM_2ND){
			printf("Remote_2 conflict with PWM1_2 \r\n");
			return E_OBJ;
		}

		if (top_reg5.bit.SDP == SDP_1ST_PINMUX) {
			printf("Remote_2 conflict with SDP_1 \r\n");
			return E_OBJ;
		}
...
static int pinmux_config_ccnt(uint32_t config)
{
	...
	if (config & (PIN_PWM_CFG_CCNT | PIN_PWM_CFG_CCNT_2ND)) {
		if (config & PIN_PWM_CFG_CCNT_2ND) {
			if (top_reg6.bit.SP2_CLK == SP2_CLK_SEL_ENUM_SP2CLK) {
				printf("PICNT conflict with to SP_CLK2_1: 0x%lx\r\n", config);
				return E_OBJ;
			}
	...
	if (config & (PIN_PWM_CFG_CCNT2 | PIN_PWM_CFG_CCNT2_2ND)) {
		if (config & PIN_PWM_CFG_CCNT2_2ND) {
			if (top_reg6.bit.ETH != ETH_ID_ENUM_GPIO) {
				printf("PICNT2_2 conflict with ETH: 0x%x\r\n",top_reg6.bit.ETH);
				return E_OBJ;
			}
	...
	if (config & (PIN_PWM_CFG_CCNT3 | PIN_PWM_CFG_CCNT3_2ND)) {
		if (config & PIN_PWM_CFG_CCNT3_2ND) {
			if (top_reg6.bit.ETH != ETH_ID_ENUM_GPIO) {
				printf("PICNT3_2 conflict with ETH: 0x%x \r\n",top_reg6.bit.ETH);
				return E_OBJ;
			}
			...
			if (top_reg6.bit.AUDIO_MCLK == AUDIO_MCLK_MCLK) {
				printf("PICNT3 conflict with I2S MCLK\r\n");
				return E_OBJ;
			}
			if (top_reg3.bit.SP_CLK == SP_CLK_SEL_ENUM_SP_2_CLK) {
				printf("PICNT3 conflict with SP_CLK_2\r\n");
				return E_OBJ;
			}
...
static int pinmux_config_audio(uint32_t config)
{
	if (config & (PIN_AUDIO_CFG_I2S)) {
		if (top_reg9.bit.UART2 == UART2_ENUM_1ST_PINMUX) {
			printf("AUDIO I2S_1 conflict with UART2_1\r\n");
			return E_OBJ;
		}
		if (top_reg7.bit.PI_CNT2 == PICNT_ENUM_PICNT) {
			printf("AUDIO I2S_1 conflict with PICNT2_1\r\n");
			return E_OBJ;
		}
		if (top_reg5.bit.SPI3 == SPI3_1ST_PINMUX) {
			printf("AUDIO I2S_1 conflict with SPI3_1\r\n");
			return E_OBJ;
		}
		...
		if(top_reg1.bit.SDIO3_BUS_WIDTH == SDIO3_BUS_WIDTH_8BITS) {
			printf("AUDIO I2S_2 conflict with SDIO3 8bits\r\n");
			return E_OBJ;
		}
		...
		if ((top_reg7.bit.PWM0 == PWM_ENUM_PWM_4TH) ||
			...
			(top_reg7.bit.PWM3 == PWM_ENUM_PWM_4TH)){
				...
				printf("AUDIO I2S_2 conflict with PWM0_4 ~PWM3_4 \r\n");
				return E_OBJ;
		}
	...
	if (config & (PIN_AUDIO_CFG_MCLK)) {
		if(top_reg5.bit.SIFCH2 == SIFCH_ENUM_SIF_2ND){
			printf("I2S_MCLK_1 conflict with SB3_2\r\n");
			return E_OBJ;
		}
		...
		if (top_reg6.bit.PWM11 == PWM_ENUM_PWM_4TH){
			printf("AUDIO I2S_2 conflict with PWM11_4\r\n");
			return E_OBJ;
		}
		top_reg_dgpio0.bit.DGPIO_7 = GPIO_ID_EMUM_FUNC;
...
static int pinmux_config_eth(uint32_t config)
{
	if (config == PIN_ETH_CFG_NONE) {
		...
		if (nvt_get_chip_id() == CHIP_NA51055) {
			printf("CHIP_NA51055 not support RMII_2\r\n");
			return E_OBJ;
		}

		if(top_reg5.bit.SIFCH2 == SIFCH_ENUM_SIF){
			printf("RMII_2 conflict with SB3_1\r\n");
			return E_OBJ;
		}
		...
		if (nvt_get_chip_id() == CHIP_NA51055) {
			printf("CHIP_NA51055 not support RGMII\r\n");
			return E_OBJ;
		}

		if(top_reg5.bit.SIFCH2 == SIFCH_ENUM_SIF){
			printf("RGMII conflict with SB3_1\r\n");
			return E_OBJ;
		}
...
static int pinmux_config_misc(uint32_t config)
{
	...
	if (config & PIN_MISC_CFG_RTCLK) {
		if (top_reg7.bit.PWM1 == PWM_ENUM_PWM) {
			printf("RTCLK conflict with PWM1_1 \r\n");
			return E_OBJ;
		}

		if (top_reg9.bit.UART2 == UART2_ENUM_3RD_PINMUX) {
			printf("RTCLK conflict with UART2_3\r\n");
			return E_OBJ;
		}

		if(top_reg5.bit.SPI2 == SPI2_2ND_PINMUX) {
			printf("RTCLK conflict with SPI2_2\r\n");
			return E_OBJ;
		}
	...
	if (config & PIN_MISC_CFG_SPCLK_NONE) {
		...
		if (top_reg6.bit.ETH == ETH_ID_ENUM_RMII) {
			printf("SP_CLK_1 conflict with ETH RMII\r\n");
			return E_OBJ;
		}
	...
	if (config & PIN_MISC_CFG_SP2CLK_NONE) {
		...
		if (top_reg6.bit.DIGITAL_MIC == DMIC_ENUM_DMIC) {
			printf("SP_CLK2 conflict with to DM_CLK: 0x%lx\r\n", config);
			return E_OBJ;
		}
...
static ER pinmux_select_primary_lcd(struct nvt_pinctrl_info *info, u32 config, u32 pinmux)
{
	...
	case PINMUX_LCD_SEL_PARALLE_RGB888:
		//printf("Primary LCD configure fail -- Not support this format !\r\n");
	...
	case PINMUX_LCD_SEL_SERIAL_MI_SDIO:
		printf("Primary LCD configure MI fail -- Not support MI !\r\n");
		return E_OBJ;
	case PINMUX_LCD_SEL_MIPI:
		break;
	default:
		printf("invalid primary LCD config: 0x%lx\r\n", pinmux);
		return E_PAR;
	}
	...
	if ((pinmux_type >= PINMUX_LCD_SEL_CCIR656) && (pinmux_type <= PINMUX_LCD_SEL_RGB_16BITS)) {
		if ((pinmux_type != PINMUX_LCD_SEL_CCIR656) && (pinmux_type != PINMUX_LCD_SEL_CCIR656_16BITS)) {
			if (!(pinmux & PINMUX_LCD_SEL_NO_HVSYNC)) {
				...
				if (nvt_get_chip_id() == CHIP_NA51084) {
					printf("[WARNNING] 528 not support PINMUX_LCD_SEL_NO_HVSYNC\r\n");
					return E_OBJ;
				}
			}
		}
...
int pinmux_select_debugport(PINMUX_DEBUGPORT uiDebug)
{
    ...
    union TOP_REGLGPIO0 tTOP_REGLGPIO0;
    union TOP_REGPGPIO0 tTOP_REGPGPIO0;
    union TOP_REGCGPIO0 tTOP_REGCGPIO0;
    union TOP_REGSGPIO0 tTOP_REGSGPIO0;
    ...
    if (uiDebug & PINMUX_DEBUGPORT_GROUP1) {
      tTOP_REGCGPIO0.reg = TOP_GETREG(info,TOP_REGCGPIO0_OFS);
      tTOP_REGCGPIO0.reg &= ~(0x7FFD0F); /*MC0~MC3 , MC5~MC22*/
      TOP_SETREG(info, TOP_REGCGPIO0_OFS, tTOP_REGCGPIO0.reg);

      tTOP_REGSGPIO0.reg = TOP_GETREG(info,TOP_REGSGPIO0_OFS);
      tTOP_REGSGPIO0.reg &= ~(0x00001); /*S_GPIO0 */
      TOP_SETREG(info, TOP_REGSGPIO0_OFS, tTOP_REGSGPIO0.reg);

    } else if (uiDebug & PINMUX_DEBUGPORT_GROUP2) {
      tTOP_REGLGPIO0.reg = TOP_GETREG(info,TOP_REGLGPIO0_OFS);
      tTOP_REGLGPIO0.reg &= ~(0x007FF); /*LCD0~LCD10*/
      TOP_SETREG(info, TOP_REGLGPIO0_OFS, tTOP_REGLGPIO0.reg);

      tTOP_REGPGPIO0.reg = TOP_GETREG(info,TOP_REGPGPIO0_OFS);
      tTOP_REGPGPIO0.reg &= ~(0x66180C); /*P_GPIO2,3,11,12 , P_GPIO17~P_GPIO18, P_GPIO21~P_GPIO22*/
      TOP_SETREG(info, TOP_REGPGPIO0_OFS, tTOP_REGPGPIO0.reg);
    } else {
      printf("Debug output ports are not selected\r\n");
    }

    TOP_SETREG(info, TOP_REG_DEBUG_OFS, uiDebug);
...
struct nvt_pinctrl_info info_get_bs[1] = {0};
u32 top_get_bs(void)
{
	union TOP_REG0 top_bs;

	if (top_reg_addr) {
		info_get_bs->top_base = top_reg_addr;
		top_bs.reg = TOP_GETREG(info_get_bs, TOP_REG0_OFS);
	} else {
		printf("invalid pinmux address\n");
		return -E_NOMEM;
	}

	return top_bs.bit.BOOT_SRC;
}