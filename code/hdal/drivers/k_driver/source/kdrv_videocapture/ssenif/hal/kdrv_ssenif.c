	...
	if (group == KDRV_SSENIF_CSI_BASE) {
		...
		*/
		...
		case KDRV_SSENIF_CSI_PATGEN_EN: {
				object->set_csi_config(DAL_SSENIFCSI_CFGID_PATGEN_EN, (UINT32)p_param);
			}
			break;

		case KDRV_SSENIF_CSI_PATGEN_MODE: {
				object->set_csi_config(DAL_SSENIFCSI_CFGID_PATGEN_MODE, (UINT32)p_param);
			}
			break;

		case KDRV_SSENIF_CSI_PATGEN_VALUE: {
				object->set_csi_config(DAL_SSENIFCSI_CFGID_PATGEN_VALUE, (UINT32)p_param);
			}
			break;
	...
	if (group == KDRV_SSENIF_CSI_BASE) {
		...
		*/
		...
		case KDRV_SSENIF_CSI_SENSOR_FS_CNT: {
			...
			break;
		case KDRV_SSENIF_CSI_FRAME_NUMBER: {
				*ret = object->get_csi_config(DAL_SSENIFCSI_CFGID_FRAME_NUMBER);
			}
			break;
