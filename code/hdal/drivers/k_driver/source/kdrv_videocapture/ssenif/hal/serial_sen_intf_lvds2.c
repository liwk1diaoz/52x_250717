...
#define MODULE_ID               LVDS_ID_LVDS2
#define DALSSENIFID             DAL_SSENIF_ID_LVDS2
#define DALLVDS_API_DEFINE(name) ssenif_##name##_lvds2
...
static void  ssenif_set_default_sync(DAL_SSENIF_LANESEL lane_select)
{
	...
	case DAL_SSENIF_SENSORTYPE_PANASONIC: {
			glvds_obj->set_config(LVDS_CONFIG_ID_DESKEW_EN, DISABLE);
...
DAL_SSENIF_INTERRUPT DALLVDS_API_DEFINE(wait_interrupt)(DAL_SSENIF_INTERRUPT waited_flag)
{
	...
	if ((waited_flag & DAL_SSENIF_INTERRUPT_VD)||(waited_flag & DAL_SSENIF_INTERRUPT_SIE2_VD)) {
		_waited_flag |= LVDS_INTERRUPT_VD;
	}
	if ((waited_flag & DAL_SSENIF_INTERRUPT_VD2)||(waited_flag & DAL_SSENIF_INTERRUPT_SIE4_VD)) {
		_waited_flag |= LVDS_INTERRUPT_VD2;
	}
	if ((waited_flag & DAL_SSENIF_INTERRUPT_VD3)||(waited_flag & DAL_SSENIF_INTERRUPT_SIE5_VD)) {
		_waited_flag |= LVDS_INTERRUPT_VD3;
	}
	if (waited_flag & DAL_SSENIF_INTERRUPT_VD4) {
	...
	if (_waited_flag & LVDS_INTERRUPT_VD) {
		ret |= (DAL_SSENIF_INTERRUPT_VD|DAL_SSENIF_INTERRUPT_SIE2_VD);
	}
	if (_waited_flag & LVDS_INTERRUPT_VD2) {
		ret |= (DAL_SSENIF_INTERRUPT_VD2|DAL_SSENIF_INTERRUPT_SIE4_VD);
	}
	if (_waited_flag & LVDS_INTERRUPT_VD3) {
		ret |= (DAL_SSENIF_INTERRUPT_VD3|DAL_SSENIF_INTERRUPT_SIE5_VD);
	}
	if (_waited_flag & LVDS_INTERRUPT_VD4) {
		ret |= DAL_SSENIF_INTERRUPT_VD4;
	}
...
void DALLVDS_API_DEFINE(dump_debug_information)(void)
{
	...
	if (glvds_obj != NULL) {
		...
		DBG_DUMP("LVDS1  %d  %s     %d     %02d    %04d    %04d     %d    %04d      %d    %02d    %02d    %02d    %02d    %d   %d   %d   %d   %d   %d   %d   %d   %d   %d\r\n",
...
void DALLVDS_API_DEFINE(set_config)(DAL_SSENIFLVDS_CFGID config_id, UINT32 value)
{
	...
	case DAL_SSENIFLVDS_CFGID_IMGID_TO_SIE5:
		configuration = LVDS_CONFIG_ID_CHID3;
	  	break;
	//case DAL_SSENIFLVDS_CFGID_IMGID_TO_SIE4:
	//  ConfigID = LVDS_CONFIG_ID_CHID4;
	//  break;
	case DAL_SSENIFLVDS_CFGID_CLANE_SWITCH:
		//configuration = LVDS_CONFIG_ID_CLANE_SWITCH;

		//if (value == DAL_SSENIF_CFGID_CLANE_LVDS2_USE_C4) {
		//	value = FALSE;
		//} else if (value == DAL_SSENIF_CFGID_CLANE_LVDS2_USE_C6) {
		//	value = TRUE;
		//} else {
...
UINT32 DALLVDS_API_DEFINE(get_config)(DAL_SSENIFLVDS_CFGID config_id)
{
	...
	case DAL_SSENIFLVDS_CFGID_CLANE_SWITCH:
		...
		//	return DAL_SSENIF_CFGID_CLANE_LVDS2_USE_C6;
		//} else {
		//	return DAL_SSENIF_CFGID_CLANE_LVDS2_USE_C4;
		//}
		return DAL_SSENIF_CFGID_CLANE_LVDS2_USE_C1;
