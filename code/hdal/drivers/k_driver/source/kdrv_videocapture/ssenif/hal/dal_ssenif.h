...
typedef enum {
	...
	DAL_SSENIFCSI_CFGID_PATGEN_EN,          ///<[OPTIONAL] pattern gen enable/disable
	DAL_SSENIFCSI_CFGID_PATGEN_MODE,        ///<[OPTIONAL] pattern gen mode
	DAL_SSENIFCSI_CFGID_PATGEN_VALUE,       ///<[OPTIONAL] pattern gen value

	DAL_SSENIFCSI_CFGID_FRAME_NUMBER,       ///<[OPTIONAL] get FS short packet frame number.
	DAL_SSENIFCSI_CFGID_SENSOR_ERR_CNT,     ///<[OPTIONAL] get error count
...
/**
    ...
    This definition is used in set_csi_config(DAL_SSENIFCSI_CFGID_PATGEN_MODE)
    to specify the Pattern Gen mode.
*/
typedef enum {
	DAL_SSENIFCSI_PATGEN_MODE_COLORBAR = 1,   ///< PG is color bar
	DAL_SSENIFCSI_PATGEN_MODE_RANDOM,         ///< PG is pseudo random(Frame base)
	DAL_SSENIFCSI_PATGEN_MODE_FIXED,          ///< PG is fixed value
	DAL_SSENIFCSI_PATGEN_MODE_1DINC,          ///< PG is incremental and reset to base pixel value when every line
	DAL_SSENIFCSI_PATGEN_MODE_2DINC,          ///< PG is incremental. The vase pixel value of ever line  st to th line number

	ENUM_DUMMY4WORD(DAL_SSENIFCSI_PATGEN_MODE)
} DAL_SSENIFCSI_PATGEN_MODE;
...
typedef enum {
	...
	DAL_SSENIFLVDS_CFGID_SENSOR_VD_CNT,     ///<[OPTIONAL] Get the sensor VD count
