...
module_param_named(ife_debug_level, ife_debug_level, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(ife_debug_level, "Debug message level");
#endif

//=============================================================================
// Global variable
//=============================================================================
static struct of_device_id ife_match_table[] = {
...
//=============================================================================
static int nvt_ife_open(struct inode *inode, struct file *file);
static int nvt_ife_release(struct inode *inode, struct file *file);
static long nvt_ife_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int nvt_ife_probe(struct platform_device *pdev);
static int nvt_ife_suspend(struct platform_device *pdev, pm_message_t state);
static int nvt_ife_resume(struct platform_device *pdev);
static int nvt_ife_remove(struct platform_device *pdev);
int __init nvt_ife_module_init(void);
void __exit nvt_ife_module_exit(void);
...
static int nvt_ife_open(struct inode *inode, struct file *file)
{
	IFE_DRV_INFO *pdrv_info;

	pdrv_info = container_of(vos_icdev(inode), IFE_DRV_INFO, cdev);
	file->private_data = pdrv_info;

	if (nvt_ife_drv_open(&pdrv_info->module_info, MINOR(vos_irdev(inode)))) {
...
static int nvt_ife_release(struct inode *inode, struct file *file)
{
	...
	nvt_ife_drv_release(&pdrv_info->module_info, MINOR(vos_irdev(inode)));
	return 0;
}
...
static long nvt_ife_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	return nvt_ife_drv_ioctl(MINOR(vos_irdev(inode)), &pdrv->module_info, cmd, arg);
}

struct file_operations nvt_xxx_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_ife_open,
	.release = nvt_ife_release,
	.unlocked_ioctl = nvt_ife_ioctl,
	.llseek  = no_llseek,
};

static int nvt_ife_probe(struct platform_device *pdev)
{
	IFE_DRV_INFO *pdrv_info;//info;
	...
	match = of_match_device(ife_match_table, &pdev->dev);
	if (!match) {
		nvt_dbg(ERR, "Platform device not found \n");
		return -EINVAL;
	}

	pdrv_info = kzalloc(sizeof(IFE_DRV_INFO), GFP_KERNEL);
	if (!pdrv_info) {
	...
	for (ucloop = 0 ; ucloop < MODULE_CLK_NUM; ucloop++) {
		...
		if (IS_ERR(pdrv_info->module_info.pclk[ucloop])) {
			nvt_dbg(ERR, "faile to get ife clock%d source\n", ucloop);
	...
	//Get pclock source
	for (ucloop = 0 ; ucloop < MODULE_CLK_NUM; ucloop++) {
		pdrv_info->module_info.p_pclk[ucloop] = clk_get(&pdev->dev, "ife_pclk");
		if (IS_ERR(pdrv_info->module_info.p_pclk[ucloop])) {
			DBG_ERR("failed to get ife pclock%d source\n", ucloop);
	...
	cdev_init(&pdrv_info->cdev, &nvt_xxx_fops);
	...
	pdrv_info->pmodule_class = class_create(THIS_MODULE, "kdrv_ife");
	...
	ret = nvt_ife_proc_init(pdrv_info);
	if (ret) {
	...
	ret = nvt_ife_drv_init(&pdrv_info->module_info);
	if (ret) {
		nvt_dbg(ERR, "failed in creating proc.\n");
		goto FAIL_DRV_INIT;
	}
	...
	/* register IRQ here*/
	if (request_irq(pdrv_info->module_info.iinterrupt_id[0], nvt_ife_drv_isr, IRQF_TRIGGER_HIGH, "IFE_INT", &pdrv_info->module_info)) {
...
static int nvt_ife_remove(struct platform_device *pdev)
{
	...
	nvt_ife_drv_remove(&pdrv_info->module_info);
...
static int nvt_ife_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_ife_drv_suspend(&pdrv_info->module_info);
...
static int nvt_ife_resume(struct platform_device *pdev)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_ife_drv_resume(&pdrv_info->module_info);

	nvt_dbg(IND, "finished\n");
	return 0;
}

static struct platform_driver nvt_ife_driver = {
	.driver = {
		.name   = "kdrv_ife",
		.owner = THIS_MODULE,
		.of_match_table = ife_match_table,
	},
	.probe      = nvt_ife_probe,
	.remove     = nvt_ife_remove,
	.suspend = nvt_ife_suspend,
	.resume = nvt_ife_resume
};
...
int __init nvt_ife_module_init(void)
{
	...
	nvt_dbg(WRN, "\n");
	ret = platform_driver_register(&nvt_ife_driver);
...
{
	nvt_dbg(WRN, "\n");
	platform_driver_unregister(&nvt_ife_driver);
}

module_init(nvt_ife_module_init);
module_exit(nvt_ife_module_exit);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION("IFE driver");
MODULE_LICENSE("GPL");
