...
unsigned int rtos_agc_debug_level = NVT_DBG_WRN;
...
BOOL audlib_agc_init(void)
{
	...
	if(bMsgDump){
		DBG_DUMP("[AGC] CH= %d       	SR= %d Hz\r\n",(int)AgcInfo.bStereo+1, (int)AgcInfo.iSampleRate);
		DBG_DUMP("[AGC] target= %d DB 	NG-THD= %d DB\r\n",(int)AgcInfo.iTargetLvl>>AGC_GAINDB_FRACTIONAL_BITS, (int)AgcInfo.iNgTHD>>AGC_GAINDB_FRACTIONAL_BITS);
		DBG_DUMP("[AGC] MaxG= %d DB	MinG= %d DB\r\n",(int)AgcInfo.iMaxGain>>1,(int)AgcInfo.iMinGain>>1);
		DBG_DUMP("[AGC] PEAKDET-THD= %d samples \r\n",(int)AgcInfo.pk_up_thd);
		DBG_DUMP("[AGC] DECAY= %d DB	ATTACK= %d DB\r\n",(int)iStepTHDDecay>>AGC_GAINDB_FRACTIONAL_BITS,(int)iStepTHDAttack>>AGC_GAINDB_FRACTIONAL_BITS);
		DBG_DUMP("[AGC] NG-DECAY= %d DB	NG-ATTACK= %d DB\r\n",(int)iNGStepTHDDecay>>AGC_GAINDB_FRACTIONAL_BITS,(int)iNGStepTHDAttack>>AGC_GAINDB_FRACTIONAL_BITS);
	}
