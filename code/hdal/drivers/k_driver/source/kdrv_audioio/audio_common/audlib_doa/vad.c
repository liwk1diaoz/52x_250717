...
static const int kValidRates[] = { 8000,11025,16000,22050,24000,32000,44100,48000 };
static const size_t kRatesSize = sizeof(kValidRates) / sizeof(*kValidRates);
#if VAD_REDUCE_STACK_SZ
static const int kMaxFrameLengthMs = 10;
#else
...
static int32_t WebRtcVad_GaussianProbability(int16_t input,
                                      int16_t mean,
                                      int16_t std,
                                      int16_t* delta) {
...
static int WebRtcVad_ValidRateAndFrameLength(int rate, int frame_length) {
  int return_value = -1;
  size_t i;
  int valid_length_ms;
  int valid_length;
  ...
  for (i = 0; i < kRatesSize; i++) {
    if (kValidRates[i] == rate) {
      for (valid_length_ms = 10; valid_length_ms <= kMaxFrameLengthMs;
          valid_length_ms += 10) {
        valid_length = (kValidRates[i] / 100 * (valid_length_ms/10));
...
static void WebRtcSpl_DownBy2IntToShort(int32_t *in, int32_t len, int16_t *out,
                                 int32_t *state)
{
...
static void WebRtcSpl_DownBy2ShortToInt(const int16_t *in,
                                  int32_t len,
                                  int32_t *out,
                                  int32_t *state)
{
...
// state:  filter state array; length = 8
static void WebRtcSpl_LPBy2IntToInt(const int32_t* in, int32_t len, int32_t* out,
                             int32_t* state)
{
    int32_t tmp0, tmp1, diff;
    int32_t i;
...
//      K: number of blocks
#if VAD_48KHZ_EN
static void WebRtcSpl_Resample48khzTo32khz(const int32_t *In, int32_t *Out,
                                    int32_t K)
{
    /////////////////////////////////////////////////////////////
    // Filter operation:
...
#if VAD_48KHZ_EN
// 48 -> 8 resampler
static void WebRtcSpl_Resample48khzTo8khz(const int16_t* in, int16_t* out,
...
static int32_t WebRtcSpl_Energy(int16_t* vector, int vector_length, int* scale_factor)
{
    int32_t en = 0;
    int i;
...
static int16_t WebRtcVad_CalculateFeatures(VadInstT* self, const int16_t* data_in,
  ...
  // the second split.
  #if VAD_REDUCE_STACK_SZ
  int16_t hp_120[40], lp_120[40];
  int16_t hp_60[20], lp_60[20];
  #else
...
static int WebRtcVad_CalcVad8khz(VadInstT* inst, const int16_t* speech_frame,
                          int frame_length)
{
...
#if VAD_48KHZ_EN
static int WebRtcVad_CalcVad48khz(VadInstT* inst, const int16_t* speech_frame, int frame_length)
{
  int vad;
  int i;
  #if VAD_REDUCE_STACK_SZ
  int16_t speech_nb[80];  // 10 ms in 8 kHz.
  #else
...
static int WebRtcVad_CalcVad32khz(VadInstT* inst, const int16_t* speech_frame,
                           int frame_length)
{
    int len, vad;
	#if VAD_REDUCE_STACK_SZ
    int16_t speechWB[160]; // Downsampled speech frame: 960 samples (30ms in SWB)
    int16_t speechNB[80]; // Downsampled speech frame: 480 samples (30ms in WB)
	#else
...
static int WebRtcVad_CalcVad16khz(VadInstT* inst, const int16_t* speech_frame,
                           int frame_length)
{
    int len, vad;
	#if VAD_REDUCE_STACK_SZ
    int16_t speechNB[80]; // Downsampled speech frame: 160 samples (10ms in WB)
	#else
    int16_t speechNB[240]; // Downsampled speech frame: 480 samples (30ms in WB)
	#endif
...
static int WebRtcVad_CalcVadOtherkhz(VadInstT* inst, const int16_t* speech_frame,
                           int frame_length, int fs) {
#if 0//ASRC_READY
  int vad=0;
  int rInput,rOutput=8000;
  int rqBufSize=0;
  int Ret;
  int handle =0;
  int i =0;
  int16_t temp_mem[589]={0}; // worst case
  #if VAD_REDUCE_STACK_SZ
  int16_t speech_nb[80];
  #else
  int16_t speech_nb[240];
  #endif
  int16_t *ptr_out,*ptr_in;
  int Mode =1;//one frame mode, 0 for continious mode

  ptr_out = &speech_nb[0];
  ptr_in  = (int16_t*)speech_frame;

  rInput = fs;

  rInput /= 100;   // source code define
  rOutput/= 100;

  rqBufSize = audlib_src_pre_init(1, rInput, rOutput, Mode); // 44->8 rqBufSize = 1178
                                                         // 24->8 rqBufSize = 672
                                                         // 22->8 rqBufSize = 620
                                                         // 11->8 rqBufSize = 344
  //DBG_ERR("%d -> 8 rqBufSize = %d\r\n",fs/1000,rqBufSize);


  Ret = audlib_src_init(&handle , 1 , rInput, rOutput, Mode, (short *)&temp_mem);
  if (Ret!=0){
      DBG_ERR("audlib_src_init fail .... returns %d %d\r\n",Ret,rqBufSize);
      return vad;
  }

    do{
        ptr_in  += i*rInput;//16bits per sample -> *2 to bytes
        ptr_out += i*rOutput;

        audlib_src_run(handle, (void *) ptr_in, (void *) ptr_out);
        i++;
 //       DBG_ERR("%d\r\n",i);
    }while((i*rInput)<frame_length);


  frame_length *=rOutput;
  frame_length /=rInput;
  //DBG_ERR("%d hz frame = %d\r\n",fs,frame_length);



  // Do VAD on an 8 kHz signal
  vad = WebRtcVad_CalcVad8khz(inst, speech_nb, frame_length);
  //DBG_ERR("vad = %d ,frame = %d\r\n",vad,frame_length);
...
static void WebRtcSpl_ResetResample48khzTo8khz(WebRtcSpl_State48khzTo8khz* state)
{
...
int WebRtcVad_Process(VadInst* handle, int fs, const int16_t* audio_frame,
  ...
  } else if ((fs == 11025) || (fs == 22050) || (fs == 24000) || (fs == 44100)){
    vad = WebRtcVad_CalcVadOtherkhz(self, audio_frame, frame_length,fs);
  }
