...
typedef struct {
	uintptr_t      ddr_id[NVTMPP_DDR_MAX_NUM];            ///< [in]:  which DDR.
	uintptr_t      base[NVTMPP_DDR_MAX_NUM];
    unsigned       long size[NVTMPP_DDR_MAX_NUM];
} NVTMPP_IOC_GET_HDAL_BASE_S;

typedef struct {
	NVTMPP_VB_BLK  blk;                                   ///< [in]: the block handle
	NVTMPP_ER      rtn;                                   ///< [out]:return value
} NVTMPP_IOC_VB_LOCK_BLK_S;

typedef struct {
	UINT32         pa;                                    ///< [in]:  the physical address
	NVTMPP_VB_BLK  blk;                                   ///< [out]: the block handle
} NVTMPP_IOC_VB_PA_TO_BLK_S;
...
#define NVTMPP_IOC_VB_GET_HDAL_BASE        _VOS_IOWR(NVTMPP_IOC_MAGIC, 25, NVTMPP_IOC_GET_HDAL_BASE_S)
#define NVTMPP_IOC_VB_LOCK_BLK             _VOS_IOWR(NVTMPP_IOC_MAGIC, 26, NVTMPP_IOC_VB_LOCK_BLK_S)
#define NVTMPP_IOC_VB_PA_TO_BLK            _VOS_IOWR(NVTMPP_IOC_MAGIC, 27, NVTMPP_IOC_VB_PA_TO_BLK_S)
