...
#define NVTMPP_VB_MAX_BLK_EACH_POOL 255
...
#define NVTMPP_VB_VCAP_OUT_POOL    29
...
typedef enum _NVTMPP_MODULE_ER {
	...
	NVTMPP_MODULE_ER_DROP              = 3,
	NVTMPP_MODULE_ER_WRN               = 4,
	NVTMPP_MODULE_ER_MISC              = 5,
...
typedef enum {
	...
	NVTMPP_DDR_3                    =   2, ///< DDR3
	NVTMPP_DDR_MAX                  =   3,
	NVTMPP_DDR_MAX_NUM              =   NVTMPP_DDR_MAX,
	NVTMPP_DDR_AUTO                 = 0xF,
...
/**
    Destroy a private pool with pool handle and pool name.
    ...
    @code
    {
        ...
        ret = nvtmpp_vb_destroy_pool_2(pool, "TEST");
        if (NVTMPP_ER_OK != ret) {
            DBG_ERR("destory pool 0x%x\r\n",pool);
        }
    }
...
/**
    ioremap vb block kernel space va.

*/
void         *nvtmpp_vb_blk_map_va(NVTMPP_VB_BLK blk);

/**
    unmap vb block kernel space va.

*/
void         nvtmpp_vb_blk_unmap_va(NVTMPP_VB_BLK blk);

/**
    get a vb block handle by physical address.
