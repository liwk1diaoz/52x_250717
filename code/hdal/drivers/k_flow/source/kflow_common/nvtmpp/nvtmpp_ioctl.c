...
long nvtmpp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case NVTMPP_IOC_VB_GET_COMM_POOL_RANGE: {
		...
		break;
	case NVTMPP_IOC_DDR_MONITOR_START: {
			NVTMPP_DDR_MONITOR_ID_S msg = {0};

			if (unlikely(copy_from_user(&msg, argp, sizeof(msg)))) {
				return -EFAULT;
			}
			if (0 != ddr_data_monitor_start(msg.ddr)) {
				return -EFAULT;
			}
		}
		break;
	case NVTMPP_IOC_DDR_MONITOR_STOP: {
			NVTMPP_DDR_MONITOR_ID_S msg = {0};

			if (unlikely(copy_from_user(&msg, argp, sizeof(msg)))) {
				return -EFAULT;
			}
			if (0 != ddr_data_monitor_stop(msg.ddr)) {
				return -EFAULT;
			}
		}
		break;
	case NVTMPP_IOC_DDR_MONITOR_RESET: {
			NVTMPP_DDR_MONITOR_ID_S msg = {0};

			if (unlikely(copy_from_user(&msg, argp, sizeof(msg)))) {
				return -EFAULT;
			}
			if (msg.ddr > NVTMPP_DDR_2) {
				DBG_ERR("Err ddr = 0x%x\r\n", (int)msg.ddr);
				return -EFAULT;
			}
			ddr_data_monitor_reset(msg.ddr);
		}
		break;
	case NVTMPP_IOC_DDR_MONITOR_DATA: {
			...
			ddr_data_monitor_get(msg.ddr, &msg.cnt, &msg.byte);
			if (unlikely(copy_to_user(argp, &msg, sizeof(msg)))) {
				return -EFAULT;
			}
		}
	...
	case NVTMPP_IOC_VB_LOCK_BLK: {
		...
		break;
	case NVTMPP_IOC_VB_PA_TO_BLK: {
			NVTMPP_IOC_VB_PA_TO_BLK_S    msg = {0};
			UINT32                       va;
			NVTMPP_VB_BLK                blk;
...
int nvtmpp_ioctl (int fd, unsigned int cmd, void *p_arg)
{
	...
	case NVTMPP_IOC_VB_LOCK_BLK: {
		...
		break;
	case NVTMPP_IOC_VB_PA_TO_BLK: {
			NVTMPP_IOC_VB_PA_TO_BLK_S *p_msg = (NVTMPP_IOC_VB_PA_TO_BLK_S *)p_arg;
			UINT32                       va;
			NVTMPP_VB_BLK                blk;
