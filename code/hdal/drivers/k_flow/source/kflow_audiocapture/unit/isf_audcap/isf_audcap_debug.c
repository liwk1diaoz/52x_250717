...
void isf_audcap_dump_status(int (*dump)(const char *fmt, ...), ISF_UNIT *p_thisunit)
{
	...
	if(!p_thisunit || !dump)
		return;
	for (i = 0; i < ISF_AUDCAP_OUT_NUM; i++)
	{
	...
	//dump out status
	dump("------------------------- AUDIOCAP %-2d OUT WORK STATUS -------------------------\r\n", 0);
	...
	//dump user status
	dump("------------------------- AUDIOCAP %-2d USER WORK STATUS ------------------------\r\n", 0);
