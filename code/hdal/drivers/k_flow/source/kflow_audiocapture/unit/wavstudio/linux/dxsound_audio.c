...
#define __MODULE__          dxsound
#define __DBGLVL__          THIS_DBGLVL
#define __DBGFLT__          "*" // *=All, [mark]=CustomClass
#if defined (__UITRON)
#include "DebugModule.h"
#else
#include "uitron_wrapper/debug.h"
#endif
///////////////////////////////////////////////////////////////////////////////
*/
...
AUDIO_GAIN gCurDrvSndGain = AUDIO_GAIN_7;
...
ER DrvSound_SetOutDevConfig(UINT32 outDevIdx, PAUDTS_OBJ pAudPlayObj)
{
	...
	if (gpCurDrvAudOutDev->vidOutType == DRVSOUND_VOUT_LCD) {
		...
		aud_set_output(AUDIO_OUTPUT_SPK); //or AUDIO_OUTPUT_MONO
		aud_set_feature(AUDIO_FEATURE_LINE_PWR_ALWAYSON, FALSE);
		aud_set_feature(AUDIO_FEATURE_INTERFACE_ALWAYS_ACTIVE, FALSE);
		aud_set_feature(AUDIO_FEATURE_DISCONNECT_HDMI, TRUE);
		aud_set_feature(AUDIO_FEATURE_PLAY2ALLCODEC, FALSE);
	} else if (gpCurDrvAudOutDev->vidOutType == DRVSOUND_VOUT_TV) {
		DBG_IND("video: device=TV\r\n");
		DBG_IND("audio: codec=DEFAULT device=LINE\r\n");
		aud_switch_codec(AUDIO_CODECSEL_DEFAULT);
		aud_set_output(AUDIO_OUTPUT_LINE); //or AUDIO_OUTPUT_HP
		aud_set_feature(AUDIO_FEATURE_LINE_PWR_ALWAYSON, TRUE);
		aud_set_feature(AUDIO_FEATURE_INTERFACE_ALWAYS_ACTIVE, FALSE);
		aud_set_feature(AUDIO_FEATURE_DISCONNECT_HDMI, TRUE);
		aud_set_feature(AUDIO_FEATURE_PLAY2ALLCODEC, FALSE);
		...
		aud_set_feature(AUDIO_FEATURE_DISCONNECT_HDMI,  FALSE);
		aud_set_feature(AUDIO_FEATURE_PLAY2ALLCODEC, FALSE);
		...
		aud_set_feature(AUDIO_FEATURE_INTERFACE_ALWAYS_ACTIVE, TRUE);
		aud_set_feature(AUDIO_FEATURE_DISCONNECT_HDMI, FALSE);
		aud_set_feature(AUDIO_FEATURE_PLAY2ALLCODEC, TRUE);
		// 後續 aud_setSamplingRate() 設定的 sampling rate 須等同 HDMI display object 端設定的 sampling rate
		// 不然需要用 aud_setResampleInfo() 啟動 audio resample
	}
...
ER DrvSound_SetCodec(PAUDIO_CODEC_FUNC codecFunc, AUDIO_CODECSEL audCodecSel)
{
	...
	aud_set_extcodec(codecFunc);
	aud_switch_codec(audCodecSel);
	gDrvSndCodecFunc = codecFunc;
	gDrvSndCodecSel = audCodecSel;
	aud_close();
	DBG_IND("CodecFunc=0x%x,codecSel=%d\r\n", (UINT32)gDrvSndCodecFunc, gDrvSndCodecSel);
	return retV;
}
ER DrvSound_SetTotalVolLvl(AUDIO_VOL_LEVEL volLvl)
{
	...
	aud_set_total_vol_level(volLvl);
	aud_close();
...
ER DrvSound_SetVol(UINT32 vol)
{
	...
	if (vol == 0) {
		...
		maxVolLvl = aud_get_total_vol_level();
...
void DrvSound_SetAudioFeature(void)
{
	...
	aud_set_feature(AUDIO_FEATURE_ALC,                                                  TRUE);//HDMI test
	aud_set_feature(AUDIO_FEATURE_NG_WITH_MICBOOST,                                    FALSE);
	aud_set_parameter(AUDIO_PARAMETER_NOISETHD_WITH_BOOST,         EAC_NG_THRESHOLD_N58P5_DB);
	aud_set_parameter(AUDIO_PARAMETER_ALC_MAXGAIN,                  EAC_ALC_MAXGAIN_P21P0_DB);
	aud_set_feature(AUDIO_FEATURE_MICBOOST,                                             TRUE);
	aud_set_parameter(AUDIO_PARAMETER_NOISEGAIN,                                         0x1);
	aud_set_parameter(AUDIO_PARAMETER_NOISEGATE_TIME_RESOLUTION, EAC_ALC_TRESO_BASIS_45000US);
	aud_set_parameter(AUDIO_PARAMETER_ALC_TIME_RESOLUTION,        EAC_ALC_TRESO_BASIS_5000US);
	aud_set_parameter(AUDIO_PARAMETER_ALC_ATTACK_TIME,                                   0x4);
	aud_set_parameter(AUDIO_PARAMETER_ALC_DECAY_TIME,                                    0x5);
	aud_set_parameter(AUDIO_PARAMETER_ALCNG_ATTACK_TIME,                                 0x6);
	aud_set_parameter(AUDIO_PARAMETER_ALCNG_DECAY_TIME,                                  0x3);
#endif
	aud_set_total_vol_level(gDrvSndTotalAudVolLvl);
	aud_set_volume(gCurDrvSndVol);
	aud_set_gain(gCurDrvSndGain);
...
ER DrvSound_SetAudGain(UINT32 vol)
{
	...
	if (aud_get_lock_status()) {
		aud_set_feature(AUDIO_FEATURE_ALC, bAudioALCEn);
		aud_set_gain(uAudGain);
	} else {
		if (E_OK == aud_open()) {
			aud_set_feature(AUDIO_FEATURE_ALC, bAudioALCEn);
			aud_set_gain(uAudGain);
...
ER DrvSound_SetSR_Resample(PDXSOUND_AUD_RESAMPLE_INFO pAudResampleInfo, PAUDTS_OBJ pAudObj)
{
	...
	DbgMsg_DrvSoundIO(("+%s:sr=%d\r\n", __func__, audSR));
	//DbgCode_DrvSound(if (aud_get_lock_status() == NO_TASK_LOCKED) DBG_ERR("%s: AudDrv not open\r\n",__func__);)
...
UINT32 DrvSound_Control(UINT32 CtrlID, UINT32 Param1, UINT32 Param2)
{
	...
	case DXSOUND_CAP_SET_DEVINITDATA:
		...
		aud_set_device_object((AUDIO_DEVICE_OBJ *)Param1);
		aud_close();
		break;
