...
#define NMR_VDOENC_DYNAMIC_CONTEXT          1    // 1: alloc context dynamically, but debug command could only be used when running
...
#define NMR_VDOENC_MSG_REQ_I      			((UINT32)0x00000001 << 11)  //0x00000800
...
typedef struct {
	...
	BOOL                        bBSRingMode;            //in: bs ring buffer mode
} NMR_VDOENC_MEMINFO;
...
typedef struct {
	...
	MP_VDOENC_OSDMASKCB             osdmaskCb;                  //in: OSD and Mask callback
#endif
	MP_VDOENC_MAQDIFF           MaqDiffInfo;            //in: set maq diff info
	...
	BOOL                        bResetIFrameByTimeStamp;		//in: reset to start encoding I frame
	UINT64                      uiTargetTimeStamp;			//in: target time stamp for vendor req_i
	...
	NMR_VDOENC_TOPINFO          uiEncTopInfo[NMI_VDOENC_TOP_MAX];
	BOOL                        bSEIBsChkSum;
	BOOL                        bLowPower;
#ifdef VDOENC_LL
	UINT32                      uiEncTopTime[NMI_VDOENC_TOP_MAX];
#endif
	...
	UINT32                      uiVidFastbootSnapshotSize;		//in: fastboot snapshot size
	BOOL                        bCommSrcOut;			//in: source out buffer using common buffer
	BOOL                        bCommSrcOutNoJPGEnc;			//in: source out buffer no need encode jpeg
	BOOL                        bTimerTrigComp;			//in: timer trigger compensation
	BOOL                        bSkipFrm;
	UINT32                      uiSkipFrmTargetFr;
	UINT32                      uiSkipFrmInputCnt;
	NMI_VDOENC_META_ALLOC_INFO  *pMetaAllocInfo; //in: pointer for meta alloc info
} NMR_VDOENC_OBJ;
...
#ifdef VDOENC_LL
extern THREAD_HANDLE _SECTION(".kercfg_data") NMR_VDOTRIG_D2DTSK_ID_WRAPBS;
#endif
...
#ifdef VDOENC_LL
extern ID _SECTION(".kercfg_data") FLG_ID_NMR_VDOTRIG_WRAPBS;
#endif
extern SEM_HANDLE _SECTION(".kercfg_data") NMR_VDOENC_SEM_ID[];
#ifdef VDOENC_LL
extern SEM_HANDLE _SECTION(".kercfg_data") NMR_VDOENC_TRIG_SEM_ID[];
#endif
