...
typedef struct {
	...
	UINT32                      bFull;                  ///< Full flag
	UINT32                      Queue[NMR_VDOTRIG_JOBQ_MAX];
} NMR_VDOTRIG_WRAPBS_JOBQ, *PNMR_VDOTRIG_WRAPBS_JOBQ;
#endif
...
typedef struct {
	...
	UINT32                      BsNow2;                 ///< out: current bs addr2
	UINT32                      BsSize2;                ///< out: current bs size2
...
#endif
	UINT32                      SkipModeFrameIdx;       ///< out: skip mode vid frame index
...
#ifdef VDOENC_LL
extern ER       NMR_VdoTrig_GetJob_WrapBS(UINT32 *pPathID);
#endif
...
#ifdef VDOENC_LL
extern UINT32   NMR_VdoTrig_GetJobCount_WrapBS(void);
#endif
extern UINT32   NMR_VdoTrig_GetBSCount(UINT32 pathID);
extern BOOL     NMR_VdoTrig_CheckBS(UINT32 pathID, UINT32 *uiAddr, UINT32 *uiSize, UINT32 *uiAddr2, UINT32 *uiSize2, UINT32 *uiStartAddr);
