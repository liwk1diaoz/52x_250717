...
typedef enum {
	...
	NMI_VDOENC_PARAM_BR_TOLERANCE,
	NMI_VDOENC_PARAM_COMM_SRCOUT_ENABLE,
	NMI_VDOENC_PARAM_SNAPSHOT_SIZE,
	NMI_VDOENC_PARAM_SNAPSHOT_RANGE,
	NMI_VDOENC_PARAM_COMM_SRCOUT_NO_JPG_ENC,
	NMI_VDOENC_PARAM_TIMER_TRIG_COMP_ENABLE,
	NMI_VDOENC_REQ_TARGET_I,
	NMI_VDOENC_PARAM_H26X_SEI_CHKSUM,
	NMI_VDOENC_PARAM_H26X_LOW_POWER,
	NMI_VDOENC_PARAM_H26X_CROP,
	NMI_VDOENC_PARAM_CHRM_QP_IDX,
	NMI_VDOENC_PARAM_SEC_CHRM_QP_IDX,
	NMI_VDOENC_PARAM_H26X_MAQ_DIFF_ENABLE,
	NMI_VDOENC_PARAM_H26X_MAQ_DIFF_STR,
	NMI_VDOENC_PARAM_H26X_MAQ_DIFF_START_IDX,
	NMI_VDOENC_PARAM_H26X_MAQ_DIFF_END_IDX,
	NMI_VDOENC_PARAM_BS_RING,
	NMI_VDOENC_PARAM_JPG_JFIF_ENABLE,
	NMI_VDOENC_PARAM_H26X_SKIP_FRM,
	NMI_VDOENC_PARAM_META_ALLOC,
} NMI_VDOENC_PARAM;
...
typedef enum {
	...
	NMI_VDOENC_EVENT_GET_BS_START_PA2,			//get bs_start2 pa
...
typedef struct {
	...
	BOOL				bCommSnapShot;			//out: snapshot buffer using common block
...
} NMI_VDOENC_REQ_TARGET_I_INFO;

typedef struct {
	BOOL   enable;           ///< enable skip frame. default: 0, range: 0~1 (0: disable, 1: enable)
	UINT32 target_fr;        ///< target frame rate
	UINT32 input_frm_cnt;    ///< real input frame number
} NMI_VDOENC_H26X_SKIP_FRM_INFO;

#define NMI_VDOENC_META_ALLOC_INFO_MAX 10
typedef struct _NMI_VDOENC_META_ALLOC_INFO {
	UINT32 sign;
	struct _NMI_VDOENC_META_ALLOC_INFO *p_next;
	UINT32 data_size;
	UINT32 header_size;
} NMI_VDOENC_META_ALLOC_INFO;
