...
typedef struct {
	...
	UINT32 				group_idx;	//ref. to CTL_SIE_ID_IDX
} CTL_SIE_PATGEN_PAR;
...
typedef struct {
	...
	UINT32 gyro_buf_sz;
	UINT64 crp_start_ts;	//timestamp for crops tart
	UINT64 crp_end_ts;		//timestamp for crop_end
	UINT64 t_crp_end_to_vd;	//crop_end to vd time diff
...
typedef struct {
	...
	UINT32					bp3_ratio_direct;	//manual ratio setting for act_start to bp3, bp3 will set by (ratio*total_line)/100
	CTL_SIE_DRAM_OUT_CTRL	ch_out_mode;
	CTL_SIE_SINGLE_OUT_CTRL	sin_out_ctl;
	CTL_SIE_RING_BUF_CTL	ring_buf_ctl;
	UINT32 ca_private_addr;
	UINT32 la_private_addr;
