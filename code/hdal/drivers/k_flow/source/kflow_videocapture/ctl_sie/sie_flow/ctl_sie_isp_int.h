...
typedef struct {
	CTL_SIE_ID id;
	UINT32 ca_addr;
	UINT32 ca_total_size;
	UINT32 la_addr;
	UINT32 la_total_size;
} CTL_SIE_ISP_SUB_OUT_PARAM;

typedef struct {
	UINT32 src_addr;
	CTL_SIE_ISP_LA_RSLT *la_rst;
} CTL_SIE_INT_ISP_LA_RSLT;

typedef struct {
	UINT32 src_addr;
	CTL_SIE_ISP_CA_RSLT *ca_rst;
} CTL_SIE_INT_ISP_CA_RSLT;

void ctl_sie_isp_init(void);
UINT32 ctl_sie_isp_get_sts(void);
INT32 ctl_sie_isp_update_sta_out(CTL_SIE_ISP_SUB_OUT_PARAM sub_out_param);
