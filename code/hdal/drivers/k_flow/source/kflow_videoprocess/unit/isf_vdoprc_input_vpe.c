...
#if (USE_VPE == ENABLE)
///////////////////////////////////////////////////////////////////////////////
#define __MODULE__          isf_vdoprc_iv
#define __DBGLVL__          8 // 0=FATAL, 1=ERR, 2=WRN, 3=UNIT, 4=FUNC, 5=IND, 6=MSG, 7=VALUE, 8=USER
#define __DBGFLT__          "*" //*=All, [mark]=CustomClass
#include "kwrap/debug.h"
unsigned int isf_vdoprc_iv_debug_level = NVT_DBG_WRN;
//module_param_named(isf_vdoprc_iv_debug_level, isf_vdoprc_iv_debug_level, int, S_IRUGO | S_IWUSR);
//MODULE_PARM_DESC(isf_vdoprc_iv_debug_level, "vdoprc1 debug level");
...
static void _isf_vdoprc_vpe_push_dummy_queue_for_all_output(ISF_UNIT *p_thisunit)
{
...
ISF_RV _vdoprc_config_vpe_in_crop(ISF_UNIT *p_thisunit, UINT32 iport)
{
	VDOPRC_CONTEXT* p_ctx = (VDOPRC_CONTEXT*)(p_thisunit->refdata);
	...
	if ((p_vdoinfo->imgaspect.w == 0) || (p_vdoinfo->imgaspect.h == 0)) {
		...
		if (crop_en) {
			...
			p_ctx->in[0].crop.mode = CTL_VPE_IN_CROP_USER;
	...
	ctl_vpe_set(p_ctx->dev_handle, CTL_VPE_ITEM_IN_CROP, (void *)&in_crop);
	return ISF_OK;
}

void _isf_vdoprc_vpe_iport_do_proc_cb(ISF_UNIT *p_thisunit, UINT32 iport, UINT32 buf_handle, UINT32 event, INT32 kr)
{
	#ifdef ISF_TS
	VDOPRC_CONTEXT* p_ctx = (VDOPRC_CONTEXT*)(p_thisunit->refdata);
	#endif
	...
	if (event == CTL_VPE_BUF_UNLOCK) {
		...
		case CTL_VPE_E_OK:
			...
			break;
		case CTL_VPE_E_DROP_INPUT_ONLY: ///< not open, or already close
			_vdoprc_iport_dropdata(p_thisunit, iport, j, ISF_IN_PROBE_PROC_DROP, ISF_ERR_NOT_OPEN_YET);
			_isf_vdoprc_vpe_push_dummy_queue_for_all_output(p_thisunit);
			break;
		#if 0//TBD
		case CTL_IPP_E_STATE: ///< not start, already stop
			_vdoprc_vpe_iport_dropdata(p_thisunit, iport, j, ISF_IN_PROBE_PROC_DROP, ISF_ERR_NOT_START);
			break;
		case CTL_IPP_E_FLUSH: ///< force drop
			_vdoprc_vpe_iport_dropdata(p_thisunit, iport, j, ISF_IN_PROBE_PROC_DROP, ISF_ERR_FLUSH_DROP);
			break;
		case CTL_IPP_E_INDATA: ///< data error
			_vdoprc_vpe_iport_dropdata(p_thisunit, iport, j, ISF_IN_PROBE_PROC_WRN, ISF_ERR_DATA_NOT_SUPPORT);
			break;
		case CTL_IPP_E_QOVR: ///< queue full
			_vdoprc_vpe_iport_dropdata(p_thisunit, iport, j, ISF_IN_PROBE_PROC_WRN, ISF_ERR_QUEUE_FULL);
			break;
		case CTL_IPP_E_PAR: 	///< illegal parameter
		case CTL_IPP_E_KDRV_SET:
			_vdoprc_vpe_iport_dropdata(p_thisunit, iport, j, ISF_IN_PROBE_PROC_WRN, ISF_ERR_PARAM_NOT_SUPPORT);
			break;
		case CTL_IPP_E_SYS: ///< process fail
			_vdoprc_vpe_iport_dropdata(p_thisunit, iport, j, ISF_IN_PROBE_PROC_ERR, ISF_ERR_PROCESS_FAIL);
			break;
		...
		case CTL_IPP_E_TIMEOUT: ///< process timeout
			_vdoprc_vpe_iport_dropdata(p_thisunit, iport, j, ISF_IN_PROBE_PROC_ERR, ISF_ERR_WAIT_TIMEOUT);
			break;
		#endif
		default:
