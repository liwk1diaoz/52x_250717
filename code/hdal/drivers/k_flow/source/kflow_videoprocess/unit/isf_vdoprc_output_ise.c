...
unsigned int isf_vdoprc_ise_debug_level = NVT_DBG_WRN;
...
#if 0
static void _vdoprc_ise_dump_outpath(ISF_UNIT *p_thisunit, CTL_ISE_OUT_PATH *p_data)
{
...
#endif
static ISF_RV _vdoprc_check_ise_out_fmt(ISF_UNIT *p_thisunit, UINT32 pid, UINT32 imgfmt)
{
...
static ISF_RV _vdoprc_check_ise_out_dir(ISF_UNIT *p_thisunit, UINT32 pid, UINT32 imgdir)
{
	VDOPRC_CONTEXT* p_ctx = (VDOPRC_CONTEXT*)(p_thisunit->refdata);
...
static ISF_RV _vdoprc_check_ise_out_crop(ISF_UNIT *p_thisunit, UINT32 pid, UINT32 imgcrop)
{
	VDOPRC_CONTEXT* p_ctx = (VDOPRC_CONTEXT*)(p_thisunit->refdata);
...
static ISF_RV _vdoprc_check_ise_pre_scl_crop(ISF_UNIT *p_thisunit, UINT32 pid)
{
	VDOPRC_CONTEXT* p_ctx = (VDOPRC_CONTEXT*)(p_thisunit->refdata);
	INT32 x, y, w, h;
	...
	if (((x + w) > (INT32)p_ctx->in[0].max_size.w) || ((y + h) > (INT32)p_ctx->in[0].max_size.h) ) {
	...
	if ((CTL_ISE_IN_CROP_MODE)p_ctx->in[0].crop.mode != CTL_ISE_IN_CROP_NONE) {
...
ISF_RV _vdoprc_config_out_ise(ISF_UNIT *p_thisunit, UINT32 pid, UINT32 en, BOOL runtime_update)
{
	VDOPRC_CONTEXT* p_ctx = (VDOPRC_CONTEXT*)(p_thisunit->refdata);
	ISF_INFO *p_dest_info = p_thisunit->port_outinfo[pid - ISF_OUT_BASE];
	ISF_VDO_INFO *p_vdoinfo = (ISF_VDO_INFO *)(p_dest_info);
	...
	if ( pid < CTL_ISE_OUT_PATH_ID_MAX ) {
		...
		if (!en) {
...
#endif
			ctl_ise_set(p_ctx->dev_handle, CTL_ISE_ITEM_OUT_PATH, (void *)&out_path);
		} else {
			UINT32 w,h,imgfmt;
			...
			rv = _vdoprc_check_ise_out_fmt(p_thisunit, pid, imgfmt);
			if (rv != ISF_OK) {
				return rv;
			}
			rv = _vdoprc_check_ise_out_crop(p_thisunit, pid, p_ctx->out_crop_mode[pid]);
			if (rv != ISF_OK) {
				return rv;
			}
			rv = _vdoprc_check_ise_out_dir(p_thisunit, pid, p_vdoinfo->direct);
			if (rv != ISF_OK) {
				return rv;
			}
			rv = _vdoprc_check_ise_pre_scl_crop(p_thisunit, pid);
			if (rv != ISF_OK) {
				return rv;
			}
#if (USE_OUT_FRC == ENABLE)
			...
			//_vdoprc_ise_dump_outpath(p_thisunit, &out_path);
			ctl_ise_set(p_ctx->dev_handle, CTL_ISE_ITEM_OUT_PATH, (void *)&out_path);
			...
			// set h_algin
			{
				CTL_ISE_OUT_PATH_HALIGN h_aln;

				h_aln.pid = pid;
				h_aln.align = p_ctx->out_h_align[pid];
				ctl_ise_set(p_ctx->dev_handle, CTL_ISE_ITEM_OUT_PATH_HALIGN, (void *)&h_aln);
...
*/
void _isf_vdoprc_ise_oport_do_out_cb(ISF_UNIT *p_thisunit, UINT32 event, void *p_in, void *p_out)
{
	VDOPRC_CONTEXT* p_ctx = (VDOPRC_CONTEXT*)(p_thisunit->refdata);
	CTL_ISE_OUT_BUF_INFO *p_info = (CTL_ISE_OUT_BUF_INFO *)p_in;
	VDO_FRAME* p_vdoframe = 0;
	UINT32 path, j;

	path = p_info->pid;

	if (event == CTL_ISE_BUF_NEW) { //new buffer
		...
		if (j == 0xff) {
			p_info->buf_id = 0xff;
			p_info->buf_addr = 0;
			if (p_ctx->ise_mode && p_ctx->user_out_blk[path]) {
				DBG_ERR("%s.out[%d]! no j!? Droped!\r\n", p_thisunit->unit_name, path);
			}
		} else {
			p_info->buf_id = j;
