...
void isf_auddec_dump_status(int (*dump)(const char *fmt, ...), ISF_UNIT *p_thisunit)
{
	#define ONE_SECOND 1000000
	//#define ISF_AUDDEC_IN_NUM          			16
	//#define ISF_AUDDEC_OUT_NUM         			16

	UINT32 i;
	UINT32 did=0;  // audiodec only 1 device
	register UINT32 k;
	UINT16 avg_cnt[16] = {0};
	BOOL do_en = FALSE;
	if(!p_thisunit || !dump)
	...
	//dump in status
	i = 0;
	dump("------------------------- AUDIODEC %-2d IN WORK STATUS --------------------------\r\n", did);
	...
	//dump out status
	dump("------------------------- AUDIODEC %-2d OUT WORK STATUS -------------------------\r\n", did);
	...
	//dump user status
	dump("------------------------- AUDIODEC %-2d USER WORK STATUS -------------------------\r\n", did);
