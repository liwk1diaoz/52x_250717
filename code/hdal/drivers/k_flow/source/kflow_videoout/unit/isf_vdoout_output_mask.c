...
void _isf_vdoout_do_output_mask(ISF_UNIT *p_thisunit, UINT32 oport, void *in, void *out)
{
	VDO_FRAME *p_in             = (VDO_FRAME*)in;
	VDOOUT_CONTEXT* p_ctx       = (VDOOUT_CONTEXT*)(p_thisunit->refdata);
	VDS_TO_VO_MASK *p_vs_ctx;
	UINT64 start = hwclock_get_longcounter();
	
	p_vs_ctx    = vds_lock_context(VDS_QS_VO_MASK, p_ctx->dev, NULL);
	...
	if(p_in && p_vs_ctx && p_vs_ctx->dirty) {
		vds_render_vo(VDS_QS_VO_MASK, p_in->addr[0], p_in->addr[1], 
			p_vs_ctx, p_in->pw[0], p_in->ph[0], NULL, p_in->loff, p_in->pxlfmt);
	 	p_vs_ctx->dirty = 0;
	}

	vds_save_latency(VDS_QS_VO_MASK, p_ctx->dev, 0, hwclock_get_longcounter() - start);
	vds_unlock_context(VDS_QS_VO_MASK, oport, 0);
}
