/**
	...
	@file video_record_with_md_extend.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_with_md_extend, argc, argv)
...
static HD_RESULT mem_init(void)
{
	...
	mem_cfg.pool_info[2].blk_size = DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(ALIGN_CEIL_16(SUB_VDO_SIZE_W), ALIGN_CEIL_16(SUB_VDO_SIZE_H), HD_VIDEO_PXLFMT_YUV420)  // align to 16 for rotate buffer
		                                             +MD_HEAD_BUFSIZE()
                                                     +MD_INFO_BUFSIZE(VDO_SIZE_W, VDO_SIZE_W);
...
static void *encode_thread(void *arg)
{
	...
	while (p_stream0->enc_exit == 0) {
		if (HD_OK == hd_videoenc_poll_list(poll_list, 3, -1)) {    // multi path poll_list , -1 = blocking mode
...
MAIN(argc, argv)
{
	...
	if (extend_out_mode == 0) {
		...
		sub_dim.w = 600;
		sub_dim.h = 800;
		sub_dir = HD_VIDEO_DIR_ROTATE_270;
	...
	if (extend_out_mode == 0) {
		...
		sub2_dim.w = 600;
		sub2_dim.h = 800;
		sub2_dir = HD_VIDEO_DIR_ROTATE_270;
	} else if (extend_out_mode == 8) {
		// test extend output with pre_rotate
		src_dir = HD_VIDEO_DIR_ROTATE_90;
		sub2_crop.x = 0;
