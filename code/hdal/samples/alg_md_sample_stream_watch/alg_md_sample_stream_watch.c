...
UINT32 g_sensi = MED_SENSI;// MED_SENSI;//HIGH_SENSI;//DEFAULT_SENSI;
...
static INT32 share_memory_init(MEM_RANGE *p_share_mem)
{
	...
	for(i=0;i<11;i++){
		...
		else if(i==3 || i==7)blk_size = IMG_BUF_SIZE*40;
		else if(i==4 || i==8)blk_size = IMG_BUF_SIZE*6;
		else if(i==5 || i==9)blk_size = ((IMG_BUF_SIZE+15)/16)*16*12;
		else if(i==6)blk_size = ((IMG_BUF_SIZE+7)/8);
		else if(i==10)blk_size = IMG_BUF_SIZE; // result transform
...
static void md_set_para_medium_sensitivity(VENDOR_MD_PARAM *mdbc_parm)
{
	...
	mdbc_parm->UpdPara.maxFgFrm       = 0;	//0x80;
...
static HD_RESULT md_set_para(MEM_RANGE *p_share_mem, UINT32 ping_pong_id, UINT32 mode, UINT32 sensi)
{
	...
	mdbc_parm.controlEn.deghost_en    = 0;	//1;
...
int md_sum( UINT8 *result, int width, int height)
{
	UINT32 i,j,size, sum=0;
	size = width*height;
...
int trigger_alarm(int alarm_th)	//alarm_th for secconds
{
	int i=0;
	for(i=0; i<alarm_th; i++)
	{
		printf("alarm!!!\n");
		sleep(1);
	}
 ...
 VOID *watch_thread(VOID *arg)
{
	...
	int stable_count = 0, need_init=0;
	int MD_count = 0;
	int MD_count_TH = IMG_WIDTH*IMG_HEIGHT/10;	// 根据区域大小可配 
	int alarm_count_TH = 3;	// alarm time
	int SC_TH = 8;	// 8秒
	struct timeval s1 ={0};
	struct timeval s2 ={0};
	...
	while(g_endFlag==0)
	{
		...
		if(counter >20) {
			...
			md_set_para(p_share_mem, ping_pong_id,is_Init,g_sensi);
			...
			if(is_Init == 1) {
				...
				MD_count = md_sum( (UINT8*)p_share_mem[6].va, IMG_WIDTH, IMG_HEIGHT);
				if(MD_count >= MD_count_TH)
				{
					stable_count++;
					...
					if((s2.tv_sec - s1.tv_sec)*1000 + (s2.tv_usec - s1.tv_usec)/1000 >= 1000*SC_TH)	
					{
						trigger_alarm(alarm_count_TH);
						stable_count = 0;
						need_init = 1;
					}
				}
...
MAIN(argc, argv)
{
	...
	for (idx = 0; idx < MD_THREAD_NUM; idx++) {
		// create thread
		ret = pthread_create(&md_thread_id[idx], NULL, watch_thread, (VOID*)share_mem);	
		if (ret < 0) {
