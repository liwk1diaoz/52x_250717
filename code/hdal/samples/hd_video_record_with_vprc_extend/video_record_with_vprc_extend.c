...
MAIN(argc, argv)
{
	...
	if (extend_out_mode == 0) {
		...
		// test extend output with mirror-x
		sub_crop.x = 0;
		sub_crop.y = 0;
		sub_crop.w = 0;
		sub_crop.h = 0;
		sub_dim.w = VDO_SIZE_W;
		sub_dim.h = VDO_SIZE_H;
		sub_dir = HD_VIDEO_DIR_MIRRORX;  // HD_VIDEO_DIR_MIRRORY.HD_VIDEO_DIR_MIRRORX
	} else { //if (extend_out_mode == 12) {		// test extend output with pre_rotate, crop, scale
		src_dir = HD_VIDEO_DIR_ROTATE_90;
		sub_crop.x = 100;
		sub_crop.y = 100;
		sub_crop.w = 480;
	...
	if (extend_out_mode == 0) {
		...
		sub2_dim.w = VDO_SIZE_W;
		sub2_dim.h = VDO_SIZE_H;
		sub2_dir = HD_VIDEO_DIR_MIRRORX;  // HD_VIDEO_DIR_MIRRORY.HD_VIDEO_DIR_MIRRORX
	} else { //if (extend_out_mode == 12) {		// test extend output with pre_rotate, crop, scale
		src_dir = HD_VIDEO_DIR_ROTATE_90;
		sub2_crop.x = 100;
		sub2_crop.y = 100;
	...
	while (1) {
        ...
        if (key == 'm') {
            ...
            stream[0].enc_exit = 1;
            set_proc_param(stream[0].proc_path, &main_dim, src_dir);
        ...
        //src crop on/off, exten path should re-start for update config
        //echo trace d0 o5 mffff > /proc/hdal/vprc/cmd
        if (key == 'n') {
        	HD_VIDEOPROC_CROP  video_out_param = {0};
    		...
    		video_out_param.win.rect.x   = 0;
    		video_out_param.win.rect.y   = 0;
    		video_out_param.win.rect.w   = 640;
    		video_out_param.win.rect.h   = 480;
    		ret = hd_videoproc_set(stream[0].proc_path, HD_VIDEOPROC_PARAM_OUT_CROP, &video_out_param);
    		hd_videoproc_start(stream[0].proc_path);
        	hd_videoproc_start(stream[1].proc_path);
	    }
