...
#define HD_VIDEOPROC_CFG_STRIP_NUM2		0x00200000  //vprc "2: force 2 stripe
#define HD_VIDEOPROC_CFG_STRIP_NUM3		0x00300000  //vprc "3: force 3 stripe
#define HD_VIDEOPROC_CFG_STRIP_NUM4		0x00400000  //vprc "4: force 4 stripe
#define HD_VIDEOPROC_CFG_STRIP_NUM5		0x00500000  //vprc "5: force 5 stripe
#define HD_VIDEOPROC_CFG_STRIP_NUM6		0x00600000  //vprc "6: force 6 stripe
#define HD_VIDEOPROC_CFG_STRIP_NUM7		0x00700000  //vprc "7: force 7 stripe
#define HD_VIDEOPROC_CFG_STRIP_NUM8		0x00800000  //vprc "8: force 8 stripe
#define HD_VIDEOPROC_CFG_STRIP_NUM9		0x00900000  //vprc "9: force 9 stripe
...
static HD_RESULT set_proc_cfg(HD_PATH_ID *p_video_proc_ctrl, HD_DIM* p_max_dim, HD_OUT_ID _out_id)
{
	...
	{
		UINT32 g_strip_rule = 0;

		if (g_prccfg == 6) {
			g_strip_rule = HD_VIDEOPROC_CFG_STRIP_NUM2;
		} else if (g_prccfg == 7) {
			g_strip_rule = HD_VIDEOPROC_CFG_STRIP_NUM3;
		} else if (g_prccfg == 8) {
			g_strip_rule = HD_VIDEOPROC_CFG_STRIP_NUM4;
		} else if (g_prccfg == 9) {
			g_strip_rule = HD_VIDEOPROC_CFG_STRIP_NUM5;
		} else if (g_prccfg == 10) {
			g_strip_rule = HD_VIDEOPROC_CFG_STRIP_NUM6;
		} else if (g_prccfg == 11) {
			g_strip_rule = HD_VIDEOPROC_CFG_STRIP_NUM7;
		} else if (g_prccfg == 12) {
			g_strip_rule = HD_VIDEOPROC_CFG_STRIP_NUM8;
		} else if (g_prccfg == 13) {
			g_strip_rule = HD_VIDEOPROC_CFG_STRIP_NUM9;
		}

		if (g_strip_rule) {
			ret = vendor_videoproc_set(video_proc_ctrl, VENDOR_VIDEOPROC_PARAM_STRIP, &g_strip_rule);
			if (ret != HD_OK) {
				return HD_ERR_NG;
			}
		}
...
MAIN(argc, argv)
{
	...
	if (argc == 1 || argc > 7) {
		...
		printf("                 6(STRP_NUM2), 7(STRP_NUM3), 8(STRP_NUM4), 9(STRP_NUM5), 10(STRP_NUM6), 11(STRP_NUM7)\r\n");
		printf("                 12(STRP_NUM8), 13(STRP_NUM9)\r\n");
		return 0;
	}
	...
	while (1) {
		...
		if (key == 'o') {
			hd_videocap_open(HD_VIDEOCAP_0_IN_0, HD_VIDEOCAP_0_OUT_0, &stream[0].cap_path);
			hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_0, &stream[0].proc_path);
			if (g_capbind == 0xff) { //no-bind mode
				// create capture_thread (pull_out frame, push_in frame then pull_out frame)
				ret = pthread_create(&stream[0].cap_thread_id, NULL, capture_thread, (void *)stream);
				if (ret < 0) {
