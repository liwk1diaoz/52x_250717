...
static HD_RESULT pdcnn_proc(UINT32 proc_id, UINT32 layer_id, UINT32 mode, UINT32 layer_param, UINT32 parm_addr)
{
	...
	//cls_prob_32_mem  1347
	rv = vendor_ai_net_get(proc_id, VENDOR_AI_NET_PARAM_OUT(112, 0), &(out_buf));
	hd_common_mem_flush_cache((VOID *)out_buf.va, out_buf.size);
	in_va[0] = out_buf.va;
	//bbox_pred_32_mem 1350
	rv = vendor_ai_net_get(proc_id, VENDOR_AI_NET_PARAM_OUT(113, 0), &(out_buf));
	hd_common_mem_flush_cache((VOID *)out_buf.va, out_buf.size);
	in_va[1] = out_buf.va;
	//cls_prob_16_mem 1426
	rv = vendor_ai_net_get(proc_id, VENDOR_AI_NET_PARAM_OUT(126, 0), &(out_buf));
	hd_common_mem_flush_cache((VOID *)out_buf.va, out_buf.size);
	in_va[2] = out_buf.va;
	//bbox_pred_16_mem 1427
	rv = vendor_ai_net_get(proc_id, VENDOR_AI_NET_PARAM_OUT(127, 0), &(out_buf));
	hd_common_mem_flush_cache((VOID *)out_buf.va, out_buf.size);
	in_va[3] = out_buf.va;
	//cls_prob_8_mem 1579
	rv = vendor_ai_net_get(proc_id, VENDOR_AI_NET_PARAM_OUT(152, 0), &(out_buf));
	hd_common_mem_flush_cache((VOID *)out_buf.va, out_buf.size);
	in_va[4] = out_buf.va;
	//bbox_pred_8_mem 1580
	rv = vendor_ai_net_get(proc_id, VENDOR_AI_NET_PARAM_OUT(155, 0), &(out_buf));
	hd_common_mem_flush_cache((VOID *)out_buf.va, out_buf.size);
	in_va[5] = out_buf.va;
