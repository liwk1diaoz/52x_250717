...
static UINT32 RESOLUTION_SET = 0; //0: 2M(IMX290), 1:5M(OS05A), 2: 2M (OS02K10), 3: 2M (AR0237IR), 4: (PATTERN_GEN), 5: 2M (F37), 6: 2M (F35), 7: 4M (GC4653), 8: 4M (GC4653_slave)
static char * SENSOR_NAME_TBL[] = {"sen_imx290", "sen_os05a10", "sen_os02k10", "sen_ar0237ir", "sen_off", "sen_f37", "sen_f35", "sen_gc4653", "sen_gc4653_slave"};
static UINT32 VDO_SIZE_W_TBL[] = {1920, 2592, 1920, 1920, 1920, 1920, 1920, 2560, 2560};
static UINT32 VDO_SIZE_H_TBL[] = {1080, 1944, 1080, 1080, 1080, 1080, 1080, 1440, 1440};
...
#if 1//(VOUT_DISP==ENABLE) /*always alloc,avoid change fastboot-hdal-mem.dtsi*/
	// config vout pool
...
static HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl)
{
	...
	case 7:
		...
		break;
	case 8:
		snprintf(cap_cfg.sen_cfg.sen_dev.driver_name, HD_VIDEOCAP_SEN_NAME_LEN-1, "nvt_sen_gc4653_slave");
		printf("Using nvt_sen_gc4653_slave\n");
		break;
	default:
	...
	if(RESOLUTION_SET == 8) {//gc4653_slave
	  cap_cfg.sen_cfg.sen_dev.if_type = HD_COMMON_VIDEO_IN_MIPI_CSI;
		cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.sensor_pinmux =  0x2A0; //PIN_SENSOR_CFG_MIPI
		printf("MIPI interface for gc4653_slave\n");
  }
...
MAIN(argc, argv)
{
	...
	if (sensor1_enabled()) {
		...
		sub_dim.w = 640;//VDO_SIZE_W;
		sub_dim.h = 480;//VDO_SIZE_H;
