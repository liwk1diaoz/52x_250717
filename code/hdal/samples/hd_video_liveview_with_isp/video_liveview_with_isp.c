...
MAIN(argc, argv)
{
	...
	while (1) {
		...
		if (key == 'a') {
			//The switching duration from 'x' (sensor HDR mode) to 'a' (vprc SHDR off) is less than switching from 'x' (sensor HDR mode, vprc SHDR on) to 'z' (sensor linear mode, vprc SHDR off).
			//However, sensor still outputs dual frames, so it consumes more power than sensor linear mode and cannot support direct mode. 
