...
static INT32 share_memory_init(VQA_MEM_RANGE *p_share_mem)
{
	...
	for(i=0; i<4; i++){
		if(i==0)
			...
			blk_size = IMG_BUF_SIZE/2;
		else if(i==3)
			blk_size = vqa_calc_buf_size();
...
static HD_RESULT vqa_set_para(UINT8 en_blur, UINT32 blur_strength, UINT32 blur_cover_th, UINT32 blur_alarm_times, UINT32 blur_w_num, UINT32 blur_h_num)
{
	...
	vqa_param.contrast_param.blur_w_num  = blur_w_num;
    	vqa_param.contrast_param.blur_h_num  = blur_h_num;
...
static VOID *vqa_thread_api(VOID *arg)
{
	...
	int en_blur=1, blur_strength=8, blur_cover_th=60, blur_alarm_times=5, blur_w_num=160, blur_h_num=94;
	int *blur_block_info;
	vqa_set_para(en_blur, blur_strength, blur_cover_th, blur_alarm_times, blur_w_num, blur_h_num);

	blur_block_info = malloc(blur_w_num * blur_h_num * sizeof(UINT32));
	vqa_rst.res_blur_block = (UINT32 *) blur_block_info;
	...
	while(g_endFlag==0) 
	{
		...
		if(counter >30) {
			...
			imgY_va = (UINT32)hd_common_mem_mmap(HD_COMMON_MEM_MEM_TYPE_CACHE, phy_addr_main, yuv_size);
			if (imgY_va == 0) {
                		printf("Error: mmap fail !!\r\n");
                		goto gen_init_fail;
            		}
			#define PHY2VIRT_MAIN(pa) (imgY_va + ((pa) - phy_addr_main))
			...
			ret = hd_common_mem_munmap((void *)imgY_va, yuv_size);
			if (HD_OK != ret) {
                		printf("mnumap error !!\r\n\r\n");
				goto gen_init_fail;
            		}
...
MAIN(argc, argv)
{
	...
	UINT32 blur_strength=8, blur_cover_th=85, blur_alarm_times=5, en_blur=1, blur_w_num=160, blur_h_num=94;
