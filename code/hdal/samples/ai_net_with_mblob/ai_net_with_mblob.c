...
static HD_RESULT input_open(NET_PATH_ID net_path)
{
	...
	// allocate in buf
	ret = mem_alloc(&p_net->input_mem, "ai_in_buf",  ALIGN_CEIL_32(buf_size) * p_net->in_cfg.b);
	if (ret != HD_OK) {
...
static HD_RESULT set_buf_by_in_path_list(VIDEO_LIVEVIEW *p_stream)
{
	...
	for (i = 0; i < p_stream->input_blob_num; i++) {
...
#if FLOAT_FEATURE_IN
		UINT32 fixed_size = 0;
		UINT32 fixed_bitdepth = 0;
		VOID *fixed_buf = NULL;
		
		if (p_in->in_cfg.is_float_feature_in != 0 ) {
			ret = vendor_ai_net_get(proc_id, in_path_list[idx], &tmp_buf);
			if (HD_OK != ret) {
				printf("proc_id(%u) get in buf fail, i(%d), in_path(0x%lx)\n", proc_id, i, in_path_list[i]);
				goto exit;
			}
			// alloc fixed buf size
			fixed_bitdepth = HD_VIDEO_PXLFMT_BITS((&tmp_buf)->fmt) ;
			fixed_size = p_in->in_cfg.h * p_in->in_cfg.w * p_in->in_cfg.c * p_in->in_cfg.b * (fixed_bitdepth / 8);
			fixed_buf = (VOID*)malloc(fixed_size);
		
			// transfer input from float to fixed
			vendor_ai_cpu_util_float2fixed((FLOAT*)p_in->input_mem.va, 1.0f, fixed_buf,  tmp_buf.fmt, p_in->in_cfg.h * p_in->in_cfg.w * p_in->in_cfg.c * p_in->in_cfg.b);

				
			if (p_in->input_mem.size < fixed_size) {
				printf("proc_id(%u) invalid size !! input_mem.size(%u) < fixed_size(%u)\n", proc_id, in_buf.size, fixed_size);
				ret = HD_ERR_FAIL;
				goto exit;
			}
			// overwrite with fixed buf
			memcpy((VOID*)p_in->input_mem.va, fixed_buf, fixed_size);
			hd_common_mem_flush_cache((VOID*)p_in->input_mem.va, fixed_size);

			// update in_buf.size from float_size -> fixed_size
			in_buf.size  = fixed_size / in_buf.batch_num ;
		
			free(fixed_buf);		
		}
#endif
		src_img_size = ALIGN_CEIL_32(in_buf.size);
		...
		if (p_in->in_cfg.is_comb_img != 0) { 
			for(k =0 ; k < in_buf.batch_num; k++) {
			in_temp_buf[k] = (UINT32 *)malloc(in_buf.size);
			memcpy((VOID*)in_temp_buf[k], (VOID*)(in_buf.va+(k*in_buf.size)), in_buf.size);
			}
		}
		...
		for(j = 0; j < in_buf.batch_num; j++) {
			if (j > 0) {
				...
				if (p_in->in_cfg.is_comb_img == 0) { // no combine image, need to use the same image as input.
					...
					UINTPTR dst_va = in_buf.va + src_img_size;
					memcpy((VOID*)dst_va, (VOID*)in_temp_buf[j], in_buf.size);
					hd_common_mem_flush_cache((VOID *)dst_va, src_img_size);
					in_buf.pa += src_img_size;
					in_buf.va += src_img_size;
				}
