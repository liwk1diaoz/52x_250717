...
static HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl)
{
	...
	#if MCLK_SRC_SYNC_FUNC
	{
		...
		if (ret != HD_OK) {
			printf("VENDOR_VIDEOCAP_PARAM_MCLK_SRC_SYNC_SET failed!(%d)\r\n", ret);
		}
	}
	#endif
	#if SW_VD_SYNC_FUNC
	{
		...
		ret = vendor_videocap_set(video_cap_ctrl, VENDOR_VIDEOCAP_PARAM_SW_VD_SYNC, &sw_vd_sync);
		if (ret != HD_OK) {
...
MAIN(argc, argv)
{
	...
	while (1) {
		...
		if (key == '0') {
			...
			get_cap_sysinfo(stream[1].cap_ctrl);
		}
	}
