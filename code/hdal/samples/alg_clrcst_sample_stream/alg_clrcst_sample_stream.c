...
#define DST_IMG_WIDTH  			320
#define DST_IMG_HEIGHT			180
...
#define VDO_YUV_BUF_SIZE	VDO_SIZE_W*VDO_SIZE_H*3/2
...
static HD_RESULT mem_init(void)
{
	...
	// config common pool (alg: color cast)
	mem_cfg.pool_info[3].type = HD_COMMON_MEM_USER_BLK;
	mem_cfg.pool_info[3].blk_size = vqa_cal_clrcst_buf_size(DST_IMG_WIDTH, DST_IMG_HEIGHT);
	mem_cfg.pool_info[3].blk_cnt = 1;
	mem_cfg.pool_info[3].ddr_id = DDR_ID0;

#if(CLRCST_NUE_PREPROC)
	// config common pool (use nue2 preproc)
	mem_cfg.pool_info[4].type = HD_COMMON_MEM_COMMON_POOL;
	mem_cfg.pool_info[4].blk_size = vendor_preproc_get_mem_size() + DST_IMG_WIDTH*DST_IMG_HEIGHT*3;
	...
	if (HD_OK != ret) {
		...
		printf("hd_common_mem_init ok: %d\r\n", ret);
	return ret;
}
...
static HD_RESULT release_mem_block(CLRCST_MEM_RANGE *p_share_mem)
{
	/* Release in buffer */
	UINT8 i;
	HD_RESULT ret = HD_OK;
		...
		if (ret != HD_OK) {
			printf("mem_uninit : (p_share_mem[%d].va) hd_common_mem_munmap fail.\r\n", i);
			return ret;
		}
		ret = hd_common_mem_release_block(p_share_mem[i].blk);
		if (ret != HD_OK) {
			printf("mem_uninit : (p_share_mem[%d].va)hd_common_mem_release_block fail.\r\n", i);
			return ret;
		}
	}
	return ret;
...
static INT32 share_memory_init(CLRCST_MEM_RANGE *p_share_mem)
{
	HD_COMMON_MEM_VB_BLK blk;
        UINT8 i;
        UINT32 pa, va;
...
#endif
                if(i==0){
			blk_size = DST_IMG_BUF_SIZE; // for videoproc
                	blk = hd_common_mem_get_block(HD_COMMON_MEM_COMMON_POOL, blk_size, ddr_id);
		}
                else if(i==1){ // for vqa color cast
			blk_size = vqa_cal_clrcst_buf_size(DST_IMG_WIDTH, DST_IMG_HEIGHT);
                	blk = hd_common_mem_get_block(HD_COMMON_MEM_USER_BLK, blk_size, ddr_id);
		}
#if(CLRCST_NUE_PREPROC)
                else if(i==2){ // for preproc
			blk_size = vendor_preproc_get_mem_size()+ DST_IMG_WIDTH*DST_IMG_HEIGHT*3;
                	blk = hd_common_mem_get_block(HD_COMMON_MEM_COMMON_POOL, blk_size, ddr_id);
		}
#endif
...
static VOID *clrcst_thread_api(VOID *arg)
{
	...
	do {
			...
			video_frame_new.phy_addr[0] = p_share_mem[0].addr; 					 // Y
			video_frame_new.phy_addr[1] = p_share_mem[0].addr + (DST_IMG_WIDTH * DST_IMG_HEIGHT);  // UV pack
			video_frame_new.blk         = p_share_mem[0].blk;
			...
			if (is_clrcst_run) {
#if(CLRCST_NUE_PREPROC)
				output_va = p_share_mem[2].va + vendor_preproc_get_mem_size();
				output_pa = p_share_mem[2].addr + vendor_preproc_get_mem_size();
				ret = vendor_preproc_run_v2(&preproc_mem, &preproc_param, NULL);
			#if(DUMP_PREPROC_RST)
				CHAR dump_path[256] = {0};
				hd_common_mem_flush_cache((VOID *)output_va, 320*180);
				snprintf(dump_path, STR_MAX_LENGTH, "out_rgb/y%d.bin", (int)run_id);
				vendor_ais_writebin(output_va, 320*180, dump_path);
				run_id += 1;
			#endif
				if (ret != 0) {
					printf("CLRCST: run preproc fail\n");
					return 0;
				}
				cl_status = (CLRCST_STATUS_S) vqa_clrcst_detect(output_va, video_frame_new.dim.w, video_frame_new.dim.h);
#else

				cl_status = (CLRCST_STATUS_S) vqa_clrcst_detect(p_share_mem[0].va, video_frame_new.dim.w, video_frame_new.dim.h);
#endif
...
#if(CLRCST_NUE_PREPROC)
INT32 preproc_init_parm(AI_PREPROC_PARM* p_param, UINT32 input_addr, UINT32 output_addr, UINT32 width, UINT32 height)
{
        ...
        // init parameter
	//printf("preproc init width %d/height%d\n", width, height);
	//printf("preproc init input addr  = 0x%08X\n", (unsigned int)input_addr);
	//printf("preproc init output addr = 0x%08X\n", (unsigned int)output_addr);
        ...
        io_info.size.width = width;
        io_info.size.height = height;
        io_info.size.channel = 3;
        io_info.ofs.line_ofs = width;
        vendor_preproc_set_io_parm(p_param, &io_info, AI_PREPROC_FMT_YUV420, 0, AI_PREPROC_CH_Y); // set Y
        io_info.pa = input_addr + io_info.size.width * io_info.size.height;
        vendor_preproc_set_io_parm(p_param, &io_info, AI_PREPROC_FMT_YUV420, 0, AI_PREPROC_CH_UVPACK); //set UV
...
MAIN(argc, argv)
{
	...
	is_clrcst_run = FALSE;
#if(CLRCST_NUE_PREPROC)
	CLRCST_MEM_RANGE share_mem[3];
	UINT32 input_y_pa = 0; //physical address for input
#else
	CLRCST_MEM_RANGE share_mem[2];
#endif
	//CLRCST_MEM_RANGE *p_share_mem = (CLRCST_MEM_RANGE*) (&share_mem);
	...
	g_stream[0].proc_alg_max_dim.w = DST_IMG_WIDTH;
	g_stream[0].proc_alg_max_dim.h = DST_IMG_HEIGHT;
	...
	ret = pthread_create(&clrcst_thread_id[0], NULL, clrcst_thread_api, (VOID*)(&share_mem));
	if (ret < 0) {
	...
	do {
		...
		if (key == 'q' || key == 0x3) {
...
#else
			cl_parm.cl_max_yuv_width = 320;
			cl_parm.cl_max_yuv_height = 180;
			cl_parm.cl_collect_time = 1;
			cl_parm.cl_warning_max = 5;
			cl_parm.cl_diff_th = 20;
			...
			cl_parm.clrcst_buf_va_addr = share_mem[1].va;
			cl_parm.clrcst_buf_pa_addr = share_mem[1].addr;
			cl_parm.cl_is_debug = 0;
#endif

#if(CLRCST_NUE_PREPROC)
			input_y_pa = share_mem[0].addr;
			g_preproc_user_parm_va[0] = share_mem[2].va;
			g_preproc_user_parm_pa[0] = share_mem[2].addr;

			g_preproc_user_buff_va[0] = g_preproc_user_parm_va[0] + vendor_preproc_get_mem_size();
			g_preproc_user_buff_pa[0] = g_preproc_user_parm_pa[0] + vendor_preproc_get_mem_size();
			g_preproc_mem_size =  vendor_preproc_get_mem_size();

			if (preproc_init_parm(&preproc_param, input_y_pa, g_preproc_user_buff_pa[0], DST_IMG_WIDTH, DST_IMG_HEIGHT) < 0){
				printf("CLRCST: preproc parameter init fail\n");
				return 0;
			}
			else{
				printf("CLRCST: preproc parameter init sucess\n");
			}
			vendor_preproc_auto_alloc_mem(g_preproc_user_parm_pa[0], g_preproc_user_parm_va[0], &preproc_mem);
#endif
...
exit:
	...
	ret = release_mem_block(share_mem);
	if (ret != HD_OK) {
		printf("mem fail=%d\n", ret);
	}
