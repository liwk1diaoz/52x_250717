...
MAIN(argc, argv)
{
	...
	while (1) {
		...
		if (key == 't') {
			...
			hd_videoproc_get(stream[0].proc_path, HD_VIDEOPROC_PARAM_FUNC_CONFIG, &video_path_param);
			if (video_path_param.out_func & HD_VIDEOPROC_OUTFUNC_DIS) {
				video_path_param.out_func &= ~HD_VIDEOPROC_OUTFUNC_DIS;
				printf("DIS OFF.\r\n");
			} else {
				video_path_param.out_func |= HD_VIDEOPROC_OUTFUNC_DIS;
			...
			hd_videoproc_set(stream[0].proc_path, HD_VIDEOPROC_PARAM_FUNC_CONFIG, &video_path_param);
			hd_videoproc_start(stream[0].proc_path);
		}
	}
