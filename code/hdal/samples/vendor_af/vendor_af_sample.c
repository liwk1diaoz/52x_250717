...
int main(int argc, char *argv[])
{
	...
	while (trig) {
			...
			printf(" 42. Get sensor RESOLUTION \n");
			...
			printf(" 83. Set sensor RESOLUTION \n");
		...
		case 42:
			do {
				printf("Set isp id (0, 1)>> \n");
				sen_resolution.id = (UINT32)get_choose_int();
			} while (0);

			result = vendor_isp_get_af(AFT_ITEM_SEN_RESOLUTION, &sen_resolution);
			if (result == HD_OK) {
				printf("id = %d, sen_resolution.sen_reso = %d \n", sen_resolution.id, sen_resolution.sen_reso);
			} else {
				printf("Get AFT_ITEM_VA_STA fail! \n");
			}
			break;			
		...
		case 81:
				...
				do {
					printf("Set backlash_comp enable (0, 1)>> \n");
		...
		case 83:
				...
				do {
					printf("Set sen resolution (2~12)>> \n");
					sen_resolution.sen_reso = (UINT32)get_choose_int();
				}while (0);
				
				result = vendor_isp_set_af(AFT_ITEM_SEN_RESOLUTION, &sen_resolution);
				if (result == HD_OK) {
					printf("id = %d, sen_resolution.sen_reso = %d \n", sen_resolution.id, sen_resolution.sen_reso);
				} else {
					printf("Get AFT_ITEM_VA_STA fail! \n");
