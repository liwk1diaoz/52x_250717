...
static HD_RESULT set_proc_cfg(HD_PATH_ID *p_video_proc_ctrl, HD_DIM* p_max_dim)
{
    ...
    ret = vendor_videoproc_set(video_proc_ctrl, VENDOR_VIDEOPROC_CFG_DIS_SCALERATIO, &ratio);
    if (ret != HD_OK)
		return ret;
    ret = vendor_videoproc_set(video_proc_ctrl ,VENDOR_VIDEOPROC_CFG_DIS_SUBSAMPLE,&subsample);
	if (ret != HD_OK)
		return ret;
    }
...
MAIN(argc, argv)
{
	...
	while (1) {
		...
		if (key == 'a') {
    		...
    		ret = vendor_videoproc_set(stream[0].proc_path, VENDOR_VIDEOPROC_PARAM_DIS_COMPENSATE, &enable);
