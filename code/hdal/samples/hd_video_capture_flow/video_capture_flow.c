...
#define TEST_CAPTURE_BW    0
#define TEST_CAPTURE_CGAIN 0
...
#define VPRC_ALG_FUNC 	HD_VIDEOPROC_FUNC_WDR | HD_VIDEOPROC_FUNC_DEFOG | HD_VIDEOPROC_FUNC_COLORNR
...
static void *flow_thread(void *arg)
{
	...
	ISPT_C_GAIN isp_c_gain = {0};
	#if (TEST_CAPTURE_CGAIN)
	ISPT_C_GAIN isp_c_gain2 = {0};
	#endif
	#if (TEST_CAPTURE_BW)
	IQT_COLOR_PARAM color = {0};
	IQT_COLOR_PARAM color2 = {0};
	#endif
#if (LOAD_ISP_CFG == 1)
	...
	while (p_stream2->flow_quit == 0 ) {
		...
		if (p_stream2->flow_run == FLOW_ON_SHOT) {
			...
			cfg_info.id = 0;
			if (RESOLUTION_SET == 0) {
				strncpy(cfg_info.path, "/mnt/app/isp/isp_imx290_0_cap.cfg", CFG_NAME_LENGTH);
			} else {
			...
			rt = vendor_isp_get_common(ISPT_ITEM_C_GAIN, &isp_c_gain);
			if (rt == HD_OK) {
				#if (TEST_CAPTURE_CGAIN)
				isp_c_gain2.gain[0] = 256;
				isp_c_gain2.gain[1] = 256;
				isp_c_gain2.gain[2] = 256;
				rt |= vendor_isp_set_common(ISPT_ITEM_C_GAIN, &isp_c_gain2);
				#else
				//printf("set capture CGAIN (%d, %d, %d) \r\n", isp_c_gain.gain[0], isp_c_gain.gain[1], isp_c_gain.gain[2]);
				rt |= vendor_isp_set_common(ISPT_ITEM_C_GAIN, &isp_c_gain);
				#endif
			} else {
				printf("set capture CGAIN fail \r\n");
			}

			#if (TEST_CAPTURE_BW)
			vendor_isp_get_iq(IQT_ITEM_COLOR_PARAM, &color);
			vendor_isp_get_iq(IQT_ITEM_COLOR_PARAM, &color2);
			color2.color.mode = 1;
			color2.color.manual_param.c_con = 0;
			vendor_isp_set_iq(IQT_ITEM_COLOR_PARAM, &color2);
			#endif
			...
			#if (TEST_CAPTURE_CGAIN)
			isp_c_gain.gain[0] = 768;
			isp_c_gain.gain[1] = 256;
			isp_c_gain.gain[2] = 768;
			rt |= vendor_isp_set_common(ISPT_ITEM_C_GAIN, &isp_c_gain);
			#else
			rt = vendor_isp_get_common(ISPT_ITEM_C_GAIN, &isp_c_gain);
			if (rt == HD_OK) {
				//printf("set preview CGAIN (%d, %d, %d) \r\n", isp_c_gain.gain[0], isp_c_gain.gain[1], isp_c_gain.gain[2]);
				rt |= vendor_isp_set_common(ISPT_ITEM_C_GAIN, &isp_c_gain);
			} else {
				printf("set preview CGAIN fail \r\n");
			}
			#endif
