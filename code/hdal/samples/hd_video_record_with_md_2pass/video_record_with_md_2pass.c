/**
	...
	@file video_record_with_md_2pass.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_with_md_2pass, argc, argv)
...
static HD_RESULT set_proc_cfg(HD_OUT_ID ctrl, HD_PATH_ID *p_video_proc_ctrl, HD_VIDEOPROC_PIPE pipe, HD_DIM* p_max_dim)
{
	...
	if (p_max_dim != NULL ) {
		...
		video_cfg_param.in_max.pxlfmt = (pipe == HD_VIDEOPROC_PIPE_RAWALL) ? CAP_OUT_FMT : HD_VIDEO_PXLFMT_YUV420;
...
static HD_RESULT set_proc_param(HD_PATH_ID video_proc_path, HD_DIM* p_dim,UINT32 md_enable)
{
	HD_RESULT ret = HD_OK;
    ...
    if(md_enable)
	{
		...
		HD_VIDEOPROC_FUNC_CONFIG video_path_param = {0};
        //DBGH(video_proc_path);
		video_path_param.out_func = VENDOR_VIDEOPROC_OUTFUNC_COPYMETA;
		ret = vendor_videoproc_set(video_proc_path, VENDOR_VIDEOPROC_PARAM_FUNC_CONFIG, &video_path_param);
	}
	return ret;
}
...
static HD_RESULT open_module(VIDEO_RECORD *p_stream, HD_DIM* p_proc0_max_dim, HD_DIM* p_proc1_max_dim)
{
    ...
    if(pipe_type == 1) {
        pipe = HD_VIDEOPROC_PIPE_YUVAUX;
    } else if(pipe_type == 2) {
        pipe = HD_VIDEOPROC_PIPE_VPE;
    } else {
        pipe = HD_VIDEOPROC_PIPE_SCALE;
    }
	...
	ret = set_proc_cfg(HD_VIDEOPROC_1_CTRL, &p_stream->proc1_ctrl, pipe, p_proc1_max_dim);
	if (ret != HD_OK) {
		printf("set proc-cfg fail=%d\n", ret);
		return HD_ERR_NG;
	}
...
MAIN(argc, argv)
{
	...
	ret = set_proc_param(stream[0].proc1_path, NULL,0);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
