...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_liveview_with_4win, argc, argv)
...
#if !MULTI_VOUT
static UINT32 g_vout_open_cnt = 0;
static UINT32 g_vout_start_cnt = 0;
...
static HD_RESULT set_cap_param(UINT32 stream_id)
{
	...
	{//select sensor mode, manually or automatically
		...
		if (stream_id == 0) {
			...
			video_in_param.sen_mode = HD_VIDEOCAP_PATGEN_MODE(HD_VIDEOCAP_SEN_PAT_RANDOM, 10);
		} else {
			video_in_param.sen_mode = HD_VIDEOCAP_PATGEN_MODE(HD_VIDEOCAP_SEN_PAT_HINCREASE, 20);
		}
		video_in_param.frc = HD_VIDEO_FRC_RATIO(p_stream->vdo_fps,1);
		video_in_param.dim.w = p_dim->w;
		video_in_param.dim.h = p_dim->h;
...
#endif
#if MULTI_VOUT
		video_cfg_param.pipe |= HD_VIDEOPROC_PIPE_COMBINE;
#endif	
...
static HD_RESULT set_proc_param(UINT32 stream_id)
{
	...
	//if (p_dim != NULL) { //if videoproc is already binding to dest module, not require to setting this!
		...
		video_out_param.rect.w = p_dim->w/2;
		video_out_param.rect.h = p_dim->h/2;

		video_out_param.rect.x = (stream_id%2)*(p_dim->w/2);
		video_out_param.rect.y = (stream_id/2)*(p_dim->h/2);
#else
...
static HD_RESULT set_out_cfg(UINT32 stream_id)
{
    ...
    {
    UINT32 status =1;
	ret = vendor_videoout_set(video_out_ctrl, VENDOR_VIDEOOUT_ITEM_MERGE_WND, &status);
	printf("VENDOR_VIDEOOUT_ITEM_MERGE_WND ret=%d\r\n", ret);
    }
    #endif
...
static HD_RESULT set_out_param(UINT32 stream_id)
{
    ...
    video_out_win.visible = 1;
    video_out_win.rect.x = (stream_id%2)*(p_dim->w/2);
    video_out_win.rect.y = (stream_id/2)*(p_dim->h/2);
    video_out_win.rect.w = p_dim->w/2;
    video_out_win.rect.h = p_dim->h/2;
    video_out_win.layer = HD_LAYER2;
    ret =  hd_videoout_set(video_out_path,HD_VIDEOOUT_PARAM_IN_WIN_ATTR, &video_out_win);
	printf("##stream_id %d  video_out_win x:%d,y:%d,w:%d,h:%d \r\n", stream_id,video_out_win.rect.x,video_out_win.rect.y,video_out_win.rect.w, video_out_win.rect.h);
...
static HD_RESULT open_module(UINT32 stream_id)
{
    ...
    printf("open_module stream_id %d \r\n",stream_id);
	// set videocap config
	...
	if ((ret = hd_videoproc_open(HD_VIDEOPROC_IN(p_stream->vproc_id, 0), HD_VIDEOPROC_OUT(p_stream->vproc_id, 0), &p_stream->proc_path)) != HD_OK)
		return ret;
#if MULTI_VOUT
	if ((ret = hd_videoout_open(HD_VIDEOOUT_IN(0,p_stream->vout_id), HD_VIDEOOUT_OUT(0, 0), &p_stream->out_path)) != HD_OK)
		return ret;
#else
...
static HD_RESULT close_module(UINT32 stream_id)
{
	HD_RESULT ret;
	VIDEO_LIVEVIEW *p_stream = &g_stream[stream_id];
    printf("close_module stream_id %d \r\n",stream_id);
#if !MULTI_VOUT
	if (g_vout_open_cnt) {
		g_vout_open_cnt--;
	}
...
static HD_RESULT stream_start(UINT32 stream_id)
{
	...
	p_stream->proc_max_dim.w = p_stream->vdo_size_w; //assign by user
	p_stream->proc_max_dim.h = p_stream->vdo_size_h; //assign by user
...
#else
	hd_videoproc_bind(HD_VIDEOPROC_OUT(p_stream->vproc_id, 0), HD_VIDEOOUT_IN(0,p_stream->vout_id));
#endif
