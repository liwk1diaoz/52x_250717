...
typedef struct _HD_VIDEOENC_BS_RING {
	BOOL enable;                             ///< enable bs ring mode. default: 0, range: 0~1 (0: disable, 1: enable)
} HD_VIDEOENC_BS_RING;
...
typedef enum _HD_VIDEOENC_PARAM_ID {
	...
	HD_VIDEOENC_PARAM_BS_RING,            ///< support get/set with i/o path, using HD_VIDEOENC_BS_RING struct
...
HD_RESULT hd_videoenc_pull_out_buf3(HD_PATH_ID path_id, HD_VIDEOENC_BS3 *p_videoenc_bs, INT32 wait_ms);
HD_RESULT hd_videoenc_release_out_buf(HD_PATH_ID path_id, HD_VIDEOENC_BS *p_videoenc_bs);
HD_RESULT hd_videoenc_release_out_buf3(HD_PATH_ID path_id, HD_VIDEOENC_BS3 *p_videoenc_bs);
