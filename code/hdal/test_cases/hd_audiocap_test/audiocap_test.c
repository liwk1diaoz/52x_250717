/**
	...
	@file test_audio_capture_only.c
...
static void *capture_thread(void *arg)
{
	...
	/* config pattern name */
	snprintf(file_path_main, sizeof(file_path_main), "/mnt/sd/audio_bs_%d_%d_%d_%lu_%d_pcm.dat", HD_AUDIO_BIT_WIDTH_16, p_cap_only->item.sample_rate, p_cap_only->item.mode, p_cap_only->item.volume, p_cap_only->item.mono);
	snprintf(file_path_len, sizeof(file_path_len), "/mnt/sd/audio_bs_%d_%d_%d_%lu_%d_pcm.len", HD_AUDIO_BIT_WIDTH_16, p_cap_only->item.sample_rate, p_cap_only->item.mode, p_cap_only->item.volume, p_cap_only->item.mono);
...
EXAMFUNC_ENTRY(hd_audiocap_test, argc, argv)
{
	HD_RESULT ret;
	//INT key;
