...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(ai_net_with_push, argc, argv)
...
#define BASIC_LOAD_SIZE 1024

// dependent buf
#define DEP_BUF_SIZE    2 * 1024 * 1024  // 2 MB
#define DEP_BUF_CNT     3
#define TOTAL_DEP_SIZE  DEP_BUF_SIZE * DEP_BUF_CNT
...
typedef struct _NET_PROC_DEP {

	MEM_PARM alloc_mem;
	NET_DEP_LOAD dep_load[DEP_BUF_CNT];
...
typedef struct _NET_PROC {
	...
	NET_PROC_DEP net_dep;
	MEM_PARM proc_mem;       // memory for basic model & model header
	//MEM_PARM dep_mem;        // memory for dependent buf loading
	NET_PROC_DEP proc_dep;
	UINT32 proc_id;
...
static UINT32 _load_model(CHAR *filename, UINT32 va, UINT32 file_size)
{
	FILE  *fd;
	UINT32 read_size = 0;
...
static HD_RESULT network_open(NET_PATH_ID net_path)
{
	...
	UINT32 basic_size = BASIC_LOAD_SIZE;
	UINT32 total_dep_size = TOTAL_DEP_SIZE;
	UINT32 loadsize = 0;
	...
	// basic model
	{
		mem_get(&p_net->proc_mem, basic_size, proc_id);
		
		// config basic model
		loadsize = _load_model(p_net->net_cfg.model_filename, p_net->proc_mem.va, basic_size);
		if (loadsize <= 0) {
			printf("proc_id(%u) load basic model fail: %s\r\n", proc_id, p_net->net_cfg.model_filename);
			return HD_ERR_NG;
		}
	...
	// model parm region
	{
		// get model header size
		vendor_ai_net_get(proc_id, VENDOR_AI_NET_PARAM_INFO, &net_info);
		if (!net_info.model_part1_size) {
			printf("proc_id(%u) get net info fail: %s, size=0\r\n", proc_id, p_net->net_cfg.model_filename);
			return HD_ERR_NG;
		}
		mem_get(&p_net->proc_mem, net_info.model_part1_size, proc_id);

		// config parm region
		loadsize = _load_model(p_net->net_cfg.model_filename, p_net->proc_mem.va, net_info.model_part1_size);
		if (loadsize <= 0) {
			printf("proc_id(%u) load basic model fail: %s\r\n", proc_id, p_net->net_cfg.model_filename);
			return HD_ERR_NG;
		}
	...
	{
		int i;
		UINT32 tmp_pa, tmp_va;
		NET_DEP_LOAD *p_load;

		// get max dep buf id
		p_net->net_cfg.dep_max = net_info.model_dep_cnt;

		// get total dep buf
		mem_get(&p_net->proc_dep.alloc_mem, total_dep_size, proc_id);

		// init dep buf
		tmp_pa = p_net->proc_dep.alloc_mem.pa;
		tmp_va = p_net->proc_dep.alloc_mem.va;
		for (i = 0; i < DEP_BUF_CNT; i++) {
			p_load = &p_net->proc_dep.dep_load[i];
			p_load->pa = tmp_pa;
			p_load->va = tmp_va;
			p_load->size = DEP_BUF_SIZE;
			p_load->is_used = 0;
			printf("init dep buf: i=%d, pa=0x%x, va=0x%x, size=0x%x, is_used=%d\n",
				i, p_load->pa, p_load->va, p_load->size, p_load->is_used);
...
static HD_RESULT network_push_in_dep(NET_PATH_ID net_path, UINT32 dep_id)
{
	HD_RESULT ret = HD_OK;
	NET_PROC* p_net = g_net + net_path;
	UINT32 proc_id = p_net->proc_id;
	...
	vendor_ai_net_get(proc_id, VENDOR_AI_NET_PARAM_DEP(dep_id), &load_size); //get data size of dep

	// .... (load data from file) ...

	vendor_ai_net_push_in_buf(proc_id, VENDOR_AI_NET_PARAM_DEP(dep_id), &in_buf, 0); //signal "read ready" to AI CORE

	return ret;
}

static HD_RESULT network_pull_out_dep(NET_PATH_ID net_path, UINT32 dep_id)
{
	HD_RESULT ret = HD_OK;
	NET_PROC* p_net = g_net + net_path;
	UINT32 proc_id = p_net->proc_id;
	...
	vendor_ai_net_pull_out_buf(proc_id, VENDOR_AI_NET_PARAM_DEP(dep_id), &out_buf, -1); //wait "write finish" form AI CORE
...
typedef struct _VIDEO_LIVEVIEW {
	...
	pthread_t  proc_push_thread_id;
	pthread_t  proc_pull_thread_id;
	UINT32 proc_start;
	UINT32 proc_exit;
	UINT32 proc_oneshot;
...
static VOID *network_user_push_thread(VOID *arg);
static VOID *network_user_pull_thread(VOID *arg);
...
static HD_RESULT network_user_start(VIDEO_LIVEVIEW *p_stream)
{
	...
	ret = pthread_create(&p_stream->proc_push_thread_id, NULL, network_user_push_thread, (VOID*)(p_stream));
	if (ret < 0) {
		return HD_ERR_FAIL;
	}
	ret = pthread_create(&p_stream->proc_pull_thread_id, NULL, network_user_pull_thread, (VOID*)(p_stream));
	if (ret < 0) {
		return HD_ERR_FAIL;
	}
...
static HD_RESULT network_user_stop(VIDEO_LIVEVIEW *p_stream)
{
	...
	if (p_stream->proc_push_thread_id) {
		pthread_join(p_stream->proc_push_thread_id, NULL);
	}
	if (p_stream->proc_pull_thread_id) {
		pthread_join(p_stream->proc_pull_thread_id, NULL);
	}
...
static VOID *network_user_push_thread(VOID *arg)
{
	...
	while (p_stream->proc_exit == 0) {
		...
		if (p_stream->proc_oneshot) {
			...
			// push in dep buf
			for (i = 0; i <= p_net->net_cfg.dep_max; i++) {
				// [ToDo] ... lock dep_buf ...
				network_push_in_dep(p_stream->net_path, i);
			}
		}
		usleep(100);
	}
...
static VOID *network_user_pull_thread(VOID *arg)
{
	...
	while (p_stream->proc_exit == 0) {
		...
		if (p_stream->proc_oneshot) {
			...
			// pull out dep buf
			for (i = 0; i <= p_net->net_cfg.dep_max; i++) {
				network_pull_out_dep(p_stream->net_path, i);
				// [ToDo] ... unlock dep buf ...
			}
...
int main(int argc, char *argv[])
{
	...
	if(argc < 3){
		printf("usage : ai_net_with_push (buf_opt)\n");
		return -1;
	}
...
#if 1
	// only support in this option
	net_cfg.job_method = VENDOR_AI_NET_JOB_OPT_LINEAR;
	net_cfg.job_wait_ms = -2; // enable sync mode (sync to dependent buffer ready)
#else
	if (argc > idx) {
