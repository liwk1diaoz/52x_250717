...
static HD_RESULT network_alloc_ronly_buf(NET_PATH_ID net_path)
{
	HD_RESULT ret = HD_OK;
	NET_PROC* p_net = g_net + net_path;
	UINT32 proc_id = p_net->proc_id;
	...
	ret = vendor_ai_net_get(proc_id, VENDOR_AI_NET_PARAM_CFG_RONLYBUF, &rbuf );
	if (ret != HD_OK) {
		printf("proc_id(%lu) get VENDOR_AI_NET_PARAM_CFG_RONLYBUF fail\r\n", proc_id);
		return HD_ERR_FAIL;
	}
	ret = mem_alloc(&p_net->rbuf_mem, "ai_rbuf_buf", rbuf.size);
	if (ret != HD_OK) {
		printf("proc_id(%lu) alloc ai_io_buf fail\r\n", proc_id);
		return HD_ERR_FAIL;
	}

	rbuf.pa = p_net->rbuf_mem.pa;
	rbuf.va = p_net->rbuf_mem.va;
	rbuf.size = p_net->rbuf_mem.size;
	ret = vendor_ai_net_set(proc_id, VENDOR_AI_NET_PARAM_CFG_RONLYBUF, &rbuf);
	if (ret != HD_OK) {
		printf("proc_id(%lu) set VENDOR_AI_NET_PARAM_CFG_RONLYBUF fail\r\n", proc_id);
		return HD_ERR_FAIL;
	}

	printf("alloc_ronly_buf: ronly buf, pa = %#lx, va = %#lx, size = %lu\r\n", rbuf.pa, rbuf.va, rbuf.size);
...
static HD_RESULT network_free_ronly_buf(NET_PATH_ID net_path)
{
	HD_RESULT ret = HD_OK;
	NET_PROC* p_net = g_net + net_path;

	if (p_net->rbuf_mem.pa && p_net->rbuf_mem.va) {
		mem_free(&p_net->rbuf_mem);
	}
	return ret;
}
...
static HD_RESULT network_open(NET_PATH_ID net_path)
{
	...
	if (p_net->net_cfg.p_share_model == NULL ) {
		...
		// set model
		vendor_ai_net_set(proc_id, VENDOR_AI_NET_PARAM_CFG_MODEL, (VENDOR_AI_NET_CFG_MODEL*)&p_share_net->proc_mem);
	}
	...
	if (share_mode == 2){
		if ((ret = network_alloc_ronly_buf(net_path)) != HD_OK)
		return ret;
	}
...
static HD_RESULT network_close(NET_PATH_ID net_path)
{
	...
	if ((ret = network_free_io_buf(net_path)) != HD_OK)
		return ret;
	if (share_mode == 2){
		if ((ret = network_free_ronly_buf(net_path)) != HD_OK)
			return ret;
	}	
	// close
