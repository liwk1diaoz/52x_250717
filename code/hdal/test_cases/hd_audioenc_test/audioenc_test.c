...
typedef struct _TEST_ITEM {
	...
	UINT32					enc_type;
} TEST_ITEM_USER_ENCONLY;
typedef struct _AUDIO_ENCODE {
	TEST_ITEM_USER_ENCONLY	item;
...
///////////////////////////////////////////////////////////////////////////////
static HD_RESULT set_enc_cfg(HD_PATH_ID audio_enc_path, TEST_ITEM_USER_ENCONLY *p_item)
{
	HD_RESULT ret = HD_OK;
	HD_AUDIOENC_PATH_CONFIG audio_path_cfg = {0};

	// set audioenc path config
	audio_path_cfg.max_mem.codec_type = p_item->enc_type;
...
static HD_RESULT set_enc_param(HD_PATH_ID audio_enc_path, TEST_ITEM_USER_ENCONLY *p_item)
{
	HD_RESULT ret = HD_OK;
	...
	audio_in_param.sample_rate = p_item->sample_rate;
	audio_in_param.sample_bit = HD_AUDIO_BIT_WIDTH_16;
	audio_in_param.mode = p_item->mode;
	...
	audio_out_param.codec_type = p_item->enc_type;
	audio_out_param.aac_adts = (p_item->enc_type == HD_AUDIO_CODEC_AAC) ? TRUE : FALSE;
...
static void *feed_frm_thread(void *arg)
{
	...
	// assign audio info
	HD_AUDIO_SR sample_rate = p_stream0->item.sample_rate;
	HD_AUDIO_BIT_WIDTH sample_bit = HD_AUDIO_BIT_WIDTH_16;
	HD_AUDIO_SOUND_MODE mode = p_stream0->item.mode;

	// open input files
	snprintf(file_path, sizeof(file_path), "/mnt/sd/audio_bs_%d_%d_%d_100_0_pcm.dat", sample_bit, sample_rate, mode);
	...
	snprintf(file_path, sizeof(file_path), "/mnt/sd/audio_bs_%d_%d_%d_100_0_pcm.len", sample_bit, sample_rate, mode);
	...
	while (p_stream0->feed_exit == 0) {
		...
		au_frame_ms = frm_size * 1000 / HD_AUDIO_SR_48000; // the time(in ms) of each audio frame
...
static void *encode_thread(void *arg)
{
	...
	HD_AUDIO_BIT_WIDTH sample_bit = HD_AUDIO_BIT_WIDTH_16;
	HD_AUDIO_SOUND_MODE mode = p_stream0->item.mode;
	switch (p_stream0->item.enc_type) {
		...
		case HD_AUDIO_CODEC_ALAW:
			...
			break;
		default:
			printf("invalid audio codec(%d)\n", p_stream0->item.enc_type);
	}

	// config pattern name
	snprintf(file_path_main, sizeof(file_path_main), "/mnt/sd/dump_bs_%d_%d_%d_%s.dat", sample_bit, sample_rate, mode, codec_name);
	snprintf(file_path_len, sizeof(file_path_len), "/mnt/sd/dump_bs_%d_%d_%d_%s.len", sample_bit, sample_rate, mode, codec_name);
...
static void DumpParam(TEST_ITEM_USER_ENCONLY *ToPrint){
	...
	switch (ToPrint->enc_type){
		case HD_AUDIO_CODEC_PCM:
			printf("enc_type = HD_AUDIO_CODEC_PCM(%d)\n", HD_AUDIO_CODEC_PCM);
			break;
		case HD_AUDIO_CODEC_AAC:
			printf("enc_type = HD_AUDIO_CODEC_AAC(%d)\n", HD_AUDIO_CODEC_AAC);
			break;
		case HD_AUDIO_CODEC_ADPCM:
			printf("enc_type = HD_AUDIO_CODEC_ADPCM(%d)\n", HD_AUDIO_CODEC_ADPCM);
			break;
		case HD_AUDIO_CODEC_ULAW:
			printf("enc_type = HD_AUDIO_CODEC_ULAW(%d)\n", HD_AUDIO_CODEC_ULAW);
			break;
		case HD_AUDIO_CODEC_ALAW:
			printf("enc_type = HD_AUDIO_CODEC_ALAW(%d)\n", HD_AUDIO_CODEC_ALAW);
			break;
		default:
			printf("Unknown enc_type!\n");
	}
	printf("======DUMPING END======\n");
}

EXAMFUNC_ENTRY(hd_audioenc_test, argc, argv)
{
	HD_RESULT ret;
	AUDIO_ENCODE stream[1] = {0}; //0: main stream
	UINT32 sample_rate_argv = 0;
	UINT32 mode_argv = 0;
	UINT32 enc_type_argv = 0;

	// query program options (only support PCM and AAC)
	//if (argc == 2) {
	...
	/******CODEC TYPE OPTION******/
	/*HD_AUDIO_CODEC_PCM	  = 1*/
	/*HD_AUDIO_CODEC_AAC	  = 2*/
	/*HD_AUDIO_CODEC_ADPCM	  = 3*/
	/*HD_AUDIO_CODEC_ULAW	  = 4*/
	/*HD_AUDIO_CODEC_ALAW	  = 5*/
	/*****************************/
	...
	//set audioenc parameter
	if (argc > 1){
		sample_rate_argv = strtoul(argv[1], NULL, 10);
		switch (sample_rate_argv){
			case 8000:
				//sample rate = 8000
				stream[0].item.sample_rate = HD_AUDIO_SR_8000;
				break;
			case 11025:
				//sample rate = 11025
				stream[0].item.sample_rate = HD_AUDIO_SR_11025;
				break;
			case 12000:
				//sample rate = 12000
				stream[0].item.sample_rate = HD_AUDIO_SR_12000;
				break;
			case 16000:
				//sample rate = 16000
				stream[0].item.sample_rate = HD_AUDIO_SR_16000;
				break;
			case 22050:
				//sample rate = 22050
				stream[0].item.sample_rate = HD_AUDIO_SR_22050;
				break;
			case 24000:
				//sample rate = 24000
				stream[0].item.sample_rate = HD_AUDIO_SR_24000;
				break;
			case 32000:
				//sample rate = 32000
				stream[0].item.sample_rate = HD_AUDIO_SR_32000;
				break;
			case 44100:
				//sample rate = 44100
				stream[0].item.sample_rate = HD_AUDIO_SR_44100;
				break;
			case 48000:
			default:
				//sample rate = 48000
				stream[0].item.sample_rate = HD_AUDIO_SR_48000;
		}
	}
	if (argc > 2){
		...
		switch (mode_argv){
			case 1:
				//mode = mono
				stream[0].item.mode = HD_AUDIO_SOUND_MODE_MONO;
				break;
			case 2:
			default:
				//mode = stereo
				stream[0].item.mode = HD_AUDIO_SOUND_MODE_STEREO;
		}
	}
	if (argc > 3){
		enc_type_argv = strtoul(argv[3], NULL, 10);
		switch (enc_type_argv){
			case HD_AUDIO_CODEC_PCM:
				//enc_type = 1
				stream[0].item.enc_type = HD_AUDIO_CODEC_PCM;
				break;
			default:
			case HD_AUDIO_CODEC_AAC:
				//enc_type = 2
				stream[0].item.enc_type = HD_AUDIO_CODEC_AAC;
				break;
			case HD_AUDIO_CODEC_ADPCM:
				//enc_type = 3
				stream[0].item.enc_type = HD_AUDIO_CODEC_ADPCM;
				break;
			case HD_AUDIO_CODEC_ULAW:
				//enc_type = 4
				stream[0].item.enc_type = HD_AUDIO_CODEC_ULAW;
				break;
			case HD_AUDIO_CODEC_ALAW:
				//enc_type = 5
				stream[0].item.enc_type = HD_AUDIO_CODEC_ALAW;
		}
	}
	// dump item
	DumpParam(&stream[0].item);
    // open all modules
	...
	// set audioenc config
	ret = set_enc_cfg(stream[0].enc_path, &stream[0].item);
	if (ret != HD_OK) {
	...
	ret = set_enc_param(stream[0].enc_path, &stream[0].item);
	if (ret != HD_OK) {
