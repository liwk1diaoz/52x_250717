...
#define SEN_SEL_GC4663   2   //4M_SHDR
...
HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl, HD_CTRL_ID ctrl_id)
{
	...
	if (sensor_sel == SEN_SEL_TC358840) {
		...
		if (chip_name != NULL && strcmp(chip_name, "CHIP_NT98562") == 0) {
...
int main(int argc, char** argv)
{
	...
	if (argc == 1) {
		...
		printf("  <sensor> :  0(IMX290) , 1(OS05A), 2(GC4663)\r\n");
		return 0;
	}
	...
	if (argc >= 12) {
		ai_mode = atoi(argv[11]);
		printf("ai_mode %d\r\n", ai_mode);
	}
	if (argc >= 13) {
		save_ai_result = atoi(argv[12]);
		printf("Whether to save the PD or FD results: %d\r\n", save_ai_result);
	}
    ...
    // start demo
    {
		printf("start demo\r\n");
		g_enc_poll = 0;
		system("alg_pdcnn_sample_stream_ipc 10 1 &");
		system("nvtrtspd_ipc &");
		while (1) {
			sleep(1);
		}
	...
	while (1) {
		...
		if (key == 'c') {
			...
			if(save_ai_result){
				if(ai_mode == 0){
					system("ai2_pdcnn_sample_stream_ipc 10 1 1 &");
				}
				else if(ai_mode == 1){
					system("ai2_fdcnn_light_sample_stream_ipc 10 1 &");
				}
				else if(ai_mode == 2){
					system("alg_cddcnn_sample_stream_ipc 10 1 1 &");
				}
				else{
					printf("ai mode selection error, please re-enter, only supports 0, 1 or 2!\r\n");
					goto exit;
				}
			}
			else{
				if(ai_mode == 0){
					...
					system("ai2_fdcnn_light_sample_stream_ipc 10 0 &");
				}
				else if(ai_mode == 2){
					system("alg_cddcnn_sample_stream_ipc 10 1 0 &");
				}
				else{
					printf("ai mode selection error, please re-enter, only supports 0, 1 or 2!\r\n");
					goto exit;
