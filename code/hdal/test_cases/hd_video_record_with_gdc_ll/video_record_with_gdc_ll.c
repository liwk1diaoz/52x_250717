...
static UINT32 g_capbind = 0;  //fixed //0:D2D, 1:direct, 2: one-buf, 0xff: no-bind
static UINT32 g_capfmt = 0; //fixed //0:RAW, 1:RAW-compress
static UINT32 g_prcfmt = 0; //fixed //0:YUV, 1:YUV-compress

static UINT32 g_prcbind = 0; //0:D2D, 1:lowlatency, 2: one-buf, 0xff: no-bind
static UINT32 g_gdc = 0; //0: OFF, 1: ON
static UINT32 g_dumptime = 0; //0: OFF, 1: ON
static UINT32 g_crop_w = 0;
static UINT32 g_crop_h = 0;
...
static HD_RESULT set_proc_param(HD_PATH_ID video_proc_path, HD_DIM* p_dim)
{
	...
	if (g_crop_w == 0 && g_crop_h == 0) {
		//HD_CROP_OFF
		HD_VIDEOPROC_CROP video_crop_param = {0};

		video_crop_param.mode = HD_CROP_OFF;
		ret = hd_videoproc_set(video_proc_path, HD_VIDEOPROC_PARAM_IN_CROP, &video_crop_param);
	} else {
		//HD_CROP_ON
		HD_VIDEOPROC_CROP video_crop_param = {0};
...
static void *encode_thread(void *arg)
{
	...
	while (p_stream0->enc_exit == 0) {
		...
		if (ret == HD_OK) {
			if (g_dumptime) printf("[LL] diff = %llu us\n", hd_gettime_us() - data_pull.timestamp);
...
MAIN(argc, argv)
{
	...
	if (argc == 1 || argc > 7) {
		printf("Usage: <enc-type> <gdc> <ll> <dump_time> <crop_w> <crop_h>\r\n");
		printf("Help:\r\n");
		printf("  <enc_type> : 0(H265), 1(H264), 2(MJPG)\r\n");
		printf("  <gdc>      : 0(OFF), 1(ON)\r\n");
		printf("  <ll>       : 0(OFF), 1(ON)\r\n");
		printf("  <dumptime> : 0(OFF), 1(ON)\r\n");
		printf("  <crop_w>   : 0(no crop), >0(crop)\r\n");
		printf("  <crop_h>   : 0(no crop), >0(crop)\r\n");
		return 0;
	}
	...
	if (argc > 2) {
		...
		printf("gdc %d\r\n", g_gdc);
		if(g_gdc > 1) {
			printf("error: not support <gdc> (%d)!\r\n", g_gdc);
			return 0;
		}
	}
	if (argc > 3) {
		g_prcbind = atoi(argv[3]);  //0:D2D, 1:lowlatency
		printf("ll %d\r\n", g_prcbind);
		if(g_prcbind > 1) {
			printf("error: not support <ll> (%d)!\r\n", g_prcbind);
			return 0;
		}
	}
	if (argc > 4) {
		g_dumptime = atoi(argv[4]);
		printf("dumptime %d\r\n", g_dumptime);
		if(g_dumptime > 1) {
			printf("error: not support <dumptime>! (%d) \r\n", g_dumptime);
			return 0;
		}
	}
	if (argc > 5) {
		g_crop_w = atoi(argv[5]);
		printf("CROP_W %d\r\n", g_crop_w);
		if(g_crop_w > VDO_SIZE_W) {
			printf("error: not support <crop_w> ! (%d) , must <= sensor width(%d)\r\n", g_crop_w, VDO_SIZE_W);
			return 0;
		}
	}
	if (argc > 6) {
		g_crop_h = atoi(argv[6]);
		printf("CROP_H %d\r\n", g_crop_h);
		if(g_crop_h > VDO_SIZE_H) {
			printf("error: not support <crop_h> ! (%d) , must <= sensor height(%d)\r\n", g_crop_h, VDO_SIZE_H);
			return 0;
		}
	}
		// assign g_capfmt
	...
	// set sysconfig
	if (g_gdc == 0) {
		hd_common_sysconfig(HD_VIDEOPROC_CFG, 0, HD_VIDEOPROC_CFG_DISABLE_GDC, 0);  // default GDC = ENABLE, so disable GDC
	}
	...
	main_dim.w = (g_crop_w == 0)? VDO_SIZE_W : g_crop_w;
	main_dim.h = (g_crop_h == 0)? VDO_SIZE_H : g_crop_h;
	// set videoproc parameter (main)
	ret = set_proc_param(stream[0].proc_path, &main_dim); //set vprc's out dim & crop dim
	if (ret != HD_OK) {
