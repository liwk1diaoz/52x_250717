...
static HD_RESULT set_enc_param(HD_PATH_ID video_enc_path, HD_DIM *p_dim, UINT32 enc_type, UINT32 bitrate)
{
	...
	if (p_dim != NULL) {
		...
		video_in_param.dim.w   = p_dim->w;
		video_in_param.dim.h   = p_dim->h;
		video_in_param.frc     = HD_VIDEO_FRC_RATIO(30,1); // trigger Normal_TimerMode with framerate 30. This could be overwrite by set VENDOR_VIDEOENC_PARAM_OUT_TIMELAPSE_TIME later to trigger Timelapse_TimerMode
		...
		//--- vendor command ---
		{
			#define TS_VID_SIZE    21     // PES_HEADER_LENGTH(14) + NAL_LENGTH(7) // NAL => H264 need 6 (0x00 00 00 01 09 10) / h265 need 7 (0x00 00 00 01 46 01 10)
			#define TS_AUD_SIZE    7      // ADTS_LENGTH(7)
			#define JPG_EXIF_SIZE  65536
...

		{
			...
			timelapse_cfg.timelapse_time = 5000; // 5 second    ( >0 : Timelapse_TimerMode , =0 : Normal_TimerMode )
			vendor_videoenc_set(video_enc_path, VENDOR_VIDEOENC_PARAM_OUT_TIMELAPSE_TIME, &timelapse_cfg);
		}
...
static void *encode_thread(void *arg)
{
	...
	while (p_stream0->enc_exit == 0) {
		//pull data
		ret = hd_videoenc_pull_out_buf(p_stream0->enc_path, &data_pull, 300); // timeout = 300 ms
...
EXAMFUNC_ENTRY(hd_video_record_with_timelapse, argc, argv)
{
	HD_RESULT ret;
	INT key;
