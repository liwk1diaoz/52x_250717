...
#define SEN_SEL_OS02K10  1
#define SEN_SEL_AR0237IR 2
#define SEN_SEL_IMX415   3   //8M_SHDR
#define SEN_SEL_OS05A10  4   //5M_SHDR
#define SEN_SEL_IMX317   5
#define SEN_SEL_IMX335   6
#define SEN_SEL_F37      7

/*
#define SENSOR_IMX291		1 //2M
#define SENSOR_IMX290		2 //2M_SHDR
#define SENSOR_OS05A10		5 //5M
#define SENSOR_IMX317		8 //8M_SHDR

#define DIM_0				0
#define DIM_250K			250 //250K
#define DIM_500K			500 //500K
#define DIM_1M				1000 //1M
#define DIM_2M				2000 //2M
#define DIM_4M				4000 //4M
...
#define MODEL_SENSOR		SENSOR_OS05A10
#define MODEL_SUB			DIM_2M
#define MODEL_DISP			DISP_LCD
#define MODEL_SUB2			DIM_1M
*/
...
#define FUNC_RELAY		    ENABLE // DISABLE: bind / ENABLE: NO bind, using relay_yuv_thread to pull_out then push_in

#define FUNC_ISP			ENABLE

#define FUNC_DEFOG			ENABLE
...
#define FUNC_512Mb          DISABLE

#define H26X_COL_MV         ENABLE

#define TEST_VPRC_EXTEND_PERF      DISABLE

static UINT32 vdo_size_w = VDO_SIZE_W_5M, vdo_size_h = VDO_SIZE_H_5M;
static UINT32 subvdo_size_w = VDO_SIZE_W_2M, subvdo_size_h = VDO_SIZE_H_2M;
static UINT32 sub2vdo_size_w = VDO_SIZE_W_VGA, sub2vdo_size_h = VDO_SIZE_H_VGA;
#if (MODEL_DISP != DISP_NONE)
static UINT32 disp_size_w = 960, disp_size_h = 240;
#endif
static UINT32 vdo_br     = (8 * 1024 * 1024);
static UINT32 subvdo_br  = (2 * 1024 * 1024);
static UINT32 sub2vdo_br = (300 * 1024);

static UINT32 shdr_mode = 1;
...
static INT32 g_out_type = 1; // default is LCD mode
static UINT32 g_yuvcompress = 0;  //0x0: noraml, 0x1: yuv compress
static UINT32 g_osg = 1;  //0x0: disable, 0x1: osg enable
static UINT32 g_rotate = 0;  //0x0: disable, 0x1: rotate 90
static INT32 g_enc3_type = 0;
static UINT32 g_ai_new = 1;
...
#if (MODEL_DISP != DISP_NONE)
#define DISP_VDO_SIZE_W	disp_size_w
#define DISP_VDO_SIZE_H	disp_size_h
#endif
...
#define STAMP_WIDTH     ALIGN_CEIL(vdo_size_w, 8)
...
#define SUB_STAMP_WIDTH     ALIGN_CEIL(subvdo_size_w, 8)
...
#define SUB2_STAMP_WIDTH   ALIGN_CEIL(sub2vdo_size_w, 8)
#define SUB2_STAMP_HEIGHT  ALIGN_CEIL(sub2vdo_size_h/8, 8)
#define SUB2_STAMP_PXLFMT  HD_VIDEO_PXLFMT_ARGB1555
#define SUB2_STAMP_X       (0)
#define SUB2_STAMP_Y  	   (sub2vdo_size_h*7/8)
#define SUB2_STAMP_C 	   ((0x1 << 15) | (0x0f << 10) | (0x1f << 5) | (0x0f)) //green

#if (MODEL_DISP != DISP_NONE)
#define DISP_STAMP_WIDTH   ALIGN_CEIL(disp_size_w, 8)
#define DISP_STAMP_HEIGHT  ALIGN_CEIL(disp_size_h/8, 8)
#define DISP_STAMP_PXLFMT  HD_VIDEO_PXLFMT_ARGB1555
#define DISP_STAMP_X       (0)
#define DISP_STAMP_Y       (disp_size_h*7/8)
#define DISP_STAMP_C 	   ((0x1 << 15) | (0x0f << 10) | (0x0f << 5) | (0x1f)) //blue
#endif
...
#if (MODEL_SENSOR == SENSOR_IMX291)
#define FUNC_SHDR			DISABLE
#define SEN_OUT_FMT		HD_VIDEO_PXLFMT_RAW12
#define CAP_OUT_FMT		HD_VIDEO_PXLFMT_RAW12
#define VDO_SIZE_W		1920
#define VDO_SIZE_H		1080
#define VDO_BR     		(1 * 1024 * 1024)
#endif
#if (MODEL_SENSOR == SENSOR_IMX290)
#define FUNC_SHDR			ENABLE
#if (FUNC_SHDR == ENABLE)
#define SEN_OUT_FMT		HD_VIDEO_PXLFMT_RAW10_SHDR2
#define CAP_OUT_FMT		HD_VIDEO_PXLFMT_RAW12_SHDR2
#else
#define SEN_OUT_FMT		HD_VIDEO_PXLFMT_RAW12
#define CAP_OUT_FMT		HD_VIDEO_PXLFMT_RAW12
#endif
...
#define VDO_BR     		(1 * 1024 * 1024)
#endif
#if (MODEL_SENSOR == SENSOR_OS05A10)
#define FUNC_SHDR			ENABLE
#if (FUNC_SHDR == ENABLE)
#define SEN_OUT_FMT		HD_VIDEO_PXLFMT_NRX12_SHDR2
#define CAP_OUT_FMT		HD_VIDEO_PXLFMT_NRX12_SHDR2
#else
#define SEN_OUT_FMT		HD_VIDEO_PXLFMT_RAW12
#define CAP_OUT_FMT		HD_VIDEO_PXLFMT_RAW12
#endif
...
#define VDO_BR     		(3 * 1024 * 1024 /2)
#endif
#if (MODEL_SENSOR == SENSOR_IMX317)
#define FUNC_SHDR			ENABLE
#if (FUNC_SHDR == ENABLE)
#define SEN_OUT_FMT		HD_VIDEO_PXLFMT_RAW10_SHDR2
#define CAP_OUT_FMT		HD_VIDEO_PXLFMT_NRX12_SHDR2 //NOTE! CAP must enable RAW-compress for 4Kp60 or 4Kp30+SHDR
#else
#define SEN_OUT_FMT		HD_VIDEO_PXLFMT_RAW12
#define CAP_OUT_FMT		HD_VIDEO_PXLFMT_RAW12
#endif
...
#define VDO_BR     		(2 * 1024 * 1024)
#endif

#if (MODEL_SUB == DIM_0)
#define SUB_VDO_SIZE_W	0
#define SUB_VDO_SIZE_H	0
#define SUB_VDO_BR		(0)
#endif
#if (MODEL_SUB == DIM_250K)
#define SUB_VDO_SIZE_W	640
#define SUB_VDO_SIZE_H	360
#define SUB_VDO_BR		(1 * 1024 * 1024 /8)
#endif
#if (MODEL_SUB == DIM_500K)
#define SUB_VDO_SIZE_W	960
#define SUB_VDO_SIZE_H	540
#define SUB_VDO_BR		(1 * 1024 * 1024 /4)
#endif
#if (MODEL_SUB == DIM_1M)
#define SUB_VDO_SIZE_W	1280
#define SUB_VDO_SIZE_H	720
#define SUB_VDO_BR		(1 * 1024 * 1024 /2)
#endif
#if (MODEL_SUB == DIM_2M)
#define SUB_VDO_SIZE_W	1920
#define SUB_VDO_SIZE_H	1080
#define SUB_VDO_BR		(1 * 1024 * 1024)
#endif
#if (MODEL_SUB == DIM_4M)
#define SUB_VDO_SIZE_W	2208
#define SUB_VDO_SIZE_H	1696
#define SUB_VDO_BR		(2 * 1024 * 1024)
#endif

#if (MODEL_DISP == DISP_NONE)
#define DISP_VDO_SIZE_W	0
#define DISP_VDO_SIZE_H	0
#endif
#if (MODEL_DISP == DISP_LCD)
#define DISP_VDO_SIZE_W	960
#define DISP_VDO_SIZE_H	240
#endif
#if (MODEL_DISP == DISP_CVBS)
#define DISP_VDO_SIZE_W	720
#define DISP_VDO_SIZE_H	572
#endif
#if (MODEL_DISP == DISP_HDMI)
#define DISP_VDO_SIZE_W	1920
#define DISP_VDO_SIZE_H	1080
#endif


#if (MODEL_SUB2 == DIM_500K)
#define SUB2_VDO_SIZE_W	960
#define SUB2_VDO_SIZE_H	540
#define SUB2_VDO_BR		(1 * 1024 * 1024 /4)
#endif
#if (MODEL_SUB2 == DIM_1M)
#define SUB2_VDO_SIZE_W	1280
#define SUB2_VDO_SIZE_H	720
#define SUB2_VDO_BR		(1 * 1024 * 1024 /2)
#endif
...
#define STAMP_WIDTH   ALIGN_CEIL(VDO_SIZE_W, 8)
#define STAMP_HEIGHT  ALIGN_CEIL(VDO_SIZE_H/4, 8)
#define STAMP_PXLFMT	HD_VIDEO_PXLFMT_ARGB1555
#define STAMP_X   	(0)
#define STAMP_Y  		(VDO_SIZE_H*3/8)
#define STAMP_C 		((0x1 << 15) | (0x1f << 10) | (0x0f << 5) | (0x0f)) //red

#define SUB_STAMP_WIDTH   ALIGN_CEIL(SUB_VDO_SIZE_W, 2)
#define SUB_STAMP_HEIGHT  ALIGN_CEIL(SUB_VDO_SIZE_H/4, 2)
#define SUB_STAMP_PXLFMT	HD_VIDEO_PXLFMT_ARGB1555
#define SUB_STAMP_X   (0)
#define SUB_STAMP_Y  	(SUB_VDO_SIZE_H*3/8)
#define SUB_STAMP_C 	((0x1 << 15) | (0x0f << 10) | (0x1f << 5) | (0x0f)) //green

#if (MODEL_DISP != DISP_NONE)
#define DISP_STAMP_WIDTH   ALIGN_CEIL(DISP_VDO_SIZE_W, 8)
#define DISP_STAMP_HEIGHT  ALIGN_CEIL(DISP_VDO_SIZE_H/4, 8)
#define DISP_STAMP_PXLFMT	HD_VIDEO_PXLFMT_ARGB1555
#define DISP_STAMP_X  (0)
#define DISP_STAMP_Y  (DISP_VDO_SIZE_H*3/8)
#define DISP_STAMP_C 	((0x1 << 15) | (0x0f << 10) | (0x0f << 5) | (0x1f)) //blue
#endif

#define SUB2_STAMP_WIDTH   ALIGN_CEIL(SUB2_VDO_SIZE_W, 2)
#define SUB2_STAMP_HEIGHT  ALIGN_CEIL(SUB2_VDO_SIZE_H/4, 2)
#define SUB2_STAMP_PXLFMT	HD_VIDEO_PXLFMT_ARGB1555
#define SUB2_STAMP_X   (0)
#define SUB2_STAMP_Y  	(SUB2_VDO_SIZE_H*3/8)
#define SUB2_STAMP_C 	((0x1 << 15) | (0x0f << 10) | (0x1f << 5) | (0x0f)) //green

*/

static int    g_quit = 0;
static UINT32 g_sub2_en = 0;
...
#if TEST_VPRC_EXTEND_PERF
#define EXTEND_VDO_SIZE_W      1280
#define EXTEND_VDO_SIZE_H      720
#endif

#define NN_TOTAL_MEM_SIZE   (0x580000)//(NETWORKS_MEM_SIZE + POSTPROC_MEM_SIZE)
...
HD_RESULT mem_init(void)
{
	...
	id = 0;
	mem_cfg.pool_info[id].type = HD_COMMON_MEM_COMMON_POOL; // HD_COMMON_MEM_COMMON_POOL;
	if (g_capbind == 1) {
		...
		mem_cfg.pool_info[id].blk_size = DBGINFO_BUFSIZE()+VDO_NRX_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H)
        													+VDO_CA_BUF_SIZE(CA_WIN_NUM_W, CA_WIN_NUM_H)
        													+VDO_LA_BUF_SIZE(LA_WIN_NUM_W, LA_WIN_NUM_H);
	}
	...
	id ++;
	mem_cfg.pool_info[id].type = HD_COMMON_MEM_COMMON_POOL;
	mem_cfg.pool_info[id].blk_size = DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(DISP_VDO_SIZE_W, DISP_VDO_SIZE_H, HD_VIDEO_PXLFMT_YUV420);
	mem_cfg.pool_info[id].blk_cnt = 3;
	mem_cfg.pool_info[id].ddr_id = DDR_ID0;
    }
#endif
	...
	// config common pool (osg disp)
	id ++;
	mem_cfg.pool_info[id].type = HD_COMMON_MEM_USER_DEFINIED_POOL+2;
	mem_cfg.pool_info[id].blk_size = osg_query_buf_size(DISP_STAMP_WIDTH, DISP_STAMP_HEIGHT, DISP_STAMP_PXLFMT);
	mem_cfg.pool_info[id].blk_cnt = 1;
	mem_cfg.pool_info[id].ddr_id = DDR_ID0;
    }
#endif
	id ++;
	mem_cfg.pool_info[id].type = HD_COMMON_MEM_USER_DEFINIED_POOL+3;
	mem_cfg.pool_info[id].blk_size = osg_query_buf_size(SUB2_STAMP_WIDTH, SUB2_STAMP_HEIGHT, SUB2_STAMP_PXLFMT);
	mem_cfg.pool_info[id].blk_cnt = 1;
	mem_cfg.pool_info[id].ddr_id = DDR_ID0;
    }
#endif
    ...
    if(g_ai_new) {
	    mem_cfg.pool_info[id].blk_size = NN_TOTAL_MEM_SIZE;
	} else {
	...
	mem_cfg.pool_info[id].blk_cnt = 1;
	mem_cfg.pool_info[id].ddr_id = DDR_ID1;

	#if TEST_VPRC_EXTEND_PERF
	id ++;
	mem_cfg.pool_info[id].type = HD_COMMON_MEM_COMMON_POOL;
	mem_cfg.pool_info[id].blk_size = DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(ALIGN_CEIL_16(EXTEND_VDO_SIZE_W), ALIGN_CEIL_16(EXTEND_VDO_SIZE_H), HD_VIDEO_PXLFMT_YUV420);
...
/*
	...
	if (1) {
		...
		cfg_info.id = 0;
		strncpy(cfg_info.path, "/etc/isp/isp_imx291_0.cfg", CFG_NAME_LENGTH);   //using vendor API to load isp config for imx391
	...
	if (0) {
		...
		cfg_info.id = 0;
		strncpy(cfg_info.path, "/etc/isp/isp_imx290_0.cfg", CFG_NAME_LENGTH);   //using vendor API to load isp config for imx391
		vendor_isp_set_iq(IQT_ITEM_RLD_CONFIG, &cfg_info);
		#if (FUNC_GDC == ENABLE)
		{
			IQT_LDC_PARAM ldc = {0};
	...
	cap_cfg.sen_cfg.sen_dev.pin_cfg.sen_2_serial_pin_map[6] = HD_VIDEOCAP_SEN_IGNORE;
	cap_cfg.sen_cfg.sen_dev.pin_cfg.sen_2_serial_pin_map[7] = HD_VIDEOCAP_SEN_IGNORE;
	cap_cfg.sen_cfg.shdr_map = HD_VIDEOCAP_SHDR_MAP(HD_VIDEOCAP_HDR_SENSOR1, (HD_VIDEOCAP_0|HD_VIDEOCAP_2));
#else
	cap_cfg.sen_cfg.sen_dev.pin_cfg.sen_2_serial_pin_map[0] = 0;
	cap_cfg.sen_cfg.sen_dev.pin_cfg.sen_2_serial_pin_map[1] = 1;
	cap_cfg.sen_cfg.sen_dev.pin_cfg.sen_2_serial_pin_map[2] = HD_VIDEOCAP_SEN_IGNORE;
	...
	///////////////////////////////////////////////////////////////////
	#if (FUNC_ISP == ENABLE)
	if (ctrl_id == HD_VIDEOCAP_0_CTRL) {
		IQT_CFG_INFO cfg_info = {0};
		if (vendor_isp_init() != HD_OK) {
			return -1;
		}
		cfg_info.id = 0;
		strncpy(cfg_info.path, "/mnt/app/isp/isp_os05a10_0.cfg", CFG_NAME_LENGTH);   //using vendor API to load isp config for imx391
		vendor_isp_set_iq(IQT_ITEM_RLD_CONFIG, &cfg_info);
		#if (FUNC_GDC == ENABLE)
		{
			IQT_LDC_PARAM ldc = {0};
	...
	if (ctrl_id == HD_VIDEOCAP_0_CTRL) {
		...
		cfg_info.id = 0;
		strncpy(cfg_info.path, "/etc/isp/isp_imx317_0.cfg", CFG_NAME_LENGTH);   //using vendor API to load isp config for imx317
		vendor_isp_set_iq(IQT_ITEM_RLD_CONFIG, &cfg_info);
		#if (FUNC_GDC == ENABLE)
		{
			IQT_LDC_PARAM ldc = {0};
...
HD_RESULT set_proc_param(HD_PATH_ID video_proc_path, HD_DIM* p_dim, HD_VIDEO_PXLFMT pxlfmt, BOOL pull,UINT32 b_one_buf)
{
	HD_RESULT ret = HD_OK;
...
HD_RESULT set_enc_cfg(HD_PATH_ID video_enc_path, HD_DIM *p_max_dim, UINT32 max_bitrate)
{
	HD_RESULT ret = HD_OK;
...
HD_RESULT set_enc_param(HD_PATH_ID video_enc_path, HD_DIM *p_dim, HD_VIDEO_PXLFMT pxlfmt, INT32 enc_type, UINT32 bitrate)
{
	HD_RESULT ret = HD_OK;
...
#if (MODEL_DISP != DISP_NONE)
HD_RESULT set_out_cfg(HD_PATH_ID *p_video_out_ctrl, UINT32 out_type,HD_VIDEOOUT_HDMI_ID hdmi_id)
{
	HD_RESULT ret = HD_OK;
...
#if (MODEL_DISP != DISP_NONE)
HD_RESULT open_module_3(DEMO *p_stream, UINT32 out_type)
{
	HD_RESULT ret;
	// set videoout config
...
static HD_RESULT open_module_5(DEMO *p_stream, HD_DIM* p_proc_max_dim)
{
	HD_RESULT ret;
...
static HD_RESULT open_module_6(DEMO *p_stream, HD_DIM* p_proc_max_dim)
{
	HD_RESULT ret;
	if ((ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_7, &p_stream->proc_path)) != HD_OK)  // out 5~15 is extend
		return ret;
	return HD_OK;
}
...
#if TEST_VPRC_EXTEND_PERF
static HD_RESULT open_module_7(DEMO *p_stream)
{
	HD_RESULT ret;
	ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_8, &p_stream->proc_path);
	return ret;
}

static HD_RESULT open_module_8(DEMO *p_stream)
{
	HD_RESULT ret;
	ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_9, &p_stream->proc_path);
	return ret;
}

static HD_RESULT open_module_9(DEMO *p_stream)
{
	HD_RESULT ret;
	ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_10, &p_stream->proc_path);
	return ret;
}

static HD_RESULT open_module_10(DEMO *p_stream)
{
	HD_RESULT ret;
	ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_11,&p_stream->proc_path);
	return ret;
}

static HD_RESULT open_module_11(DEMO *p_stream)
{
	HD_RESULT ret;
	ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_12, &p_stream->proc_path);
	return ret;
}

static HD_RESULT open_module_12(DEMO *p_stream)
{
	HD_RESULT ret;
	ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_13, &p_stream->proc_path);
	return ret;
}

static HD_RESULT open_module_13(DEMO *p_stream)
{
	HD_RESULT ret;
	ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_14, &p_stream->proc_path);
	return ret;
}
...
HD_RESULT close_module_5(DEMO *p_stream)
{
	HD_RESULT ret;
	if ((ret = hd_videoproc_close(p_stream->proc_path)) != HD_OK)
		return ret;
...
HD_RESULT close_module_6(DEMO *p_stream)
{
	HD_RESULT ret;
	if ((ret = hd_videoproc_close(p_stream->proc_path)) != HD_OK)
		return ret;
...
#if TEST_VPRC_EXTEND_PERF
HD_RESULT close_module_7(DEMO *p_stream)
{
	HD_RESULT ret;
	if ((ret = hd_videoproc_close(p_stream->proc_path)) != HD_OK)
		return ret;
...
HD_RESULT close_module_8(DEMO *p_stream)
{
	HD_RESULT ret;
	if ((ret = hd_videoproc_close(p_stream->proc_path)) != HD_OK)
		return ret;
...
HD_RESULT close_module_9(DEMO *p_stream)
{
	HD_RESULT ret;
	if ((ret = hd_videoproc_close(p_stream->proc_path)) != HD_OK)
		return ret;
...
HD_RESULT close_module_10(DEMO *p_stream)
{
	HD_RESULT ret;
	if ((ret = hd_videoproc_close(p_stream->proc_path)) != HD_OK)
		return ret;
...
HD_RESULT close_module_11(DEMO *p_stream)
{
	HD_RESULT ret;
	if ((ret = hd_videoproc_close(p_stream->proc_path)) != HD_OK)
		return ret;
...
HD_RESULT close_module_12(DEMO *p_stream)
{
	HD_RESULT ret;
	if ((ret = hd_videoproc_close(p_stream->proc_path)) != HD_OK)
		return ret;
...
HD_RESULT close_module_13(DEMO *p_stream)
{
	HD_RESULT ret;
	if ((ret = hd_videoproc_close(p_stream->proc_path)) != HD_OK)
		return ret;
...
static void *relay_yuv_thread(void *arg)
{
	...
	DEMO* p_stream2 = p_stream0 + 3;
	HD_RESULT ret = HD_OK;
	HD_VIDEO_FRAME video_frame = {0};
	HD_VIDEOPROC_POLL_LIST poll_list[3];
	...
	poll_list[2].path_id = p_stream2->proc_path;

	//--------- pull data test ---------
	while (p_stream0->proc_exit == 0) {
		if (HD_OK == hd_videoproc_poll_list(poll_list, 3, 1000)) {    // multi path poll_list , -1 = blocking mode
			...
			if (TRUE == poll_list[1].revent.event) {
				...
				//printf("stream 1 reserved[0] = %d, w = %d, h =%d\r\n", video_frame.reserved[0], video_frame.dim.w, video_frame.dim.h);
			...
			if (TRUE == poll_list[2].revent.event) {
				ret = hd_videoproc_pull_out_buf(p_stream2->proc_path, &video_frame, -1); // -1 = blocking mode
				if (ret != HD_OK) {
					printf("pull_out [2] error !!\r\n\r\n");
					continue;
				}
				//printf("stream 2 reserved[0] = %d, w = %d, h =%d\r\n", video_frame.reserved[0], video_frame.dim.w, video_frame.dim.h);



				ret = hd_videoenc_push_in_buf(p_stream2->enc_path, &video_frame, NULL, -1); // -1 = blocking mode
				if (ret != HD_OK) {
					printf("push_in [2] error !!\r\n");
				}

				ret = hd_videoproc_release_out_buf(p_stream2->proc_path, &video_frame);
				if (ret != HD_OK) {
					printf("release_out [2] error !!\r\n\r\n");
				}
			}
		}
	}
...
static void *encode_thread(void *arg)
{
	...
	if (g_enc3_type >= 0) {
		hd_common_mem_munmap((void *)vir_addr_sub2, phy_buf_sub2.buf_info.buf_size);
	}
...
static void *gfxcpy_thread(void *arg)
{
	...
	COPY_INFO            *info = (COPY_INFO *)arg;
	UINT32               frame_count;
	UINT32               i, *pdata, *pdata2;
	BOOL                 hasError = FALSE;

	if (NULL != info) {
		size = info->copy_size;
	}
	ret = hd_common_mem_alloc("test1", &pa1, (void **)&va1, size + 0x20000, info->ddr_id);
	if (ret != HD_OK) {
		printf("err:alloc size 0x%x, ddr %d\r\n", size, ddr_id+1);
		return 0;
	}
	ret = hd_common_mem_alloc("test2", &pa2, (void **)&va2, size + 0x20000, info->ddr_id);
	if (ret != HD_OK) {
		printf("err:alloc size 0x%x, ddr %d\r\n", size, ddr_id+1);
		return 0;
	}
	...
	while (!g_quit) {
		...
		vendor_common_mem_cache_sync((void*)va1, size, VENDOR_COMMON_MEM_DMA_BIDIRECTIONAL);
		hd_gfx_copy(&param);
		vendor_common_mem_cache_sync((void*)va2, size, VENDOR_COMMON_MEM_DMA_BIDIRECTIONAL);
		if (info->sleep_us) {
			usleep(info->sleep_us);
		}
		total_size += size;
...
static void *cacheflush_by_cpu_thread(void *arg)
{
	void                 *va, *va2, *frame_va;
	UINT32               pa, pa2, i;
	UINT32               size = 0x800000;
	UINT32               image_size, y_size;
	HD_COMMON_MEM_DDR_ID ddr_id = DDR_ID0;
	HD_RESULT            ret;
	UINT64               time_b, time_e;
	UINT32               cpu_id = 1;
	DEMO                 *p_stream = (DEMO *)arg;
	HD_VIDEO_FRAME       video_frame = {0};
	HD_GFX_COPY          param = {0};
	UINT32               frame_count;
	UINT32               *pdata, *pdata2;
	...
	printf("\r\ntest_cacheflush_perf_by_cpu id %d\r\n", cpu_id);
	...
	while (!g_quit) {
		frame_count ++;
		ret = hd_videoproc_pull_out_buf(p_stream->proc_path, &video_frame, 1000); // -1 = blocking mode
		if (ret != HD_OK) {
			printf("pull_out [0] error %d !!\r\n\r\n",ret);
			continue;
		}
		//printf("frame_count = %d reserved[0] = %d, w = %d, h =%d \r\n", frame_count, video_frame.reserved[0], video_frame.dim.w, video_frame.dim.h);

		//printf("video_frame.phy_addr[0] = 0x%x, video_frame.phy_addr[1] = 0x%x\r\n", video_frame.phy_addr[0], video_frame.phy_addr[1]);

		y_size = video_frame.loff[0] * video_frame.dim.h;
		image_size = y_size * 3 / 2;

		param.src_img.dim.w            = video_frame.dim.w;
		param.src_img.dim.h            = video_frame.dim.h;
		param.src_img.format           = HD_VIDEO_PXLFMT_YUV420;
		param.src_img.p_phy_addr[0]    = video_frame.phy_addr[0];
		param.src_img.lineoffset[0]    = video_frame.loff[0];
		param.src_img.p_phy_addr[1]    = video_frame.phy_addr[1];
		param.src_img.lineoffset[1]    = video_frame.loff[1];
		param.dst_img.dim.w            = video_frame.dim.w;
		param.dst_img.dim.h            = video_frame.dim.h;
		param.dst_img.format           = HD_VIDEO_PXLFMT_YUV420;
		param.dst_img.p_phy_addr[0]    = pa;
		param.dst_img.lineoffset[0]    = video_frame.loff[0];
		param.dst_img.p_phy_addr[1]    = pa + y_size;
		param.dst_img.lineoffset[1]    = video_frame.loff[1];
		...
		// make sure the source frame cache is clean
		frame_va = hd_common_mem_mmap(HD_COMMON_MEM_MEM_TYPE_CACHE, video_frame.phy_addr[0], image_size);
		if (frame_va == NULL) {
			printf("mmap fail\r\n");
		}
		vendor_common_mem_cache_sync(frame_va, image_size, VENDOR_COMMON_MEM_DMA_BIDIRECTIONAL);
		hd_common_mem_munmap(frame_va, image_size);

		//printf("dst_img.phy_addr[0] = 0x%x, dst_img.phy_addr[1] = 0x%x\r\n", param.dst_img.p_phy_addr[0], param.dst_img.p_phy_addr[1]);
		vendor_gfx_copy_no_flush(&param);
		...
		vendor_common_mem_cache_sync_all(va, image_size, VENDOR_COMMON_MEM_DMA_BIDIRECTIONAL);
		time_e = hd_gettime_us();
		printf("cacheflush_by_cpu size 0x%x, time = %lld us\r\n", (int)image_size, time_e-time_b);
		pa2 = pa + image_size;
		va2 = va + image_size;
		param.src_img.dim.w            = video_frame.dim.w;
		param.src_img.dim.h            = video_frame.dim.h;
		param.src_img.format           = HD_VIDEO_PXLFMT_YUV420;
		param.src_img.p_phy_addr[0]    = pa;
		param.src_img.lineoffset[0]    = video_frame.loff[0];
		param.src_img.p_phy_addr[1]    = pa + y_size;
		param.src_img.lineoffset[1]    = video_frame.loff[1];
		param.dst_img.dim.w            = video_frame.dim.w;
		param.dst_img.dim.h            = video_frame.dim.h;
		param.dst_img.format           = HD_VIDEO_PXLFMT_YUV420;
		param.dst_img.p_phy_addr[0]    = pa2;
		param.dst_img.lineoffset[0]    = video_frame.loff[0];
		param.dst_img.p_phy_addr[1]    = pa2 + y_size;
		param.dst_img.lineoffset[1]    = video_frame.loff[1];
		...
		param.alpha                    = 255;
		vendor_gfx_copy_no_flush(&param);
		vendor_common_mem_cache_sync_all(va2, image_size, VENDOR_COMMON_MEM_DMA_BIDIRECTIONAL);
		// cpu compare
		pdata = (UINT32 *)va;
		pdata2 = (UINT32 *)va2;
		printf("framecount = %d\r\n", *pdata);
...
int main(int argc, char** argv)
{
	HD_RESULT ret;
	INT key;
	DEMO stream[15] = {0}; //0: main stream, 1: sub stream, 2: disp stream, 3:sub2 stream, 4:AI path
	INT32 enc_type = 0;
	INT32 enc2_type = 0;
	HD_DIM main_dim;
	HD_DIM sub_dim;
	...
	pthread_t  cacheflush_thread_id;
	...
	BOOL       is_start_cacheflush_test = FALSE;
	int    ve_in_dir = 0;
	int    vprc_out_dir = 0;
	...
	int    cap_size_h = 0;
#if (FUNC_RELAY == ENABLE)
	UINT32 pxlfmt = HD_VIDEO_PXLFMT_YUV420;
#endif
	...
	if (argc == 1) {
		...
		printf("  <display type>  : LCD only\r\n");
		printf("  <enable sub2 stream>  : 0(disable), 1(enable)\r\n");
		printf("  <sub2 stream enc_type>  : 0(H265), 1(H264), 2(MJPG)\r\n");
		printf("  <cap_out_bind>: 0(D2D Mode), 1(Direct Mode)\r\n");
		printf("  <enable shdr>  : 0(disable), 1(enable)\r\n");
		printf("  <fps>  : 20,25,30\r\n");
		printf("  <main streamsize> : 3(3M),4(4M),5(5M)\r\n");
		printf("  <one buffer> :  0(disable), 1(enable)\r\n");
		printf("  <yuvcompress> :  0(disable), 1(enable)\r\n");
		printf("  <osg> :  0(disable), 1(enable)\r\n");
		printf("  <sub streamsize> : 2(2M),1(1M),400(400k),300(300k)\r\n");
		printf("  <rotate> : 0(disable), 1(enable)\r\n");
		printf("  <sub2 streamsize> : 2(2M),1(1M),600(600k),400(400k),300(300k)\r\n");
	...
	if (argc >= 4) {
		g_out_type = atoi(argv[3]);
		printf("g_out_type %d\r\n", g_out_type);
		if(g_out_type > 2) {
			printf("error: not support g_out_type\r\n");
			return 0;
		}
	}
	if (argc >= 5) {
		g_sub2_en = atoi(argv[4]);
		printf("g_sub2_en %d\r\n", g_sub2_en);
	}
	if (argc >= 6) {
		g_enc3_type = atoi(argv[5]);
		printf("g_enc3_type %d\r\n", g_enc3_type);
		if(g_enc3_type > 2) {
			printf("error: not support g_enc3_type!\r\n");
			return 0;
		}
	}
	if (argc >= 7) {
		g_capbind = atoi(argv[6]);
		printf("CAP-BIND %d\r\n", g_capbind);
		if(g_capbind > 1) {
	...
	if (argc >= 10) {
		main_strem_size = atoi(argv[9]);
		printf("main stream size %d\r\n", main_strem_size);
	}
	if (argc >= 11) {
		g_one_buf = atoi(argv[10]);
		printf("one buffer %d\r\n", g_one_buf);
	}
	if (argc >= 12) {
		g_yuvcompress = atoi(argv[11]);
	...
	if (argc >= 14) {
		sub_strem_size = atoi(argv[13]);
		printf("sub_strem_size %d\r\n", sub_strem_size);
	}
	if (argc >= 15) {
		g_rotate = atoi(argv[14]);
		printf("g_rotate %d\r\n", g_rotate);
	}
	if (argc >= 16) {
		sub2_strem_size = atoi(argv[15]);
		printf("sub2_strem_size %d\r\n", sub2_strem_size);
	}

	// check if chip is 528
	if (chip_name != NULL && strcmp(chip_name, "CHIP_NA51084") == 0) {
		sensor_sel = SEN_SEL_IMX415;
	} else {
		sensor_sel = SEN_SEL_OS05A10;
		//sensor_sel = SEN_SEL_IMX290;
	}
	if (sensor_sel == SEN_SEL_OS05A10){
		vdo_size_w = VDO_SIZE_W_5M;
		vdo_size_h = VDO_SIZE_H_5M;
		cap_size_w = vdo_size_w;
		cap_size_h = vdo_size_h;
		vdo_br = 5 * 1024 *1024;
	} else if (sensor_sel == SEN_SEL_IMX317 || sensor_sel == SEN_SEL_IMX415) {
    ...
    if (sub2_strem_size != 0) {
		switch (sub2_strem_size) {
    		...
    		case 1:
				...
				break;
    		case 600:
				sub2vdo_size_w = VDO_SIZE_W_AI;
				sub2vdo_size_h = VDO_SIZE_H_AI;
				sub2vdo_br = (VDO_SIZE_W_AI * VDO_SIZE_H_AI);
                g_ai_new = 1;
				break;
    		case 400:
...
#if (MODEL_DISP != DISP_NONE)
	if (g_out_type >= 0) {
	osg_init(2, &(stream[2].osg),
		DISP_STAMP_WIDTH, DISP_STAMP_HEIGHT, DISP_STAMP_PXLFMT, DISP_STAMP_C);
	}
#endif
	if (g_enc3_type >= 0) {
	osg_init(3, &(stream[3].osg),
		SUB2_STAMP_WIDTH, SUB2_STAMP_HEIGHT, SUB2_STAMP_PXLFMT, SUB2_STAMP_C);
	}
    }
#endif
...
#if (MODEL_DISP != DISP_NONE)
	if (g_out_type >= 0) {
	osg_open(2, &(stream[2].osg), OSG_TYPE_VIDEOOUT, HD_VIDEOOUT_0_IN_0);
	}
#endif
	if (g_enc3_type >= 0) {
		osg_open(3, &(stream[3].osg), OSG_TYPE_VIDEOENC, HD_VIDEOENC_0_IN_2);
	}
...
#if (MODEL_DISP != DISP_NONE)
	if (g_out_type >= 0) {
		// open demo modules (disp)
	    stream[2].hdmi_id=HD_VIDEOOUT_HDMI_1920X1080I60;//default
		ret = open_module_3(&stream[2], g_out_type);
		if (ret != HD_OK) {
			printf("open fail=%d\n", ret);
			goto exit;
		}
	...
	ret = open_module_5(&stream[4], &stream[4].proc_max_dim);
	if (ret != HD_OK) {
		printf("open fail=%d\n", ret);
		goto exit;
	}
	stream[5].proc_max_dim.w = SUB2_VDO_SIZE_W; //assign by user
	stream[5].proc_max_dim.h = SUB2_VDO_SIZE_H; //assign by user
	ret = open_module_6(&stream[5], &stream[5].proc_max_dim);
	if (ret != HD_OK) {
		printf("open fail=%d\n", ret);
		goto exit;
	}
	...
	ret = open_module_8(&stream[7]);
	if (ret != HD_OK) {
		printf("open fail=%d\n", ret);
		goto exit;
	}
	ret = open_module_9(&stream[8]);
	if (ret != HD_OK) {
		printf("open fail=%d\n", ret);
		goto exit;
	}
	ret = open_module_10(&stream[9]);
	if (ret != HD_OK) {
		printf("open fail=%d\n", ret);
		goto exit;
	}
	ret = open_module_11(&stream[10]);
	if (ret != HD_OK) {
		printf("open fail=%d\n", ret);
		goto exit;
	}
	ret = open_module_12(&stream[11]);
	if (ret != HD_OK) {
		printf("open fail=%d\n", ret);
		goto exit;
	}
	ret = open_module_13(&stream[12]);
	if (ret != HD_OK) {
		printf("open fail=%d\n", ret);
		goto exit;
	}
...
#if (MODEL_DISP != DISP_NONE)
	osg_start(2, &(stream[2].osg), OSG_TYPE_VIDEOOUT, DISP_STAMP_X, DISP_STAMP_Y);
#endif
	if (g_enc3_type >= 0) {
		osg_start(3, &(stream[3].osg), OSG_TYPE_VIDEOENC, SUB2_STAMP_X, SUB2_STAMP_Y);
	}
...
#if (MODEL_DISP != DISP_NONE)
	disp_dim.w = DISP_VDO_SIZE_W; //assign by user
	disp_dim.h = DISP_VDO_SIZE_H; //assign by user
#endif
	...
	if (g_yuvcompress == 1 && ((enc_type == 0)||(enc_type == 1))) {
		pxlfmt = HD_VIDEO_PXLFMT_YUV420_NVX2;
	} else {
		pxlfmt = HD_VIDEO_PXLFMT_YUV420;
	}
	ret = set_proc_param(stream[0].proc_path, &main_dim, pxlfmt, TRUE,0); //no bind, must set out dim, must enable pull
#else
	ret = set_proc_param(stream[0].proc_path, NULL, 0, FALSE,g_one_buf);
#endif
	if (ret != HD_OK) {
	...
	ret = set_proc_param(stream[1].proc_path, &sub_dim, pxlfmt, TRUE,0); //no bind, must set out dim, must enable pull
#else
	ret = set_proc_param(stream[1].proc_path, NULL, 0, FALSE,g_one_buf);
#endif
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	...
	/*
	{
		...
		video_path_param.ddr_id = 1;
		ret = hd_videoproc_set(stream[1].proc_path, HD_VIDEOPROC_PARAM_FUNC_CONFIG, &video_path_param);
        if (ret != HD_OK) {
			return HD_ERR_NG;
		}
	}
	*/
	...
	if (g_out_type >= 0) {
		// set videoproc parameter (disp)
		ret = set_proc_param(stream[2].proc_path, NULL, 0, FALSE,0);
		if (ret != HD_OK) {
			printf("set proc fail=%d\n", ret);
			goto exit;
		}
...
#endif
#if (FUNC_RELAY == ENABLE)
	ret = set_proc_param(stream[3].proc_path, &sub2_dim, HD_VIDEO_PXLFMT_YUV420,TRUE,0); //no bind, must set out dim, must enable pull
#else
    if ((g_sub2_en==1)&&(g_enc3_type == -1)) {
	ret = set_proc_param(stream[3].proc_path, &sub2_dim, HD_VIDEO_PXLFMT_YUV420,TRUE,0); //no bind, must set out dim, must enable pull
    } else {
	ret = set_proc_param(stream[3].proc_path, NULL, 0,FALSE,0);
    }
#endif
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
	...
	ret = set_proc_param_extend(stream[5].proc_path, HD_VIDEOPROC_0_OUT_1, NULL, &sub2_dim, TRUE, 0);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
#if TEST_VPRC_EXTEND_PERF
	ext_dim.w = EXTEND_VDO_SIZE_W;
	ext_dim.h = EXTEND_VDO_SIZE_H;
	ret = set_proc_param_extend(stream[6].proc_path, HD_VIDEOPROC_0_OUT_0, NULL, &ext_dim, TRUE, 0);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
	ret = set_proc_param_extend(stream[7].proc_path, HD_VIDEOPROC_0_OUT_0, NULL, &ext_dim, TRUE, 0);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
	ret = set_proc_param_extend(stream[8].proc_path, HD_VIDEOPROC_0_OUT_0, NULL, &ext_dim, TRUE, 0);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
	ext_dim.w = 960;
	ext_dim.h = 720;
	ret = set_proc_param_extend(stream[9].proc_path, HD_VIDEOPROC_0_OUT_0, NULL, &ext_dim, TRUE, 0);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
	ext_dim.w = 640;
	ext_dim.h = 480;
	ret = set_proc_param_extend(stream[10].proc_path, HD_VIDEOPROC_0_OUT_0, NULL, &ext_dim, TRUE, 0);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
	ext_dim.w = 320;
	ext_dim.h = 240;
	ret = set_proc_param_extend(stream[11].proc_path, HD_VIDEOPROC_0_OUT_0, NULL, &ext_dim, TRUE, 0);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
	ext_dim.w = 160;
	ext_dim.h = 120;
	ret = set_proc_param_extend(stream[12].proc_path, HD_VIDEOPROC_0_OUT_0, NULL, &ext_dim, TRUE, 0);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
    ...
    if (g_yuvcompress == 1 && ((enc_type == 0)||(enc_type == 1))) {
    	ret = set_enc_param(stream[0].enc_path, &stream[0].enc_dim, HD_VIDEO_PXLFMT_YUV420_NVX2, enc_type, VDO_BR);
    } else {
	    ret = set_enc_param(stream[0].enc_path, &stream[0].enc_dim, 0, enc_type, VDO_BR);
    }
	if (ret != HD_OK) {
	...
	if (enc2_type >= 0) {
		...
		if (g_yuvcompress == 1 && ((enc2_type == 0)||(enc2_type == 1))) {
			ret = set_enc_param(stream[1].enc_path, &stream[1].enc_dim, HD_VIDEO_PXLFMT_YUV420_NVX2, enc2_type, SUB_VDO_BR);
		} else {
			ret = set_enc_param(stream[1].enc_path, &stream[1].enc_dim, 0, enc2_type, SUB_VDO_BR);
		}
		if (ret != HD_OK) {
			printf("set enc fail=%d\n", ret);
			goto exit;
	...
	if (g_out_type >= 0) {
		...
		//printf("set display dim %dx%d\r\n", stream[2].out_dim.w, stream[2].out_dim.h);
		if((stream[2].out_dim.w*stream[2].out_dim.h) > (disp_dim.w*disp_dim.h)) {
			printf("not enough buffer for out dim %dx%d!\n", stream[2].out_dim.w, stream[2].out_dim.h);
			goto exit;
		}
	...
	if (g_enc3_type >= 0) {
    	stream[3].enc_max_dim.w = sub2_dim.w;
    	stream[3].enc_max_dim.h = sub2_dim.h;
    	ret = set_enc_cfg(stream[3].enc_path, &stream[3].enc_max_dim, SUB2_VDO_BR);
    	if (ret != HD_OK) {
    		printf("set enc-cfg fail=%d\n", ret);
    		goto exit;
    	}
    	...
    	stream[3].enc_dim.w = sub2_dim.w;
    	stream[3].enc_dim.h = sub2_dim.h;
    	ret = set_enc_param(stream[3].enc_path, &stream[3].enc_dim, 0, g_enc3_type, SUB2_VDO_BR);
    	if (ret != HD_OK) {
    		printf("set enc fail=%d\n", ret);
    		goto exit;
    	}
	...
	// start demo modules (disp)
	if (g_out_type >= 0) {
		hd_videoproc_start(stream[2].proc_path);
	}
	// start demo modules (sub2)
	if (g_sub2_en) {
		hd_videoproc_start(stream[3].proc_path);
	}
	// start demo modules (alg)
	hd_videoproc_start(stream[4].proc_path);
	hd_videoproc_start(stream[5].proc_path);
#if TEST_VPRC_EXTEND_PERF
	hd_videoproc_start(stream[6].proc_path);
	hd_videoproc_start(stream[7].proc_path);
	hd_videoproc_start(stream[8].proc_path);
	hd_videoproc_start(stream[9].proc_path);
	hd_videoproc_start(stream[10].proc_path);
	hd_videoproc_start(stream[11].proc_path);
	hd_videoproc_start(stream[12].proc_path);
#endif
	...
	printf("Enter 6 to stop output\n");
	printf("Enter 7 to start output\n");
	...
	printf("Enter c to start flushall test\n");
	printf("Enter m to test memcpy\n");
	printf("Enter n to test gfxcpy\n");

	printf("Enter t to rotate of videoenc.in\n");
	while (1) {
		key = getchar();
		if (key == 'q' || key == 0x3) {
		...
		if (key == 'c') {
			if (TRUE == is_start_cacheflush_test) {
				continue;
			}
			printf("start cacheflush test\r\n");
			ret = pthread_create(&cacheflush_thread_id, NULL, cacheflush_by_cpu_thread, &stream[5]);
			if (ret < 0) {
				printf("create cacheflush test failed");
				goto exit;
			}
			is_start_cacheflush_test = TRUE;
		}
		...
		if (key == 'n') {
			copy_info[1].copy_size = 0x500000;
			copy_info[1].ddr_id = DDR_ID1;
			copy_info[1].sleep_us = 0;

			if (TRUE == is_start_gfxcpy_test) {
				continue;
			}
			printf("start gfxcpy test\r\n");
		...
		if (key == 'o') {
			...
			copy_info[2].ddr_id = DDR_ID1;
			copy_info[2].sleep_us = 5000;

			if (TRUE == is_start_memcpy_test2) {
				continue;
			}
			printf("start memcpy test2\r\n");
		...
		if (key == 'p') {
			copy_info[3].copy_size = 0x500000;
			copy_info[3].ddr_id = DDR_ID0;
			copy_info[3].sleep_us = 5000;

			if (TRUE == is_start_gfxscale_test) {
				continue;
			}
			printf("start gfxscale test\r\n");
		...
		if (key == 't') {
			HD_VIDEOENC_IN video_in_param = {0};
			{
                ...
                hd_videoenc_stop(stream[1].enc_path);
				hd_videoenc_get(stream[1].enc_path, HD_VIDEOENC_PARAM_IN, &video_in_param);
				if(ve_in_dir == 0)
					video_in_param.dir = HD_VIDEO_DIR_NONE;
				else if(ve_in_dir == 1)
					video_in_param.dir = HD_VIDEO_DIR_ROTATE_90;
					...
					video_in_param.dir = HD_VIDEO_DIR_ROTATE_270;
				hd_videoenc_set(stream[1].enc_path, HD_VIDEOENC_PARAM_IN, &video_in_param);
				hd_videoenc_start(stream[1].enc_path);


	            if (g_enc3_type >= 0) {
					hd_videoenc_stop(stream[3].enc_path);
					hd_videoenc_get(stream[3].enc_path, HD_VIDEOENC_PARAM_IN, &video_in_param);
					if(ve_in_dir == 0)
						video_in_param.dir = HD_VIDEO_DIR_NONE;
					else if(ve_in_dir == 1)
						video_in_param.dir = HD_VIDEO_DIR_ROTATE_90;
						...
						video_in_param.dir = HD_VIDEO_DIR_ROTATE_270;
					hd_videoenc_set(stream[3].enc_path, HD_VIDEOENC_PARAM_IN, &video_in_param);
					hd_videoenc_start(stream[3].enc_path);
				}
			}
		}
		if (key == 'r') {
			HD_VIDEOPROC_OUT_EX vproc_outex_param = {0};
			HD_VIDEOENC_IN video_in_param = {0};
			{
				// next dir
				vprc_out_dir ++;
				if(vprc_out_dir > 3) vprc_out_dir = 0;
				printf("*** set vprc_out_dir dir = (%d) ***\r\n", vprc_out_dir);

				if (g_sub2_en) {
					hd_videoproc_stop(stream[3].proc_path);
					hd_videoproc_get(stream[3].proc_path, HD_VIDEOPROC_PARAM_OUT_EX, &vproc_outex_param);
					if(vprc_out_dir == 0) {
						vproc_outex_param.dir = HD_VIDEO_DIR_NONE;
						vproc_outex_param.dim.w = SUB2_VDO_SIZE_W;
						vproc_outex_param.dim.h = SUB2_VDO_SIZE_H;
					}
					else if(vprc_out_dir == 1) {
						vproc_outex_param.dir = HD_VIDEO_DIR_ROTATE_90;
						vproc_outex_param.dim.w = SUB2_VDO_SIZE_H;
						vproc_outex_param.dim.h = SUB2_VDO_SIZE_W;
					}
					else if(vprc_out_dir == 2) {
						vproc_outex_param.dir = HD_VIDEO_DIR_ROTATE_180;
						vproc_outex_param.dim.w = SUB2_VDO_SIZE_W;
						vproc_outex_param.dim.h = SUB2_VDO_SIZE_H;
					}
					else {
						vproc_outex_param.dir = HD_VIDEO_DIR_ROTATE_270;
						vproc_outex_param.dim.w = SUB2_VDO_SIZE_H;
						vproc_outex_param.dim.h = SUB2_VDO_SIZE_W;
					}
					hd_videoproc_set(stream[3].proc_path, HD_VIDEOPROC_PARAM_OUT_EX, &vproc_outex_param);
					hd_videoproc_start(stream[3].proc_path);
					...
					hd_videoenc_stop(stream[3].enc_path);
					hd_videoenc_get(stream[3].enc_path, HD_VIDEOENC_PARAM_IN, &video_in_param);
					video_in_param.dim.w = vproc_outex_param.dim.w;
					video_in_param.dim.h = vproc_outex_param.dim.h;
	...
	hd_videoproc_stop(stream[5].proc_path);
#if TEST_VPRC_EXTEND_PERF
	hd_videoproc_stop(stream[6].proc_path);
	hd_videoproc_stop(stream[7].proc_path);
	hd_videoproc_stop(stream[8].proc_path);
	hd_videoproc_stop(stream[9].proc_path);
	hd_videoproc_stop(stream[10].proc_path);
	hd_videoproc_stop(stream[11].proc_path);
	hd_videoproc_stop(stream[12].proc_path);
#endif
...
exit:
	...
	if (TRUE == is_start_memcpy_test2) {
		pthread_join(memcpy_thread_id2, NULL);
	}
	if (TRUE == is_start_gfxcpy_test) {
		pthread_join(gfxcpy_thread_id, NULL);
	}
	if (TRUE == is_start_gfxscale_test) {
		pthread_join(gfxscale_thread_id, NULL);
	}
	if (TRUE == is_start_cacheflush_test) {
		pthread_join(cacheflush_thread_id, NULL);
	}
...
#if (MODEL_DISP != DISP_NONE)
	osg_close(2, &(stream[2].osg), OSG_TYPE_VIDEOOUT);
#endif
	if (g_enc3_type >= 0) {
		osg_close(3, &(stream[3].osg), OSG_TYPE_VIDEOENC);
	}
	...
	ret = close_module_5(&stream[4]);
	if (ret != HD_OK) {
		printf("close fail=%d\n", ret);
	}
	ret = close_module_6(&stream[5]);
	if (ret != HD_OK) {
		printf("close fail=%d\n", ret);
	}
#if TEST_VPRC_EXTEND_PERF
	ret = close_module_7(&stream[6]);
	if (ret != HD_OK) {
		printf("close fail=%d\n", ret);
	}
	ret = close_module_8(&stream[7]);
	if (ret != HD_OK) {
		printf("close fail=%d\n", ret);
	}
	ret = close_module_9(&stream[8]);
	if (ret != HD_OK) {
		printf("close fail=%d\n", ret);
	}
	ret = close_module_10(&stream[9]);
	if (ret != HD_OK) {
		printf("close fail=%d\n", ret);
	}
	ret = close_module_11(&stream[10]);
	if (ret != HD_OK) {
		printf("close fail=%d\n", ret);
	}
	ret = close_module_12(&stream[11]);
	if (ret != HD_OK) {
		printf("close fail=%d\n", ret);
	}
	ret = close_module_13(&stream[12]);
	if (ret != HD_OK) {
		printf("close fail=%d\n", ret);
	}
...
#if (MODEL_DISP != DISP_NONE)
	osg_uninit(2, &(stream[2].osg));
#endif
	if (g_enc3_type >= 0) {
		osg_uninit(3, &(stream[3].osg));
	}
