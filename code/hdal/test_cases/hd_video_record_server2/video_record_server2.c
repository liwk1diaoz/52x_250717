/**
	@brief Sample code of video record server for multi-process.\n

	@file video_record_server2.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_server2, argc, argv)
...
#define HD_COMMON_CFG				0xf0000000	//common R4
...
static void *encode_thread(void *arg)
{
	...
	if ((g_pid == 0)) {
		...
		if ((f_out_main = fopen(file_path_sub, "wb")) == NULL) {
			HD_VIDEOENC_ERR("open file (%s) fail....\r\n", file_path_sub);
		} else {
...
#define HD_VIDEOCAP_PATH(dev_id, in_id, out_id) (((dev_id) << 16) | (((in_id) & 0x00ff) << 8)| ((out_id) & 0x00ff))
	...
	if (g_prcbind == 0xff) //no-bind mode
		ret = set_proc_param(stream[1].proc_path, &sub_dim); //must set vprc's out dim
	else
		ret = set_proc_param(stream[1].proc_path, NULL); //keep vprc's out dim = {0,0}, it means auto sync from vout's in dim
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
...
if ((g_pid == 0)) {
	...
	if (g_prcbind == 0xff) { //no-bind mode
		...
		ret = pthread_create(&stream[0].prc_thread_id, NULL, process_thread, (void *)&stream[0]);
		if (ret < 0) {
	...
	if (g_prcbind == 0xff) { //no-bind mode
		// create process_thread (pull_out frame, push_in frame then pull_out bitstream)
		ret = pthread_create(&stream[1].prc_thread_id, NULL, process_thread, (void *)&stream[1]);
		if (ret < 0) {
			printf("create process_thread failed");
			goto exit;
		}
...
if ((g_pid == 0)) {
	...
	ret = pthread_create(&stream[0].enc_thread_id, NULL, encode_thread, (void *)&stream[0]);
	if (ret < 0) {
	...
	// create encode_thread (pull_out bitstream)
	ret = pthread_create(&stream[1].enc_thread_id, NULL, encode_thread, (void *)&stream[1]);
	if (ret < 0) {
		printf("create encode thread failed");
		goto exit;
	}
...
#if defined(__LINUX)
	printf("pidof <server> to get pid\n");
...
//} else {
	if (g_prcbind == 0xff) { //no-bind mode
		// destroy process_thread
		pthread_join(stream[1].prc_thread_id, NULL);  //NOTE: before destory, call stop to breaking pull(-1)
	} 
}
