...
static HD_RESULT mem_init(void)
{
	...
	mem_cfg.pool_info[2].blk_size = DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(ALIGN_CEIL_64(VDO_SIZE_W), ALIGN_CEIL_64(VDO_SIZE_H), HD_VIDEO_PXLFMT_YUV420);
...
static HD_RESULT set_enc_cfg(HD_PATH_ID video_enc_path, HD_DIM *p_max_dim, UINT32 max_bitrate)
{
	...
	h26x_comm_recfrm.h26x_comm_base_recfrm_en = 1;
	h26x_comm_recfrm.h26x_comm_svc_recfrm_en = 0;
	h26x_comm_recfrm.h26x_comm_ltr_recfrm_en = 1;
...
static HD_RESULT set_enc_param(HD_PATH_ID video_enc_path, HD_DIM *p_dim, UINT32 enc_type, UINT32 bitrate)
{
	...
	if (p_dim != NULL) {
		...
		if (enc_type == 0) {
			...
			video_out_param.h26x.ltr_interval  = 7;
			video_out_param.h26x.ltr_pre_ref   = 1;
