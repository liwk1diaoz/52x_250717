/**
	@brief Sample code of video playback partial.\n

	@file video_playback_partial.c
...
static HD_RESULT set_dec_param(HD_PATH_ID video_dec_path, UINT32 dec_type)
{
	...
	if (dec_type == HD_CODEC_TYPE_H264 || dec_type == HD_CODEC_TYPE_H265) {
		...
		sprintf(file_name, "/mnt/sd/video_bs_%d_%d_%s_svc_partial.dat", VDO_SIZE_W, VDO_SIZE_H, codec_name);
#else
		sprintf(file_name, "/mnt/sd/video_bs_%d_%d_%s_partial.dat", VDO_SIZE_W, VDO_SIZE_H, codec_name);
#endif
...
#if SVC_BITSTREAM_TESTING
		snprintf(file_name, sizeof(file_name), "/mnt/sd/video_bs_%d_%d_%s_svc_partial.len", VDO_SIZE_W, VDO_SIZE_H, codec_name);
#else
		snprintf(file_name, sizeof(file_name), "/mnt/sd/video_bs_%d_%d_%s_partial.len", VDO_SIZE_W, VDO_SIZE_H, codec_name);
#endif
...
static BOOL check_test_pattern(UINT32 dec_type)
{
	...
	// check .dat file
#if SVC_BITSTREAM_TESTING
	sprintf(filename, "video_bs_%d_%d_%s_svc_partial.dat", VDO_SIZE_W, VDO_SIZE_H, codec_name);
#else
	sprintf(filename, "video_bs_%d_%d_%s_partial.dat", VDO_SIZE_W, VDO_SIZE_H, codec_name);
#endif
	...
	// check .len file
#if SVC_BITSTREAM_TESTING
	sprintf(filename, "video_bs_%d_%d_%s_svc_partial.len", VDO_SIZE_W, VDO_SIZE_H, codec_name);
#else
	sprintf(filename, "video_bs_%d_%d_%s_partial.len", VDO_SIZE_W, VDO_SIZE_H, codec_name);
#endif
	sprintf(filepath, "/mnt/sd/%s", filename);
...
static void *feed_bs_thread(void *arg)
{
	...
	char codec_name[8], file_name[128];
	FILE *bs_fd = 0, *bslen_fd = 0, *bslen_fd_total = 0;
	HD_RESULT ret = HD_OK;
	HD_COMMON_MEM_VB_BLK blk;
	HD_COMMON_MEM_DDR_ID ddr_id = DDR_ID0;
	UINT32 blk_size = BS_BLK_SIZE;
	UINT32 pa = 0, va = 0, bs_buf_start = 0, bs_buf_curr = 0, bs_buf_end = 0;
	INT bs_size = 0, bs_size_total = 0, read_len = 0, bs_size_sum = 0;
	BOOL is_desc = FALSE;
	HD_VIDEO_FRAME video_frame = {0};
	BOOL bGetVideo = FALSE, bPushInErr = FALSE;
	...
	snprintf(file_name, sizeof(file_name), "/mnt/sd/video_bs_%d_%d_%s_total.len", VDO_SIZE_W, VDO_SIZE_H, codec_name);
	if ((bslen_fd_total = fopen(file_name, "rb")) == NULL) {
		printf("open file (%s) fail !!....\r\nPlease copy test pattern to SD Card !!\r\n\r\n", file_name);
		goto quit_rel_fd;
	}
	printf("bslen total file: [%s]\n", file_name);
	...
	while (p_stream0->dec_exit == 0) {
...
#if 0
		UINT8 *ptrbs2 = (UINT8 *)bs_buf_curr;
		if (bGetVideo && ((*ptrbs2 == 0x0 && *(ptrbs2+1) == 0x0 && *(ptrbs2+2) == 0x0 && *(ptrbs2+3) == 0x1))) {
			// release video frame buf
			printf("2 rel blk 0x%x, 0x%x\r\n", video_frame.blk, bs_size);
		...
		if (!is_desc) { // only push I or P
			...
			UINT8 *ptrbs = (UINT8 *)bs_buf_curr;
			if (bPushInErr || (*ptrbs == 0x0 && *(ptrbs+1) == 0x0 && *(ptrbs+2) == 0x0 && *(ptrbs+3) == 0x1)) {
				// get video frame
				...
				// get bs size
				if (fscanf(bslen_fd_total, "%d\n", &bs_size_total) == EOF) {
					// reach EOF, read from the beginning
					fseek(bslen_fd_total, 0, SEEK_SET);
					if (fscanf(bslen_fd_total, "%d\n", &bs_size_total) == EOF) {
						printf("[ERROR] bs_size_total fscanf error\n");
						continue;
					}
				}
			}
			...
			if (bs_size_sum == bs_size_total) {
				video_bitstream.count         = bs_size_total;
				bs_size_sum = 0;
			} else {
				video_bitstream.count         = 0x800000;
			}
...
#endif
			ret = hd_videodec_push_in_buf(p_stream0->dec_path, &video_bitstream, &video_frame, -99);
			if (ret != HD_OK) {
				printf("push_in error(%d) !!\r\n", ret);
				// release video frame buf
				//printf("1 rel blk 0x%x, 0x%x\r\n", video_frame.blk, bs_size);
				ret = hd_videodec_release_out_buf(p_stream0->dec_path, &video_frame);
				bGetVideo = FALSE;
				bPushInErr = TRUE;
				if (ret != HD_OK) {
		...
		usleep(15000); // sleep 500 ms, to avoid pull_out write card too slow
	}
...
quit_rel_fd:
	if (bs_fd) fclose(bs_fd);
	if (bslen_fd) fclose(bslen_fd);
	if (bslen_fd_total) fclose(bslen_fd_total);
...
EXAMFUNC_ENTRY(hd_video_playback_partial, argc, argv)
{
	HD_RESULT ret;
    INT key;
	...
	ret = pthread_create(&stream[0].feed_thread_id, NULL, decode_thread, (void *)stream);
	if (ret < 0) {
