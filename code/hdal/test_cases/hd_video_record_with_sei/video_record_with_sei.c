 ...
 *  codec_type: 0: H265, 1: H264
 * 
 * Return:
 *  length of output bitstream
 *************************************/
static UINT32 encode_gen_sei(UINT8 *out_stream, UINT8 *data, UINT32 length, HD_VIDEO_CODEC codec_type)
{
	UINT32 byte_pos = 0;
	UINT32 tmp = length;
	UINT32 i;
	...
	if (HD_CODEC_TYPE_H264 == codec_type) {	//for H264
	...
	else if (HD_CODEC_TYPE_H265 == codec_type) {	//for H265
		// 1. encode start code & SEI header
		out_stream[byte_pos++] = 0x00;
		out_stream[byte_pos++] = 0x00;
		out_stream[byte_pos++] = 0x00;
	...
	while (p_stream0->enc_exit == 0) {
		...
		if (ret == HD_OK) {
			for (j=0; j< data_pull.pack_num; j++) {
				...
				// packing sei @ I frame and between PPS & slice
				if ((data_pull.frame_type == HD_FRAME_TYPE_IDR || data_pull.frame_type == HD_FRAME_TYPE_I) && (j == data_pull.pack_num - 2)) {
					char sei_data[20] = "NovatekTestSEI";
					UINT8 sei_pack[32];
					len = encode_gen_sei(sei_pack, (UINT8 *)sei_data, strlen(sei_data), data_pull.vcodec_format);
					if (f_out_main) fwrite(sei_pack, 1, len, f_out_main);
					if (f_out_main) fflush(f_out_main);
				}
			}
