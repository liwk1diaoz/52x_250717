...
static HD_RESULT mem_get(MEM_PARM *mem_parm, UINT32 size, UINT32 id)
{
	...
	if (HD_COMMON_MEM_VB_INVALID_BLK == blk) {
		printf("hd_common_mem_get_block fail, try next block\r\n");
		blk = hd_common_mem_get_block(HD_COMMON_MEM_COMMON_POOL, size, DDR_ID0);
		if (HD_COMMON_MEM_VB_INVALID_BLK == blk) {
...
*/
static HD_RESULT mem_alloc(MEM_PARM *mem_parm, CHAR* name, UINT32 size, int ddr_id)
{
	HD_RESULT ret = HD_OK;
	UINT32 pa   = 0;
	void  *va   = NULL;
	...
	//alloc private pool
	ret = hd_common_mem_alloc(name, &pa, (void**)&va, size, ddr_id);
	if (ret!= HD_OK) {
		return ret;
	}
...
static INT32 mem_load(MEM_PARM *mem_parm, const CHAR *filename)
{
	...
	if (mem_parm->size < (UINT32)size) {
		printf("ERROR: ai_in_buf(%lu) is not enough, input file(%lu)\r\n", mem_parm->size, (UINT32)size);
		size = -1;
		goto exit;
	}
...
static HD_RESULT input_open(NET_PATH_ID net_path)
{
	...
	snprintf(buf_name, 256, "ai_in_buf_%lu", proc_id);
	ret = mem_alloc(&p_net->input_mem, buf_name, AI_RGB_BUFSIZE(p_net->in_cfg.w, p_net->in_cfg.h), DDR_ID0);
	if (ret != HD_OK) {
	...
	ret = _load_buf(&p_net->input_mem, 
		...
		p_net->in_cfg.fmt);
	if (ret != HD_OK) {
		printf("proc_id(%lu) input_open fail=%d\n", proc_id, ret);
	}
...
typedef struct _VIDEO_LIVEVIEW {
	...
	UINT32 proc_oneshot;
	UINT32 process_id;
	UINT32 disable_multi_process;

} VIDEO_LIVEVIEW;

static HD_RESULT network_init(VIDEO_LIVEVIEW *p_stream)
{
	HD_RESULT ret = HD_OK;
	//int i;
	/*
	...
	*/
	// config extend engine plugin, process scheduler
	{
		...
		vendor_ai_cfg_set(VENDOR_AI_CFG_MULTI_THREAD, &is_multi);
		if (p_stream->disable_multi_process != 1) {
			vendor_ai_cfg_set(VENDOR_AI_CFG_MULTI_PROCESS, &is_multi_process);
		}
	}
...
static INT32 network_mem_config(NET_PATH_ID net_path, HD_COMMON_MEM_INIT_CONFIG* p_mem_cfg, void* p_cfg, INT32 i)
{
	...
	printf("proc_id(%lu) set net_mem_cfg: model-file(%s), binsize=%d\r\n", 
		proc_id,
	...
	p_mem_cfg->pool_info[i].blk_size = p_net->net_cfg.binsize * 2;
...
static HD_RESULT network_alloc_io_buf(NET_PATH_ID net_path)
{
	...
	snprintf(buf_name, 256, "ai_io_buf_%lu", proc_id);
	ret = mem_alloc(&p_net->io_mem, buf_name, wbuf.size, DDR_ID0);
	if (ret != HD_OK) {
...
static HD_RESULT network_open(NET_PATH_ID net_path)
{
	...
	snprintf(buf_name, 256, "ai_model_buf_%lu", proc_id);
	ret = mem_alloc(&p_net->proc_mem, buf_name, p_net->net_cfg.binsize, DDR_ID1);
	...
	if (ret != HD_OK) {
		printf("proc_id(%lu) load_label fail=%d\n", proc_id, ret);
		return HD_ERR_FAIL;
	}
...
static HD_RESULT init_module(VIDEO_LIVEVIEW *p_stream)
{
	HD_RESULT ret;
	//if ((ret = input_init()) != HD_OK)
	//	return ret;
	if ((ret = network_init(p_stream)) != HD_OK)
		return ret;
	return HD_OK;
}
...
static VOID *network_user_thread(VOID *arg)
{
	...
	NET_IN_CONFIG* p_in_cfg = &p_stream->net_in_cfg;

	NET_PROC_CONFIG* p_proc_cfg = &p_stream->net_proc_cfg;

	UINT32 select_proc_id;
	...
	for (j = 0; j < 100; j++) {
		...
		if (p_stream->proc_oneshot) {
			//printf("before get_id, process_id = %u, net_path = %u\n",p_stream->process_id,p_stream->net_path);
			vendor_ai_get_id(&select_proc_id);
			//printf("after get_id, process_id = %u, net_path = %u, select_proc_id = %u\n",p_stream->process_id,p_stream->net_path,select_proc_id);
			p_net->proc_id = select_proc_id;
			//p_net->proc_id = (p_stream->process_id - 1)*NN_RUN_NET_NUM + p_stream->net_path; 
			// set open config
			ret = input_set_config(p_stream->in_path, p_in_cfg);
			if (HD_OK != ret) {
				printf("proc_id(%lu) input_set_config fail=%d\n", p_stream->in_path, ret);
				goto skip;
			}
			ret = network_set_config(p_stream->net_path, p_proc_cfg);
			if (HD_OK != ret) {
				printf("proc_id(%lu) network_set_config fail=%d\n", p_stream->net_path, ret);
				goto skip;
			}
			// open video_liveview modules
			ret = open_module(p_stream);
			if (ret != HD_OK) {
				printf("open fail=%d\n", ret);
				goto skip;
			}
			...
			// set input image
			ret = vendor_ai_net_set(p_net->proc_id, VENDOR_AI_NET_PARAM_IN(0, 0), &in_buf);
			if (HD_OK != ret) {
			...
			// get output result
			ret = vendor_ai_net_get(p_net->proc_id, VENDOR_AI_NET_PARAM_OUT(VENDOR_AI_MAXLAYER, 0), &out_buf);
			if (HD_OK != ret) {
				printf("proc_id(%lu) output get fail !!\n", p_stream->net_path);
				goto skip;
			}
			//printf("proc_id(%u) oneshot done!\n", p_stream->net_path);
			printf("proc done, process_id = %lu, net_path = %lu, proc id = %lu, proc_count = %lu!\n",p_stream->process_id,p_stream->net_path,p_net->proc_id,proc_count);
			...
			vendor_ai_release_id(select_proc_id);
		}
		usleep(100);
	}
	// uninit all modules
...
int main(int argc, char *argv[])
{
	...
	//INT key;
	UINT32 process_id = 0;
	UINT32 disable_multi_process = 0;
	UINT32 i/*,j*/;

	//net_in
	NET_IN_CONFIG in_cfg[NN_RUN_NET_NUM];
	NET_IN_CONFIG in_cfg_ = {
	...
	for (i = 0; i < NN_RUN_NET_NUM; i++) {
		memcpy(&in_cfg[i],&in_cfg_,sizeof(NET_IN_CONFIG));
		memcpy(&net_cfg[i],&net_cfg_,sizeof(NET_PROC_CONFIG));
	}

	if(argc < 3){
		printf("usage : ai_net_with_mprocess (job_opt) (job_sync) (buf_opt) (process_id) (disable_multi_process)\n");
		return -1;
	}
	...
	int job_method = 0, job_wait_ms = 0, buf_method = 0;
	if (argc > idx) {
	...
	if (argc > idx) {
		sscanf(argv[idx++], "%lu", &process_id);
	}
	if (argc > idx) {
		sscanf(argv[idx++], "%lu", &disable_multi_process);
	}
	for (i = 0; i < NN_RUN_NET_NUM; i++) {
	...
	for (i = 0; i < NN_RUN_NET_NUM; i++) {
		stream[i].in_path = i;
		stream[i].net_path = i;
		stream[i].process_id = process_id;
		stream[i].disable_multi_process = disable_multi_process;	
	}
	...
	if(process_id > 0) {
		...
		*/
		...
		for (i = 0; i < NN_RUN_NET_NUM; i++) {
			...
			stream[i].net_in_cfg = in_cfg[i];
			stream[i].net_proc_cfg = net_cfg[i];
