/**
	@brief Source file of vendor ai net with buffer list sample code.

	@file ai_net_with_buf_list.c
...
#define INPUT_BY_BUF_LIST       1  // input by ai_buf_list (y/uv or r/g/b separated)
...
#if INPUT_BY_BUF_LIST
static HD_RESULT input_pull_buf_by_channel(NET_PATH_ID net_path, VENDOR_AI_BUF *p_in, INT32 wait_ms)
{
	HD_RESULT ret = HD_OK;
	NET_IN* p_net = g_in + net_path;
	INT i;
	...
	for (i = 0; i < INPUT_CHANNEL_NUM; i++) {
		memcpy((void*)&p_in[i], (void*)&(p_net->src_img), sizeof(VENDOR_AI_BUF));
		if (i == 0) {
			printf("input y plane:\n");
			p_in[i].fmt = HD_VIDEO_PXLFMT_Y8;
		} else {
			printf("input uv plane:\n");
			p_in[i].fmt = HD_VIDEO_PXLFMT_UV;
			p_in[i].pa = p_net->src_img.pa + p_net->src_img.width * p_net->src_img.height;
			p_in[i].va = p_net->src_img.va + p_net->src_img.width * p_net->src_img.height;
		}
		printf("    fmt: 0x%lx\n", p_in[i].fmt);
		printf("    pa:  0x%lx\n", p_in[i].pa);
		printf("    va:  0x%lx\n", p_in[i].va);
...
#if CONFIG_INTERNAL_BUFFER
static HD_RESULT network_alloc_intl_buf(NET_PATH_ID net_path)
{
	HD_RESULT ret = HD_OK;
	NET_PROC* p_net = g_net + net_path;
	UINT32 proc_id = p_net->proc_id;
	...
	if (ret != HD_OK) {
		printf("proc_id(%lu) get VENDOR_AI_NET_PARAM_CFG_INTLBUF fail\r\n", proc_id);
		return HD_ERR_FAIL;
	}
	ret = mem_alloc(&p_net->intl_mem, "ai_intl_buf", buf.size);
	if (ret != HD_OK) {
		printf("proc_id(%lu) alloc ai_io_buf fail\r\n", proc_id);
		return HD_ERR_FAIL;
	}

	buf.pa = p_net->intl_mem.pa;
	buf.va = p_net->intl_mem.va;
	buf.size = p_net->intl_mem.size;
	ret = vendor_ai_net_set(proc_id, VENDOR_AI_NET_PARAM_CFG_INTLBUF, &buf);
	if (ret != HD_OK) {
		printf("proc_id(%lu) set VENDOR_AI_NET_PARAM_CFG_INTLBUF fail\r\n", proc_id);
		return HD_ERR_FAIL;
	}

	printf("alloc_intl_buf: internal buf, pa = %#lx, va = %#lx, size = %lu\r\n", buf.pa, buf.va, buf.size);
...
#if CONFIG_INTERNAL_BUFFER
	if ((ret = network_alloc_intl_buf(net_path)) != HD_OK)
		return ret;
#endif
	// open
...
static VOID *network_user_thread(VOID *arg)
{
	...
	while (p_stream->proc_exit == 0) {
		...
		if (p_stream->proc_oneshot) {
#if INPUT_BY_BUF_LIST
			VENDOR_AI_BUF	in_buf[INPUT_CHANNEL_NUM] = {0};
			VENDOR_AI_BUF_LIST in_buf_list = {0};
#else
...
#if INPUT_BY_BUF_LIST
			input_pull_buf_by_channel(p_stream->in_path, &in_buf[0], 0);
			if (HD_OK != ret) {
			...
			// setup in_buf_list
			in_buf_list.sign = MAKEFOURCC('A','L','S','T');
			in_buf_list.buf_count = INPUT_CHANNEL_NUM;
			in_buf_list.buf_list = (VENDOR_AI_BUF *)&in_buf[0];

			printf("proc_id(%u) oneshot ...\n", p_stream->net_path);
			// set input image
			ret = vendor_ai_net_set(p_stream->net_path, VENDOR_AI_NET_PARAM_IN(0, 0), &in_buf_list);
			if (HD_OK != ret) {
