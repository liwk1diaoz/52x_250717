...
static void *encode_thread(void *arg)
{
	...
	//--------- pull data test ---------
	BOOL b_first_partial = TRUE;
	UINT32  handled_byte = 0;

	while (p_stream0->enc_exit == 0 || handled_byte != 0) {   // "handled_byte" to check last partial frame done
		//pull data
		ret = hd_videoenc_pull_out_buf(p_stream0->enc_path, &data_pull, -99); // -99 = partial pull mode

		if ((ret == HD_OK) || (ret == HD_ERR_INTERMEDIATE)) {
			// vps/sps/pps
			if (b_first_partial) {
#if DEBUG_PARTIAL
				printf("--------------------------\n");
#endif
				for (j=0; j< data_pull.pack_num-1; j++) {
				...
				b_first_partial = FALSE;
			}

			// data
			{
				UINT8 *ptr = (UINT8 *)PHY2VIRT_MAIN(data_pull.video_pack[data_pull.pack_num-1].phy_addr);
				UINT32 len = data_pull.video_pack[data_pull.pack_num-1].size;
				UINT32 need_handle_byte = len - handled_byte;

				if (ret != HD_OK && need_handle_byte < 1024) {
					// too less bytes, just sleep a while, pull again
					usleep(200); // sleep 200 us ( 0.2 ms)
					continue;
				}
				
				if (need_handle_byte > 0) {
#if DEBUG_PARTIAL
				printf("total len = %5d, handled_len = %5d, need_handle_len = %5d\n", len, handled_byte, need_handle_byte);
#endif
					if (f_out_main) fwrite(ptr + handled_byte, 1, need_handle_byte, f_out_main);
					if (f_out_main) fflush(f_out_main);
					handled_byte += need_handle_byte;
				}
			}
...
EXAMFUNC_ENTRY(hd_video_record, argc, argv)
{
	HD_RESULT ret;
	INT key;
