...
#endif
///////////////////////////////////////////////////////////////////////////////
UINT32 g_CAP_OUT_FMT  = 0; // option for Bayer compress
UINT32 g_PROC_OUT_FMT = 0; // option for YCC compress
UINT32 g_3DNR = 0;
UINT32 g_DATA_MODE = 0; //option for vcap output directly to vprc
///////////////////////////////////////////////////////////////////////////////
static char func_name[32]={0};
...
static HD_RESULT mem_init(void)
{
	...
	mem_cfg.pool_info[0].blk_size = DBGINFO_BUFSIZE()+VDO_CA_BUF_SIZE(CA_WIN_NUM_W, CA_WIN_NUM_H)+VDO_LA_BUF_SIZE(LA_WIN_NUM_W, LA_WIN_NUM_H);
	if (g_DATA_MODE == 0) {//D2D mode
		if (g_CAP_OUT_FMT == HD_VIDEO_PXLFMT_NRX12)
			mem_cfg.pool_info[0].blk_size += VDO_NRX_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H);
		else
			mem_cfg.pool_info[0].blk_size += VDO_RAW_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H, g_CAP_OUT_FMT);
	}
	...
	// config common pool (lcd)
	mem_cfg.pool_info[2].type = HD_COMMON_MEM_COMMON_POOL;
	mem_cfg.pool_info[2].blk_size = DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(960, 240, HD_VIDEO_PXLFMT_YUV420);
...
static HD_RESULT set_cap_param(HD_PATH_ID video_cap_path, HD_DIM *p_dim)
{
	...
	#endif
	{
		...
		video_out_param.pxlfmt = g_CAP_OUT_FMT;
...
static HD_RESULT set_proc_cfg(HD_PATH_ID *p_video_proc_ctrl, HD_DIM* p_max_dim)
{
	...
	if (p_max_dim != NULL ) {
		...
		video_cfg_param.in_max.pxlfmt = g_CAP_OUT_FMT;
	...
	if (g_3DNR == 0) {
		...
		/* Please make sure that it shall be follow rule -> "videocapture output size == videoprocess output size"  */
...
static HD_RESULT set_enc_param(HD_PATH_ID video_enc_path, HD_DIM *p_dim, UINT32 enc_type, UINT32 bitrate)
{
	...
	if (p_dim != NULL) {
		...
		video_in_param.pxl_fmt = g_PROC_OUT_FMT;
		video_in_param.dim.w   = p_dim->w;
		video_in_param.dim.h   = p_dim->h;
...
static HD_RESULT open_module_rec(VIDEO_RECORD *p_stream, HD_DIM* p_proc_max_dim)
{
	HD_RESULT ret;
	// set videocap config
...
static HD_RESULT open_module_lv(VIDEO_LIVEVIEW *p_stream, HD_DIM* p_proc_max_dim, UINT32 out_type)
{
	HD_RESULT ret;
...
static HD_RESULT close_module(VIDEO_RECORD *stream_rec, VIDEO_LIVEVIEW *stream_lv)
{
	HD_RESULT ret;
	if ((ret = hd_videocap_close(stream_rec->cap_path)) != HD_OK)
		return ret;

	if ((ret = hd_videoproc_close(stream_rec->proc_path)) != HD_OK)
		return ret;
	if ((ret = hd_videoenc_close(stream_rec->enc_path)) != HD_OK)
		return ret;
	if ((ret = hd_videoproc_close(stream_lv->proc_path)) != HD_OK)
		return ret;
	if ((ret = hd_videoout_close(stream_lv->out_path)) != HD_OK)
		return ret;
...
static void *encode_thread(void *arg)
{
    ...
    sprintf(file_path_main,"%s%s.dat","/mnt/sd/dump_bs_rec_lv",func_name);
	//------ wait flow_start ------
...
static HD_RESULT OPEN_MODULE(VIDEO_RECORD *stream_rec, VIDEO_LIVEVIEW *stream_lv, UINT32 out_type)
{
	HD_RESULT ret = HD_OK;

	// open video_record modules (main)
	stream_rec->proc_max_dim.w = VDO_SIZE_W; //assign by user
	stream_rec->proc_max_dim.h = VDO_SIZE_H; //assign by user
	ret = open_module_rec(stream_rec, &stream_rec->proc_max_dim);
	if (ret != HD_OK) {
		printf("open fail=%d\n", ret);
		goto exit;
	}
	...
	stream_lv->proc_max_dim.w = VDO_SIZE_W; //assign by user
	stream_lv->proc_max_dim.h = VDO_SIZE_H; //assign by user
	ret = open_module_lv(stream_lv, &stream_lv->proc_max_dim, out_type);
	if (ret != HD_OK) {
		printf("open fail=%d\n", ret);
		goto exit;
	}
...
static HD_RESULT SET_CAPS(VIDEO_RECORD *stream_rec)
{
	HD_RESULT ret = HD_OK;

	// get videocap capability
	ret = get_cap_caps(stream_rec->cap_ctrl, &stream_rec->cap_syscaps);
	if (ret != HD_OK) {
	...
	stream_rec->cap_dim.w = VDO_SIZE_W; //assign by user
	stream_rec->cap_dim.h = VDO_SIZE_H; //assign by user
	ret = set_cap_param(stream_rec->cap_path, &stream_rec->cap_dim);
	if (ret != HD_OK) {
...
static HD_RESULT SET_PROC(HD_PATH_ID proc_path_rec, HD_PATH_ID proc_path_lv)
{
	HD_RESULT ret = HD_OK;

	// set videoproc parameter (record)
	ret = set_proc_param(proc_path_rec, NULL);
	if (ret != HD_OK) {
	...
	ret = set_proc_param(proc_path_lv, NULL);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
...
static HD_RESULT SET_ENC(VIDEO_RECORD *stream_rec, UINT32 enc_type)
{
	HD_RESULT ret = HD_OK;
	HD_DIM main_dim;
	...
	stream_rec->enc_max_dim.w = main_dim.w;
	stream_rec->enc_max_dim.h = main_dim.h;
	ret = set_enc_cfg(stream_rec->enc_path, &stream_rec->enc_max_dim, 2 * 1024 * 1024);
	if (ret != HD_OK) {
	...
	stream_rec->enc_dim.w = main_dim.w;
	stream_rec->enc_dim.h = main_dim.h;
	ret = set_enc_param(stream_rec->enc_path, &stream_rec->enc_dim, enc_type, 2 * 1024 * 1024);
	if (ret != HD_OK) {
...
static HD_RESULT SET_VOUT(VIDEO_LIVEVIEW *stream_lv)
{
	HD_RESULT ret = HD_OK;

	// get videoout capability
	ret = get_out_caps(stream_lv->out_ctrl, &stream_lv->out_syscaps);
	if (ret != HD_OK) {
		printf("get out-caps fail=%d\n", ret);
		goto exit;
	}
	stream_lv->out_max_dim = stream_lv->out_syscaps.output_dim;


	// set videoout parameter (main)
	stream_lv->out_dim.w = stream_lv->out_max_dim.w; //using device max dim.w
	stream_lv->out_dim.h = stream_lv->out_max_dim.h; //using device max dim.h
	ret = set_out_param(stream_lv->out_path, &stream_lv->out_dim);
	if (ret != HD_OK) {
...
EXAMFUNC_ENTRY(hd_video_rec_liview, argc, argv)
{
	HD_RESULT        ret;
	INT key;
	VIDEO_RECORD     stream_rec = {0};
	VIDEO_LIVEVIEW   stream_lv  = {0};
	UINT32           enc_type   = 0;
	UINT32           out_type   = 0;
	...
	if (argc != 7) {
		...
		printf("Usage: <enc_type> <out_type> <cap_out_fmt> <proc_out_fmt> <3dnr> <data_mode>.\r\n");
		printf("Help:\r\n");
		printf("  <enc_type>     : 0(H265),     1(H264), 2(JPEG)\r\n");
		printf("  <out_type>     : 0(CVBS),     1(LCD),  2(HDMI 1920x1080 I60)\r\n");
		printf("  <cap_out_fmt>  : 0(RAW12),    1(NRX12 - Bayer compress)\r\n");
		printf("  <proc_out_fmt> : 0(YUV420),   1(NVX - YCC compress)\r\n");
		printf("  <3dnr>         : 0(DISABLE),  1(ENABLE)\r\n");
		printf("  <data_mode>    : 0(D2D Mode), 1(Direct Mode)\r\n");
		return 0;
	} else {
		enc_type       = atoi(argv[1]);
		out_type       = atoi(argv[2]);
		g_CAP_OUT_FMT  = atoi(argv[3]);
		g_PROC_OUT_FMT = atoi(argv[4]);
		g_3DNR         = atoi(argv[5]);
		g_DATA_MODE    = atoi(argv[6]);

		printf("enc_type     : %d\r\n", enc_type);
		printf("out_type     : %d\r\n", out_type);
		printf("cap_out_fmt  : %d\r\n", g_CAP_OUT_FMT);
		printf("proc_out_fmt : %d\r\n", g_PROC_OUT_FMT);
		printf("3dnr         : %d\r\n", g_3DNR);
		printf("data_mode    : %d\r\n", g_DATA_MODE);

        memset((void *)func_name,0,sizeof(func_name));
		//renaming by param
        sprintf(func_name,"_%d_%d_%d_%d_%d_%d",(int)enc_type,(int)out_type,(int)g_CAP_OUT_FMT,(int)g_PROC_OUT_FMT,(int)g_3DNR,(int)g_DATA_MODE);
		...
		if(out_type > 2) {
			printf("error: not support out_type!(%d)\r\n", out_type);
			return 0;
		}
		if(g_CAP_OUT_FMT > 1) {
			printf("error: not support cap_out_fmt!(%d)\r\n", g_CAP_OUT_FMT);
			return 0;
		}
		if(g_PROC_OUT_FMT > 1) {
			printf("error: not support proc_out_fmt!(%d)\r\n", g_PROC_OUT_FMT);
			return 0;
		}
		if(g_3DNR > 1) {
		...
		// Assign g_CAP_OUT_FMT
		if (g_CAP_OUT_FMT == 0) {
			g_CAP_OUT_FMT = HD_VIDEO_PXLFMT_RAW12;
		} else {
			g_CAP_OUT_FMT =  HD_VIDEO_PXLFMT_NRX12;
		}

		// Assign g_PROC_OUT_FMT
		if (g_PROC_OUT_FMT == 0) {
			g_PROC_OUT_FMT = HD_VIDEO_PXLFMT_YUV420;
		} else {
			#if 0 //680
				if (enc_type == 0) {
					g_PROC_OUT_FMT = HD_VIDEO_PXLFMT_YUV420_NVX1_H265;
				} else if (enc_type == 1) {
					g_PROC_OUT_FMT = HD_VIDEO_PXLFMT_YUV420_NVX1_H264;
				} else {
					g_PROC_OUT_FMT = HD_VIDEO_PXLFMT_YUV420;
				}
			#else //520
				g_PROC_OUT_FMT = HD_VIDEO_PXLFMT_YUV420_NVX2;
			#endif
		}
	}

	//temp setting for IQ parameter
	#if defined(__LINUX)
	if (g_3DNR) {
		char cmd[256];

		snprintf(cmd, 256, "echo w ctltest reg_isp exam_isp 0x00010000 > /proc/kflow_ctl_ipp/cmd");
		system(cmd);
	}
	#endif

	stream_lv.hdmi_id=HD_VIDEOOUT_HDMI_1920X1080I60;//default
	...
	ret = OPEN_MODULE(&stream_rec, &stream_lv, out_type);
	if (ret != HD_OK) {
		printf("OPEN_MODULE fail=%d\n", ret);
		goto exit;
	}
	ret = SET_CAPS(&stream_rec);
	if (ret != HD_OK) {
		printf("SET_CAPS fail=%d\n", ret);
		goto exit;
	}
	ret = SET_PROC(stream_rec.proc_path, stream_lv.proc_path);
	if (ret != HD_OK) {
		printf("SET_PROC fail=%d\n", ret);
		goto exit;
	}
	ret = SET_ENC(&stream_rec, enc_type);
	if (ret != HD_OK) {
		printf("SET_ENC fail=%d\n", ret);
		goto exit;
	}
	ret = SET_VOUT(&stream_lv);
	if (ret != HD_OK) {
		printf("SET_VOUT fail=%d\n", ret);
		goto exit;
	}
	...
	ret = pthread_create(&stream_rec.enc_thread_id, NULL, encode_thread, (void *)&stream_rec);
	if (ret < 0) {
		printf("create encode thread failed");
		goto exit;
	}

	// start video record / video liveview
	if (g_DATA_MODE) {
		//direct NOTE: ensure videocap start after 1st videoproc phy path start
		hd_videoproc_start(stream_rec.proc_path);
		hd_videocap_start(stream_rec.cap_path);
		hd_videoproc_start(stream_lv.proc_path);
	} else {
		hd_videocap_start(stream_rec.cap_path);
		hd_videoproc_start(stream_rec.proc_path);
		hd_videoproc_start(stream_lv.proc_path);
	...
	sleep(1);
	hd_videoenc_start(stream_rec.enc_path);
	hd_videoout_start(stream_lv.out_path);

	// let encode_thread start to work
	stream_rec.flow_start = 1;
	...
	pthread_join(stream_rec.enc_thread_id, NULL);

	// stop video record / video liveview
	if (g_DATA_MODE) {
		//direct NOTE: ensure videocap stop after all videoproc path stop
		hd_videoproc_stop(stream_rec.proc_path);
		hd_videoproc_stop(stream_lv.proc_path);
		hd_videocap_stop(stream_rec.cap_path);
	} else {
		hd_videocap_stop(stream_rec.cap_path);
		hd_videoproc_stop(stream_rec.proc_path);
		hd_videoproc_stop(stream_lv.proc_path);
	}
	hd_videoenc_stop(stream_rec.enc_path);
	hd_videoout_stop(stream_lv.out_path);
...
exit:
	...
	ret = close_module(&stream_rec, &stream_lv);
	if (ret != HD_OK) {
		printf("close fail=%d\n", ret);
	}
