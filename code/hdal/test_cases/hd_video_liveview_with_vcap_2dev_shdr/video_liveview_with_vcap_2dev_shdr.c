/**
	...
	@file video_liveview_with_vcap_2dev_shdr.c

	@author Ben Wang

	@ingroup mhdal

	@note This file is modified from video_liveview_with_vcap_2dev.c.
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_liveview_with_vcap_2dev_shdr, argc, argv)
...
#define SEN1_SHDR_VCAP_MAP (HD_VIDEOCAP_0|HD_VIDEOCAP_3)
#define SEN2_SHDR_VCAP_MAP (HD_VIDEOCAP_1|HD_VIDEOCAP_4)
///////////////////////////////////////////////////////////////////////////////

static UINT32 shdr_mode = 0;
static UINT32 slave_mode = 0;
static UINT32 slave_sync = 0;
static UINT32 sensor_en_map = 3; //1 for sensor 1, 2 for sensor 2 and 3 for all
static UINT32 mclk_src_sync = 0;
...
static HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl)
{
	...
	if (slave_mode) {
		...
		if (slave_sync) {
			cap_cfg.sen_cfg.sen_dev.if_cfg.tge.vcap_sync_set = SEN1_SHDR_VCAP_MAP;
		}
		cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.sensor_pinmux =  0x2A0; //PIN_SENSOR_CFG_MIPI | PIN_SENSOR_CFG_MCLK | PIN_SENSOR_CFG_LVDS_VDHD
		...
		cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.serial_if_pinmux = 0x301;//PIN_MIPI_LVDS_CFG_CLK1 | PIN_MIPI_LVDS_CFG_DAT0 | PIN_MIPI_LVDS_CFG_DAT1
		cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.cmd_if_pinmux = 0x10;//PIN_I2C_CFG_CH2
	}
	...
	if (mclk_src_sync) {
		UINT32 mclk_sync_set = ((1<<SEN1_VCAP_ID)|(1<<SEN2_VCAP_ID));
...
static HD_RESULT set_cap2_cfg(HD_PATH_ID *p_video_cap_ctrl)
{
	...
	if (slave_mode) {
		...
		cap_cfg.sen_cfg.sen_dev.if_cfg.tge.vcap_vd_src = HD_VIDEOCAP_SEN_TGE_CH1_VD_TO_VCAP0;
		if (slave_sync) {
			cap_cfg.sen_cfg.sen_dev.if_cfg.tge.vcap_sync_set = SEN2_SHDR_VCAP_MAP;
		}
		cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.sensor_pinmux =   0xA0; //PIN_SENSOR_CFG_MIPI | PIN_SENSOR2_CFG_LVDS_VDHD
		...
		if (mclk_src_sync) {
			...
			cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.sensor_pinmux = 0x1020; //PIN_SENSOR2_CFG_MIPI|PIN_SENSOR2_CFG_SN_MCLK
		}
		cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.serial_if_pinmux = 0xC02;//PIN_MIPI_LVDS_CFG_CLK1 | PIN_MIPI_LVDS_CFG_DAT2 | PIN_MIPI_LVDS_CFG_DAT3
		cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.cmd_if_pinmux = 0x01;//PIN_I2C_CFG_CH1
	}
...
MAIN(argc, argv)
{
	...
	//default dual sensor linear and master mode
	shdr_mode = 0;
	slave_mode = 0;
	slave_sync = 0;

	if (argc > 1) {
		shdr_mode = atoi(argv[1]);
		printf("shdr_mode %d\r\n", shdr_mode);
	}
	if (argc > 2) {
		slave_mode = atoi(argv[2]);
		printf("slave_mode %d\r\n", slave_mode);
	}
	if (argc > 3) {
		slave_sync = atoi(argv[3]);
		printf("slave_sync %d\r\n", slave_sync);
	}
	if (argc > 4) {
		mclk_src_sync = atoi(argv[4]);
		printf("mclk_src_sync %d\r\n", mclk_src_sync);
	}
	//###########  NOT support yet ############
	if (slave_mode || slave_sync) {
		printf("NOT support TGE yet!!!!!!!!!!!\n");
		goto exit;
	}
	//########################################
	if (argc > 5) {
	...
	if (argc > 6) {
		sensor_en_map = atoi(argv[6]);
		printf("sensor_en_map %d\r\n", sensor_en_map);
		if (sensor_en_map == 1)
			display = 0;
		else if (sensor_en_map == 2)
			display = 1;
	}
...
exit:
	...
	if ((ret = hd_videoout_close(stream[0].out_path)) != HD_OK) {
		if (ret != HD_OK)
			printf("close out fail=%d\n", ret);
	}
