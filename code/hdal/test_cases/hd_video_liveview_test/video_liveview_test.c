...
static HD_RESULT mem_init(void)
{
	...
	if ((g_prcbind == 0) || (g_prcbind == 0xff)) {
		...
		mem_cfg.pool_info[2].blk_cnt = 2; //no-bind and no pull
		mem_cfg.pool_info[2].ddr_id = DDR_ID0;
		if (g_capbind == 1) {
			//direct
			mem_cfg.pool_info[2].blk_cnt += 1; //direct will pre-new 1
		}
	} else if (g_prcbind == 2) {
		//one-buf
		mem_cfg.pool_info[2].blk_size = DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H, HD_VIDEO_PXLFMT_YUV420);
		...
		//not support
		mem_cfg.pool_info[2].blk_size = 0;
		mem_cfg.pool_info[2].blk_cnt = 0;
		mem_cfg.pool_info[2].ddr_id = DDR_ID0;
	}
	}

	#if DUMP_RAW_DEBUG //this pool is only for debug usage
	{
		UINT32 debug_pool;
		if (g_3dnr == 1) {
			debug_pool = 3;
		...
		mem_cfg.pool_info[debug_pool].type = HD_COMMON_MEM_COMMON_POOL;
		mem_cfg.pool_info[debug_pool].blk_size = DBGINFO_BUFSIZE() + VDO_CA_BUF_SIZE(CA_WIN_NUM_W, CA_WIN_NUM_H)
															+ VDO_LA_BUF_SIZE(LA_WIN_NUM_W, LA_WIN_NUM_H);
		mem_cfg.pool_info[debug_pool].blk_size += VDO_RAW_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H, g_capfmt);
		mem_cfg.pool_info[debug_pool].blk_cnt = 2;
		mem_cfg.pool_info[debug_pool].ddr_id = DDR_ID0;
	}
	#endif
...
MAIN(argc, argv)
{
	...
	if (argc == 1) {
		printf("Usage: <out-type> <cap_out_bind> <cap_out_fmt> <prc_out_bind> <prc_out_fmt> <shdr> <3dnr>.\r\n");
		printf("Help:\r\n");
	...
	if (argc > 8) {
		g_remap_vcap_sie = atoi(argv[8]);
		printf("remap_vcap_sie %d\r\n", g_remap_vcap_sie);
	}
	if (argc > 9) {
		g_pattern_gen = atoi(argv[9]);
