...
typedef struct _NN_DIFF_PARM_MULTICUST {
	...
	UINT32 process_cycle;
#endif
	UINT32 multi_in_width[7];      
	UINT32 multi_in_height[7];
	UINT32 multi_in_batch[7]; 
    UINT32 multi_in_channel[7]; 
	NN_DIFF_OFS multi_in_ofs[5];
}NN_DIFF_PARM_MULTICUST;

typedef struct _NN_DIFF_OUTPUT_PARM {
  	UINT16 mctrl_id;
  	UINT16 port_id;
  	UINT32 width;
  	UINT32 height;
...
typedef struct _NN_DIFF_BATCH_HEAD {
    UINT32 batch_id;         ///< configuration of batch model     
    UINT32 batch_parm_size;  // batch parameters size
    UINT32 io_buff_size;
} NN_DIFF_BATCH_HEAD;

typedef struct NN_DIFF_BATCH_PARM {
    UINT32 in_batch_ratio;   ///<  max batch num / in batch_num 
    UINT32 out_batch_ratio;
    BOOL skip_en;
}NN_DIFF_BATCH_PARM; 
/*
nvt_diff_batch_model in external
...
//diff model 1
////////////////////////  ////////////////  //////////////////////// 
// NN_DIFF_BATCH_HEAD //  //  NN_IOMEM  //  // NN_DIFF_BATCH_PARM // 
////////////////////////  ////////////////  //////////////////////// 
