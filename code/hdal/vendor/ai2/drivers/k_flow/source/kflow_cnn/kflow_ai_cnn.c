...
//=============================================================
#define __CLASS__ 				"[ai][kflow][cnn]"
#include "kflow_ai_debug.h"
//=============================================================
#include "kflow_cnn/kflow_cnn_platform.h"
...
static SEM_HANDLE SEMID_KFLOW_CNN;
static KFLOW_AI_CHANNEL_CTX kflow_cnn_ch1;
#if defined(_BSP_NA51090_) || defined(_BSP_NA51102_) || defined(_BSP_NA51103_)
static INT32 kflow_cnn_ch1_isrcb(UINT32 cycle_eng, UINT32 cycle_ll, UINT32 cycle_dma, UINT32 intstatus, UINT32 eng, void *parm);
static INT32 kflow_cnn_ch2_isrcb(UINT32 cycle_eng, UINT32 cycle_ll, UINT32 cycle_dma, UINT32 intstatus, UINT32 eng, void *parm);
#else
static INT32 kflow_cnn_ch1_isrcb(UINT32 handle, UINT32 intstatus, UINT32 eng, void *parm);
static INT32 kflow_cnn_ch2_isrcb(UINT32 handle, UINT32 intstatus, UINT32 eng, void *parm);
#endif

static SEM_HANDLE SEMID_KFLOW_CNN2;
static KFLOW_AI_CHANNEL_CTX kflow_cnn_ch2;
...
int kflow_cnn_set_output_path(char *path)
{
	if (path == NULL) {
		return -1;
	}
...
EXPORT_SYMBOL(kflow_cnn_set_output_path);


static int _kflow_cnn_write_file(char *filename, void *data, size_t count, unsigned long long *offset)
{
	int ret = 0;
	int fd;
...
static void _kflow_cnn_dump_engine_register(KFLOW_AI_CHANNEL_CTX *p_ch, KFLOW_AI_JOB* p_job)
{
	if (!p_ch) {
	...
	if (strcmp(p_ch->name, "cnn1") == 0) {
		debug_dumpmem(0xf0cb0000, 0x220);
	} else if (strcmp(p_ch->name, "cnn2") == 0) {
		debug_dumpmem(0xf0d60000, 0x220);
	}
}

static void _kflow_cnn_dump_out_buf(KFLOW_AI_CHANNEL_CTX *p_ch, KFLOW_AI_JOB *p_job)
{
	...
	for (out_id = 0; out_id < omem_cnt; out_id++) {
		if (p_sao[out_id].size > 0) {
			...
			DBG_DUMP("write file: %s\n\n\n", filename);
			vos_cpu_dcache_sync(p_sao[out_id].va, p_sao[out_id].size, VOS_DMA_FROM_DEVICE);
			_kflow_cnn_write_file(filename, (void *)p_sao[out_id].va, p_sao[out_id].size, 0);
		}
	}
#endif
}

static void kflow_cnn_ch1_open(KFLOW_AI_CHANNEL_CTX *p_ch)
{
	const UINT32 chip = 0;
	...
	SEM_CREATE(SEMID_KFLOW_CNN, 1);
	SEM_WAIT(SEMID_KFLOW_CNN);
	...
	id = KDRV_DEV_ID(chip, KDRV_AI_ENGINE_CNN, channel);
	kdrv_ai_set(id, KDRV_AI_PARAM_OPENCFG, &cfg.opencfg);
	kdrv_ai_set(id, KDRV_AI_PARAM_ISR_CB, kflow_cnn_ch1_isrcb);
	kdrv_ai_open(chip, KDRV_AI_ENGINE_CNN); //clk set to 600MHz
	INIT_LIST_HEAD(p_ch->ready_queue);

	p_ch->wait_ms = -1;

	SEM_SIGNAL(SEMID_KFLOW_CNN);
}

static void kflow_cnn_ch1_close(KFLOW_AI_CHANNEL_CTX *p_ch)
{
	...
	kdrv_ai_close(chip, KDRV_AI_ENGINE_CNN);
	//TODO: remove all jobs still remain in p_ch->ready_queue

	SEM_SIGNAL(SEMID_KFLOW_CNN);
	SEM_DESTROY(SEMID_KFLOW_CNN);
}

static void kflow_cnn_ch1_trig(KFLOW_AI_CHANNEL_CTX *p_ch, KFLOW_AI_JOB *p_job)
{
	const UINT32 chip = 0, channel = 0;
...
#if (SHOW_FLOW == 1)
	{
		DBG_DUMP("----- proc[%d].job[%d] cnn1 trig: layer=%d, eng=%d, mode=%d, trig=%d, cnt=%d\r\n",
	...
	case NN_GEN_TRIG_LL_AI_DRV:
		if (p_mctrl->tot_trig_eng_times) {
			...
			omem_cnt = p_mctrl->iomem.omem_cnt;
			p_sao    = (NN_DATA*)p_mctrl->iomem.omem_addr;
			for (out_id = 0; out_id < OUT_BUF_NUM(p_mctrl); out_id++) {
				if (p_sao[out_id].size > 0) {
...
static void kflow_cnn_ch1_debug(KFLOW_AI_CHANNEL_CTX *p_ch, KFLOW_AI_JOB *p_job, UINT32 info)
{
	/* dump engine register */
	if (info & KFLOW_AI_DBG_CTX) {
		
		_kflow_cnn_dump_engine_register(p_ch, p_job);
	}

	/* dump output buffer */
	if (info & KFLOW_AI_DBG_OBUF) {
		_kflow_cnn_dump_out_buf(p_ch, p_job);
	}
}
...
#endif
{
	KFLOW_AI_CHANNEL_CTX *p_ch = &kflow_cnn_ch1;
	KFLOW_AI_JOB *p_job = p_ch->run_job;
	UINT32 *p_parm = (UINT32 *)parm;
...
#endif
	if (p_job == NULL) {
		DBG_DUMP("kflow_cnn_ch1_isrcb ERR - NULL run_job????????????\r\n");
		return 0;
	}

	if (p_parm[0] == AI_TRIG_MODE_APP) {
		p_job->exec_cnt++;
		p_mctrl = (NN_GEN_MODE_CTRL *)(p_job->p_op_info);
		if (p_job->exec_cnt == (INT32)p_mctrl->tot_trig_eng_times) {  // app mode will enter here tot_trig_eng_times
			if (p_ch->onfinish != 0) {
#if defined(_BSP_NA51090_) || defined(_BSP_NA51102_) || defined(_BSP_NA51103_)
	...
	} else if (p_parm[0] == AI_TRIG_MODE_LL) {
		p_job->exec_cnt++;
		if (p_ch->onfinish != 0) {
#if defined(_BSP_NA51090_) || defined(_BSP_NA51102_) || defined(_BSP_NA51103_)
			p_ch->onfinish(p_ch, p_ch->run_job, cycle_eng, macc);
...
static void kflow_cnn_ch1_reset2(struct _KFLOW_AI_CHANNEL_CTX *p_ch)
{
	KFLOW_AI_JOB *p_job = p_ch->run_job;
	//NN_GEN_MODE_CTRL *p_mctrl = (NN_GEN_MODE_CTRL*)(p_job->p_op_info);

	//DBG_IND("- do reset CNN1!\r\n");
...
static void kflow_cnn_ch2_open(KFLOW_AI_CHANNEL_CTX *p_ch)
{
	const UINT32 chip = 0;
	const UINT32 channel = 0;
	UINT32 id;
	...
	SEM_CREATE(SEMID_KFLOW_CNN2, 1);
	SEM_WAIT(SEMID_KFLOW_CNN2);

	cfg.opencfg.clock_sel = 0;
	cfg.engine = AI_ENG_CNN2;
	cfg.net_id = 0;

	kdrv_ai_config_flow(1); //config to new AI flow

	id = KDRV_DEV_ID(chip, KDRV_AI_ENGINE_CNN2, channel);
	kdrv_ai_set(id, KDRV_AI_PARAM_OPENCFG, &cfg.opencfg);
	kdrv_ai_set(id, KDRV_AI_PARAM_ISR_CB, kflow_cnn_ch2_isrcb);
	kdrv_ai_open(chip, KDRV_AI_ENGINE_CNN2); //clk set to 600MHz
	INIT_LIST_HEAD(p_ch->ready_queue);

	p_ch->wait_ms = -1;

	SEM_SIGNAL(SEMID_KFLOW_CNN2);
}

static void kflow_cnn_ch2_close(KFLOW_AI_CHANNEL_CTX *p_ch)
{
	const UINT32 chip = 0, channel = 0;
	KDRV_DEV_ENGINE engine = 0;
	UINT32 id = 0;
	...
	kdrv_ai_close(chip, KDRV_AI_ENGINE_CNN2);
	//TODO: remove all jobs still remain in p_ch->ready_queue

	SEM_SIGNAL(SEMID_KFLOW_CNN2);
	SEM_DESTROY(SEMID_KFLOW_CNN2);
}

static void kflow_cnn_ch2_trig(KFLOW_AI_CHANNEL_CTX *p_ch, KFLOW_AI_JOB *p_job)
{
	const UINT32 chip = 0, channel = 0;
	NN_GEN_ENG_TYPE ctrl_type;
	KDRV_DEV_ENGINE engine = 0;
...
#if (SHOW_FLOW == 1)
	{
		DBG_DUMP("----- proc[%d].job[%d] cnn2 trig: layer=%d, eng=%d, mode=%d, trig=%d, cnt=%d\r\n",
				 (int)p_job->proc_id, (int)p_job->job_id, p_mctrl->layer_index, p_mctrl->eng, p_mctrl->mode, p_mctrl->trig_src, p_mctrl->tot_trig_eng_times);
	}
#endif
#if (DUMMY == 1)
...
static void kflow_cnn_ch2_debug(KFLOW_AI_CHANNEL_CTX *p_ch, KFLOW_AI_JOB *p_job, UINT32 info)
{
	/* dump engine register */
	if (info & KFLOW_AI_DBG_CTX) {
		_kflow_cnn_dump_engine_register(p_ch, p_job);
...
#endif
{
	KFLOW_AI_CHANNEL_CTX *p_ch = &kflow_cnn_ch2;
	KFLOW_AI_JOB *p_job = p_ch->run_job;
	UINT32 *p_parm = (UINT32 *)parm;
	NN_GEN_MODE_CTRL *p_mctrl;
#if defined(_BSP_NA51090_) || defined(_BSP_NA51102_) || defined(_BSP_NA51103_)
...
#endif
	if (p_job == NULL) {
		DBG_DUMP("kflow_cnn_ch2_isrcb ERR - NULL run_job????????????\r\n");
		return 0;
	}
...
static void kflow_cnn_ch2_reset2(struct _KFLOW_AI_CHANNEL_CTX *p_ch)
{
	KFLOW_AI_JOB *p_job = p_ch->run_job;
	//NN_GEN_MODE_CTRL *p_mctrl = (NN_GEN_MODE_CTRL*)(p_job->p_op_info);

	//DBG_IND("- do reset CNN2!\r\n");
...
static LIST_HEAD kflow_cnn1_rq = {0};
static KFLOW_AI_CHANNEL_CTX kflow_cnn_ch1 =
{
	.name = "cnn1",
	.ready_queue = &kflow_cnn1_rq,
	.run_job = 0,
	.open = kflow_cnn_ch1_open,
	.close = kflow_cnn_ch1_close,
	.trigger = kflow_cnn_ch1_trig,
	.debug = kflow_cnn_ch1_debug,
	.reset2 = kflow_cnn_ch1_reset2,
};

static LIST_HEAD kflow_cnn2_rq = {0};
static KFLOW_AI_CHANNEL_CTX kflow_cnn_ch2 =
{
	.name = "cnn2",
	.ready_queue = &kflow_cnn2_rq,
	.run_job = 0,
	.open = kflow_cnn_ch2_open,
	.close = kflow_cnn_ch2_close,
	.trigger = kflow_cnn_ch2_trig,
	.debug = kflow_cnn_ch2_debug,
	.reset2 = kflow_cnn_ch2_reset2,
};
...
#define MAX_CHN 1
#endif
static LIST_HEAD kflow_cnn_wq = {0};
static KFLOW_AI_CHANNEL_CTX *kflow_cnn_ch[MAX_CHN] = {0};
static KFLOW_AI_ENGINE_CTX kflow_cnn_eng;

static void kflow_cnn_init(KFLOW_AI_ENGINE_CTX *p_eng)
{
	INT idx = 0;
	INIT_LIST_HEAD(p_eng->wait_queue);

	if (kdrv_ai_get_eng_caps(AI_ENG_CNN)) {
		kflow_cnn_ch[idx] = &kflow_cnn_ch1;
		idx++;
	}
...
#else
#if defined(_BSP_NA51055_)
	if (kdrv_ai_get_eng_caps(AI_ENG_CNN2)) {
		kflow_cnn_ch[idx] = &kflow_cnn_ch2;
		idx++;
	}
#elif defined(_BSP_NA51090_)
	if (kdrv_ai_get_eng_caps(AI_ENG_CNN2)) {
	...
	if (kdrv_ai_get_eng_caps(AI_ENG_CNN3)) {
		kflow_cnn_ch[idx] = &kflow_cnn_ch3;
		idx++;
	}
#endif
#endif
...
static void kflow_cnn_uninit(KFLOW_AI_ENGINE_CTX *p_eng)
{
}

static UINT32 kflow_cnn_getcfg(KFLOW_AI_ENGINE_CTX *p_eng, KFLOW_AI_JOB *p_job, UINT32 cfg)
{
	UINT32 r = 0;
	switch (cfg) {
	case 0:
		...
		if (p_job) {
			...
			if (p_job->engine_op & 0x00008000) {
				//should be a NUE job, pass it to real NUE engine
				KFLOW_AI_ENGINE_CTX *p_nue_eng = p_eng->geteng(1); // 1 = VENDOR_AI_ENGINE_NUE
				return p_nue_eng->getcfg(p_nue_eng, p_job, cfg);
			}
		}
#endif
		if (p_job) {
...
{
	.name = "cnn",
	.wait_queue = &kflow_cnn_wq,
	.channel_max = 0xff,
	.channel_count = MAX_CHN,
	.p_ch = kflow_cnn_ch,
	.attr = 0, //fast
	.init = kflow_cnn_init,
	.uninit = kflow_cnn_uninit,
	.getcfg = kflow_cnn_getcfg,
};

KFLOW_AI_ENGINE_CTX *kflow_cnn_get_engine(void)
{
	return &kflow_cnn_eng;
}
EXPORT_SYMBOL(kflow_cnn_get_engine);
