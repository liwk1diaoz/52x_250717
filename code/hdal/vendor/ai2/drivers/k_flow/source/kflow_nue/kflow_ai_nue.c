...
//=============================================================
#define __CLASS__ 				"[ai][kflow][nue]"
#include "kflow_ai_debug.h"
//=============================================================
#include "kflow_nue/kflow_nue.h"
#include "kflow_nue/kflow_nue_platform.h"
...
static SEM_HANDLE SEMID_KFLOW_NUE;
static KFLOW_AI_CHANNEL_CTX kflow_nue_ch1;
#if defined(_BSP_NA51090_) || defined(_BSP_NA51102_) || defined(_BSP_NA51103_)
static INT32 kflow_nue_ch1_isrcb(UINT32 cycle_eng, UINT32 cycle_ll, UINT32 cycle_dma, UINT32 intstatus, UINT32 eng, void *parm);
#else
static INT32 kflow_nue_ch1_isrcb(UINT32 handle, UINT32 intstatus, UINT32 eng, void *parm);
#endif
...
int kflow_nue_set_output_path(char *path)
{
	if (path == NULL) {
		return -1;
	}
...
EXPORT_SYMBOL(kflow_nue_set_output_path);

static int _kflow_nue_write_file(char *filename, void *data, size_t count, unsigned long long *offset)
{
	int ret = 0;
	int fd;
...
static void _kflow_nue_dump_engine_register(KFLOW_AI_CHANNEL_CTX *p_ch, KFLOW_AI_JOB* p_job)
{
	if (!p_ch) {
	...
	if (strcmp(p_ch->name, "nue1") == 0) {
		debug_dumpmem(0xf0c60000, 0x200);
	}
}
	
static void _kflow_nue_dump_out_buf(KFLOW_AI_CHANNEL_CTX *p_ch, KFLOW_AI_JOB* p_job)
{
	...
	for (out_id = 0; out_id < 3; out_id++) {
		if (p_io_mem->SAO[out_id].size > 0) {
			...
			_kflow_nue_write_file(filename, (void *)p_io_mem->SAO[out_id].va, p_io_mem->SAO[out_id].size, 0);
		}
	}
#else
	...
	for (out_id = 0; out_id < omem_cnt; out_id++) {
		if (p_sao[out_id].size > 0) {
			...
			DBG_DUMP("write file: %s\n\n\n", filename);
			vos_cpu_dcache_sync(p_sao[out_id].va, p_sao[out_id].size, VOS_DMA_FROM_DEVICE);
			_kflow_nue_write_file(filename, (void *)p_sao[out_id].va, p_sao[out_id].size, 0);
		}
	}
#endif
}

static void kflow_nue_ch1_open(KFLOW_AI_CHANNEL_CTX* p_ch)
{
	const UINT32 chip = 0;
	...
	SEM_CREATE(SEMID_KFLOW_NUE, 1);
	SEM_WAIT(SEMID_KFLOW_NUE);
	...
	id = KDRV_DEV_ID(chip, KDRV_AI_ENGINE_NUE, channel);
	kdrv_ai_set(id, KDRV_AI_PARAM_OPENCFG, &cfg.opencfg);
	kdrv_ai_set(id, KDRV_AI_PARAM_ISR_CB, kflow_nue_ch1_isrcb);
	kdrv_ai_open(chip, KDRV_AI_ENGINE_NUE); //clk set to 600MHz
	INIT_LIST_HEAD(p_ch->ready_queue);

	p_ch->wait_ms = -1;
	
	SEM_SIGNAL(SEMID_KFLOW_NUE);
}

static void kflow_nue_ch1_close(KFLOW_AI_CHANNEL_CTX* p_ch)
{
	...
	kdrv_ai_close(chip, KDRV_AI_ENGINE_NUE);
	
	SEM_SIGNAL(SEMID_KFLOW_NUE);
	SEM_DESTROY(SEMID_KFLOW_NUE);
}

static void kflow_nue_ch1_trig(KFLOW_AI_CHANNEL_CTX* p_ch, KFLOW_AI_JOB* p_job)
{
	const UINT32 chip = 0, channel = 0;
...
#if (SHOW_FLOW == 1)
	{
		DBG_DUMP("----- proc[%d].job[%d] nue trig: layer=%d, eng=%d, mode=%d, trig=%d, cnt=%d\r\n",
	...
	if (1) { 
		//should be a job pass from CNN engine
		p_ch->run_job = p_job; //set it as NUE's job
		//run this job by NUE engine...
	}
#endif
	ctrl_type = p_mctrl->eng;
	if (ctrl_type == NN_GEN_ENG_NUE)
		engine = KDRV_AI_ENGINE_NUE;
	...
	case NN_GEN_TRIG_USER_AI_DRV:
		if (p_mctrl->tot_trig_eng_times) {
			KDRV_AI_TRIG_MODE mode = AI_TRIG_MODE_FC;
...
static void kflow_nue_ch1_debug(KFLOW_AI_CHANNEL_CTX* p_ch, KFLOW_AI_JOB* p_job, UINT32 info)
{
	/* dump engine register */
	if (info & KFLOW_AI_DBG_CTX) {
		_kflow_nue_dump_engine_register(p_ch, p_job);
	}

	/* dump output buffer */
	if (info & KFLOW_AI_DBG_OBUF) {
		_kflow_nue_dump_out_buf(p_ch, p_job);
	}
}
...
#endif
{
	KFLOW_AI_CHANNEL_CTX* p_ch = &kflow_nue_ch1;
...
#endif
	if (p_job == NULL) {
		DBG_DUMP("kflow_nue_ch1_isrcb ERR - NULL run_job????????????\r\n");
		return 0;
	}
	...
	if (1) { 
		//should be a job pass from CNN engine
		KFLOW_AI_ENGINE_CTX* p_cnn_eng = p_job->p_eng->geteng(0); // 0 = VENDOR_AI_ENGINE_CNN
		KFLOW_AI_CHANNEL_CTX* p_cnn_ch = p_cnn_eng->p_ch[0];
		p_ch->run_job = 0; //clear NUE's run job
		p_ch = p_cnn_ch; //processing like a cnn channel
	}
#endif
	if (p_ch->onfinish != 0) {
#if defined(_BSP_NA51090_) || defined(_BSP_NA51102_) || defined(_BSP_NA51103_)
...
static void kflow_nue_ch1_reset2(struct _KFLOW_AI_CHANNEL_CTX* p_ch)
{
	KFLOW_AI_JOB* p_job = p_ch->run_job;
	//NN_GEN_MODE_CTRL *p_mctrl = (NN_GEN_MODE_CTRL*)(p_job->p_op_info);
	
	//DBG_IND("- do reset NUE!\r\n");
...
static LIST_HEAD kflow_nue1_rq = {0};
static KFLOW_AI_CHANNEL_CTX kflow_nue_ch1 =
{
	.name = "nue1",
	.ready_queue = &kflow_nue1_rq,
	.run_job = 0,
	.open = kflow_nue_ch1_open,
	.close = kflow_nue_ch1_close,
	.trigger = kflow_nue_ch1_trig,
	.debug = kflow_nue_ch1_debug,
	.reset2 = kflow_nue_ch1_reset2,
};

#define MAX_CHN 1
static LIST_HEAD kflow_nue_wq = {0};
static KFLOW_AI_CHANNEL_CTX* kflow_nue_ch[MAX_CHN] = {&kflow_nue_ch1};

static void kflow_nue_init(KFLOW_AI_ENGINE_CTX* p_eng)
{
	INT idx = 0;
	INIT_LIST_HEAD(p_eng->wait_queue);

	if (kdrv_ai_get_eng_caps(AI_ENG_NUE)) {
		kflow_nue_ch[idx] = &kflow_nue_ch1;
		idx++;
	}
...
static void kflow_nue_uninit(KFLOW_AI_ENGINE_CTX* p_eng)
{
}

static UINT32 _kflow_nue_getcfg(KFLOW_AI_ENGINE_CTX* p_eng, KFLOW_AI_JOB* p_job, UINT32 cfg)
{
	UINT32 r = 0;
	switch(cfg) {
	case 0:
...
static KFLOW_AI_ENGINE_CTX kflow_nue_eng =
{
	.name = "nue",
	.wait_queue = &kflow_nue_wq,
	.channel_max = 0xff,
	.channel_count = MAX_CHN,
	.p_ch = kflow_nue_ch,
	.attr = 0, //fast
	.init = kflow_nue_init,
	.uninit = kflow_nue_uninit,
	.getcfg = _kflow_nue_getcfg,
};

KFLOW_AI_ENGINE_CTX* kflow_nue_get_engine(void)
{
	return &kflow_nue_eng;
}
EXPORT_SYMBOL(kflow_nue_get_engine);
