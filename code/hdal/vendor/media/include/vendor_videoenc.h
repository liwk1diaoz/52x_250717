...
typedef struct {
	...
	UINT8 avc_inter_luma_coeff_cost_th;		// default: 5, range: 0~15, //INTER Y coefficient cost threshold to eliminate coefficient
	UINT8 avc_inter_chroma_coeff_cost_th;	// default: 5, range: 0~15, //INTER C coefficient cost threshold to eliminate coefficient
...
//------
typedef struct _VENDOR_VIDEOENC_JPG_CFG {
	UINT32 jfif_en;
} VENDOR_VIDEOENC_JPG_CFG;
...
typedef enum _VENDOR_VIDEOENC_PARAM_ID {
	...
	VENDOR_VIDEOENC_PARAM_OUT_JPG_ENC_PARAM,      ///< IPC only   support set with i/o path, using VENDOR_VIDEOENC_JPG_CFG struct
