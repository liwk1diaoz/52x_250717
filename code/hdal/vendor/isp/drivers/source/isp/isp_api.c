...
#define ISP_IF_IPP_REG_EVENT        ISP_EVENT_IPP_PROCEND | ISP_EVENT_IPP_CFGEND | ISP_EVENT_IPP_CFGEND_IMM | ISP_EVENT_PARAM_RST
#define ISP_IF_ENC_REG_EVENT        ISP_EVENT_ENC_TNR | ISP_EVENT_ENC_RATIO | ISP_EVENT_PARAM_RST
...
void isp_api_reg_if(UINT32 id_list)
{
	...
	for (i = 0; i < ISP_ID_MAX_NUM; i++) {
		...
		if (pdev_info != NULL) {
			...
			pdev_info->va_rslt[i] = isp_uti_vmem_alloc(sizeof(ISP_VA_RSLT));
			pdev_info->va_indep_rslt[i] = isp_uti_vmem_alloc(sizeof(ISP_VA_INDEP_RSLT));
			pdev_info->md_rslt[i] = isp_uti_vmem_alloc(sizeof(ISP_MD_RSLT));
...
void isp_api_unreg_if(void)
{
	...
	for (i = 0; i < ISP_ID_MAX_NUM; i++) {
		if (isp_id_valid[i] && (pdev_info != NULL)) {
			...
			isp_uti_vmem_free(pdev_info->va_rslt[i]);
			isp_uti_vmem_free(pdev_info->va_indep_rslt[i]);
...
static INT32 isp_api_cb_sie(ISP_ID id, ISP_EVENT evt, UINT32 frame_cnt, void *param)
{
	...
	if (evt & ISP_EVENT_SIE_CRPEND) {
		...
		if (fun_en[sie_id] & ISP_FUNC_EN_AE) {
			sie_ret = ctl_sie_isp_get(id, CTL_SIE_ISP_ITEM_LA_RSLT, (void *)pdev_info->la_rslt[sie_id]);
			if (sie_ret != CTL_SIE_E_OK) {
				PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_SIE_ISP_ITEM_LA_RSLT fail (%d) \r\n", sie_ret);
			}
			...
			if (sie_ret == CTL_SIE_E_OK) {
				...
				PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_SIE_ISP_ITEM_MD_RSLT fail (%d) \r\n", sie_ret);
			}
		}

		if (fun_en[sie_id] & ISP_FUNC_EN_AWB) {
			sie_ret = ctl_sie_isp_get(id, CTL_SIE_ISP_ITEM_CA_RSLT, (void *)pdev_info->ca_rslt[sie_id]);
			if (sie_ret != CTL_SIE_E_OK) {
				PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_SIE_ISP_ITEM_CA_RSLT fail (%d) \r\n", sie_ret);
			}
		}
	}
...
static INT32 isp_api_cb_ipp(ISP_ID id, ISP_EVENT evt, UINT32 frame_cnt, void *param)
{
	...
	case ISP_EVENT_IPP_CFGEND_IMM:
		...
		if (fun_en & ISP_FUNC_EN_DEFOG) {
			...
			if ((evt != ISP_EVENT_PARAM_RST) || isp_api_get_fastboot_valid()) {
...
static void isp_api_get_fastboot_sync_info(UINT32 id)
{
	...
	if (isp_builtin_get_sensor_valid(id)) {
		...
		iq_init_info.smoothed_tm_ratio = isp_builtin_get_shdr_smoothed_tm_ratio(id);
...
void isp_api_set_iq_param(ISP_ID id, ISP_IQ_ITEM isp_iq_item, void *param)
{
	...
	case ISP_IQ_ITEM_IFE_PARAM:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_IFE_PARAM)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_IFE_IQ_PARAM, (KDRV_IFE_PARAM_IQ_ALL_PARAM *)param);
	...
	case ISP_IQ_ITEM_DCE_PARAM:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_DCE_PARAM)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_DCE_IQ_PARAM, (KDRV_DCE_PARAM_IQ_ALL_PARAM *)param);
		}
	break;

	case ISP_IQ_ITEM_IPE_PARAM:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_IPE_PARAM)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_IPE_IQ_PARAM, (KDRV_IPE_PARAM_IQ_ALL_PARAM *)param);
		}
	break;

	case ISP_IQ_ITEM_IFE2_PARAM:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_IFE2_PARAM)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_IFE2_IQ_PARAM, (KDRV_IFE2_PARAM_IQ_ALL_PARAM *)param);
		}
	break;

	case ISP_IQ_ITEM_IME_PARAM:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_IME_PARAM)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_IME_IQ_PARAM, (KDRV_IME_PARAM_IQ_ALL_PARAM *)param);
		}
	break;
	...
	case ISP_IQ_ITEM_ENC_PARAM:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_ENC_PARAM)) {
			...
			PRINT_ISP(isp_dbg_get_dbg_mode(isp_id) & ISP_DBG_ENC_SET, "kflow_videoenc_set (%d) ENC_IQ_PARAM \r\n", id);
		}
	break;
