...
static INT32 do_ioc_common(ISP_DEV_INFO *pdev_info, UINT32 cmd, UINT32 arg)
{
	...
	if (!(isp_dbg_get_ioc_control() & ISP_IOC_BYPASS_COMMON)) {
		common_ioctl(cmd, arg, buf_addr);
		PRINT_ISP(isp_dbg_get_ioc_control() & ISP_IOC_PRINT_COMMON, "com ioc, id: %d, %s cmd: %2d %s \r\n", *((UINT32 *)buf_addr), _IOC_DIR(cmd) == 3 ? "r" : "w", _IOC_NR(cmd), isp_dbg_get_ispt_item(_IOC_NR(cmd)));
	}

	return ret;
}

static INT32 do_ioc_common_yuv(ISP_DEV_INFO *pdev_info, UINT32 cmd, UINT32 arg)
{
	INT32 ret = 0;
	ISPT_YUV_INFO yuv_info = {0};

	if (!(isp_dbg_get_ioc_control() & ISP_IOC_BYPASS_COMMON)) {
		common_ioctl(cmd, arg, (UINT32 *)&yuv_info);
		PRINT_ISP(isp_dbg_get_ioc_control() & ISP_IOC_PRINT_COMMON, "com ioc, id: %d, %s cmd: %2d %s \r\n", *((UINT32 *)&yuv_info), _IOC_DIR(cmd) == 3 ? "r" : "w", _IOC_NR(cmd), isp_dbg_get_ispt_item(_IOC_NR(cmd)));
	}

	return ret;
}

static INT32 do_ioc_common_raw(ISP_DEV_INFO *pdev_info, UINT32 cmd, UINT32 arg)
{
	INT32 ret = 0;
	ISPT_RAW_INFO raw_info = {0};

	if (!(isp_dbg_get_ioc_control() & ISP_IOC_BYPASS_COMMON)) {
		common_ioctl(cmd, arg, (UINT32 *)&raw_info);
		PRINT_ISP(isp_dbg_get_ioc_control() & ISP_IOC_PRINT_COMMON, "com ioc, id: %d, %s cmd: %2d %s \r\n", *((UINT32 *)&raw_info), _IOC_DIR(cmd) == 3 ? "r" : "w", _IOC_NR(cmd), isp_dbg_get_ispt_item(_IOC_NR(cmd)));
	}
...
static INT32 do_ioc_common_wait_vd(ISP_DEV_INFO *pdev_info, UINT32 cmd, UINT32 arg)
{
	...
	if (!(isp_dbg_get_ioc_control() & ISP_IOC_BYPASS_VD)) {
		common_ioctl(cmd, arg, (UINT32 *)&wait_vd);
		PRINT_ISP(isp_dbg_get_ioc_control() & ISP_IOC_PRINT_VD, "vd  ioc, id %d \r\n", wait_vd.id);
	}
...
static INT32 do_ioc_ae(ISP_DEV_INFO *pdev_info, UINT32 cmd, UINT32 arg)
{
	...
	if (!(isp_dbg_get_ioc_control() & ISP_IOC_BYPASS_AE)) {
		ret = pdev_info->isp_ae->fn_ioctl(cmd, arg, buf_addr);
		PRINT_ISP(isp_dbg_get_ioc_control() & ISP_IOC_PRINT_AE, "ae  ioc, id: %d, %s cmd: %2d %s \r\n", *((UINT32 *)buf_addr), _IOC_DIR(cmd) == 3 ? "r" : "w", _IOC_NR(cmd), isp_dbg_get_aet_item(_IOC_NR(cmd)));
	}
...
static INT32 do_ioc_awb(ISP_DEV_INFO *pdev_info, UINT32 cmd, UINT32 arg)
{
	...
	if (!(isp_dbg_get_ioc_control() & ISP_IOC_BYPASS_AWB)) {
		ret = pdev_info->isp_awb->fn_ioctl(cmd, arg, buf_addr);
		PRINT_ISP(isp_dbg_get_ioc_control() & ISP_IOC_PRINT_AWB, "awb ioc, id: %d, %s cmd: %2d %s \r\n", *((UINT32 *)buf_addr), _IOC_DIR(cmd) == 3 ? "r" : "w", _IOC_NR(cmd), isp_dbg_get_awbt_item(_IOC_NR(cmd)));
	}
...
static INT32 do_ioc_af(ISP_DEV_INFO *pdev_info, UINT32 cmd, UINT32 arg)
{
	...
	if (!(isp_dbg_get_ioc_control() & ISP_IOC_BYPASS_AF)) {
		ret = pdev_info->isp_af->fn_ioctl(cmd, arg, buf_addr);
		PRINT_ISP(isp_dbg_get_ioc_control() & ISP_IOC_PRINT_AF, "af  ioc, id: %d, %s cmd: %2d %s \r\n", *((UINT32 *)buf_addr), _IOC_DIR(cmd) == 3 ? "r" : "w", _IOC_NR(cmd), isp_dbg_get_aft_item(_IOC_NR(cmd)));
	}
...
static INT32 do_ioc_iq(ISP_DEV_INFO *pdev_info, UINT32 cmd, UINT32 arg)
{
	...
	if (!(isp_dbg_get_ioc_control() & ISP_IOC_BYPASS_IQ)) {
		ret = pdev_info->isp_iq->fn_ioctl(cmd, arg, buf_addr);
		PRINT_ISP(isp_dbg_get_ioc_control() & ISP_IOC_PRINT_IQ, "iq  ioc, id: %d, %s cmd: %2d %s \r\n", *((UINT32 *)buf_addr), _IOC_DIR(cmd) == 3 ? "r" : "w", _IOC_NR(cmd), isp_dbg_get_iqt_item(_IOC_NR(cmd)));
	}
...
{
	ISP_DRV_INFO *pdrv_info = NULL;
	ISP_DEV_INFO *pdev_info = NULL;
	INT32 ret = 0;

	pdrv_info = (ISP_DRV_INFO *)filp->private_data;
	if (pdrv_info == NULL) {
		return -ENXIO;
	}
	...
	} else if ((_IOC_TYPE(cmd) == ISP_IOC_COMMON) && (_IOC_NR(cmd) == ISPT_ITEM_YUV)) {
		ret = do_ioc_common_yuv(pdev_info, cmd, arg);
	} else if ((_IOC_TYPE(cmd) == ISP_IOC_COMMON) && (_IOC_NR(cmd) == ISPT_ITEM_RAW)) {
		ret = do_ioc_common_raw(pdev_info, cmd, arg);
	} else {
