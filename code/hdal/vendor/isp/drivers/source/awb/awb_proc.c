...
static UINT32 win_f = AWB_ID_MAX_NUM;
...
static INT32 awb_proc_cmd_set_ca_th(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	...
	awb_cmd_printf("id = %d, en = %d\n", id, ca_th.enable);
	return 0;
}

static INT32 awb_proc_cmd_get_win(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AWB_ID id;

	awb_cmd_printf("awb_proc_cmd_get_win \n");
...
static INT32 awb_proc_cmd_set_win(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AWB_ID id;
	UINT32 mode;

	awb_cmd_printf("awb_proc_cmd_set_win \n");
	...
	awb_flow_set_win(id, mode);

	awb_cmd_printf("id = %d, mode = %d\n", id, mode);
	return 0;
}
#endif
...
static AWB_PROC_CMD awb_proc_r_cmd_list[] = {
	...
	{ "win",                awb_proc_cmd_get_win,            "get win, param1 is awb_id(0~2)"}
	#endif
};
...
static AWB_PROC_CMD awb_proc_w_cmd_list[] = {
	...
	{ "win",                awb_proc_cmd_set_win,            "set win, param1 is awb_id(0~2), param2 is en(0 or 1)"},
	#endif
...
{
	if (g_cmd_msg.buf == NULL && ca_f >= AWB_ID_MAX_NUM && flag_f >= AWB_ID_MAX_NUM && win_f >= AWB_ID_MAX_NUM && param_f >= AWB_ID_MAX_NUM) {
		return -EINVAL;
	}
	...
	awb_flow_get_win(sfile, win_f);
	win_f = AWB_ID_MAX_NUM;
...
static int awb_proc_help_show(struct seq_file *sfile, void *v)
{
	...
	seq_printf(sfile, " 'echo r win ID > /proc/hdal/vendor/awb/cmd;cat /proc/hdal/vendor/awb/cmd'                            - Get AWB win parameter\n");
	seq_printf(sfile, " 'echo w win ID mode > /proc/hdal/vendor/awb/cmd;cat /proc/hdal/vendor/awb/cmd'                       - Set AWB win parameter\n");
	seq_printf(sfile, "======================================================================================================================================\n");
