...
{
	g_cmd_msg.buf = isp_uti_vmem_alloc(PROC_MSG_BUFSIZE);
...
{
	if (g_cmd_msg.buf) {
		isp_uti_vmem_free(g_cmd_msg.buf);
		g_cmd_msg.buf = NULL;
	}
}
...
{
	AE_ID id;
	u32 speed, down_speed, up_speed1, up_speed2;
	AE_PROC_CMD_OBJ *ae_proc_cmd;
	...
	down_speed = simple_strtoul(argv[4], NULL, 0);
	up_speed1 = simple_strtoul(argv[5], NULL, 0);
	up_speed2 = simple_strtoul(argv[6], NULL, 0);
	
	ae_proc_cmd->speed = speed;
	ae_proc_cmd->down_speed = down_speed;
	ae_proc_cmd->up_speed1 = up_speed1;
	ae_proc_cmd->up_speed2 = up_speed2;

	ae_cmd_printf("id = %d, speed=%d, down_speed=%d, up_speed1=%d, up_speed2=%d\n", id, ae_proc_cmd->speed, ae_proc_cmd->down_speed, ae_proc_cmd->up_speed1, ae_proc_cmd->up_speed2);
...
static s32 ae_proc_cmd_drv_ratio(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id = AE_ID_1;
	s32 drv_ratio = 0;
	AE_PROC_CMD_OBJ *ae_proc_cmd= NULL;
	...
	ae_proc_cmd->drv_ratio = drv_ratio;

	ae_proc_cmd->cmd = AE_PROC_CMD_DCIRIS_DRV_RATIO;

	ae_cmd_printf("set ae_info(%d) drv_ratio(%d) \n", id, ae_proc_cmd->drv_ratio);
	return 0;
}

static s32 ae_proc_cmd_sensor_test(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
	AE_PROC_CMD_OBJ *ae_proc_cmd;
	...
	ae_proc_cmd->sensor_test[0] = simple_strtoul(argv[3], NULL, 0);
	ae_proc_cmd->sensor_test[1] = simple_strtoul(argv[4], NULL, 0);
	ae_proc_cmd->sensor_test[2] = simple_strtoul(argv[5], NULL, 0);
	ae_proc_cmd->sensor_test[3] = simple_strtoul(argv[6], NULL, 0);

	ae_cmd_printf("set ae_info(%d) exp0(%d), gain0(%d), exp1(%d), gain1(%d)  \n", id, ae_proc_cmd->sensor_test[0], ae_proc_cmd->sensor_test[1], ae_proc_cmd->sensor_test[2], ae_proc_cmd->sensor_test[3]);

	ae_proc_cmd->cmd = AE_PROC_CMD_SENSOR_TEST;
	return 0;
}

static s32 ae_proc_cmd_ae_test(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
	AE_PROC_CMD_OBJ *ae_proc_cmd;
	...
	ae_proc_cmd->ae_test[0] = simple_strtoul(argv[3], NULL, 0);
	ae_proc_cmd->ae_test[1] = simple_strtoul(argv[4], NULL, 0);
	ae_proc_cmd->ae_test[2] = simple_strtoul(argv[5], NULL, 0);
	ae_proc_cmd->ae_test[3] = simple_strtoul(argv[6], NULL, 0);
	ae_proc_cmd->ae_test[4] = simple_strtoul(argv[7], NULL, 0);
	ae_proc_cmd->ae_test[5] = simple_strtoul(argv[8], NULL, 0);
	ae_proc_cmd->ae_test[6] = simple_strtoul(argv[9], NULL, 0);

	//ae_cmd_printf("set ae_info(%d) exp0(%d), gain0(%d), exp1(%d), gain1(%d)  \n", id, ae_proc_cmd->sensor_test[0], ae_proc_cmd->sensor_test[1], ae_proc_cmd->sensor_test[2], ae_proc_cmd->sensor_test[3]);

	ae_proc_cmd->cmd = AE_PROC_CMD_AE_TEST;
	return 0;
}


static s32 ae_proc_cmd_drv_cali(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
	s32 max_th, min_th, i_th;
	AE_PROC_CMD_OBJ *ae_proc_cmd;
	...
	id = simple_strtoul(argv[2], NULL, 0);
	max_th = simple_strtoul(argv[3], NULL, 0);
	min_th = simple_strtoul(argv[4], NULL, 0);
	i_th = simple_strtoul(argv[5], NULL, 0);
	ae_proc_cmd = ae_flow_get_proc_cmd(id);
	ae_proc_cmd->max_th = max_th;
	ae_proc_cmd->min_th = min_th;
	ae_proc_cmd->i_th = i_th;
	ae_proc_cmd->cmd = AE_PROC_DCIRIS_CALI;
	//printk("proc max_th=%d, min_th=%d \r\n", ae_proc_cmd->max_th, ae_proc_cmd->min_th);
...
static s32 ae_proc_cmd_cal_dr(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	UINT32 dr_min, dr_max;
	...
	dr_min = simple_strtoul(argv[2], NULL, 0);
	dr_max = simple_strtoul(argv[3], NULL, 0);
	
	ae_cmd_printf("dr_range = %d, %d\n", dr_min, dr_max);
	ae_cmd_printf("dr_level = %d dB\n", ae_flow_cal_dr(dr_min, dr_max));

	return 0;
}

static s32 ae_proc_cmd_skip_la(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
	AE_PROC_CMD_OBJ *ae_proc_cmd;
	...
	id = simple_strtoul(argv[2], NULL, 0);
	ae_proc_cmd = ae_flow_get_proc_cmd(id);
	ae_proc_cmd->skip_la = simple_strtoul(argv[3], NULL, 0);

	ae_proc_cmd->cmd = AE_PROC_CMD_SKIP_LA;
	return 0;
}


static s32 ae_proc_cmd_autotm_sel(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
	u32 value;
	AE_PROC_CMD_OBJ *ae_proc_cmd;
	...
	value = simple_strtoul(argv[3], NULL, 0);
	ae_proc_cmd->autotm_sel = value;

	ae_cmd_printf("id = %d, autotm_sel = %d\n", id, ae_proc_cmd->autotm_sel);

	ae_proc_cmd->cmd = AE_PROC_CMD_AUTOTM_SEL;

	return 0;
}

static s32 ae_proc_cmd_auto_tm_weight(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
	u32 value;
	AE_PROC_CMD_OBJ *ae_proc_cmd;
	...
	value = simple_strtoul(argv[3], NULL, 0);
	ae_proc_cmd->auto_tm_weight = value;

	// base 16
	ae_cmd_printf("id = %d, auto_tm_weight = %d\n", id, ae_proc_cmd->auto_tm_weight);

	ae_proc_cmd->cmd = AE_PROC_CMD_AUTO_TM_WEIGHT;

	return 0;
}

static s32 ae_proc_cmd_tm_ratio(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
	u32 value;
	AE_PROC_CMD_OBJ *ae_proc_cmd;
	...
	value = simple_strtoul(argv[3], NULL, 0);
	ae_proc_cmd->auto_tm_ratio = value;

	// base 16
	ae_cmd_printf("id = %d, auto_tm_ratio = %d\n", id, ae_proc_cmd->auto_tm_ratio);

	ae_proc_cmd->cmd = AE_PROC_CMD_AUTO_TM_RATIO;

	return 0;
}

static s32 ae_proc_cmd_shdr_hbs(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
	u32 value1;
	AE_PROC_CMD_OBJ *ae_proc_cmd;
	...
	value1 = simple_strtoul(argv[3], NULL, 0);
	ae_proc_cmd->hbs_en = value1;

	ae_cmd_printf("id = %d, hbs_en = %d \n", id, ae_proc_cmd->hbs_en);

	ae_proc_cmd->cmd = AE_PROC_CMD_HBS;
...
static AE_PROC_CMD ae_proc_w_cmd_list[] = {
	...
	{ "ae_mode",            ae_proc_cmd_mode,          	  "set ae mode, param: [id] [ae_mode], 0: auto / 1: manual /2: lock"},
	...
	{ "drv_cali",           ae_proc_cmd_drv_cali,      "set drv_cali, param: [id] [max_th][min_th] [i_th]"},
	{ "sensor_test",        ae_proc_cmd_sensor_test,   "set sensor_test, param: [id] [exp0] [gain0] [exp1] [gain1]"},
	...
	{ "skip_la",            ae_proc_cmd_skip_la,       "skip la, param: [id] [skip_la]"},
	{ "autotm_sel",        	ae_proc_cmd_autotm_sel,       "set autotm_sel, param: [id] [autotm_sel]"},
	{ "auto_tm_weight",     ae_proc_cmd_auto_tm_weight,  "set auto_tm_weight, param: [id] [auto_tm_weight]"},
	{ "shdr_hbs",           ae_proc_cmd_shdr_hbs,          "set hbs, param: [id] [hbs_en]"},
	{ "cal_dr",        		ae_proc_cmd_cal_dr,            "cal dr range, param: [id] [dr_min] [dr_max]"},
	{ "tm_ratio",           ae_proc_cmd_tm_ratio,           "select tm ratio, param: [id] [tm_ratio]"},
	{ "ae_test",           ae_proc_cmd_ae_test,           "select ae test, param: [id] [tm_ratio]"}
};
...
#define LA_WIN_Y 32
	...
	if (param_f < AE_ID_MAX_NUM) {
		...
		seq_printf(sfile, "\n");
		for (i = 0; i < AEALG_DYNAMIC_LV_NUM; i++) {
			seq_printf(sfile, "%d ", ae_param[param_f]->shdr->adj_ratio[i]);
		}
		seq_printf(sfile, "\n");
		for (i = 0; i < AEALG_DYNAMIC_LV_NUM; i++) {
		...
		seq_printf(sfile, "\n");
		seq_printf(sfile, "%d \n", ae_param[param_f]->shdr->mode_sel);
...
static INT32 ae_proc_help_show(struct seq_file *sfile, void *v)
{
	...
	seq_printf(sfile, "Ex3: 'echo w ae_mode 0 2 > /proc/hdal/vendor/ae/cmd ;cat /proc/hdal/vendor/ae/cmd' \r\n");
	...
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_UI              |                                                              \n", AE_DBG_UI);
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_CONV            |                                                              \n", AE_DBG_CONV);
	...
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_LA_ROI          |                                                              \n", AE_DBG_LA_ROI);
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_BOUND           |                                                              \n", AE_DBG_BOUND);
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_LOCK            |                                                              \n", AE_DBG_LOCK);
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_IRIS            |                                                              \n", AE_DBG_IRIS);
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_STITCH          |                                                              \n", AE_DBG_STITCH);
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_NNSC            |                                                              \n", AE_DBG_NNSC);
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_STATUS          |                                                              \n", AE_DBG_STATUS);
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_CAPTURE         |                                                              \n", AE_DBG_CAPTURE);
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_PERFORMANCE     |                                                              \n", AE_DBG_PERFORMANCE);
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_AUTOTM          |                                                              \n", AE_DBG_AUTOTM);
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_MORPHOLOGY      |                                                              \n", AE_DBG_MORPHOLOGY);
	seq_printf(sfile, "|              |            | 0x%8x: AE_DBG_LA_LE           |                                                              \n", AE_DBG_LA_LE);
	seq_printf(sfile, "|--------------|------------|-------------------------------|--------------------------------------------------------------\n");
