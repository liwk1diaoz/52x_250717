...
UINT32 nue_perf_en     = 0;
UINT32 nue_perf_status = 0;
UINT32 nue_eng_time    = 0;
#if defined(__LINUX)
static struct timeval nue_tstart, nue_tend;
static struct timeval nue_total_tstart, nue_total_tend;
#endif
UINT32 nue_total_time = 0;
...
VOID nue_set_perf_en(UINT32 enable)
{
#if defined(__LINUX)
	if (enable) {
		nue_perf_en = enable;
		nue_eng_time = 0;
		do_gettimeofday(&nue_total_tstart);
	} else {
		if (nue_perf_en == 1) {
			do_gettimeofday(&nue_total_tend); 
			nue_total_time = (nue_total_tend.tv_sec - nue_total_tstart.tv_sec) * 1000000 + (nue_total_tend.tv_usec - nue_total_tstart.tv_usec);
			nue_perf_en = 0;
		}
	}
#endif
...
UINT32 nue_get_eng_time(VOID)
{
	return nue_eng_time;
}

UINT32 nue_get_eng_util(VOID)
{
	if (nue_total_time == 0) {
		return 0;
	}
	
	return 100*nue_eng_time/nue_total_time;
}
#if NUE_CYCLE_TEST
...
UINT64 nue_dump_total_cycle(UINT32 debug_layer)
{
    ...
    if(debug_layer==1) {
        UINT32 i = 0;
    	for (i = 0; i < nue_cnt; i++) {
    		DBG_DUMP("item %d nue cycle = %d\n", i, (unsigned int)nue_dbg_cnt[i]);
    	}
    }
    //#else
        //DBG_DUMP("NUE_CYCLE_LAYER is disable now.\n");
    #endif
...
#if defined(__LINUX)
		if (nue_perf_status == 1) {
			do_gettimeofday(&nue_tend); 
			nue_eng_time += ((nue_tend.tv_sec - nue_tstart.tv_sec) * 1000000 + (nue_tend.tv_usec - nue_tstart.tv_usec));
			nue_perf_status = 0;
		}
#endif
...
ER nue_set_para2eng(NUE_OPMODE mode, NUE_PARM *p_parm)
{
	...
	case NUE_OPMODE_ROIPOOLING:
		...
		if ((p_parm->roipool_parm.olofs & 0x3) != 0){
			DBG_ERR("NUE output lineoffset should be word-align.\r\n");
			return E_PAR;
		}
	...
	if ((p_parm->userdef.eng_mode == NUE_PERMUTE) && (p_parm->permute_parm.mode == 0)) {
		...
		if (p_parm->userdef.permute_stripe_en != 0) {
			DBG_ERR("NUE permute stripe_en should be set 0.\r\n");
			return E_PAR;
		}
		nue_set_permute_stripe(p_parm->userdef.permute_stripe_en);
	}
...
VOID nue_reset_status(VOID)
{
	...
	if (g_nue_lock_status != NO_TASK_LOCKED) {
		er_return = nue_close();
		if (er_return != E_OK) {
			DBG_ERR("nue_reset_status: Error at nue_close().\r\n");
			goto err_exit;
		}
	}
	g_nue_status = NUE_ENGINE_IDLE;
...
VOID nue_dma_abort(VOID)
{
    ...
    if (is_dma_disable == 1) {
        DBG_IND("NUE_DMA_ABORT: it has been in dma_disable state.\r\n");
        return;
    }
