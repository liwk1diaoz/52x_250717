...
static UINT32 nvt_ai_chk_net_id(UINT32 net_id) {
	...
	if (net_id > net_id_upb) {
		DBG_WRN("network id is overflow, clamp -> %d\r\n", (int)net_id_upb);
		net_id = net_id_upb;
	}
	return net_id;
}
...
static BOOL nvt_ai_drv_is_mem_map(UINT32 net_id)
{
	...
	if ((p_mem->kerl_pa == 0) || (p_mem->kerl_va == 0) || (p_mem->user_pa == 0) || (p_mem->user_va == 0)) {
		DBG_ERR("map mem user va=%08x, user pa=%08x, kernel va=%08x, kernel pa=%08x\r\n"
...
void nvt_ai_drv_init_rtos(void)
{
	...
	if (nvt_get_chip_id() == CHIP_NA51055) {//520 & 321
		g_cnn_clk_528 = 0;
	}
	else {
		g_cnn_clk_528 = 1;
	...
	if (iRet < 0) {
		DBG_ERR("Error, to do vmalloc for global array.\r\n");
	}*/
}
...
//#endif
	...
	case AI_IOC_ENG_INIT:
		iRet = nue_init();
		if (iRet != 0) {
            DBG_ERR("AI_IOCTL: Error, nue_init failed.\r\n");
            goto AI_IOC_ENG_INIT_EXIT;
		}
		iRet = cnn_init(0);
		if (iRet != 0) {
            DBG_ERR("AI_IOCTL: Error, cnn_init failed.\r\n");
            goto AI_IOC_ENG_INIT_EXIT;
		}
		iRet = nue2_init();
		if (iRet != 0) {
            DBG_ERR("AI_IOCTL: Error, nue2_init failed.\r\n");
            goto AI_IOC_ENG_INIT_EXIT;
		}
...
AI_IOC_ENG_INIT_EXIT:
	...
	case AI_IOC_ENG_UNINIT:
		iRet = nue_uninit();
		if (iRet != 0) {
            DBG_ERR("AI_IOCTL: Error, nue_uninit failed.\r\n");
            goto AI_IOC_ENG_UNINIT_EXIT;
        }
		iRet = cnn_uninit(0);
		if (iRet != 0) {
            DBG_ERR("AI_IOCTL: Error, cnn_uninit failed.\r\n");
            goto AI_IOC_ENG_UNINIT_EXIT;
        }
		iRet = nue2_uninit();
		if (iRet != 0) {
            DBG_ERR("AI_IOCTL: Error, nue2_uninit failed.\r\n");
            goto AI_IOC_ENG_UNINIT_EXIT;
        }	
...
AI_IOC_ENG_UNINIT_EXIT:
	...
	case AI_IOC_CNN2_INIT:
		iRet = cnn_init(1);
		if (iRet != 0) {
            DBG_ERR("AI_IOCTL: Error, cnn2_init failed.\r\n");
...
AI_IOC_CNN2_INIT_EXIT:
	...
	case AI_IOC_CNN2_UNINIT:
		iRet = cnn_uninit(1);
		if (iRet != 0) {
            DBG_ERR("AI_IOCTL: Error, cnn1_uninit failed.\r\n");
...
AI_IOC_CNN2_UNINIT_EXIT:
	...
	case AI_IOC_OPEN:
		...
		if (er_return != E_OK) {
            DBG_ERR("AI_IOCTL: Error, kdrv_ai_open failed.\r\n");
...
AI_IOC_OPEN_EXIT:
	...
	case AI_IOC_CLOSE:
		...
		if (er_return != E_OK) {
            DBG_ERR("AI_IOCTL: Error, kdrv_ai_close failed.\r\n");
...
AI_IOC_CLOSE_EXIT:
	...
	case AI_IOC_OPENCFG:
		...
		if (er_return != E_OK) {
            DBG_ERR("AI_IOCTL: Error, kdrv_ai_set failed.\r\n");
...
AI_IOC_SET_MODE_EXIT:
	...
	case AI_IOC_SET_APP:
		...
		if (!nvt_ai_drv_is_mem_map(ai_app_info.net_id)) {
			DBG_ERR("memory should be mapped before app parms set\r\n");
			goto exit;
		}
...
AI_IOC_SET_APP_EXIT:
	...
	case AI_IOC_SET_LL:
		...
		if (!nvt_ai_drv_is_mem_map(ai_ll_info.net_id)) {
			DBG_ERR("memory should be mapped before ll parms set\r\n");
			goto exit;
		}
...
AI_IOC_SET_LL_EXIT:
	...
	case AI_IOC_GET_MODE:
		...
		if (er_return != E_OK) {
            DBG_ERR("AI_IOCTL: Error, kdrv_ai_get failed.\r\n");
...
AI_IOC_GET_MODE_EXIT:
	...
	case AI_IOC_GET_APP:
		...
		if (!nvt_ai_drv_is_mem_map(ai_app_info.net_id)) {
			DBG_ERR("memory should be mapped before app parms get\r\n");
			goto exit;
		}
		id = KDRV_DEV_ID(chip, kdrv_ai_trans_eng2id(ai_app_info.engine), channel);
...
AI_IOC_GET_APP_EXIT:
	...
	case AI_IOC_GET_LL:
		memcpy(&ai_ll_info, p_arg, sizeof(ai_ll_info));
		if (!nvt_ai_drv_is_mem_map(ai_ll_info.net_id)) {
			DBG_ERR("memory should be mapped before ll parms get\r\n");
			goto exit;
		}
		id = KDRV_DEV_ID(chip, kdrv_ai_trans_eng2id(ai_ll_info.engine), channel);
...
#endif
		if (er_return != E_OK) {
            DBG_ERR("AI_IOCTL: Error, kdrv_ai_trigger failed.\r\n");
...
AI_IOC_TRIGGER_EXIT:
	...
	case AI_IOC_WAITDONE:
		...
		if (er_return != E_OK) {
            DBG_ERR("AI_IOCTL: Error, kdrv_ai_waitdone failed.\r\n");
...
AI_IOC_WAITDONE_EXIT:
	...
	case AI_IOC_RESET:
		...
		if (er_return != E_OK) {
            DBG_ERR("AI_IOCTL: Error, kdrv_ai_reset failed.\r\n");
...
AI_IOC_RESET_EXIT:
	...
	case AI_IOC_SET_MAP_ADDR:
		...
		if (net_id > (AI_SUPPORT_NET_MAX -1)) {
			DBG_WRN("network id is overflow(%d), clamp -> %d\r\n", (int) net_id, (int) (AI_SUPPORT_NET_MAX -1));
	...
	case AI_IOC_SET_LL_USR_INFO: {	
		...
		if (er_return != E_OK) {
            DBG_ERR("AI_IOCTL: Error, kdrv_ai_link_ll_update_addr failed.\r\n");
...
AI_IOC_SET_LL_USR_INFO_EXIT:
	...
	case AI_IOC_UNINIT_LINK_INFO: {
		...
		if (er_return != E_OK) {
            DBG_ERR("AI_IOCTL: Error, kdrv_ai_link_uninit failed.\r\n");
...
AI_IOC_UNINIT_LINK_INFO_EXIT:
	...
	case AI_IOC_UNINIT_LL_BUF: {
		...
		if (g_buf_handle[net_id] == NULL) {
			DBG_ERR("free buf: handle %d is null!\n", (int)net_id);
			return -1;
		} else {
	...
	case NUE_IOC_RUN_FC: {
		...
		DBG_ERR("AI_FASTBOOT: Error, NUE_IOC_RUN_FC can't be called at fastboot.\n");
#else
		...
		if (er_return != E_OK) {
            DBG_ERR("AI_IOCTL: Error, nue_open failed.\r\n");
		...
		for (run_idx = 0; run_idx < run_time; run_idx++) {
			...
			if (nue_setmode(NUE_OPMODE_USERDEFINE, &nue_parm) == E_OK) {
				er_return = nue_start();
				if (er_return != E_OK) {
					DBG_ERR("AI_IOCTL: Error, nue_start failed.\r\n");
			...
			er_return = nue_pause();
			if (er_return != E_OK) {
				DBG_ERR("AI_IOCTL: Error, nue_pause failed.\r\n");
				goto NUE_IOC_RUN_FC_1_EXIT;
			}
...
NUE_IOC_RUN_FC_1_EXIT:
		...
		er_return = nue_close();
		if (er_return != E_OK) {
			DBG_ERR("AI_IOCTL: Error, nue_close failed.\r\n");
			goto NUE_IOC_RUN_FC_EXIT;
		}
...
NUE_IOC_UNINIT_EXIT:
	...
	case AI_IOC_DMA_ABORT:
		memcpy(&ai_eng, p_arg, sizeof(KDRV_AI_ENG));
        er_return = kdrv_ai_dma_abort(chip, kdrv_ai_trans_eng2id(ai_eng));
        if (er_return != E_OK) {
            DBG_ERR("AI_IOCTL: Error, kdrv_ai_dma_abort failed.\r\n");
...
AI_IOC_DMA_ABORT_EXIT:
        ...
        break;
	default :
		DBG_ERR("unknown ai ioctl cmd: %08x\r\n", uiCmd);
		break;
	}
exit:
	return iRet;
