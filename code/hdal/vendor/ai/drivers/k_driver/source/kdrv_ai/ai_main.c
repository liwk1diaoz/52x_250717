...
unsigned int ai_default_net_supported_num = 1;
#else
...
unsigned int ai_default_net_supported_num = 16;
#endif
...
UINT32 is_ai_builtin_on=KDRV_AI_BUILTIN_MAGIC_NUM; //0:off, 1:on, 0xf:don't care
UINT32 is_ai_on=KDRV_AI_BUILTIN_MAGIC_NUM; //0:off, 1:on, 0x:don't care
...
static int nvt_handle_fb_ai_on_off(VOID)
{
	...
	if (is_ai_enable_dtsi == 1) {
		if ((is_ai_builtin_on == 1) && (is_ai_on == 1)) {
			DBG_ERR("KDRV_AI: Error, built_ai and ai can't be both opened at dtsi(ai_enable).\n");
			ret = -ENODEV;
			goto EXIT_ERR;
		} else {
#if (KDRV_AI_MINI_FOR_FASTBOOT == 1)
...
static int nvt_ai_probe(struct platform_device *pdev)
{
	...
	if (ret != 0) {
		DBG_ERR("KDRV_AI: Error, to do nvt_handle_fb_ai_on_off().\n");
		ret = -ENODEV;
		goto FAIL_FREE_BUF;
	}
	...
	//DBG_IND(" get clock source\r\n");


	if (of_property_read_u32_array(pdev->dev.of_node, "clock-frequency", ai_freq_from_dtsi, MODULE_CLK_NUM)) {
		DBG_IND("%s_%d, no dtsi input for clock-frequency.\r\n", __FUNCTION__, __LINE__);
	}

	net_supported_num = ai_default_net_supported_num;
	if (of_property_read_u32_array(pdev->dev.of_node, "net_supported_num", (UINT32 *) &net_supported_num, 1)) {
        DBG_IND("%s_%d, no dtsi input for net_supported_num.\r\n", __FUNCTION__, __LINE__);
    }
	if (net_supported_num != ai_default_net_supported_num) {
		ai_net_supported_num = net_supported_num;
		if (ai_net_supported_num > AI_SUPPORT_NET_MAX) {
			DBG_ERR("AI_MAIN: Error, to get net supported num = %d > %d, force to %d\n",
						ai_net_supported_num, AI_SUPPORT_NET_MAX, AI_SUPPORT_NET_MAX);
			ai_net_supported_num = AI_SUPPORT_NET_MAX;
		}
	}
