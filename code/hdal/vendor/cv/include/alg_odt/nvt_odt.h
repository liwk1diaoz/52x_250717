...
/**
     ...
     @param[in] p_buf: the work buffer for ODT process. the required buffer size can be get by odt_calcbuffsize(FALSE).
     ...
     @code
     {
         ...
         buf.addr = pbuf;
         buf.size = odt_calcbuffsize(FALSE);
...
/**
     ...
     @param[in] odt_with_feature: odt only with rect or with rect and feature, if with feature,please set TRUE,else set FALSE.

     @return the required buffer size.
*/
extern UINT32 odt_calcbuffsize(BOOL odt_with_feature);
...
/**
     Get odt version.
*/
extern HD_RESULT odt_get_version(CHAR* odt_version);
