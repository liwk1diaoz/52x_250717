...
typedef struct _DIS_ROI_MV_INFO {
	INT32   ix;          ///< x component
	INT32   iy;          ///< y component
	BOOL    b_valid;
	UINT32  frm_cnt;
...
typedef enum {
	DIS_SUBSAMPLE_SEL_1X = 0,	    /* use all pixel for motion vector */
	DIS_SUBSAMPLE_SEL_2X = 1,		/* use 1/2 pixel for motion vector, only support 528 */
	DIS_SUBSAMPLE_SEL_4X = 2,		/* use 1/4 pixel for motion vector, only support 528*/
	ENUM_DUMMY4WORD(DIS_SUBSEL)
} DIS_SUBSEL;
...
/**
    ...
    @return HD_RESULT,HD_OK for ok. other for error.
*/
extern ER         dis_process(void);
/**
    process the dis_alg without compensation, need set disinfor firstly.
    @return HD_RESULT,HD_OK for ok. other for error.
*/
extern ER         dis_process_no_compen_info(void);
...
/**
    ...
    @return HD_RESULT,HD_OK for ok. other for error.
*/
extern ER         dis_set_disinfor(DIS_PARAM *p_disinfo);
...
extern BOOL             dis_set_only_mv(UINT32 dis_only_proc_mv);
extern UINT32           dis_get_only_mv(void);

extern DIS_SUBSEL       dis_get_subin_info(void);
extern BOOL             dis_set_subin(DIS_SUBSEL subsel);

extern VOID             dis_reset_compensation_mv(void);
