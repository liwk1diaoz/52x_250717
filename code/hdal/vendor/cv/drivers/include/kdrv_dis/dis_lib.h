...
//@{
typedef enum {
	DIS_SUBIN_1x = 0,  ///< original
	DIS_SUBIN_2x = 1,  ///< 1/2 subsample
	DIS_SUBIN_4x = 2,  ///< 1/4 subsample
	ENUM_DUMMY4WORD(DIS_SUBIN)
} DIS_SUBIN;

typedef enum {
	DIS_SUBIN_CHOOSE_0 = 0,  ///< choose x%4 = 0 OR choose x%2 = 0
	DIS_SUBIN_CHOOSE_1 = 1,  ///< choose x%4 = 1 OR choose x%2 = 1
	DIS_SUBIN_CHOOSE_2 = 2,  ///< choose x%4 = 2
	DIS_SUBIN_CHOOSE_3 = 3,  ///< choose x%4 = 3
	ENUM_DUMMY4WORD(DIS_SUBIN_SEL)
} DIS_SUBIN_SEL;
...
///    UINT8   uiSrc2RefY;     ///< y shift between source and reference blocks
    DIS_SUBIN       SubIn;         ///< subsample input
    DIS_SUBIN_SEL   SubInSel;      ///< subsample input pixel
...
extern UINT32   dis_getCurIppbID(void);
extern UINT32   dis_getPrevIppbID(void);
extern UINT32   dis_getCurOppbID(void);

extern void dis_create_resource(UINT32 clk_freq);
