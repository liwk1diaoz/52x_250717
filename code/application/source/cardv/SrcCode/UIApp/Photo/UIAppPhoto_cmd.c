...
static BOOL cmd_fd(unsigned char argc, char **argv)
{
	UINT32 value;
...
static BOOL cmd_ev(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_iso(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_wb(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_size(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_quality(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_datestamp(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_sharp(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_qviewtime(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_selftimer(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_contshot(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_scene(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_wdr(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_defog(unsigned char argc, char **argv)
{
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_delete_file(unsigned char argc, char **argv)
{
	...
	sscanf_s(argv[0], "%d", &reservSizeMB);
...
BOOL cmd_cap(unsigned char argc, char **argv)
{
	/*
	UINT32 value;
	sscanf_s(argv[0], "%d", &value);
...
static BOOL cmd_stream(unsigned char argc, char **argv)
{
	/*
	...
	sscanf_s(argv[0], "%d %d %d %d %d", &stream_type,&codec,&width,&height,&target_byte_rate);
...
static BOOL cmd_setfps(unsigned char argc, char **argv)
{
	/*
	UINT32 src_fps = 30,dst_fps = 30;

	sscanf_s(argv[0], "%d %d", &src_fps,&dst_fps);
...
static BOOL cmd_quefull(unsigned char argc, char **argv)
{
	UINT32 i;
	for (i=0;i<257;i++)
...
BOOL cmd_photo_dumpfdb(unsigned char argc, char **argv)
{
#if (USE_FILEDB==ENABLE)
	FileDB_DumpInfo(0);
#endif
...
BOOL cmd_photo(unsigned char argc, char **argv)
{
...
static BOOL cmd_photo_trigger_cap(unsigned char argc, char **argv)
{
	#if (PHOTO_MODE==ENABLE)
...
static int uiphoto_cmd_showhelp(int (*dump)(const char *fmt, ...))
{
	UINT32 cmd_num = SXCMD_NUM(uiphoto_cmd_tbl);
	UINT32 loop = 1;
	...
	for (loop = 1 ; loop <= cmd_num ; loop++) {
		dump("%15s : %s\r\n", uiphoto_cmd_tbl[loop].p_name, uiphoto_cmd_tbl[loop].p_desc);
	}
	return 0;
}

MAINFUNC_ENTRY(uiphoto, argc, argv)
{
	UINT32 cmd_num = SXCMD_NUM(uiphoto_cmd_tbl);
	UINT32 loop;
	int    ret;
	...
	if (strncmp(argv[1], "?", 2) == 0) {
		uiphoto_cmd_showhelp(vk_printk);
		return 0;
	}
	for (loop = 1 ; loop <= cmd_num ; loop++) {
		if (strncmp(argv[1], uiphoto_cmd_tbl[loop].p_name, strlen(argv[1])) == 0) {
			ret = uiphoto_cmd_tbl[loop].p_func(argc-2, &argv[2]);
			return ret;
		}
	}
	return 0;
