...
#define __MODULE__          UIMENUWIFIQRCODE
...
INT32 UIMenuWndWiFiQRCodeScan_OnKeyEnter(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	case NVTEVT_KEY_PRESS:
	    if (System_GetState(SYS_STATE_CURRMODE) == PRIMARY_MODE_MOVIE){
	    	Ux_SendEvent(&CustomMovieObjCtrl, NVTEVT_EXE_MOVIE_QRCODE_STOP, 0);
...
INT32 UIMenuWndWiFiQRCodeScan_OnBackgroundDone(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	switch (event) {
	case NVTEVT_BKW_QRCODE_SCAN_DONE:
#if (WIFI_FUNC==ENABLE)
		Ux_OpenWindow(&UIMenuWndWiFiLivestreamCtrl, 0);
#endif
		break;
	}

	Ux_DefaultEvent(pCtrl,NVTEVT_BACKGROUND_DONE,paramNum,paramArray);
    return NVTEVT_CONSUME;
}
