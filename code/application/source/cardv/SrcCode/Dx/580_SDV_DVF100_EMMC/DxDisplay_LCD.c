...
#define LCD_ASPECT_H                    1



#define LCD2_MAX_W                      240//device w of DISP_LCDMODE_xxx
#define LCD2_MAX_H                      240  //device h of DISP_LCDMODE_xxx

#define LCD2_ASPECT_W                   1
#define LCD2_ASPECT_H                   1
...
#define DRVLCD_BRT_LEVEL_MAX      21
#define DRVLCD_BRT_LEVEL_INIT     10
...
//			int mode = (CapID & 0xffff0000) >> 16;
	...
	case DISPLAY_CAPS_MAXSIZE: {
			...
			if (pSizeInfo) {
				...
				if(1)//GetLcdType_Frontbehind()==LCD_FRONT)
				{
					pSizeInfo->w = LCD_MAX_W;
					pSizeInfo->h = LCD_MAX_H;
				}else
				{
					pSizeInfo->w = LCD2_MAX_W;
					pSizeInfo->h = LCD2_MAX_H;
				}
			}
		}
		break;
...
void Dx_SetFlipMirror(UINT32 value)
{
        static INT32 g_lcdmirrorflipfd = -1;

		
	//return;
	
        g_lcdmirrorflipfd = open("/dev/lcdmirrorflip", O_RDWR);
        vos_util_delay_ms(200);
        if (g_lcdmirrorflipfd < 0)
        {
            DBG_ERR("open [%s] failed\n\r","/lcdmirrorflip");
        } else {
            //printf(" Dx_SetFlipMirror  value=%d\n\r",value);
	    if(1)
	    {
	        if(value)
	            ioctl(g_lcdmirrorflipfd, IOC_LCD_DIR_FLIP_OSDLAYER, NULL);
	       else
                   ioctl(g_lcdmirrorflipfd, IOC_LCD_DIR_MIRROR_OSDLAYER, NULL);
	    }else{
	        if(value)
	            ioctl(g_lcdmirrorflipfd, IOC_LCD_DIR_MIRROR_FLIP_OSDLAYER, NULL);
	       else
                   ioctl(g_lcdmirrorflipfd, IOC_LCD_DIR_NORMAL_OSDLAYER, NULL);
	    }
            close(g_lcdmirrorflipfd);
        }
}
static UINT32 DrvLCDState(UINT32 StateID, UINT32 Value) // General Properties
{
	...
	if (StateID & DXGET) {
		...
		case DRVDISP_STATE_DIRECT:
			...
			printf("DRVDISP_STATE_DIRECT--->Value=%d\r\n",Value);
			Dx_SetFlipMirror(Value);
			break;
...
#define IOC_LCD_BACKLIGHT_HIGH             	_IOWR('w', 3, unsigned int) //
#define IOC_LCD_BACKLIGHT_MID             	_IOWR('w', 4, unsigned int) //
#define IOC_LCD_BACKLIGHT_LOW             	_IOWR('w', 5, unsigned int) //

#define IOC_LCD2_BACKLIGHT_ON              	_IOWR('w', 6, unsigned int)
#define IOC_LCD2_BACKLIGHT_OFF              	_IOWR('w', 7, unsigned int)

#define HAL_LCD_DEV "/dev/lcdpwm"

static    INT32 lcdpwmfd = -1;

static BOOL User_LcdPwm_SetLuma(UINT32 uiAdjValue,BOOL onoff,BOOL IsFrontLCD)
{
          unsigned int uiAdjValuetemp=uiAdjValue;
	...
	// return;
	   ...
	   if(lcdpwmfd<0)
	   {
	    ...
	    if (lcdpwmfd < 0)
	    {
	        ...
	        if(IsFrontLCD==1){
		    if(onoff==OFF)
			ioctl(lcdpwmfd, IOC_LCD_BACKLIGHT_OFF, &uiAdjValuetemp);
		    else
	    	        ioctl(lcdpwmfd, IOC_LCD_BACKLIGHT_ON, &uiAdjValuetemp);
	        }else {//if(IsFrontLCD==0){
		    if(onoff==OFF)
			ioctl(lcdpwmfd, IOC_LCD2_BACKLIGHT_OFF, &uiAdjValuetemp);
		    else
	    	        ioctl(lcdpwmfd, IOC_LCD2_BACKLIGHT_ON, &uiAdjValuetemp);
	        }
	      //  close(lcdpwmfd);
	    }
...
#define HAL_GYRO_DEV "/dev/gyro_icm42607"
#define IOC_GYRO_GET_XYZ_DATA			_IOWR('r', 1, unsigned int)


/*liu modify 21*/
 void Get_kernal_GyroData(INT32  *gyrodata)
{
...
void GPIOMap_LCDPwmInit(UINT32 levelpoweron)
{
	g_LCDBacklightLvl=levelpoweron;
}
/**
  Turn on LCD power
...
/*static void GPIOMap_TurnOnOffSmallLcd(BOOL onoff)
{
	//printf("======GPIOMap_TurnOnOffSmallLcd onoff=%d\r\n",onoff);
	gpio_setDir(P_GPIO_5, GPIO_DIR_OUTPUT);
	if(onoff)
		gpio_setPin(P_GPIO_5);
	else
...
#if (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_PWM)
		if(1)
			User_LcdPwm_SetLuma(g_LCDBacklightLvlPWMDuty[g_LCDBacklightLvl],ON,1);
		else
			//GPIOMap_TurnOnOffSmallLcd(ON);
			User_LcdPwm_SetLuma(g_LCDBacklightLvlPWMDuty[g_LCDBacklightLvl],ON,0);
...
#if (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_PWM)
		if(1)
			User_LcdPwm_SetLuma(0,OFF,1);
		else
			//GPIOMap_TurnOnOffSmallLcd(OFF);
			User_LcdPwm_SetLuma(0,OFF,0);
			
#elif (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_GPIO)
		gpio_setDir(GPIO_LCD_BLG_PCTL, GPIO_DIR_OUTPUT);
		gpio_clearPin(GPIO_LCD_BLG_PCTL);
		printf("GPIO_LCD_BLG_PCTL CLEAR\r\n");
...
static void GPIOMap_AdjustLCDBacklight(UINT32 uiAdjValue)
{
	//	printf("GPIOMap_AdjustLCDBacklight ----\r\n");
...
void JGHT_GPIOMap_TurnOnOffLCDBacklight(BOOL onoff)
{

         printf("JGHT_GPIOMap_TurnOnOffLCDBacklight---onoff=%d\r\n",onoff);
