...
void IO_InitGPIO(void)
{
	...
	DBG_IND("GPIO END\r\n");
#endif
    gpio_setDir(GPIO_EHT_CAM_1_PWR, GPIO_DIR_OUTPUT);
    gpio_setPin(GPIO_EHT_CAM_1_PWR);
    
    gpio_setDir(GPIO_EHT_CAM_2_PWR, GPIO_DIR_OUTPUT);
    gpio_setPin(GPIO_EHT_CAM_2_PWR);



    DBG_DUMP("IO_InitGPIO");

    
	gpio_setPin(GPIO_ETH_RESET);
	vos_util_delay_ms(100);
	gpio_clearPin(GPIO_ETH_RESET);
	vos_util_delay_ms(100);
	gpio_setPin(GPIO_ETH_RESET);
