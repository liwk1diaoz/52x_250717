...
typedef enum _PB_PRMID {
	...
	PBPRMID_JPG_DECRYPT_KEY     = 0x1A, ///< JPG decryption key
	PBPRMID_JPG_DECRYPT_LEN     = 0x1B, ///< JPG decryption length
	PBPRMID_VID_DECRYPT_MODE    = 0x1C, ///< Video decryption mode. 1:AES128 2:AES256
	PBPRMID_VID_DECRYPT_POS     = 0x1D, ///< Video decryption pos. refer to PB_DECRYPT_TYPE
	...
	PBPRMID_MAX_FILE_SIZE       = 0x97, ///< Max File Size by project
	PBPRMID_MAX_RAW_SIZE        = 0x98, ///< Max decode YUV size
	PBPRMID_MAX_DECODE_WIDTH    = 0x99, ///< Decode max width
	PBPRMID_MAX_DECODE_HEIGHT   = 0x9A, ///< Decode max height
	PBPRMID_PIXEL_FORMAT        = 0x9B, ///< VDOUT pixel by project
	PBPRMID_HD_VIDEODEC_PATH    = 0x9C, ///< Set/Get the hd_videodec path from ImageApp_Play
	...
	PBPRMID_ENABLE_DEBUG_MSG    = 0xB0, ///< Enable the debug msg or not
...
typedef enum {
	PB_DEC_PRIMARY,        ///< decode primary image
	PB_DEC_THUMBNAIL,      ///< decode thumbnail, or decode primary image
	PB_DEC_HIDDEN,         ///< decode hidden thumb, or decode primary image
	PB_DEC_THUMB_ONLY,     ///< decode thumbnail only
} PB_JPG_DEC_TYPE;


typedef enum _PB_JPG_YUV_FORMAT {
	PB_JPG_FMT_YUV211  = 0,   ///< YCbCr 2h11
	PB_JPG_FMT_YUV422  = 0,   ///< YCbCr 2h11
	PB_JPG_FMT_YUV420  = 1,   ///< YCbCr 411
	PB_JPG_FMT_YUV211V = 3,   ///< YCbCr 2v11
//	PB_JPG_FMT_YUV100  = 4,   ///< YCbCr 100 (Y plane only)
	ENUM_DUMMY4WORD(PB_JPG_YUV_FORMAT)
} PB_JPG_YUV_FORMAT;

typedef struct _PB_HD_COM_BUF{
	UINT32 pa;
	UINT32 va;
    UINT32 blk_size;
	HD_COMMON_MEM_VB_BLK blk; //block ID
}PB_HD_COM_BUF, *PPB_HD_COM_BUF;

typedef enum {
	PB_DECRYPT_TYPE_NONE              = 0x00000000,
	PB_DECRYPT_TYPE_CONTAINER         = 0x00000001,
	PB_DECRYPT_TYPE_I_FRAME           = 0x00000002,
	PB_DECRYPT_TYPE_P_FRAME           = 0x00000004,
	PB_DECRYPT_TYPE_ALL_FRAME         = 0x00000006,
	ENUM_DUMMY4WORD(PB_DECRYPT_TYPE)
} PB_DECRYPT_TYPE;

typedef enum {
	PB_DECRYPT_MODE_AES128            = 2,
	PB_DECRYPT_MODE_AES256			 = 3,
	ENUM_DUMMY4WORD(PB_DECRYPT_MODE)
} PB_DECRYPT_MODE;
...
*/
typedef void (*PB_DISPTRIG_CB)(HD_VIDEO_FRAME *pimg_buf, BOOL trigger_draw);
...
*/
typedef void (*PB_ONDRAW_CB)(PB_VIEW_STATE view_state, HD_VIDEO_FRAME *pHdVdoFrame);
//------------------------------------------------------------------------------
// Structures
//------------------------------------------------------------------------------
/**
...
/**
    ...
    @return void
*/
extern void PB_InstallID(void);

//------------------------------------------------------------------------------
// Export Functions
//------------------------------------------------------------------------------
/**
    Uninstall Playback task, flag and semaphore ID

    @return void
*/
extern void PB_UninstallID(void);
...
#if 0
extern BOOL PB_DispSrv_OnLock(UINT32 *puiLocked, VDO_FRAME *pImg);
#else
...
extern void PB_SeekCurrent(void);
