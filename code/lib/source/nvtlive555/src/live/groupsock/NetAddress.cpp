...
NetAddressList::NetAddressList(char const* hostname)
...
#if defined(__LINUX)
  struct ifaddrs * ifAddrStruct=NULL, *p=NULL;
  ...
  p = ifAddrStruct;
  while (p!=NULL) {
    if (p->ifa_addr->sa_family==AF_INET) { // check it is IPv4
      // is a valid IPv4 Address
      fAddressArray[i++] = new NetAddress((u_int8_t const*)&(((struct sockaddr_in*)p->ifa_addr)->sin_addr.s_addr), 4);
    } else if (p->ifa_addr->sa_family==AF_INET6) { // check it is IPv6
      // is a valid IPv6 Address
	  fAddressArray[i++] = new NetAddress((u_int8_t const*)&(((struct sockaddr_in*)p->ifa_addr)->sin_addr.s_addr), 16);
    }
...
void AddressString::init(netAddressBits addr) {
  ...
  sprintf(fVal, "%u.%u.%u.%u", (int)((addrNBO>>24)&0xFF), (int)((addrNBO>>16)&0xFF), (int)((addrNBO>>8)&0xFF), (int)(addrNBO&0xFF));
}
