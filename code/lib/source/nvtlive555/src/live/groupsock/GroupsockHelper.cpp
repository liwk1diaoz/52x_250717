...
#else
#if defined(SO_REUSEPORT) && !defined(__FREERTOS) //NVT_MODIFIED, FIXME
...
Boolean makeSocketNonBlocking(int sock) {
  ...
  int arg = 1;
  return lwip_ioctl(sock, FIONBIO, (void *)&arg) == 0;
#else
...
Boolean writeSocket(UsageEnvironment& env,
		    ...
		    u_int8_t ttlArg,
		    unsigned char* buffer, unsigned bufferSize, int* pByteSent) { //NVT_MODIFIED, WIFI-TEAM
	do {
		if (ttlArg != 0) {
...
#define TTL_TYPE u_int8_t
...
#if 0 //show seq number
		if (bufferSize > 0x4) {
			unsigned char *p_seq = (unsigned char *)&buffer[0x0];
			int seq_num = (((int)p_seq[2]) << 8) | p_seq[3];
			printf("%d\n", seq_num);
		}
#endif
#else
...
#endif
        if(pByteSent != NULL) //NVT_MODIFIED, WIFI-TEAM
        {
            *pByteSent = bytesSent;
        }
		...
		if (bytesSent != (int)bufferSize) {
			...
			if (bytesSent != -1) {
				printf("live555: failed to send=%d, dst-port=%d\r\n", bytesSent, ntohs(port.num()));
			}

#if 0 //shows sendto fail
            unsigned char* p = (unsigned char*)buffer;
            unsigned int seq =(((unsigned int)p[2]) << 8) | (unsigned int)p[3];

            if(p[1]!=0xC8)                 
                printf("RTP send error, seq:%d, wrote %d bytes instead of %u\r\n",seq, bytesSent, bufferSize);            
            else            
                printf("RTCP send error\r\n");            
#endif
			//break;
		}
...
netAddressBits ourIPAddress(UsageEnvironment& env) {
  ...
  if (ourAddress == 0) {
    ...
    if (!loopbackWorks) do {
	  ...
	  int result = 0;
	  sprintf(hostname, "unknown host %d", (unsigned)(our_random() * 0x7FFFFFFF));
#endif
...
int gettimeofday(struct timeval* tp, int* /*tz*/) {
  ...
  if (!isInitialized) {
    if(1 == InterlockedIncrement(&initializeLock_gettimeofday)) {
      ...
      //NVT_MODIFIED
      u_int64_t tm64 = tickNow.QuadPart*1000000/tickFrequency.QuadPart;
