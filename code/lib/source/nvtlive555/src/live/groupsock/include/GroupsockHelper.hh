...
Boolean writeSocket(UsageEnvironment& env,
		    ...
		    u_int8_t ttlArg,
		    unsigned char* buffer, unsigned bufferSize, int* pByteSent=NULL); //NVT_MODIFIED, WIFI-TEAM
...
//NVT_MODIFIED, fix bind LOOPBACK failed on eCos (because eCos use memcmp to compare var)
#define MAKE_SOCKADDR_IN(var,adr,prt) /*adr,prt must be in network order*/\
    struct sockaddr_in var = {0};\
