...
Boolean OutputSocket::write(netAddressBits address, Port port, u_int8_t ttl,
			    unsigned char* buffer, unsigned bufferSize, int idx_socket, int* pByteSent) { //NVT_MODIFIED, Multi-UDP Source //NVT_MODIFIED, WIFI-TEAM
  if (ttl == fLastSentTTL) {
  ...
  if (!writeSocket(env(), socketNum(idx_socket), destAddr, port, ttl, //NVT_MODIFIED, Multi-UDP Source
		   buffer, bufferSize, pByteSent)) //NVT_MODIFIED, WIFI-TEAM
    return False;

  if (sourcePortNum() == 0) {
    // Now that we've sent a packet, we can find out what the
    // kernel chose as our ephemeral source port number:
    if (!getSourcePort(env(), socketNum(idx_socket), fSourcePort)) { //NVT_MODIFIED, Multi-UDP Source
      if (DebugLevel >= 1)
	env() << *this
...
Boolean Groupsock::output(UsageEnvironment& env, u_int8_t ttlToSend,
			  ...
			  DirectedNetInterface* interfaceNotToFwdBackTo,int idx_socket, int* pByteSent) { //NVT_MODIFIED, Multi-UDP Source //NVT_MODIFIED, WIFI-TEAM
  do {
    // First, do the datagram send, to each destination:
    Boolean writeSuccess = True;
    for (destRecord* dests = fDests; dests != NULL; dests = dests->fNext) {
      if (!write(dests->fGroupEId.groupAddress().s_addr, dests->fPort, ttlToSend,
		 buffer, bufferSize,idx_socket,pByteSent)) { //NVT_MODIFIED, Multi-UDP Source //NVT_MODIFIED, WIFI-TEAM
	writeSuccess = False;
	break;
      }
    }
