...
#include <unix.h>
#endif
#include "GroupsockHelper.hh" //NVT_MODIFIED
#include "NvtMgr.hh"
////////// BasicTaskScheduler //////////
...
void BasicTaskScheduler::SingleStep(unsigned maxDelayTime) {
  ...
  while ((handler = iter.next()) != NULL) {
    ...
    if (selectResult && FD_ISSET(sock, &readSet) && FD_ISSET(sock, &fReadSet)/*sanity check*/) resultConditionSet |= SOCKET_READABLE;
    if (selectResult && FD_ISSET(sock, &writeSet) && FD_ISSET(sock, &fWriteSet)/*sanity check*/) resultConditionSet |= SOCKET_WRITABLE;
    if (selectResult && FD_ISSET(sock, &exceptionSet) && FD_ISSET(sock, &fExceptionSet)/*sanity check*/) resultConditionSet |= SOCKET_EXCEPTION;
