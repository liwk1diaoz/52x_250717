...
Boolean RTPInterface::sendPacket(unsigned char* packet, unsigned packetSize, int idx_socket, int* pByteSent) { //NVT_MODIFIED, Multi-UDP Source //NVT_MODIFIED, WIFI-TEAM
  Boolean success = True; // we'll return False instead if any of the sends fail

  // Normal case: Send as a UDP packet:
  if (!fGS->output(envir(), fGS->ttl(), packet, packetSize,NULL,idx_socket,pByteSent)) success = False; //NVT_MODIFIED, Multi-UDP Source
...
Boolean RTPInterface::sendRTPorRTCPPacketOverTCP(u_int8_t* packet, unsigned packetSize,
  ...
  do {
    ...
    {
        printf("live555: tcp send timeout-1 (%d)\r\n",streamChannelId);
        break; 
    }
    if (!sendDataOverTCP(socketNum, packet, packetSize, True)) 
    {
        printf("live555: tcp send timeou-2 (%d)\r\n",streamChannelId);
        break;
    }
