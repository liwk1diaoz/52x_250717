...
RTPSink::RTPSink(UsageEnvironment& env,
  ...
  : MediaSink(env), fRTPInterface(this, rtpGS),
    ...
    fNumChannels(numChannels), fTimestampOffset(0) {
...
u_int32_t RTPSink::convertToRTPTimestamp(struct timeval tv) {
  ...
  if (fNextTimestampHasBeenPreset) {
	//NVT_MODIFIED, NVT might skip frame at begging (for wait first I), so the start rtp time cannot be the same with first I's timestamp
	//fTimestampBase = random time + begin time, so don't use below as new fTimestampBase
	//fTimestampBase =  (fTimestampFrequency*timeNow.tv_sec) + (u_int32_t)(fTimestampFrequency*(timeNow.tv_usec / 1000000.0) + 0.5)
	if (fTimestampOffset == 0) { // because seek will trigger fNextTimestampHasBeenPreset once again, use fTimestampOffset == to prevent it
	  struct timeval timeNow;
	  gettimeofday(&timeNow, NULL);
	  timeNow.tv_sec -= fCreationTime.tv_sec;
	  timeNow.tv_usec -= fCreationTime.tv_usec;
	  fTimestampOffset = (fTimestampFrequency*timeNow.tv_sec) + (u_int32_t)(fTimestampFrequency*(timeNow.tv_usec / 1000000.0) + 0.5);
	}
  ...
  u_int32_t const rtpTimestamp = fTimestampBase + timestampIncrement + fTimestampOffset;
