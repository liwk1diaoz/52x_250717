...
Boolean MediaSink::startPlaying(MediaSource& source,
  ...
  // to avoid thread starting too early to cause sink not ready, so execute here
  NvtStreamFramer *pNvtFramer = dynamic_cast<NvtStreamFramer*>(&source);
  if (pNvtFramer) {
	  pNvtFramer->Execute();
	  if (pNvtFramer->IsPollingMode()) {
		  return continuePlaying();
	  } else {
		  return True; //use thread to trigger frame
	  }
  } else {
	  return continuePlaying();
  }
...
OutPacketBuffer::OutPacketBuffer(unsigned preferredPacketSize,
  ...
  //fLimit = maxNumPackets*maxPacketSize; //NVT_MODIFIED
  fLimit = maxPacketSize;
