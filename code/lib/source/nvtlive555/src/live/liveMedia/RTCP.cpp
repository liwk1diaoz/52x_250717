...
#endif
  m_tv_last_rtcp_receive.tv_sec = 0; //NVT_MODIFIED, client timeout mechanism    
  m_tv_last_rtcp_receive.tv_usec = 0; //NVT_MODIFIED, client timeout mechanism 
...
void RTCPInstance::incomingReportHandler1() {
  do {
    ...
    if (!readResult){
        //NVT_MODIFIED
        fRTCPInterface.stopNetworkReading();  
        //Notify RTPSource and RTPSink, there error occurred so we should stop
        if(fSink)  
            fSink->stopPlaying();  
        if(fSource)  
        {
    ...
    NotifyRtcpReceive(); //NVT_MODIFIED, client timeout mechanism
...
void RTCPInstance::onExpire1() {
  ...
  if(IsRtcpReceiveTimeout(CFG_CLIENT_TIMEOUT_SEC)) //NVT_MODIFIED, client timeout mechanism
  {
      if(fSource)  
      {
          printf("live555cli: one of stream disconnected because of timeout\r\n");
          nextTask() = envir().taskScheduler().scheduleDelayedTask(0,
              (TaskFunc*)fSource->handleClosure, (void*)fSource);      
          return;
      }
  }
...
void RTCPInstance::NotifyRtcpReceive() { //NVT_MODIFIED, client timeout mechanism
    gettimeofday(&m_tv_last_rtcp_receive,NULL);
}

Boolean RTCPInstance::IsRtcpReceiveTimeout(long timeout_sec) { //NVT_MODIFIED, client timeout mechanism    
    if(m_tv_last_rtcp_receive.tv_sec==0)
        return False;

    struct timeval timeNow;
    gettimeofday(&timeNow, NULL);

    if(timeNow.tv_sec-m_tv_last_rtcp_receive.tv_sec >= timeout_sec)
        return True;
...
extern "C" void SendRTCPReport(event e) {
  ...
  if (pNvtMgr && pNvtMgr->IsSupportRtcp() == 0) {
	  //printf("live555: disable rtcp.\r\n");
	  //disable it will cause VLC get several garbage frames at start
  } else {
	  instance->sendReport();
  }
}
