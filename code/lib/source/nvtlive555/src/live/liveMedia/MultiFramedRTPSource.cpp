...
#define CFG_MAX_PACKET_COUNTS 32
...
public:
  ...
  void releaseUsedPacket(BufferedPacket* packet); //NVT_MODIFIED
  void freePacket(BufferedPacket* packet);

  Boolean isEmpty() const; //NVT_MODIFIED
  Boolean isFull() const {return m_nPacketInQueue>=CFG_MAX_PACKET_COUNTS;} //NVT_MODIFIED
...
private:
  ...
  int m_nPacketInQueue; //NVT_MODIFIED
};
...
::MultiFramedRTPSource(UsageEnvironment& env, Groupsock* RTPgs,
  ...
  increaseReceiveBufferTo(env, RTPgs->socketNum(), 8192*1024); //NVT_MODIFIED
}
...
void MultiFramedRTPSource::networkReadHandler1() {
  ...
  do {
    ...
    if(fReorderingBuffer->isFull()) //NVT_MODIFIED
    {
        printf("live555cli: drop current packet due to buffer.\n");
        break;
    }
    else if (!fReorderingBuffer->storePacket(bPacket))
    {   
        break;
    }
...
#define MAX_PACKET_SIZE 16384 //NVT_MODIFIED, RTP Packet Size, Normally 8K, we use 16K to be safe.
...
::ReorderingPacketBuffer(BufferedPacketFactory* packetFactory)
  : fThresholdTime(100000) /* default reordering threshold: 100 ms */,
    fHaveSeenFirstPacket(False), fHeadPacket(NULL), fTailPacket(NULL), fSavedPacket(NULL), fSavedPacketFree(True), m_nPacketInQueue(0) {
...
Boolean ReorderingPacketBuffer::isEmpty() const 
{ 
    if(fHeadPacket == NULL && m_nPacketInQueue>1) //1: is kept head
    {
        printf("live555cli: assert isEmpty get:%d",m_nPacketInQueue);
    }
...
void ReorderingPacketBuffer::freePacket(BufferedPacket* packet) {
