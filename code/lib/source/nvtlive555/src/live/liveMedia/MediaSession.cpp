...
MediaSession::MediaSession(UsageEnvironment& env)
  ...
  const unsigned maxCNAMElen = 60; //NVT_MODIFIED
...
MediaSubsession::MediaSubsession(MediaSession& parent)
  : serverPortNum(0), sink(NULL), miscPtr(NULL),
	...
	fSpropVps(NULL), fSpropSps(NULL), fSpropPps(NULL),
...
MediaSubsession::~MediaSubsession() {
  ...
  delete[] fSpropVps; delete[] fSpropSps; delete[] fSpropPps;
  delete[] fAbsStartTime; delete[] fAbsEndTime;
  delete[] fSessionId;
...
#endif
      if (strcmp(fCodecName, "JPEG") == 0) { // motion JPEG
        if (fReceiveRawJPEGFrames) {
		  ...
		  Boolean expectDONFields = False; //NVT_MODIFIED, H265
		  fReadSource = fRTPSource
