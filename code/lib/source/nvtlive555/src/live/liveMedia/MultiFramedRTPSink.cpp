...
#define CFG_DETECT_SENDING_TIME 1
#define CFG_SENDING_TIME_EXPIRE_MS 100
...
MultiFramedRTPSink::MultiFramedRTPSink(UsageEnvironment& env,
  ...
  : RTPSink(env, rtpGS, rtpPayloadType, rtpTimestampFrequency,
    ...
    fOnSendErrorFunc(NULL), fOnSendErrorData(NULL), fOutBuf(NULL), fpReplace(NULL), bSkipThisSend(0), fSocketIdx(0), fByteSent(0) { //NVT_MODIFIED, Multi-UDP Source //NVT_MODIFIED, WIFI-TEAM
  setPacketSizes(1000, 1448); //8192 //1448 //2048 for QuickTime //NVT_MODIFIED
      // Default max packet size (1500, minus allowance for IP, UDP, UMTP headers)
      // (Also, make it a multiple of 4 bytes, just in case that matters.)
  fNextSendTime.tv_sec = fNextSendTime.tv_usec = 0;
}
...
//NVT_MODIFIED
void MultiFramedRTPSink::NotifyFinishSendPacket(void) {
  return;
}
...
void MultiFramedRTPSink::stopPlaying() {
  ...
  NvtStreamFramer *pFramer = (NvtStreamFramer *)fSource;
  if (pFramer) {
	  pFramer->Pause();
  }

  // Then call the default "stopPlaying()" function:
  MediaSink::stopPlaying(); //after this, fSource change to NULL
}
...
void MultiFramedRTPSink::packFrame() {
  ...
  if (fOutBuf->haveOverflowData()) {
	...
	//NVT_MODIFIED, because in pause, fSource is NULL, but we have to flush old frame in multi-thread mechanism
	NvtRTPSink *pNvtSink = (NvtRTPSink *)this;
	pNvtSink->GetFramer()->getNextFrame(fOutBuf->curPtr(), fOutBuf->totalBytesAvailable(),
...
::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
  ...
  if(bSkipThisSend) //NVT_MODIFIED
  {
	  ...
	  NotifyFinishSendPacket(); //NVT_MODIFIED
	  NvtStreamFramer *pFramer = (NvtStreamFramer *)fSource;
	  if (pFramer->IsContinueToNext()) {
  ...
  if (fNumFramesUsedSoFar > 0) {
	printf("live555: exception on: fNumFramesUsedSoFar > 0.\n");
...
#endif
    {
        unsigned char* pPacket = fOutBuf->packet();
        if(fpReplace!=NULL)
        {
            u_int32_t hdr_size = 12+specialHeaderSize();
            pPacket = (unsigned char*)(fpReplace-hdr_size);
            memcpy(pPacket, fOutBuf->packet(),hdr_size);
        }

#if (CFG_DETECT_SENDING_TIME)
...
#if 1       
        if (!fRTPInterface.sendPacket(pPacket, fOutBuf->curPacketSize(),fSocketIdx, &fByteSent))  //NVT_MODIFIED, Multi-UDP Source //NVT_MODIFIED, WIFI-TEAM
        {
        ...
        int64_t Diff_Ms = ((int64_t)(timeEnd.tv_sec - timeStart.tv_sec)*1000000 + (timeEnd.tv_usec - timeStart.tv_usec))/1000;
        if(Diff_Ms > CFG_SENDING_TIME_EXPIRE_MS)
        {
            printf("live555: sending packet time too long: %d ms\r\n",(int)Diff_Ms);
        }
#endif
    }      
    ++fPacketCount;
...
void MultiFramedRTPSink::sendNew(void* firstArg) {
	MultiFramedRTPSink* sink = (MultiFramedRTPSink*)firstArg;
