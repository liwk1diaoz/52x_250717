...
ServerMediaSession* RTSPServer::lookupServerMediaSession(char const* streamName,int isForceClean, char const* url) { //NVT_MODIFIED, in order to clean sms at each rtsp-desc command
...
#if 0 //#ifndef ALLOW_RTSP_SERVER_PORT_REUSE //NVT_MODIFIED, we allow to reuse
    NoReuse dummy(env); // Don't use this socket if there's already a local server using it
#endif

    ourSocket = setupStreamSocket(env, ourPort, False); //NVT_MODIFIED
    if (ourSocket < 0) break;

#if !defined(__FREERTOS) //lwip not support on SO_SNDBUF
    // Make sure we have a big send buffer:
    if (!increaseSendBufferTo(env, ourSocket, pNvtMgr->Get_SendBufSize())) break; //NVT_MODIFIED
#endif

    int tos = pNvtMgr->Get_TypeOfService();
    if(tos!=0)
    {
        int ret = setsockopt( ourSocket, IPPROTO_IP, IP_TOS, (char*) &tos, sizeof(tos) );
        if (ret == -1)
...
RTSPServer::RTSPServer(UsageEnvironment& env,
  ...
  : Medium(env),
    ...
    fAuthDB(authDatabase), fReclamationTestSeconds(reclamationTestSeconds), m_current_connected_sn(0){ //NVT_MODIFIED, fix sms leaking
  NvtMgr *pNvtMgr = NvtMgr_GetHandle();
  ignoreSigPipeOnSocket(ourSocket); // so that clients on the same host that are killed don't also kill us
  //NVT_MODIFIED, for H Customer, first RTCP cause their app have a bug.
  if (pNvtMgr->IsSupportRtcp()==0) {
	  //printf("live555: disable rtcp");
	  fReclamationTestSeconds = 0;
  }
...
void RTSPServer::incomingConnectionHandler(int serverSocket) {
  ...
  increaseSendBufferTo(envir(), clientSocket, pNvtMgr->Get_SendBufSize()); //NVT_MODIFIED

  {//NVT_MODIFIED, set tcp timeout and tos value

      struct timeval timeout;
      timeout.tv_sec = pNvtMgr->Get_TCPTimeOutSec();
      timeout.tv_usec = 0;
      ...
      if(tos!=0)
      {
          int ret = setsockopt(clientSocket, IPPROTO_IP, IP_TOS, (char*) &tos, sizeof(tos) );
          if (ret == -1)
              printf("Couldn't setsockopt(IP_TOS)\n");
      }
	  ...
	  if (setsockopt(clientSocket, SOL_SOCKET, SO_LINGER,
		  (const char*)&so_linger, sizeof so_linger) < 0) {
  ...
  //NVT_MODIFIED, fix sms leaking
  (void)createNewClientConnection(clientSocket, clientAddr, m_current_connected_sn++);
}
...
int RTSPServer::continueRegisterStream(RegisterRequestRecord* registerRequest) {
  ...
  char* cmdBuffer = new char[strlen(formatStr) + strlen(ourURL) + 1]; // big enough to hold the command
...
//NVT_MODIFIED, fix sms leaking
::RTSPClientConnection(RTSPServer& ourServer, int clientSocket, struct sockaddr_in clientAddr, unsigned int connected_sn)
  : fOurServer(ourServer), fIsActive(True),
    ...
    fRecursionCount(0), fOurSessionCookie(NULL), m_ppRTSPClientConnection(NULL), m_connected_sn(connected_sn) {
...
RTSPServer::RTSPClientConnection::~RTSPClientConnection() {
  ...
  //NVT_MODIFIED, fix timeout without release accepted socket. (fix side effect)
  if (m_ppRTSPClientConnection) {
	  RTSPClientConnection **pp = (RTSPClientConnection **)m_ppRTSPClientConnection;
	  *pp = NULL;
  }
}
...
void RTSPServer::RTSPClientConnection::handleCmd_OPTIONS() {
  ...
  NvtMgr_GetHandle()->SendEvent(LIVE555_SERVER_EVENT_OPTIONS); //NVT_MODIFIED
}
...
::handleCmd_DESCRIBE(char const* urlPreSuffix, char const* urlSuffix, char const* fullRequestStr) {
  //NVT_MODIFIED, we allow to only one client. after anyone connect, we close the session before did.
  //NVT_MODIFIED, TRY to lock, if lock unsupported, we only allow one connection
  NvtMgr *pNvtMgr = NvtMgr_GetHandle();
  int max_connections = pNvtMgr->Get_MaxConnections();
  if (max_connections == 1) {
	  RTSPServer::RTSPClientSession* clientSession;
	  while ((clientSession = (RTSPServer::RTSPClientSession*)fOurServer.fClientSessions->getFirst()) != NULL) {
		  delete clientSession;
	  }
  } else if (max_connections == (int)fOurServer.fClientSessions->numEntries()) {
	  printf("live555: reach to max connection-counts: %d\r\n", max_connections);
	  return;
  }
  ...
  do {
    ...
    //NVT_MODIFIED, fix sms leaking
	char session_name[10] = { 0 };
	snprintf(session_name, 9, "%08X", m_connected_sn);
    ServerMediaSession* session = fOurServer.lookupServerMediaSession(session_name, 1, urlTotalSuffix); //urlTotalSuffix
    if (session == NULL) {
      handleCmd_notFound();
      break;
    }
  ...
  delete[] rtspURL;
  pNvtMgr->SendEvent(LIVE555_SERVER_EVENT_DESCRIBE); //NVT_MODIFIED
}
  ...
  do {
	...
	if (sessionIdStr[0] == '\0') {
	  ...
	  do {
	    ...
	    sprintf(sessionIdStr, "%08X", (int)sessionId);
	  } while (sessionId == 0 || fOurServer.fClientSessions->Lookup(sessionIdStr) != NULL);
	  clientSession = fOurServer.createNewClientSession(sessionId, this); //NVT_MODIFIED, fix timeout without release accepted socket.
...
#if (!CFG_TEST_AUTH)
  //NVT_MODIFIED, auth mechanis
  if(NvtMgr_GetHandle()->Authenticate(fullRequestStr,strlen(fullRequestStr),(char*)fResponseBuffer, sizeof(fResponseBuffer))==200)
  {
      return True;
  }
  else
...
//NVT_MODIFIED, fix timeout without release accepted socket. (fix side effect)
void RTSPServer::RTSPClientConnection::setNotifySuicide(void **ppRTSPClientConnection)
{
	m_ppRTSPClientConnection = ppRTSPClientConnection;
}
...
::RTSPClientSession(RTSPServer& ourServer, u_int32_t sessionId, RTSPClientConnection* ourClientConnection) //NVT_MODIFIED, fix timeout without release accepted socket.
  : fOurServer(ourServer), fOurSessionId(sessionId), fOurServerMediaSession(NULL), fIsMulticast(False), fStreamAfterSETUP(False),
    fTCPStreamIdCount(0), fLivenessCheckTask(NULL), fNumStreamStates(0), fOurClientConnection(ourClientConnection), fStreamStates(NULL){ //NVT_MODIFIED, fix timeout without release accepted socket.
	//NVT_MODIFIED, fix timeout without release accepted socket. (fix side effect)
	fOurClientConnection->setNotifySuicide((void **)&fOurClientConnection);
  noteLiveness();
}
...
RTSPServer::RTSPClientSession::~RTSPClientSession() {
  ...
  //NVT_MODIFIED, fix timeout without release accepted socket. (fix side effect)
  if (fOurClientConnection) {
	  fOurClientConnection->setNotifySuicide((void **)NULL);
  }

  // Remove ourself from the server's 'client sessions' hash table before we go:
  char sessionIdStr[9];
  sprintf(sessionIdStr, "%08X", (int)fOurSessionId);
  ...
  if (fOurServerMediaSession != NULL) {
	...
	// && fOurServerMediaSession->deleteWhenUnreferenced()) { //NVT_MODIFIED, fix bug that don't kill sms when disconnecting
...
::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  do {
    // First, make sure the specified stream name exists:
      //NVT_MODIFIED, fix sms leaking
	  char session_name[10] = { 0 };
	  snprintf(session_name, 9, "%08X", ourClientConnection->m_connected_sn);
    ServerMediaSession* sms = fOurServer.lookupServerMediaSession(session_name, 0, streamName); //streamName
    if (sms == NULL) {
  ...
  NvtMgr_GetHandle()->SendEvent(LIVE555_SERVER_EVENT_SETUP); //NVT_MODIFIED
}
...
::handleCmd_TEARDOWN(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  NvtMgr_GetHandle()->SendEvent(LIVE555_SERVER_EVENT_TEARDOWN); //NVT_MODIFIED
}
...
::handleCmd_PLAY(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  if (!sawRangeHeader) {
    ...
    u_int64_t numBytes;
    rangeStart = fStreamStates[i].subsession->getCurrentNPT(fStreamStates[i].streamToken);
    fStreamStates[i].subsession->seekStream(fOurSessionId, fStreamStates[i].streamToken,
        rangeStart, 0, numBytes);
	float npt = (float)rangeStart;
    npt_end = fStreamStates[i].subsession->duration(); //NVT_MODIFIED
  ...
  NvtMgr_GetHandle()->SendEvent(LIVE555_SERVER_EVENT_PLAY); //NVT_MODIFIED
}
...
::handleCmd_PAUSE(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  NvtMgr_GetHandle()->SendEvent(LIVE555_SERVER_EVENT_PAUSE); //NVT_MODIFIED
}
...
//NVT_MODIFIED, fix sms leaking
RTSPServer::RTSPClientConnection*
RTSPServer::createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr, unsigned int connected_sn) {
  return new RTSPClientConnection(*this, clientSocket, clientAddr, connected_sn);
}

RTSPServer::RTSPClientSession*
RTSPServer::createNewClientSession(u_int32_t sessionId, RTSPClientConnection* ourClientConnection) { //NVT_MODIFIED, fix timeout without release accepted socket.
  return new RTSPClientSession(*this, sessionId, ourClientConnection); //NVT_MODIFIED, fix timeout without release accepted socket.
}
  ...
  if (fOurServer.fReclamationTestSeconds > 0) {
    envir().taskScheduler()
      .rescheduleDelayedTask(fLivenessCheckTask,
			     (int64_t)fOurServer.fReclamationTestSeconds*1000000,
...
//#ifdef DEBUG
  ...
  fprintf(stderr, "RTSP client session (id \"%08X\", stream name \"%s\") has timed out (due to inactivity)\n",
	  (int)clientSession->fOurSessionId, streamName);
//#endif
	//NVT_MODIFIED, fix timeout without release accepted socket. (fix side effect)
  if (clientSession->fOurClientConnection) {
	  clientSession->fOurClientConnection->closeSockets(); //NVT_MODIFIED, fix timeout without release accepted socket.
  }
  delete clientSession;
}
...
void RTSPServer::RegisterRequestRecord::incomingResponseHandler1() {
  ...
  if (bytesRead > 0 && bytesRead < REGISTER_RESPONSE_BUFFER_SIZE) {
	...
	(void)fOurServer.createNewClientConnection(sock, fRemoteAddress, fOurServer.m_current_connected_sn++); //NVT_MODIFIED, fix sms leaking
      }
    }
  }
