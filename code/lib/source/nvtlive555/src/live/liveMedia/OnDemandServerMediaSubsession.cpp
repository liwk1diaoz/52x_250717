...
::OnDemandServerMediaSubsession(UsageEnvironment& env,
  ...
  sprintf(fCNAME, "unknown host %d", (unsigned)(our_random() * 0x7FFFFFFF));
#endif
...
::getStreamParameters(unsigned clientSessionId,
  ...
  if (fLastStreamToken != NULL && fReuseFirstSource) {
	...
	if (rtcpGroupsock != NULL) {
	  ...
	  if (tos != 0) {
		  int ret = setsockopt(rtcpGroupsock->socketNum(), IPPROTO_IP, IP_TOS, (char*)&tos, sizeof(tos));
		  if (ret == -1)
    ...
    if (rtpGroupsock != NULL) {
      ...
      increaseSendBufferTo(envir(), rtpGroupsock->socketNum(), pNvtMgr->Get_SendBufSize()); //NVT_MODIFIED
      
      int tos = pNvtMgr->Get_TypeOfService();
      if(tos!=0)
      {
          int ret = setsockopt( rtpGroupsock->socketNum(), IPPROTO_IP, IP_TOS, (char*) &tos, sizeof(tos) );
          if (ret == -1)
              printf("Couldn't setsockopt(IP_TOS)\n");
      }
    }
...
static void afterPlayingStreamState(void* clientData) {
  ...
  if (streamState->streamDuration() == 0.0) {
    ...
    //streamState->reclaim(); //NVT_MODIFIED
    streamState->pause();
    streamState->bye();
  }
