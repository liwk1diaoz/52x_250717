...
{
	p_cfg->support_rtcp = live555_support_rtcp;
	p_cfg->support_meta = 0;
#if defined(__ECOS) || defined(__FREERTOS) || defined(__LINUX520)
	p_cfg->max_clients = 1; // because hdal mechanism
#endif
#if !defined(_WIN32)
	// force set to 0 since blocking and non-smooth on some mobile phone (refer to IVOT_N12070_CO-254), 20210113
	p_cfg->tos = 0; //0xC0;
#endif
	ImageApp_Common_DUMP("live555 on_get_cfg: rtcp=%d, meta=%d, clients=%d, tos=%x\r\n", p_cfg->support_rtcp, p_cfg->support_meta, p_cfg->max_clients, p_cfg->tos);
	return 0;
}
...
{
	if (g_ia_common_rtsp_parse_url_cb) {
		return g_ia_common_rtsp_parse_url_cb(url, p_info);
	}

	if (live555_rtsp_fixed_stream0 == FALSE) {
	...
	} else {        // make "rtsp://192.168.0.1:554/livestream/12" be workable.
		p_info->channel_id = 0;
		p_info->svc_level = 0;
	}
	return 0;
...
NVTLIVE555_HDAL_CB m_hdal_cb = {0};
int nvtrtspd_init(NVTLIVE555_HDAL_CB *p_hdal_cb)
{
	m_hdal_cb = *p_hdal_cb;
	return 0;
}
