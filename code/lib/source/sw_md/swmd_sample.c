...
UINT32 MD_BlkDiff_Thr[MD_PROC_CNT] = {MD_BLKDIFF_THR, MD_BLKDIFF_THR,MD_BLKDIFF_THR};
UINT32 MD_BlkCnt_Thr[MD_PROC_CNT] = {MD_BLKCNT_THR, MD_BLKCNT_THR,MD_BLKCNT_THR};
UINT32 MD_DiffSum_Thr[MD_PROC_CNT] = {MD_DIFFSUM_THR, MD_DIFFSUM_THR,MD_DIFFSUM_THR};
UINT32 gMDRatio[MD_PROC_CNT] = {0};
BOOL MD_Det_Win_Init[MD_PROC_CNT] = {FALSE, FALSE, FALSE};
...
UINT32 MD_BlkDiff_Thr_Ref[MD_PROC_CNT] = {MD_BLKDIFF_THR, MD_BLKDIFF_THR,MD_BLKDIFF_THR};
//#NT#2016/10/05#Harry Tsai -end

BOOL MD_Process(UINT32 Id,   HD_VIDEO_FRAME* pBuf);
...
static void MD_getImeY(UINT32 Id, UINT16 *Lum1, UINT16 *Lum2, UINT16 *Histo, UINT32 *WinXNum, UINT32 *WinYNum, HD_VIDEO_FRAME *pBuf)
{
    UINT16 i,j,Lum,Idx;
   ...
   // offset = FB_para.ch[0].line_ofs;
	...
	v_size = pBuf->ph[0];
	offset = pBuf->loff[0];
	va = (UINT32)hd_common_mem_mmap(HD_COMMON_MEM_MEM_TYPE_CACHE,pBuf->phy_addr[0],pBuf->loff[0]*pBuf->ph[0]);
    for(j=0;j<MD_LAWIN_Y;j++)
    {
        for(i=0;i<MD_LAWIN_X;i++)
        {
            Idx = j*MD_DETWIN_X+i;
            Lum = BlockAverageY(va,((i*h_size)/MD_LAWIN_X),((j*v_size)/MD_LAWIN_Y),(h_size/MD_LAWIN_X),(v_size/MD_LAWIN_Y),offset);
            Lum1[Idx] = Lum;
            Lum2[Idx] = Lum;
	...
	hd_common_mem_munmap((void *)va, pBuf->loff[0]*pBuf->ph[0]);
