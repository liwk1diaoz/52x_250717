...
#if (FORCE_SW_8888 == 1) //some project only use SW without any HW
		return _DC_BitBlt_ARGB8888_sw(_pDestDC, pDestRect, _pSrcDC, pSrcRect, uiRop, uiParam, 0);
#else
    {
		...
		case ROP_COPY:
			...
			break;
		case ROP_KEY:
			cparam[0] = COLOR_RGBA_GET_COLOR(uiParam);
			break;
		case ROP_DESTFILL:
		...
		case ROP_MUL_CNST_ALPHA:
			...
			cparam[0] = GRPH_TEXT_MULT_PROPTY(GRPH_TEXTMUL_BYTE, GRPH_TEXTMUL_UNSIGNED, uiParam, 8);
			break;
		case ROP_BLEND:
        ...
        if ((rop == ROP_COPY)||(rop == ROP_KEY)){
        	...
        	ERcode = vf_init(&param.src_img, _DC_bltSrcWidth, _DC_bltSrcHeight, HD_VIDEO_PXLFMT_ARGB8888, _DC_bltSrcPitch, vir_meminfo.pa, _DC_bltSrcPitch*_DC_bltSrcHeight);
        	if(ERcode){
        	...
        	ERcode = vf_init(&param.dst_img, _DC_bltDestWidth, _DC_bltDestHeight, HD_VIDEO_PXLFMT_ARGB8888, _DC_bltDestPitch,vir_meminfo.pa, _DC_bltDestPitch*_DC_bltDestHeight);
        	if(ERcode){
...
static UINT32 ARGB8888_OpAlphaBlending(UINT32 SrcValue, UINT32 DestValue)
{
	UINT32 OutValue;
	CVALUE r1, g1, b1, a1;
	CVALUE r2, g2, b2, a2;
	...
	a1 = COLOR_RGBA_GET_A(SrcValue);
	r1 = COLOR_RGBA_GET_R(SrcValue);
	g1 = COLOR_RGBA_GET_G(SrcValue);
	b1 = COLOR_RGBA_GET_B(SrcValue);
	a2 = COLOR_RGBA_GET_A(DestValue);
	r2 = COLOR_RGBA_GET_R(DestValue);
	g2 = COLOR_RGBA_GET_G(DestValue);
	b2 = COLOR_RGBA_GET_B(DestValue);
	if (a1 == 255) {
		a3 = a2; //not change
		r3 = r1;
		g3 = g1;
	...
	OutValue = COLOR_MAKE_RGBA(r3, g3, b3, a3);
	return OutValue;
}
							  ...
							  UINT32 uiRop, UINT32 uiParam, const MAPPING_ITEM *pTable)
{
	...
	if (rop == ROP_DESTFILL) {
		return _DC_FillRect_ARGB8888_sw(_pDestDC, pSrcRect, uiRop, uiParam, 0);
	}
	switch (rop) {
	case ROP_COPY:
		break;
	...
	if (_DC_IS_ZERO_RGB_PK(pSrcDC, &SrcRect)) {
		return GX_OK;    //skip this draw
	}
	...
	if (rop == ROP_COPY) { //ROP_COPY
		...
		for (j = 0; j < SrcRect.h; j++) {
			...
			for (i = 0; i < SrcRect.w; i++) {
				...
				//================================
				//pixel op: blend
				BlendValue = ARGB8888_OpAlphaBlending(SrcValue, DestValue);
...
//condition : _pDestDC is not null
//condition : _pDestDC is PXLFMT_RGBA8888_PK format
//condition : pDestRect is not null
//condition : pDestRect is validated and already clipped by pSrcRect
//condition : _pSrcDC is not null
...
//condition : pSrcRect is not null
//condition : pSrcRect is validated and already clipped by pDestRect
//condition : pTable is not null (for INDEX to RGB888 convert)
	...
	if (rop == ROP_COPY) { //ROP_COPY
		...
		for (j = 0; j < SrcRect.h; j++) {
			...
			for (i = 0; i < SrcRect.w; i++) {
				...
				if (COLOR_RGBA_GET_COLOR(SrcValue) == COLOR_RGBA_GET_COLOR(colorkey)) {
					//do color key
				} else {
					//================================
							   ...
							   DC_CTRL *pc)
{
	...
	return _DC_StretchBlt_ARGB8888_sw(_pDestDC, pDestRect, _pSrcDC, pSrcRect, pc);
...

	{
    	...
    	ERcode = vf_init(&param.src_img, _DC_SclSrcWidth, _DC_SclSrcHeight, HD_VIDEO_PXLFMT_ARGB8888, _DC_SclSrcPitch, vir_meminfo.pa, _DC_SclSrcPitch*_DC_SclSrcHeight);
    	if(ERcode){
    		DBG_ERR("fail to init source image\n");
    		return ERcode;
    	}
    	...
    	ERcode = vf_init(&param.dst_img, _DC_SclDestWidth, _DC_SclDestHeight, HD_VIDEO_PXLFMT_ARGB8888, _DC_SclDestPitch, vir_meminfo.pa, _DC_SclDestPitch*_DC_SclDestHeight);
    	if(ERcode){
    		DBG_ERR("fail to init dest image\n");
    		return ERcode;
    	}
...
RESULT _DC_StretchBlt_ARGB8888_sw
		...
		while (yNumPixels-- > 0) {
			...
			while (xNumPixels-- > 0) {
				...
				if ((AccValue_A / AccCnt) > 0) {
					...
					UINT32 src_b = AccValue_B / AccCnt;
					UINT32 dst_a = COLOR_RGBA_GET_A(DestValue);
					UINT32 dst_r = COLOR_RGBA_GET_R(DestValue) * dst_a / 255;
					UINT32 dst_g = COLOR_RGBA_GET_G(DestValue) * dst_a / 255;
					UINT32 dst_b = COLOR_RGBA_GET_B(DestValue) * dst_a / 255;
					/*if(dst_a == 0)
					{
					dst_a = src_a;
					dst_r = src_a*src_r/255;
...
//condition : pSrcRect is not null
//condition : pSrcRect is validated and already clipped by pDestRect
RESULT _DC_StretchBlt_INDEX2RGBA8888_sw
(DC *_pDestDC, const IRECT *pDestRect, const DC *_pSrcDC, const IRECT *pSrcRect,
 DC_CTRL *pc)
{
	DISPLAY_CONTEXT *pSrcDC = (DISPLAY_CONTEXT *)_pSrcDC;
...
//  PIXEL_2D
//--------------------------------------------------------------------------------------
typedef struct _PIXEL_2D_RGBA8888 {
...
static void PIXEL_2D_RGBA8888_Begin(PIXEL_2D *_p, IRECT *pRect)
{
	PIXEL_2D_RGBA8888 *p = (PIXEL_2D_RGBA8888 *)_p;
	...
	p->uiOffset =
		p->pRect->y * (p->uiPitch) + ((p->pRect->x) << 2);

	p->x = 0;
	p->y = 0;
}
static void PIXEL_2D_RGBA8888_BeginLine(PIXEL_2D *_p)
{
	PIXEL_2D_RGBA8888 *p = (PIXEL_2D_RGBA8888 *)_p;
...
static void PIXEL_2D_RGBA8888_BeginPixel(PIXEL_2D *_p)
{
	PIXEL_2D_RGBA8888 *p = (PIXEL_2D_RGBA8888 *)_p;

	p->pData = p->pLine + ((p->x) << 2); //get first byte

}
static UINT32 PIXEL_2D_RGBA8888_Read(PIXEL_2D *_p)
{
	PIXEL_2D_RGBA8888 *p = (PIXEL_2D_RGBA8888 *)_p;
	UINT32 pdata = 0;
	...
	pdata |= ((*(UINT8 *)(p->pData + 2)) << 16);
	pdata |= ((*(UINT8 *)(p->pData + 3)) << 24);
	return pdata;
}
static void PIXEL_2D_RGBA8888_Write(PIXEL_2D *_p, UINT32 value)
{
	PIXEL_2D_RGBA8888 *p = (PIXEL_2D_RGBA8888 *)_p;

	*(p->pData) = (DATA_PACK)((value & 0x000000ff) >> 0);
	*(p->pData + 1) = (DATA_PACK)((value & 0x0000ff00) >> 8);
	*(p->pData + 2) = (DATA_PACK)((value & 0x00ff0000) >> 16);
	*(p->pData + 3) = (DATA_PACK)((value & 0xff000000) >> 24);

}
static void PIXEL_2D_RGBA8888_EndPixel(PIXEL_2D *_p, BOOL bForceFlush, BOOL bNext)
{
	PIXEL_2D_RGBA8888 *p = (PIXEL_2D_RGBA8888 *)_p;
...
static void PIXEL_2D_RGBA8888_EndLine(PIXEL_2D *_p)
{
	PIXEL_2D_RGBA8888 *p = (PIXEL_2D_RGBA8888 *)_p;
...
static void PIXEL_2D_RGBA8888_End(PIXEL_2D *p)
{
}
static void PIXEL_2D_RGBA8888_WriteOffset(PIXEL_2D *_p, UINT32 value, INT32 dx, INT32 dy)
{
	PIXEL_2D_RGBA8888 *p = (PIXEL_2D_RGBA8888 *)_p;

	INT32 offset = (p->uiPitch) * dy + (dx << 2);
	//NOTE: boundary checking is ignored
	*(p->pData + offset) = value;
}
static void PIXEL_2D_RGBA8888_GotoXY(PIXEL_2D *_p, int x, int y)
{
	PIXEL_2D_RGBA8888 *p = (PIXEL_2D_RGBA8888 *)_p;
...
static UINT8 *PIXEL_2D_RGBA8888_LineBuf(struct _PIXEL_2D *_p, int y)
{
	PIXEL_2D_RGBA8888 *p = (PIXEL_2D_RGBA8888 *)_p;
...
static UINT8 *PIXEL_2D_RGBA8888_PlaneBuf(struct _PIXEL_2D *_p, int i)
{
	PIXEL_2D_RGBA8888 *p = (PIXEL_2D_RGBA8888 *)_p;
...
PIXEL_2D _gPixel2D_RGBA8888 = {
	PIXEL_2D_RGBA8888_Begin,
	PIXEL_2D_RGBA8888_BeginLine,
	PIXEL_2D_RGBA8888_BeginPixel,
	PIXEL_2D_RGBA8888_Read,
	PIXEL_2D_RGBA8888_Write,
	PIXEL_2D_RGBA8888_EndPixel,
	PIXEL_2D_RGBA8888_EndLine,
	PIXEL_2D_RGBA8888_End,
	PIXEL_2D_RGBA8888_WriteOffset,
	PIXEL_2D_RGBA8888_GotoXY,
	PIXEL_2D_RGBA8888_LineBuf,
	PIXEL_2D_RGBA8888_PlaneBuf,
};
