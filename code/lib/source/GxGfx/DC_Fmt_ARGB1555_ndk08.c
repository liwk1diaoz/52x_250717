...
//  --------------------------------------------------------------------------------------------
*/
#define COLOR_RGBA5551_GET_COLOR(c)   (COLOR_ITEM)(((c) & 0x00007fff) >> 0) ///< get RGB channel value of RGBA5551 color
#define COLOR_RGBA5551_GET_A(c)       (CVALUE)(((c) & 0x00008000) >> 15) ///< get A channel value of RGBA5551 color
...
#if (FORCE_SW_1555 == 1)
		//return _DC_BitBlt_ARGB1555_sw(_pDestDC, pDestRect, _pSrcDC, pSrcRect, uiRop, uiParam, 0);
		DBG_ERR("not sup\r\n");
        return E_SYS;
#else
	{
		...
		case ROP_COPY:
			...
			break;
		case ROP_KEY:
			cparam[0] = COLOR_RGBA5551_GET_COLOR(uiParam);
		...
		case ROP_MUL_CNST_ALPHA:
			...
			cparam[0] = GRPH_TEXT_MULT_PROPTY(0, 0, uiParam, 1);
			break;
#endif
#if 0
		case ROP_BLEND:
        ...
        if ((rop == ROP_COPY)||(rop == ROP_KEY)){
        	...
        	ERcode = vf_init(&param.src_img, _DC_bltSrcWidth, _DC_bltSrcHeight, HD_VIDEO_PXLFMT_ARGB1555, _DC_bltSrcPitch, vos_cpu_get_phy_addr(_DC_bltSrcOffset), _DC_bltSrcPitch*_DC_bltSrcHeight);
        	if(ERcode){
        		DBG_ERR("fail to init source image\n");
        		return ERcode;
        	}
        	ERcode = vf_init(&param.dst_img, _DC_bltDestWidth, _DC_bltDestHeight, HD_VIDEO_PXLFMT_ARGB1555, _DC_bltDestPitch, vos_cpu_get_phy_addr(_DC_bltDestOffset), _DC_bltDestPitch*_DC_bltDestHeight);
        	if(ERcode){
...
//condition : _pDestDC is not null
//condition : _pDestDC is PXLFMT_RGBA5551_PK format
//condition : pDestRect is not null
//condition : pDestRect is validated and already clipped by pSrcRect
//condition : _pSrcDC is not null
...
//condition : pSrcRect is not null
//condition : pSrcRect is validated and already clipped by pDestRect
//condition : pTable is not null (for INDEX to RGB555 convert)
	...
	if (rop == ROP_COPY) { //ROP_COPY
		...
		for (j = 0; j < SrcRect.h; j++) {
			...
			for (i = 0; i < SrcRect.w; i++) {
				...
				if (COLOR_RGBA5551_GET_COLOR(SrcValue) == COLOR_RGBA5551_GET_COLOR(colorkey)) {
					//do color key
				} else {
					//================================
...
#else //slow path
	{
		...
		for (j = 0; j < SrcRect.h; j++) {
			...
			for (i = 0; i < SrcRect.w; i++) {
				...
				if (DestValue != (COLOR_TRANSPARENT&0x00007fff)) {
							   ...
							   DC_CTRL *pc)
{
...
#if (FORCE_SW_1555 == 1) //some project only use SW without any HW

    return _DC_StretchBlt_ARGB1555_sw(_pDestDC,pDestRect,_pSrcDC,pSrcRect,pc);
	...
	//stretch rect with ISE
	{
    	...
    	ERcode = vf_init(&param.src_img, _DC_SclSrcWidth, _DC_SclSrcHeight, HD_VIDEO_PXLFMT_ARGB1555, _DC_SclSrcPitch, vos_cpu_get_phy_addr(_DC_SclSrcOffsetY), _DC_SclSrcPitch*_DC_SclSrcHeight);
    	if(ERcode){
    		DBG_ERR("fail to init source image\n");
    		return ERcode;
    	}
    	ERcode = vf_init(&param.dst_img, _DC_SclDestWidth, _DC_SclDestHeight, HD_VIDEO_PXLFMT_ARGB1555, _DC_SclDestPitch, vos_cpu_get_phy_addr(_DC_SclDestOffsetY), _DC_SclDestPitch*_DC_SclDestHeight);
    	if(ERcode){
    		DBG_ERR("fail to init dest image\n");
    		return ERcode;
    	}
...
RESULT _DC_StretchBlt_ARGB1555_sw
...
//condition : pSrcRect is not null
//condition : pSrcRect is validated and already clipped by pDestRect
RESULT _DC_StretchBlt_INDEX2RGBA5551_sw
(DC *_pDestDC, const IRECT *pDestRect, const DC *_pSrcDC, const IRECT *pSrcRect,
 DC_CTRL *pc)
{
	DISPLAY_CONTEXT *pSrcDC = (DISPLAY_CONTEXT *)_pSrcDC;
...
//  PIXEL_2D
//--------------------------------------------------------------------------------------
typedef struct _PIXEL_2D_RGBA5551 {
...
static void PIXEL_2D_RGBA5551_Begin(PIXEL_2D *_p, IRECT *pRect)
{
	PIXEL_2D_RGBA5551 *p = (PIXEL_2D_RGBA5551 *)_p;
...
static void PIXEL_2D_RGBA5551_BeginLine(PIXEL_2D *_p)
{
	PIXEL_2D_RGBA5551 *p = (PIXEL_2D_RGBA5551 *)_p;
...
static void PIXEL_2D_RGBA5551_BeginPixel(PIXEL_2D *_p)
{
	PIXEL_2D_RGBA5551 *p = (PIXEL_2D_RGBA5551 *)_p;
...
static UINT32 PIXEL_2D_RGBA5551_Read(PIXEL_2D *_p)
{
	PIXEL_2D_RGBA5551 *p = (PIXEL_2D_RGBA5551 *)_p;
	UINT32 pdata = 0;
...
static void PIXEL_2D_RGBA5551_Write(PIXEL_2D *_p, UINT32 value)
{
	PIXEL_2D_RGBA5551 *p = (PIXEL_2D_RGBA5551 *)_p;
...
static void PIXEL_2D_RGBA5551_EndPixel(PIXEL_2D *_p, BOOL bForceFlush, BOOL bNext)
{
	PIXEL_2D_RGBA5551 *p = (PIXEL_2D_RGBA5551 *)_p;
...
static void PIXEL_2D_RGBA5551_EndLine(PIXEL_2D *_p)
{
	PIXEL_2D_RGBA5551 *p = (PIXEL_2D_RGBA5551 *)_p;
...
static void PIXEL_2D_RGBA5551_End(PIXEL_2D *p)
{
}
static void PIXEL_2D_RGBA5551_WriteOffset(PIXEL_2D *_p, UINT32 value, INT32 dx, INT32 dy)
{
	PIXEL_2D_RGBA5551 *p = (PIXEL_2D_RGBA5551 *)_p;
...
static void PIXEL_2D_RGBA5551_GotoXY(PIXEL_2D *_p, int x, int y)
{
	PIXEL_2D_RGBA5551 *p = (PIXEL_2D_RGBA5551 *)_p;
...
static UINT8 *PIXEL_2D_RGBA5551_LineBuf(struct _PIXEL_2D *_p, int y)
{
	PIXEL_2D_RGBA5551 *p = (PIXEL_2D_RGBA5551 *)_p;
...
static UINT8 *PIXEL_2D_RGBA5551_PlaneBuf(struct _PIXEL_2D *_p, int i)
{
	PIXEL_2D_RGBA5551 *p = (PIXEL_2D_RGBA5551 *)_p;
...
PIXEL_2D _gPixel2D_RGBA5551 = {
	PIXEL_2D_RGBA5551_Begin,
	PIXEL_2D_RGBA5551_BeginLine,
	PIXEL_2D_RGBA5551_BeginPixel,
	PIXEL_2D_RGBA5551_Read,
	PIXEL_2D_RGBA5551_Write,
	PIXEL_2D_RGBA5551_EndPixel,
	PIXEL_2D_RGBA5551_EndLine,
	PIXEL_2D_RGBA5551_End,
	PIXEL_2D_RGBA5551_WriteOffset,
	PIXEL_2D_RGBA5551_GotoXY,
	PIXEL_2D_RGBA5551_LineBuf,
	PIXEL_2D_RGBA5551_PlaneBuf,
};
