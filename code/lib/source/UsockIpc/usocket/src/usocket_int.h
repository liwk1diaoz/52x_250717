...
#if 0 //debug
#if defined(__ECOS)
#define USOCKET_PRINT(id,fmtstr, args...) diag_printf("[%s:%d]"fmtstr"", __func__,id, ##args)
#else
#define USOCKET_PRINT(id,fmtstr, args...) printf("[usocket:%s:%d]"fmtstr"", __func__,id, ##args)
#endif
#else //release
#define USOCKET_PRINT(...)
#endif

#if defined(__ECOS)
#define USOCKET_ERR(id,fmtstr, args...) diag_printf("[ERR:%s:%d]"fmtstr"", __func__,id, ##args)
#else
#define USOCKET_ERR(id,fmtstr, args...) printf("[usocket:ERR:%s:%d]"fmtstr"", __func__id,, ##args)
#endif

#endif //_USOCKET_DEBUG_H_
...
#define FLAG_SETPTN(cond,flag,ptn,mtx) {pthread_mutex_lock(&mtx);flag|=ptn;USOCKET_PRINT("setflag 0x%x, ptn 0x%x\r\n",flag,ptn);pthread_cond_broadcast(&cond);pthread_mutex_unlock(&mtx);}
#define FLAG_CLRPTN(flag,ptn,mtx) {pthread_mutex_lock(&mtx);flag &= ~(ptn);USOCKET_PRINT("clrflag 0x%x, ptn 0x%x\r\n",flag,ptn);pthread_mutex_unlock(&mtx);}
#define FLAG_WAITPTN(cond,flag,ptn,mtx,clr) {pthread_mutex_lock(&mtx);while (1) { USOCKET_PRINT("waitflag b 0x%x, ptn 0x%x\r\n",flag,ptn);\
                                                                    if (flag & ptn){if (clr) {flag &= ~(ptn);}pthread_mutex_unlock(&mtx);USOCKET_PRINT("waitflag e 0x%x, ptn 0x%x\r\n",flag,ptn);break;}\
                                                                  pthread_cond_wait(&cond, &mtx);\
                                                                  if (flag & ptn) {\
                                                                  if (clr) {flag &= ~(ptn);}\
																  pthread_mutex_unlock(&mtx);\
																  USOCKET_PRINT("waitflag e 0x%x, ptn 0x%x\r\n",flag,ptn);\
                                                                  break;}\
                                                                  }}

#define FLAG_WAITPTN_TIMEOUT(cond,flag,ptn,mtx,clr,timeout,rtn){struct timespec to;int err;rtn=0;\
                                                         pthread_mutex_lock(&mtx);\
                                                         while (1) { USOCKET_PRINT("waitflag 0x%x\r\n",flag);\
	                                                               if (flag & ptn){if (clr) {flag &= ~(ptn);}pthread_mutex_unlock(&mtx);break;}\
                                                                   USOCKET_PRINT("cond_wait 0x%x\r\n",flag);\
