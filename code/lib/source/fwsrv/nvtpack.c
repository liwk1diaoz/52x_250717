...
#define NVTPACK_UITRON // NVTPACK_WIN32, NVTPACK_UITRON, NVTPACK_UBOOT, NVTPACK_LINUX_KERNEL, NVTPACK_LINUX_APP

#if defined(NVTPACK_WIN32)
#include <wtypes.h>
#include <stdio.h>
#elif defined(NVTPACK_LOADER)
#include "string.h"
#include "stdlib.h"
#include "constant.h"
#include "fuart.h"
...
static NVTPACK_ER nvtpack_er(NVTPACK_ER er)
{
	...
	printf("nvtpack:%d\r\n", er);
    #elif defined(NVTPACK_LOADER)
    uart_putSystemUARTStr("nvtpack:");
	uart_putSystemUARTStr(Dec2HexStr((UINT32)er));
...
_NVTPACK_EXTERN_C NVTPACK_ER nvtpack_getver(NVTPACK_MEM *p_mem, NVTPACK_VER *p_ver)
{
	NVTPACK_MEM mem_uitron = {0};
	if(IsEqualGUID(&GUID_FW2, (GUID *)p_mem->p_data)) {
		*p_ver = NVTPACK_VER_16072017;
		return nvtpack_er(NVTPACK_ER_SUCCESS);
	} else {
		...
		if(!IsEqualGUID(&GUID_FW, (GUID *)p_mem->p_data)) {
			if(nvtpack_get_uitron_offset(p_mem, &mem_uitron) != NVTPACK_ER_SUCCESS) {
				return nvtpack_er(NVTPACK_ER_PARAM);
			}
			p_curr += mem_uitron.len;
			if(p_curr >= p_end) {
				*p_ver = NVTPACK_VER_13012816;
				return nvtpack_er(NVTPACK_ER_SUCCESS);
			}
		}
	...
	return NVTPACK_ER_UNKNOWN_FORMAT;
}
...
_NVTPACK_EXTERN_C NVTPACK_ER nvtpack_get_partition(NVTPACK_GET_PARTITION_INPUT *p_input, NVTPACK_MEM *p_out)
{
	...
	if(IsEqualGUID(&GUID_FW2, (GUID *)p_in->p_data)) {
		...
		pPartition = (NVTPACK_PARTITION_HDR *)((unsigned char*)p_in->p_data + sizeof(NVTPACK_FW_HDR2));
	} else {
		NVTPACK_MEM mem_uitron = {0};
		...
		if(IsEqualGUID(&GUID_FW, (GUID *)p_curr)) {
			NVTPACK_FW_HDR *pFw = (NVTPACK_FW_HDR *)p_curr;
			TotalRes = pFw->TotalRes;
			pPartition = (NVTPACK_PARTITION_HDR *)((unsigned char*)p_curr + sizeof(NVTPACK_FW_HDR));
		} else {
			return nvtpack_er(NVTPACK_ER_UNKNOWN_FORMAT);
		}
	}
...
static NVTPACK_ER nvtpack_make_parition_pkg_fw1(NVTPACK_MAKE_PARTITION_INPUT *p_input, NVTPACK_MEM *p_out)
{
	...
	memcpy(&p_fw_hdr->guid, &GUID_FW, sizeof(GUID_FW));
...
static NVTPACK_ER nvtpack_make_parition_pkg_fw2(NVTPACK_MAKE_PARTITION_INPUT *p_input, NVTPACK_MEM *p_out)
{
	...
	memset(p_fw_hdr, 0 , sizeof(*p_fw_hdr));
	memcpy(&p_fw_hdr->guid, &GUID_FW2, sizeof(GUID_FW2));
	...
	while(num > 0) {
		...
		if(p_curr + ALIGN_CEIL_64(p_pat_curr->Size) > p_end) {
			return nvtpack_er(NVTPACK_ER_MEM_NOT_ENOUGH);
		}
		memset(p_curr, 0, ALIGN_CEIL_64(p_pat_curr->Size));
		memcpy(p_curr, p_list_curr->mem.p_data, p_pat_curr->Size);

		p_curr += ALIGN_CEIL_64(p_pat_curr->Size);
		p_pat_curr++;
		p_list_curr++;
		num--;
	}
...
_NVTPACK_EXTERN_C NVTPACK_ER nvtpack_make_resource_pkg(NVTPACK_MAKE_RESOURCE_INPUT *p_input, NVTPACK_MEM *p_out)
{
	...
	for(i = 0; i < num; i++) {
		...
		p_curr += sizeof(*p_hdr);
		memcpy(&p_hdr->guid, &GUID_RES, sizeof(GUID_RES));
