...
#if (LV_COLOR_DEPTH != 32 && LV_COLOR_DEPTH != 24) && LV_COLOR_SCREEN_TRANSP != 0
#error "LV_COLOR_SCREEN_TRANSP requires LV_COLOR_DEPTH == 32 / 24. Set it in lv_conf.h"
#endif
...
#else
#error "Invalid LV_COLOR_DEPTH in lv_conf.h! Set it to 1, 8, 16, 24 or 32!"
#endif
...
# define LV_COLOR_SET_R24(c, v) (c).ch.red = (uint8_t)((v) & 0x1FU)
# define LV_COLOR_SET_G24(c, v) (c).ch.green = (uint8_t)((v) & 0x3FU)
# define LV_COLOR_SET_B24(c, v) (c).ch.blue = (uint8_t)((v) & 0x1FU)
# define LV_COLOR_SET_A24(c, v) (c).ext_ch.alpha = (uint8_t)((v) & 0xFF)

# define LV_COLOR_GET_R24(c) (c).ch.red
# define LV_COLOR_GET_G24(c) (c).ch.green
# define LV_COLOR_GET_B24(c) (c).ch.blue
# define LV_COLOR_GET_A24(c) (c).ext_ch.alpha

# define _LV_COLOR_ZERO_INITIALIZER24 {{0x00, 0x00, 0x00}, {0x00}}
# define LV_COLOR_MAKE24(r8, g8, b8) (_LV_COLOR_MAKE_TYPE_HELPER{{(uint8_t)((b8 >> 3) & 0x1FU), (uint8_t)((g8 >> 2) & 0x3FU), (uint8_t)((r8 >> 3) & 0x1FU)}, {0xff}}) /*Fix 0xff alpha*/
...
#elif LV_COLOR_DEPTH == 8
    lv_color24_t ret;
    LV_COLOR_SET_R24(ret, LV_COLOR_GET_R(color) * 4);  /*(2^5 - 1)/(2^3 - 1) = 31/7 = 4*/
    LV_COLOR_SET_G24(ret, LV_COLOR_GET_G(color) * 9);  /*(2^6 - 1)/(2^3 - 1) = 63/7 = 9*/
    LV_COLOR_SET_B24(ret, LV_COLOR_GET_B(color) * 10); /*(2^5 - 1)/(2^2 - 1) = 31/3 = 10*/
    LV_COLOR_SET_A24(ret, 0xFF);
    return ret.full;
#elif LV_COLOR_DEPTH == 16
    lv_color24_t ret;
    LV_COLOR_SET_R24(ret, LV_COLOR_GET_R(color));
    LV_COLOR_SET_G24(ret, LV_COLOR_GET_G(color));
    LV_COLOR_SET_B24(ret, LV_COLOR_GET_B(color));
    LV_COLOR_SET_A24(ret, 0xFF);
    return ret.full;
#elif LV_COLOR_DEPTH == 24
    return color.full;
#elif LV_COLOR_DEPTH == 32
    lv_color24_t ret;
    LV_COLOR_SET_R24(ret, LV_COLOR_GET_R(color) >> 3); /* 8 - 5  = 3*/
    LV_COLOR_SET_G24(ret, LV_COLOR_GET_G(color) >> 2); /* 8 - 6  = 2*/
    LV_COLOR_SET_B24(ret, LV_COLOR_GET_B(color) >> 3); /* 8 - 5  = 3*/
    LV_COLOR_SET_A24(ret, LV_COLOR_GET_A(color));
    return ret.full;
#endif
}
...
static inline uint32_t lv_color_to32(lv_color_t color)
{
    ...
    LV_COLOR_SET_G32(ret, (LV_COLOR_GET_G(color) * 259 + 3) >> 6);
    LV_COLOR_SET_B32(ret, (LV_COLOR_GET_B(color) * 263 + 7) >> 5);
    LV_COLOR_SET_A32(ret, LV_COLOR_GET_A(color));
    return ret.full;
#elif LV_COLOR_DEPTH == 32
    return color.full;
#endif
...
#if LV_COLOR_DEPTH == 24
        res_color->ext_ch.alpha = fg_color.ext_ch.alpha;
#endif
        *res_opa = fg_opa;
    }
...
#if LV_COLOR_DEPTH == 24
        res_color->ext_ch.alpha = bg_color.ext_ch.alpha;
#endif
        *res_opa = bg_opa;
    }
...
#if LV_COLOR_DEPTH == 24
        fg_color_save.ext_ch.alpha = fg_color.ext_ch.alpha;
        bg_color_save.ext_ch.alpha = bg_color.ext_ch.alpha;
#endif
            /*Info:
...
#if LV_COLOR_DEPTH == 24
        res_color->ext_ch.alpha = res_color_saved.ext_ch.alpha;
#endif
