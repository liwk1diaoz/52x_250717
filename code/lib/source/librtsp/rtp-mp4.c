...
static void finish_config(struct rtp_mpeg4 *out)
{
	...
	o += sprintf(o, "profile-level-id=%d;config=", 1);
									 ...
									 unsigned char *d, int len)
{
	...
	//printf("vop_time_increment_resolution=%d \n", i);
	if (! get_field(d, 1, &off)) {
		spook_log(SL_WARN, "rtp-mpeg4: missing marker");
		return;
	}
...
static int mpeg4_send(struct rtp_endpoint *ep, void *d, unsigned int curr_time, unsigned int *delay)
{
	struct rtp_mpeg4 *out = (struct rtp_mpeg4 *)d;
	int i, j, space, off;
	...
	while (i < out->iov_count) {
		...
		if (space < v[j].iov_len) {
			...
			if (send_rtp_packet(ep, v, j, ep->last_timestamp, 0) < 0) {
				return -1;
			}
			off += space;
			space = ep->max_data_size;
	...
	if (send_rtp_packet(ep, v, j, ep->last_timestamp, 1) < 0) {
		return -1;
	}
...
static int mpeg4_get_payload(int payload, void *d)
{
	return 96;
}
...
struct rtp_media *new_rtp_media_mpeg4(void)
{
	...
	// Carl
	m = new_rtp_media(mpeg4_get_sdp, mpeg4_get_payload, mpeg4_process_frame,
					  mpeg4_send, mpeg4_release, out, 90000);     // Carl
	if (m == NULL) {
		free(out);
	}
