...
static int g711_get_sdp(char *dest, int len, int payload, int port, void *d)
{
	struct rtp_g711 *out = (struct rtp_g711 *)d;
...
static int g711a_get_payload(int payload, void *d)
{
	return 8;
}

static int g711u_get_payload(int payload, void *d)
{
	return 0;
}

static int g711_process_frame(struct frame *f, void *d)
{
	struct rtp_g711 *out = (struct rtp_g711 *)d;
	//int timediff;
	if (f) {
		out->g711_data = f->d;
		out->g711_len = f->length;
...
static int g711_send(struct rtp_endpoint *ep, void *d, unsigned int curr_time, unsigned int *delay)
{
	struct rtp_g711 *out = (struct rtp_g711 *)d;
	int i, plen, mark;
	struct iovec v[2];
	...
	if (timediff > 16 * 3000) {
		if (timediff > ep->last_timestamp + 16 * 200) {
			...
			for (i = 0; i < out->g711_len; i += plen) {
				plen = out->g711_len - i;
				if (plen > ep->max_data_size) {
					plen = ep->max_data_size;
					plen -= plen % 1 ;
				}
				v[1].iov_base = out->g711_data + i;
				v[1].iov_len = plen;
...
static int g711_release(void *p)
{
	if (p == NULL) {
		return -1;
	}
...
struct rtp_media *new_rtp_media_g711a(struct stream *stream)
{
	...
	out = (struct rtp_g711 *)malloc(sizeof(struct rtp_g711));
	...
	return new_rtp_media(g711_get_sdp, g711a_get_payload, g711_process_frame,
						 g711_send, g711_release, out, out->rate);
...
struct rtp_media *new_rtp_media_g711u(struct stream *stream)
{
	...
	return new_rtp_media(g711_get_sdp, g711u_get_payload, g711_process_frame,
						 g711_send, g711_release, out, out->rate);
