...
#include <rtp.h>
#include <rtp_media.h>
#include <priv.h>   /* Carl, 20091109. */
#include "gm_memory.h"

//#define CHECK_MJPEG_WIDTH_HEIGHT_DIV_8
...
#ifdef CHECK_MJPEG_WIDTH_HEIGHT_DIV_8
...
static int jpeg_process_frame(struct frame *f, void *d)
{
	...
	out->keyframe = 0; // init keyframe, only set in get Huffman table
	for (i = 0; i < 16; ++i) {
		out->quant[i] = NULL;
	}
...
static int jpeg_get_sdp(char *dest, int len, int payload, int port, void *d)
{
	...
	//printf("JPEG width =%d height=%d framerate =%d\n", out->width, out->height, out->framerate);
	return snprintf(dest, len, "m=video %d RTP/AVP 26 \r\na=fmtp:26 config=%02x%02x%02x\r\n", port, out->width, out->height, out->framerate);
}
...
static int jpeg_send(struct rtp_endpoint *ep, void *d, unsigned int curr_time, unsigned int *delay)
{
	struct rtp_jpeg *out = (struct rtp_jpeg *)d;
	...
	for (i = 0; i < out->scan_data_len; i += plen) {
		...
		if (send_rtp_packet(ep, v, vcnt + 1, ep->last_timestamp,
...
struct rtp_media *new_rtp_media_jpeg_stream(struct stream *stream)
{
	...
	// Carl
	m =  new_rtp_media(jpeg_get_sdp, jpeg_get_payload,
					   jpeg_process_frame, jpeg_send, jpeg_release, out, 90000);
	if (m == NULL) {
		free(out);
	}
