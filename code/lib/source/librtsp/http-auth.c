...
static void md5_hash(char **v, int count, char *hash)
{
	...
	for (i = 0; i < count; ++i) {
		if (i > 0) {
			RTSP_MD5Update(&md5, (unsigned char *)":", 1);
		}
		RTSP_MD5Update(&md5, (unsigned char *)v[i], strlen(v[i]));
	}
	RTSP_MD5Final(bin, &md5);

	for (i = 0; i < 16; ++i) {
		sprintf(hash + (i << 1), "%02x", bin[i]);
	}
	hash[32] = 0;
}
...
static int parse_auth_header(char *header, struct digest_auth_info *auth)
{
	...
	if (strncasecmp(header, "digest ", 7)) {
		return -1;
	}
	n = header + 7;
	while (*n) {
						  ...
						  char *username, char *password)
{
	...
	if (!(hdr = get_header(msg, "authorization"))) {
		return -1;
	}
	...
	//if( strcasecmp( auth.username, username ) )     /* Case-insensitive usernames */
	if (strcmp(auth.username, username)) {          /* Case-sensitive usernames */
		spook_log(SL_VERBOSE,
...
int describe_parse_username(struct req *req, char *username, int username_len)
{
	...
	if (!(hdr = get_header(req->req, "authorization"))) {
		return 0;
	}
	if (parse_auth_header(hdr, &auth) < 0) {
		return 0;
	...
	strncpy(username, auth.username, username_len);
