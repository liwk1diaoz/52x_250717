								...
								unsigned int clock_rate_hz)     // Carl
{
	...
	m->frame = frame;
	m->send = send;
	m->release = release;   // Carl
	m->private = private;
	m->clock_rate_hz = clock_rate_hz;
	return m;
}


// Carl
int remove_rtp_media(void *p)
{
	struct rtp_media *m = (struct rtp_media *) p;
	if (m == NULL) {
		return -1;
	}
	if ((m->release) && (m->private)) {
		m->release(m->private);
	}
	free(m);
	return 0;
...
void del_session(struct session *sess)
{
	...
	if (sess->control_close) {
		sess->control_close(sess);
	}
	free(sess);
}
