...
#define JUMP_ADDR 0xfe000000
void nvt_ddr_scan(u32 type)
{
	int ratio = 0;

	ratio = readl(0xf00244c0|0x20) | (readl(0xf00244c0|0x24) << 8) | (readl(0xf00244c0|0x28) << 16);
	ratio = (ratio*12*8)/131072;
	printf("\nDRAM1_");
	if((readl(0xf0080008)>>27)&0x1) {
		if((readl(0xf0080008)>>30)&0x1) {
			printf("DDR4x2_");
		} else {
			printf("DDR4x1_");
		}
	...
	if((readl(0xf0080008)>>28)&0x1) {
		printf("16B\n");
	} else {
		printf("32B\n");
	}
	
	if ((readl(0xf0020010)&0xc00000) != 0x0) {
		ratio = readl(0xf0024840|0x20) | (readl(0xf0024840|0x24) << 8) | (readl(0xf0024840|0x28) << 16);
		ratio = (ratio*12*8)/131072;
		printf("\nDRAM2_");
		if((readl(0xf0090008)>>27)&0x1) {
			if((readl(0xf0090008)>>30)&0x1) {
				printf("DDR4x2_");
			} else {
				printf("DDR4x1_");
			}
