...
static void link_speed_change(UINT32 speed) {
	u32 reg;
	int i;
...
printf("%s: no CONFIG_NVT_PCIE_TEST_MODE\r\n", __func__);
	...
	rc_phy_setting(&nvt_rc);
	rc_phy_setting(&nvt_ep);

	link_speed_change(3);	// switch to higher speed after phy is setup
