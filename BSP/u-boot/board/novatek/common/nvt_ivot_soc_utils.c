...
int nvt_runfw_update_over_eth(void)
{
	printf("NVT update fw over ethernet.....\n");
	char cmd[128]={0};
	sprintf(cmd, "setenv serverip 192.168.0.3");
	run_command(cmd, 0);
	sprintf(cmd, "setenv ipaddr 192.168.0.12");
	run_command(cmd, 0);
	sprintf(cmd, "tftp 0x8000000 EthcamTxFW.bin");
	run_command(cmd, 0);
	sprintf(cmd, "nvt_update_all 0x8000000 0");
	run_command(cmd, 0);
	printf("reboot...\n");
	sprintf(cmd, "nvt_boot");
	run_command(cmd, 0);
	return 0;
}
int nvt_boot_check_wdt_udata(void)
{

	//printf("NVT check wdt user data.....\n");
	...
	uiUserData =WDT_GETREG(WDT_DATA_OFS);
	//printf("WDT_DATA_OFS, UserData=0x%x\n",uiUserData);
	if(uiUserData == MAKEFOURCC('T','X','U','D')){
		WDT_SETREG(WDT_DATA_OFS, 0);
		return 1;
	}
...
static int nvt_boot_fw_init_detect(void)
{
	...
	if(nvt_boot_check_wdt_udata()){
		printf("ethcam Tx update FW over eth...\n");
		nvt_runfw_update_over_eth();
		return 0;
	}
