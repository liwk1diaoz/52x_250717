...
static  int curr_device = CONFIG_NVT_IVOT_EMMC;
#endif
...
int nvt_fs_set_blk_dev(void)
{
    ...
    char cmd[128] = {0};
    	...
    	for (i = 1;;i++) {
    		ret = part_get_info(mmc_dev, i, &info);
    		if (ret < 0) {
                printf("Find total part number, break loop.\n\r");
    			break;
    		} else {
    		    PartNum++;
    		}
    	}
        sprintf(cmd, "%d:%d", curr_device, PartNum);
        printf("nvt_fs_set_blk_dev: cmd=%s\n\r", cmd);
    	if (fs_set_blk_dev("mmc", cmd, FS_TYPE_FAT)) {
            for (i = 8; i >= 3; i--) {  //refer to "active="true"" in nvt-na51055-storage-partition.dtsi
                sprintf(cmd, "2:%d", i);
    			if (fs_set_blk_dev("mmc", cmd, FS_TYPE_FAT) == 0) {
                    printf("Searching FAT OK again\n\r");
    			    return 0;
    			}
            }
...
int nvt_getfdt_emb(ulong fdt_addr, EMB_PARTITION *pemb)
{
	...
	#if defined(CONFIG_NVT_IVOT_EMMC)
	#if (defined(CONFIG_TARGET_NA51090) || defined(CONFIG_TARGET_NA51090_A64))
	sprintf(path, "/mmc@%x/nvtpack/index", IOADDR_SDIO2_REG_BASE);
	#else
...
int nvt_get_nvtpack_label_from_id(void *fdt_addr, char *root, unsigned int id, unsigned long *label_addr)
{
	...
	*label_addr = (unsigned long )fdt_getprop((const void*)fdt_addr, nextoffset, "label", NULL);
	if (label_addr == NULL) {
		return -1;
	} else {
		return 0;
...
#if defined(CONFIG_NVT_IVOT_EMMC)
#if (defined(CONFIG_TARGET_NA51090) || defined(CONFIG_TARGET_NA51090_A64))
	sprintf(root, "/mmc@%x/nvtpack/index", IOADDR_SDIO2_REG_BASE);
#else
...
#endif
	ret = nvt_get_nvtpack_label_from_id(nvt_fdt_buffer, root, id, &label_addr);
	if (ret < 0) {
		return false;
	}
	if (strncmp((char *)label_addr, "ep-rootfs", 9) == 0) {
		return true;
	} else {
		return false;
	}
