...
#define PLL_CLKSEL_APB_150        	(0x01 << PLL_CLKSEL_APB)      //< Select APB 120MHz
...
void otp_init(void)
{
	...
	if ((uiReg & PLL_CLKSEL_APB_MASK) == PLL_CLKSEL_APB_120) {
		DBG_DUMP("120MHz\r\n");
		OUTREG32(KEY_MANAGER_TIMING_CONFIG_ADDRESS, OTP_120_TIMING);
	} else if ((uiReg & PLL_CLKSEL_APB_MASK) == PLL_CLKSEL_APB_150) {
		DBG_DUMP("150MHz\r\n");
		OUTREG32(KEY_MANAGER_TIMING_CONFIG_ADDRESS, OTP_150_TIMING);
	}
...
ER efuse_get_unique_id(UINT32 *id_L, UINT32 *id_H)
{
	...
	if(is_new_key_rule()) {
		lot_id_1_reg.Reg = otp_key_manager(EFUSE_LOT_ID1_NEW);
		lot_id_2_reg.Reg = otp_key_manager(EFUSE_LOT_ID2_NEW);
	} else {
...
s32 otp_write_key(EFUSE_OTP_KEY_SET_FIELD key_set_index, UINT8 *uc_key)
{
	...
	for (index_cnt = 0; index_cnt < EFUSE_OTP_KEY_FIELD_CNT; index_cnt++) {
		...
		if (key_field_index >= 29 && ((param & OTP_NEW_KEY_RULE_ID) == (u32)OTP_NEW_KEY_RULE_ID)) {
			key_field_index -= 20;
		}
...
s32 otp_set_key_destination(EFUSE_OTP_KEY_SET_FIELD key_set_index)
{
	...
	if(key_field_start_index == 28 && ((param & OTP_NEW_KEY_RULE_ID) == (u32)OTP_NEW_KEY_RULE_ID)) {	//Now = 28 + 1
