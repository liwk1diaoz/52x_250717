 ...
 * na51090:
 *    NOVATEK's NA51090 chip. This configuration uses an AXI master/DMA bus, an
...
static void extphy1_clk_en(phy_interface_t phy_intf, int phy_clk, int ref_clk_out , int pin_num)
{
    ...
    if(pin_num == 1){
        // reset external phy
        gpio_request(E_GPIO(17), "PHY1_RST");
        ...
        // reset external phy
        gpio_request(S_GPIO(19), "PHY1_RST");
        gpio_direction_output(S_GPIO(19), 0);
        mdelay(10);	// assert low at least 10ms
        gpio_set_value(S_GPIO(19), 1);
        mdelay(50);	// wait 50ms before access phy register
        gpio_free(S_GPIO(19));
    }
...
*/
	...
	if (phy_intf & v_nvt_plat_info[ETH_QOS_INDEX].pinmux_rmii) {
        ...
        printf("%s: don't have sup embd. return \r\n",__func__);
...
int na51090_eth_initialize(bd_t *bis)
{
	int ret = 0;
	u32 i;
	char path[20] = {0};
