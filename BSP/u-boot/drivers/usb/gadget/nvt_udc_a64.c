...
static inline int ep_reset(struct nvt_udc_chip *chip, uint8_t ep_addr)
{
	...
	if (nvt_get_chip_id() == CHIP_NA51055) {
		...
		/* reset endpoint */
		setbits_le64(&regs->ep[ep - 1], EP_RESET);
		mdelay(1);
		clrbits_le64(&regs->ep[ep - 1], EP_RESET);
		/* clear endpoint stall */
		clrbits_le64(&regs->ep[ep - 1], EP_STALL);
	} else {
		...
		*/
		...
		if ((req->bRequestType & USB_TYPE_MASK) == USB_TYPE_STANDARD) {
			...
			case USB_REQ_SET_FEATURE:
				...
				case 0:    /* Endpoint Halt */
					id = req->wIndex & 0xf;
					setbits_le64(&regs->ep[id - 1], EP_STALL);
					ret = CX_FINISH;
					break;
