...
void disable_endpoints(struct usb_composite_dev *cdev,
		...
		struct usb_ep *iso_in, struct usb_ep *iso_out, struct usb_ep *int_in, struct usb_ep *int_out)
{
	disable_ep(cdev, in);
	disable_ep(cdev, out);
	if (iso_in)
...
no_iso:
	...
	fs_int_source_desc.wMaxPacketSize = ss->int_maxpacket > 1023 ? 1023 : ss->int_maxpacket;
	fs_int_source_desc.bInterval = ss->int_interval;
	fs_int_sink_desc.wMaxPacketSize = ss->int_maxpacket > 1023 ? 1023 : ss->int_maxpacket;
...
no_int:
	...
	ss_int_source_comp_desc.wBytesPerInterval = ss->int_maxpacket * (ss->int_mult + 1) * (ss->int_maxburst + 1);
...
fail3:
	...
	/* Configure for periodic interrupt endpoint */
		...
		if (ep) {
			...
			result = source_sink_start_ep(ss, true, EP_INTERRUPT, speed);
			if (result < 0) {
fail4:
				ep = ss->int_in_ep;
				if (ep) {
		...
		/* one interrupt endpoint reads (sinks) anything OUT (from the host) */
		ep = ss->int_out_ep;
		if (ep) {
			result = config_ep_by_speed(cdev->gadget, &(ss->function), ep);
			...
			result = source_sink_start_ep(ss, false, EP_INTERRUPT, speed);
...
static struct usb_function_instance *source_sink_alloc_inst(void)
{
	...
	ss_opts->int_interval = GZERO_ISOC_INTERVAL;
	ss_opts->int_maxpacket = GZERO_ISOC_MAXPACKET;
