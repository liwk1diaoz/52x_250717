...
MODEL_BUILD_LIST=(
		...
		"lunch rtos cfg_RTOS_BOOT_LINUX_EVB gcc-6.5-newlib-2.4-2019.11-arm-ca9-eabihf"
		"lunch Linux cfg_RTOS_BOOT_LINUX_EVB arm-ca9-linux-uclibcgnueabihf-6.5"
)
	...
	*)
		...
		do
			...
			if [[ $n == *"cfg_528_FPGA"* ]]; then
				sed -i 's/^export NVT_FPGA=OFF.*/export NVT_FPGA=OFF/g' build/envoption.sh
			...
			if [ ! -f $LINUX_BUILD_TOP/prebuilt_list_linux_code.txt ]; then
				echo "Prebuilt file list" 					>> $LINUX_BUILD_TOP/prebuilt_list_linux_code.txt
				find base/linux-code/ -name *.a -o -name *.so -o -name *.o 	>> $LINUX_BUILD_TOP/prebuilt_list_linux_code.txt
				echo "Prebuilt file list" 					>> $LINUX_BUILD_TOP/prebuilt_list_rtos_code.txt
				find base/rtos-code/ -name *.a -o -name *.so -o -name *.o 	>> $LINUX_BUILD_TOP/prebuilt_list_rtos_code.txt
				echo "Prebuilt file list" 					>> $LINUX_BUILD_TOP/prebuilt_list_hdal.txt
				find base/hdal/ -name *.a -o -name *.so -o -name *.o 		>> $LINUX_BUILD_TOP/prebuilt_list_hdal.txt
			fi
			make all -j1
#			make sample
#			cd $NVT_HDAL_DIR;
#			make test_install;
			...
			cd $LINUX_BUILD_TOP;
			cp prebuilt_list* BIN/$NVT_PRJCFG_CFG/$MODEL/
...
#			cp $NVT_HDAL_DIR/test_cases/output BIN/$NVT_PRJCFG_CFG/$MODEL -rf
		...
		# just run once to check sanity of all publish.json
		make publish
